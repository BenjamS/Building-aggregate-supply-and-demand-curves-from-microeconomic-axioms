...
Farms are thus limited to decreasing returns to scale, as the yield envelope is undefined at constant returns to scale, and increasing returns to scale ($h > 1$) imply a negatively sloped supply curve, in violation of Axiom 3.
...
\begin{equation}
x_v^* = \frac{\alpha_v}{w_v}(y_0 P \lambda^{-1} \beta_w^{-1} \beta_x \beta_k)^{\frac{1}{1 - h}}
\label{eq:inputDem2}
\end{equation}

Note that input demand is undefined at $h = 1$. The own price elasticity of input demand is

\begin{equation}
\begin{split}
\frac{\partial log(x_v^*)}{\partial log(w_v)} &= - 1 - \frac{1}{1 - h} \frac{\partial \ln(\beta_w)}{\partial \ln(w_v)} \\
\frac{\partial log(x_v^*)}{\partial log(w_v)} &= \frac{-\alpha_v}{1 - h} - 1
\end{split}
\label{eq:ownPriceElas}
\end{equation}

Where Axiom 4 (negatively sloped demand curves) requires $\frac{\partial log(x_v^*)}{\partial log(w_v)} < 0 \:;\:\:\forall v$, thus imposing the following domain on $h$.

\begin{equation}
h \in (-\infty, 1 + \underline{\alpha}) \:\:; \:\:\: \underline{\alpha} = \min(\boldsymbol{\alpha})
\end{equation}

Considering that $\underline{\alpha} > 0$, the effective domain of $h$ satisfying both Axioms 3 and 4 is
...
Note that the cross price and output price elasticities of input demand also bear the expected sign under the effective domain of $h$.

\begin{equation}
\frac{\partial log(x_v^*)}{\partial log(w_{j \neq v})} = \frac{-\alpha_j}{1 - h}
\label{eq:CrossPriceElas}
\end{equation}

\begin{equation}
\frac{\partial log(x_v^*)}{\partial log(P)} = \frac{1}{1 - h}
\end{equation}
...
### Deriving the domains of the budget shadow price, optimal yield, and optimal input level

Dotting the first order conditions (equations \ref{eq:eqFOC}) through by $\ref{\mathbf{x}}$ and rearranging gives the following formula for the farm budget shadow price.
...
\begin{equation}
x_v^* < \frac{\alpha_v}{w_v} \left(y_0 P h^{-1} \frac{\beta_x \beta_k}{\beta_w} \right)
...
Most economists interpret this as meaning that the income elasticity of demand is $1$. If this is true, then the Cobb-Douglas model contradicts well documented empirical laws, particularly Engel's law, that clearly indicate changing demand for different items in a given basket as income changes.
...
In either case, equations \ref{eq:inputDem2} and \ref{eq:YdEnvCompact} clearly demonstrate that $C^*$ is a function of basal yield, prices, the budget shadow price, input elasticities, and control variables together with their respective elasticities, in which income $B$ plays no part. As such, $C^*$ may be greater or less than income. To be clear, then, equation \ref{eq:optExpElasInpDem} says that the income elsticity of _optimal expenditure_, _not of income_, is equal to $1$.
...
Moreover, the empirically observed relation between expenditures and income can be explained by Axiom 1
...Axiom 1 implies that demand does change with income.
...
\begin{equation}
x_D^* = \alpha_D P^{\frac{-\alpha_D}{1 - h_D} - 1} \left(y_0^D \frac{P_D}{\lambda_D} \frac{\beta_x^D \beta_k^D}{\tilde{\beta}_w^D} \left)^{\frac{1}{1 - h_D}}
...
And the consumer's farm output price ($P$) elasticity of demand then follows as
...
It follows, then, that Axioms 3 and 4 impose the following domain on $\varepsilon$.
...
Considering that $h_D = \sum_i \alpha_i^D$ and $\alpha_i^D > 0\:;\:\: \forall i$, the lower bound $\alpha_D / h_D$ is always greater than $0$ and less than $1$. Note, moreover, that $\alpha_D - h_D < 0$, and so the upper bound is also less than $1$.
...
Under Axiom 6, then, increasing returns to scale are possible if $\varepsilon < \alpha_D$. Note that, as a general rule, Axiom 5 applies to mills and factories, but not to farms. Farms are generally not at liberty to raise their output price in response to a higher input price, especially not in the developing world. An important asymmetry is thereby introduced between the supply and demand side of the primary sector, whereby producers are limited to decreasing returns to scale while consumers may attain increasing returns to scale if the condition $\varepsilon < \alpha_D$ is met.

The effective domain of $h_D$ in turn imposes the following domains on the consumer's output price elasticity of supply and the consumer's farm output price elasticity of demand, respectively.
...
### Upper bounds on consumer supply and demand
