...
To be clear, then, equation \ref{eq:optExpElasInpDem} says that the income elsticity of _optimal expenditure_, _not of income_, is equal to $1$. A relationship between demand $x_v^*$ and budget $B$ does not follow from Axioms 2 and 3. However, the empirical consumer expenditure laws observed in Figure \ref can be derived from Axiom 1 alone.
...
Applying the first order condition for a maximum ($\frac{\partial \ln(U)}{\partial (w_v x_v)} = 0$) and rearranging,
...
By Axiom 1 (equation \ref), this can be rewritten
...
\begin{equation}
\frac{d \ln(B)}{d B} = \frac{d \ln(B)}{d (w_v x_v)}
\end{equation}
...
\begin{equation}
\begin{split}
\alpha_B \frac{d \ln(B)}{d (w_v x_v)} &= \alpha_v \frac{d \ln(x_v)}{dx_v} \\
\frac{\alpha_B}{w_v} \frac{d \ln(B)}{d x_v} &= \alpha_v \frac{d \ln(x_v)}{dx_v}
\end{split}
\label{eq:margUtilAssump}
\end{equation}
...
\begin{equation}
\begin{split}
\frac{\alpha_B}{w_v} \int \frac{\partial \ln(B)}{\partial x_v} \: dx_v &= \alpha_v \int \frac{d \ln(x_v)}{d x_v} \: dx_v \\
\alpha_B (\ln(B) + k_B)  &= \alpha_v (\ln(x_v) + k_v)
\end{split}
\end{equation}
...
\begin{equation}
m_v = \frac{\alpha_B}{\alpha_v}, \:\:\: b_v = \frac{\alpha_B}{\alpha_v} k_B - k_v
\label{eq:incomElast}
\end{equation}
