...
And the farm price elasticity of demand $\eta(P, x_D^*)$ becomes
...
Which was shown to be true in the previous paragraph. Thus the supposition that $\varepsilon$'s upper bound is always greater than its lower bound is true.
...
The effective domain of $h_D$ derived in the previous section then imposes the following domains on the consumer's output price elasticity of supply and the consumer's feedstock (farm output) price elasticity of demand, respectively.
...
\begin{equation}
Q_S \bigr|_{\beta_k} = N_S \left(y_0 \frac{\beta_x \beta_k}{\beta_w} P^h \right)^{\frac{1}{1 - h}} \int_{0}^{\bar{\chi}} \chi f_{\lambda}(\lambda) \: d \lambda
...
And where the law of the unconscious statistician has been invoked to replace $f(y^*) \: dy^*$ with $f_{\lambda}(\lambda) \: d \lambda$.

Aggregate supply is then found by integrating eqaution \ref{eq:QSeqPcetPar} over the density of $\beta_k^{\frac{1}{1 - h}}$.

\begin{equation}
\begin{split}
Q_S &= \int_{0}^{\infty} \left[ N_S \left(y_0 \frac{\beta_x \beta_k}{\beta_w} P^h \right)^{\frac{1}{1 - h}} \int_{0}^{\bar{\chi}} \chi f_{\lambda}(\lambda) \: d \lambda \right] \: g_k(\beta_k^{\frac{1}{1 - h}}) d \beta_k^{\frac{1}{1 - h}} \\
&= N_S \left(y_0 \frac{\beta_x}{\beta_w} P^h \right)^{\frac{1}{1 - h}} \int_{0}^{\infty} \beta_k^{\frac{1}{1 - h}} f_k(\beta_k) \: d \beta_k \int_{0}^{\bar{\chi}} \chi f_{\lambda}(\lambda) \: d \lambda \\
&= N_S \left(y_0 \frac{\beta_x}{\beta_w} P^h \right)^{\frac{1}{1 - h}} \mu_k \int_{0}^{\bar{\chi}} \chi f_{\lambda}(\lambda) \: d \lambda
\end{split}
\label{eq:QSeqP}
\end{equation}

Where $\mu_k = E(\beta_k^{\frac{1}{1 - h}})$; and where the law of the unconscious statistician has again been invoked to replace $g_k(\beta_k^{\frac{1}{1 - h}}) \: d \beta_k^{\frac{1}{1 - h}}$ with $f_k(\beta_k) \: d \beta_k$.
...
Axiom 7: The economic yield density function, $f(y^*)$.
...
\begin{equation}
\int_{0}^{\bar{\chi}} \chi f_{\lambda}(\lambda) \: d \lambda = \mu_{\lambda} \Phi(\bar{\gamma})
\end{equation}

Where

\begin{equation}
\mu_{\lambda} = e^{\frac{-h}{1 - h} m_{\lambda} + \frac{1}{2} \frac{h^2}{(1 - h)^2} s_{\lambda}^2} \:\:;\:\:\: m_{\lambda} = E(\ln(\lambda)) \:, \:\: s_{\lambda}^2 = Var(\lambda)
\end{equation}
...
\begin{equation}
\begin{split}
\bar{\gamma} &= \frac{\ln(\bar{\chi}) - m_{\chi}}{s_{\chi}} - s_{\chi} \\
&= -\frac{\ln(\underline{\lambda}) - m_{\lambda}}{s_{\lambda}} + \frac{h}{1 - h} s_{\lambda} \\
&= -\frac{\ln(h) - m_{\lambda}}{s_{\lambda}} - \frac{h}{1 - h} s_{\lambda}
\end{split}
\end{equation}
...
The aggregate supply curve may then be rewritten as follows.

\begin{equation}
Q_S = N_S \left(y_0 \frac{\beta_x}{\beta_w} P^h \right)^{\frac{1}{1 - h}} \mu_k \mu_{\chi} \Phi(\bar{\chi})
\label{eq:QSHull}
\end{equation}
...
\begin{equation}
\mu_k = e^{\frac{1}{1 - h} m_k + \frac{1}{2} \frac{1}{(1 - h)^2} s_k^2} \:\:;\:\:\: m_k = E(\ln(\beta_k)) \:, \:\: s_k^2 = Var(\beta_k)
\end{equation}

### The output price elasticity of aggregate supply \label{sec:pElastQS}

In section \ref{sec:degHomog} it was shown that the farm supply curve is a power law. The aggregate supply curve derived in equation \ref{eq:QSeqP} is also a power law or constant elasticity of supply curve. This becomes more evident if the equation is expressed in following compact form.
...
\begin{equation}
\theta_S = N_S \left(y_0 \frac{\beta_x}{\beta_w} \right)^{\frac{1}{1 - h}} \mu_k \int_{0}^{\bar{\chi}} \chi f_{\lambda}(\lambda) \: d \lambda
\end{equation}
...
<!-- \begin{equation} -->
<!-- Q_S = N_S (y_0 \beta_x)^{\frac{1}{1 - h}} \mu_r \mu_k \int_{0}^{\bar{\chi}} \chi f_{\lambda}(\lambda) \: d\lambda -->
<!-- \end{equation} -->
...
\begin{equation}
Q_D = N_D \left( \frac{\alpha_D}{P} \right)^{\frac{\varepsilon - \alpha_D}{1 - h_D} + 1} \left( y_0^D b \frac{\beta_k^D \tilde{\beta}_x^D}{\tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}} \int_0^{\bar{\chi}_D} \chi_D f_{\lambda}^D \left(\lambda_D \right) \: d \lambda_D
\label{eq:QDcetPar}
\end{equation}
...
And where the law of the unconscious statistician has been invoked to replace $f_D \left(x_D^* \right)dx_D^*$ with $f_{\lambda}^D \left( \lambda_D \right) d\chi_D$.

Aggregate demand is then found by integrating eqaution \ref{eq:QDcetPar} over the density of ${\beta_k^D}^{\frac{1}{1 - h}}$.

\begin{equation}
\begin{split}
Q_D &= \int_0^{\infty} \left[ N_D \left( \frac{\alpha_D}{P} \right)^{\frac{\varepsilon - \alpha_D}{1 - h_D} + 1} \left( y_0^D b \frac{ \beta_k^D \tilde{\beta}_x^D}{\tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}} \int_0^{\infty} {\beta_k^D}^{\frac{1}{1 - h}} \int_0^{\bar{\chi}_D} \chi_D f_{\lambda}^D \left(\lambda_D \right) \: d \lambda_D \right] g_k^D \left( {\beta_k^D}^{\frac{1}{1 - h}} \right) \: d \left( {\beta_k^D}^{\frac{1}{1 - h}} \right) \\
&= N_D \left( \frac{\alpha_D}{P} \right)^{\frac{\varepsilon - \alpha_D}{1 - h_D} + 1} \left( y_0^D b \frac{ \tilde{\beta}_x^D}{\tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}} \int_0^{\infty} {\beta_k^D}^{\frac{1}{1 - h}} g_k^D \left( {\beta_k^D}^{\frac{1}{1 - h}} \right) \: d \left( {\beta_k^D}^{\frac{1}{1 - h}} \right) \int_0^{\bar{\chi}_D} \chi_D f_{\lambda}^D \left(\lambda_D \right) \: d \lambda_D \\
&= N_D \left( \frac{\alpha_D}{P} \right)^{\frac{\varepsilon - \alpha_D}{1 - h_D} + 1} \left( y_0^D b \frac{ \tilde{\beta}_x^D}{\tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}} \int_0^{\infty} {\beta_k^D}^{\frac{1}{1 - h}} f_k^D \left( \beta_k^D \right) \: d \beta_k^D \int_0^{\bar{\chi}_D} \chi_D f_{\lambda}^D \left(\lambda_D \right) \: d \lambda_D \\
&= N_D \left( \frac{\alpha_D}{P} \right)^{\frac{\varepsilon - \alpha_D}{1 - h_D} + 1} \left( y_0^D b \frac{ \tilde{\beta}_x^D}{\tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}} \mu_k^D \int_0^{\bar{\chi}_D} \chi_D f_{\lambda}^D \left(\lambda_D \right) \: d \lambda_D
\end{split}
\label{eq:QD}
\end{equation}

Where $\mu_k^D = E({\beta_k^D}^{\frac{1}{1 - h_D}})$; and where the law of the unconscious statistician has again been invoked to replace $g_k^D({\beta_k^D}^{\frac{1}{1 - h_D}}) \: d {\beta_k^D}^{\frac{1}{1 - h_D}}$ with $f_k^D(\beta_k^D) \: d \beta_k^D$.
...
Axiom 8: The consumer demand density function, $f_D(x_D^*)$.
...
\begin{equation}
\int_{0}^{\bar{\chi}_D} \chi_D f_{\lambda}^D(\lambda_D) \: d \lambda_D = \mu_{\lambda}^D \Phi(\bar{\gamma}_D)
\end{equation}

Where

\begin{equation}
\mu_{\lambda}^D = e^{\frac{-h_D}{1 - h_D} m_{\lambda}^D + \frac{1}{2} \frac{h_D^2}{(1 - h_D)^2} {s_{\lambda}^D}^2} \:\:;\:\:\: m_{\lambda}^D = E(\ln(\lambda_D)) \:, \:\: {s_{\lambda}^D}^2 = Var(\lambda_D)
\end{equation}
...
\begin{equation}
\begin{split}
\bar{\gamma}_D &= \frac{\ln(\bar{\chi}_D) - m_{\chi}^D}{s_{\chi}^D} - s_{\chi}^D \\
&= -\frac{\ln(\underline{\lambda_D}) - m_{\lambda}^D}{s_{\lambda}^D} + \frac{h_D}{1 - h_D} s_{\lambda}^D \\
&= -\frac{\ln(h_D) - m_{\lambda}^D}{s_{\lambda}^D} - \frac{h_D}{1 - h_D} s_{\lambda}^D
\end{split}
\end{equation}
...
The aggregate demand curve may then be rewritten as follows.

\begin{equation}
Q_D = N_D \left( \frac{\alpha_D}{P} \right)^{\frac{\varepsilon - \alpha_D}{1 - h_D} + 1} \left( y_0^D b \frac{ \tilde{\beta}_x^D}{\tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}} \mu_k^D \mu_{\lambda}^D \Phi(\bar{\gamma}_D)
\label{eq:QDHull}
\end{equation}
...
\begin{equation}
\mu_k^D = e^{\frac{1}{1 - h} m_k^D + \frac{1}{2} \frac{1}{(1 - h)^2} {s_k^D}^2} \:\:;\:\:\: m_k^D = E(\ln(\beta_k^D)) \:, \:\: {s_k^D}^2 = Var(\beta_k^D)
\end{equation}

### The output price elasticity of aggregate supply \label{sec:pElastQS}

In section \ref{sec:consDem} it was shown that the consumer demand curve is a power law. The aggregate demand curve derived in equation \ref{eq:QD} is also a power law. This becomes more evident if the equation is expressed in following compact form.
...
\begin{equation}
\begin{split}
\frac{\partial \ln(\theta_S)}{\partial \alpha_v} &= \frac{1}{1 - h} \left[1 - \boldsymbol{\alpha} \cdot \frac{\partial \mathbf{w}}{\partial \alpha_v} + \ln \left( \frac{\alpha_v}{w_v} \right) + \ln(\theta_S) - \ln(\mu_k) - \ln(F(\bar{\chi})) \right] + \frac{\partial \ln(\mu_k)}{\partial \alpha_v} + \frac{\ln(F(\bar{\chi}))}{\partial \alpha_v}
...
\begin{equation}
a = \int_{-\infty}^{-1} \: g(\eta_D) d \eta_D
\end{equation}

Where $g()$ is the assumed density function of $\eta_D$ (Axiom 10).
...
Where $f(y^*)$ is the assumed density of $y^*$ (Axiom 11). More compactly,
