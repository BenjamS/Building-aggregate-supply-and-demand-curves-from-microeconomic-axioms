...
Axiom 3: Farms will not enter the market if their expected net benefit is negative.
Axiom 4: Upward sloping supply curves and downward sloping demand curves (It will be seen that Axioms 1-2 require that the former implies the latter, and vice versa.)
Axiom 5: Relation between consumer output price and farm output price
Axiom 6: the shape of the relevant density functions: farm yield, consumer yield/utility, and technology adjusted price ratio.
...
### Convexity of the farm supply curve
...
## Consumer utility/yield and demand
...
But note that the consumer's "input price" $w_v^D$ is just the producer's output price $P$. Substituting $P$ for $w_v^D$ and factoring out $P$, then, optimal consumer yield and demand can be rewritten in a way that clarifies the role of price $P$.
...
\begin{equation}
x_D^* = \left(\frac{\alpha_D}{P} \right)^{\frac{\alpha_D}{1 - h_D} + 1} \left(y_0^D \frac{P_D}{\lambda_D} \frac{\tilde{\beta}_x^D \beta_k^D}{\tilde{\beta}_w^D} \left)^{\frac{1}{1 - h_D}}
\label{eq:inputDemDEM0}
\end{equation}

Where, to reduce notational clutter, the index $v$ is henceforth dropped such that $x_v^{D*}$ is replaced with $x_D^*$ and $\alpha_v^D$ is replaced with $\alpha_D$; and where

\begin{equation}
\begin{split}
\tilde{\beta}_w^D &= P^{-\alpha_D}\beta_w^D \\
\tilde{\beta}_x^D &= \alpha_D^{-\alpha_D}\beta_x^D
\end{split}
\end{equation}
...
Equation \ref{eq:PDasFunOfP} is a surmise or definition, and not a result derived from the Axioms. For this reason, it takes its place among the Axioms as Axiom 5. Nonetheless, the domain of $\varepsilon$ _is_ derived from the preceding Axioms as follows.

Substituting $bP^{\varepsilon}$ for $P_D$ and factoring out $P$ further clarifies the role of $P$ in consumer yield and demand.
<!-- \ref{eq:consYdEnv0} and \ref{eq:inputDemDEM0} -->

\begin{equation}
y_D^* = P^{\frac{\varepsilon h_D - \alpha_D}{1 - h_D}} \left(y_0^D \left( \frac{b}{\lambda_D} \right)^h_D \frac{\beta_x^D \beta_k^D}{\tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}}
\label{eq:consYdEnv}
\end{equation}

\begin{equation}
x_D^* = \left(\frac{\alpha_D}{P} \right)^{\frac{\alpha_D - \varepsilon}{1 - h_D} + 1} \left(\frac{y_0^D b \tilde{\beta_x}^D  \beta_k^D}{\lambda_D \tilde{\beta}_w^D} \right)^{\frac{1}{1 - h_D}}
\label{eq:inputDemDEM}
\end{equation}

(If there is no relation between $P$ and $P_D$, then the original consumer yield and demand expressions in equations \ref{eq:consYdEnv} and \ref{eq:inputDemDEM0} are recovered by setting $b = P_D$ and $\varepsilon = 0$.)

The consumer's output price ($P_D$) elasticity of yield then follows as

\begin{equation}
\frac{\partial \ln(y_D^*)}{\partial \ln(P_D)} = \frac{\varepsilon h_D - \alpha_D}{\varepsilon (1 - h_D)}}
\end{equation}

Where Axiom 4 (positively sloped supply curves) requires the following lower bound on $\varepsilon$.

\begin{equation}
\varepsilon > \frac{\alpha_D}{h_D}}
\end{equation}

The consumer's farm price ($P$) elasticity of demand, meanwhile, follows from equation \ref{eq:inputDemDEM} as
...
Where Axiom 4 (negatively sloped demand curves) requires the following upper bound on $\varepsilon$.

\begin{equation}
\varepsilon < 1 - h_D + \alpha_D
\end{equation}

The domain of $\varepsilon$ thus follows from the $P_D$ elasticity of consumer yield and $P$ elasticity of consumer demand as

\begin{equation}
\frac{\alpha_D}{h_D}} < \varepsilon < 1 - h_D + \alpha_D
\end{equation}

This, in turn, restricts the domain of the output price ($P_D$) elasticity of consumer yield and the farm price ($P$) elasticity of consumer demand as follows.

\begin{equation}
0 < \frac{\partial \ln(y_D^*)}{\partial \ln(P_D)} < h_D - \alpha_D
\end{equation}

\begin{equation}
-1 < \frac{\partial \ln(x_D^*)}{\partial \ln(P)} < \frac{\alpha_D}{h_D}}
\end{equation}

Since $0 < h_D < 1$ and $0 < \alpha_D < h_D$, it follows that $\frac{\partial \ln(y_D^*)}{\partial \ln(P_D)} < 1$ when Axiom 5 is added. That is, insofar as equation \ref{eq:PDasFunOfP} is a realistic model of the consumer's output price (and insofar as the other Axioms are also true), then the increase in consumer output given a $1$% increase in consumer output price $P_D$ is less than proportionate to the increase in price. In economics jargon, such an output curve is said to be inelastic. The consumer's output curve approaches perfect inelasticity as $\varepsilon$ approaches its lower bound.

Likewise, the domain of $\frac{\partial \ln(x_D^*)}{\partial \ln(P)}$ implies that the consumer's demand for the farm product is inelastic when Axiom 5 is added, such that the increase in consumer demand given a $1$% decrease in farm price $P$ is less than proportionate to the fall in price.

### Domain of consumer demand

Assuming consumers enter the market only if they expect to increase their net utility (Axiom 3), then
...
\begin{equation}
\mathcal{L}_D^* = \mathcal{L_D} \bigr_{x_D = x_D^*} = P_D y_D^* - \lambda_D (C_D^* - B_D)
\end{equation}
...
Note from equation \ref{eq:dLnYstardLny0} that the output expansion condition $\frac{\partial \ln(y^*)}{\partial \ln(y_0)} > 0$ implies

\begin{equation}
\partial \ln(y_0) &> \boldsymbol{\alpha} \cdot \partial \ln(\mathbf{w}) - h \partial \ln(P)
\end{equation}

Considering that $0 < h < 1$, it follows that all adopting farms increase their output.
...
\begin{equation}
\theta_D = N_D (\alpha_D)^{\frac{\varepsilon - \alpha_D}{1 - h_D} + 1} \left(\frac{y_0^D b \tilde{\beta}_x^D}{\tilde{\beta}_w^} \right)^{\frac{1}{1 - h_D}} \mu_k^D
\end{equation}
...
Given the form of $\mathcal{L}_D^*$ derived from Axioms 1 and 2 in equation \ref{eq:NBenvDem}, $P_max = \infty$, and $\mathcal{L}_D^* = $
