...
\varepsilon \in
\begin{cases}
(\alpha_D / h_D, 1 + \alpha_D - h_D), & \text{if $h_D \in (0, 1)$} \\
(1 + \alpha_D - h_D, \alpha_D / h_D), & \text{if $h_D \in (1, \infty)$}
\end{cases}
\label{eq:domEpsilon}
\end{equation}
...
The first case is trivially true. Note also that inequality \ref{eq:ax4boundEps} implies that $h_D < 1 \rightarrow \varepsilon > \alpha_D$ and $h_D < 1 \rightarrow \varepsilon < \alpha_D$, so the bound on $h_D$ can be rewritten
...
Under Axiom 4, the elasticity of demand in equation \ref{eq:PelasDem} imposes the following domain on the demand side degree of homogeneity.

\begin{equation}
h_D \in (0, \min(1 + \underline{\alpha}_D, 1 - \varepsilon + \alpha_D))
\end{equation}
...
However, within the theoretical framework derived above, the method remains a problematic welfare measure for reasons not yet enumerated in the literature. First of all, the calculation of an individual consumer's surplus as the area between the aggregate demand curve and equilibrium price hinges upon an identity between price and the consumer's marginal benefit. The reasoning is that price is equal to the consumer's marginal cost, and that net beneft maximization implies that marginal cost is equal to marginal benefit. However, this identity between marginal cost and marginal benefit holds only when no budget constraint is included in the consumer's net benefit maximization problem. When a budget constraint is included, as in Axiom 2, the identity no longer holds. Rather,
