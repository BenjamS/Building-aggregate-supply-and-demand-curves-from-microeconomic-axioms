...
&= \frac{\eta_D}{(\eta_D - \eta_S) (1 - h)} \left(1 - \boldsymbol{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} \right)
\end{split}
\end{equation}
...
Which is always true on adopting farms. (Recall the adoption condition in equation \ref{eq:ydAdoptCondy0}.)

The percentage change in equilibrium price given a $1$% increase in basal yield, meanwhile, is given by

\begin{equation}
\begin{split}
\frac{1}{\eta_D - \eta_S} \frac{\partial \ln(\theta_S)}{\partial \ln(y_0)} \\
&= \frac{1}{(\eta_D - \eta_S)(1 - h)} \left(1 - \boldsymbol{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} \right)
\end{split}
\end{equation}
...
Using equation \ref to substitute for $\frac{\partial \ln(P_e)}{\partial \ln(y_0)}$, this becomes
