...
### The farm supply curve is a power law with convexity dependent upon $h$ \label{sec:pwrLaw}

The yield envelope derived in equation \ref{eq:YdEnvCompact} implies that the farm supply curve is a power law---or, in economics jargon, a constant elasticity of supply curve. This becomes clearer if the yield envelope is rewritten as follows.
...
Both the scale parameter $t$ and exponent $\eta(P, y^*)$ depend critically upon the degree of homogeneity. The exponent implies that the farm supply curve is convex for $1 / 2 > h > 1$, concave for $0 < h < 1 / 2$, and linear with slope $t$ when $h = 1 / 2$.
...
The exponent $\eta(P, y^*)$ also implies that farm supply approaches, but never reaches, perfect elasticity as $h$ approaches $1$; and that farm supply approaches, but never reaches, perfect inelasticity as $h$ approaches $0$.
...
### The demand side yield or utility envelope
...
This means that consumer demand for a given farm output $x_i^{D*}$ is just a demand side analogue of farm input demand, which was derived above in equation \ref{eq:inputDem2}.
...
(Where, to reduce notational clutter, the index $i$ is henceforth dropped such that $x_i^{D*}$ is replaced with $x_D^*$ and $\alpha_i^D$ is replaced with $\alpha_D$.)
...
Equation \ref{eq:inputDemDEM} says that the consumer demand curve is a power law. This becomes clearer if the equation is written more compactly as follows.
...
\begin{equation}
t_D = \alpha_D^{\frac{\alpha_D}{1 - h_D} + 1} \left(y_0^D \frac{P_D \beta_x^D  \beta_k^D}{\lambda_D \tilde{\beta}_w^D})^{\frac{1}{1 - h_D}}
\end{equation}

And where the exponent $\eta(P, x_D^*)$ is the consumer's price elasticity of demand (equation \ref{eq:PelasDem}).

Moreover, equation \ref{eq:inputDemDEMpwrLaw} implies that the consumer demand curve is convex for all possible values of $h_D$ (keeping in mind that Axioms 1-3 limit the domain of $h_D$ to $(0, 1)$), such that the consumer's demand response to a a decrease in price more than proportionate to the price decrease, regardless of the degree of homogeneity $h_D$.
<!-- The same is not true, however, for changes in the prices of other goods in the consumer's basket. Equation \ref{eq:crossPelasDem} implies that the consumer demand curve is convex in a given cross price over $0 < h_D < 1 - \alpha_j^D$ and concave over $1 - \alpha_j^D < h_D < 1$. -->
...
The change in farm level net benefit resulting from introduction of the new technology is, by definition, the percentage change in net benefit times the baseline level of net benefit. That is, for basal yield enhancing technology,

\begin{equation}
\begin{split}
\text{Change in Net Benefit} &= \frac{\partial \ln(\mathcal{L}^*)}{\partial \ln(y_0)} \mathcal{L}^* \\
&= \frac{\partial \mathcal{L}^*}{\partial \ln(y_0)}
\end{split}
\end{equation}

And, likewise, for input enhancing technology,

\begin{equation}
\begin{split}
\text{Change in Net Benefit} &= \frac{\partial \ln(\mathcal{L}^*)}{\partial \alpha_i} \mathcal{L}^* \\
&= \frac{\partial \mathcal{L}^*}{\partial \alpha_i}
\end{split}
\end{equation}

By Axiom 2, it is assumed that a farm adopts a given new technology only if the expected change in net benefit is positive. That is, the adoption conditions for basal yield enhancing and input enhancing technology release may be expressed, respectively,
...
Here it is important to keep in mind that the release and uptake of yield enhancing technology tends to boost supply, which in turn (by Axiom 3) lowers the output price $P$. At the same time, uptake of the new technology usually implies increased use of (i.e. demand for) inputs, which (again by Axiom 3) drives up input prices. Also, the price of improved seeds (an input) is typically higher than the price of the status quo seed variety. Hence,
...
Demand side technology is typically left unaltered by a change in farm technology.^[Although there are important exceptions. Small granule cassava simluaneously reduces farm basal yield and increases bioethanol factory efficiency.] However, new farm technology does have an indirect impact on demand side net benefit $\mathcal{L}_D^*$ through its influence on the farm output price. For basal yield enhancing technology, the change in consumer net benefit works as follows.
...
## Aggregate supply and demand, equilibrium price and quantity
...
\begin{eqution}
Q_D = \int_0^{\bar{x_D}^*} x_d^* N_D f_D \left(x_d^* \right) d x_d^*
\end{equation}
...
\begin{equation}
Q_D = N_D \left( \frac{\alpha_D}{P} \right)^{\frac{\alpha_D}{1 - h_D} + 1} (y_0^D P_D \tilde{\beta}_w^{D - 1} \beta_x^D )^{\frac{1}{1 - h_D}} \mu_k^D \int_0^{\bar{\chi}_D} \chi_D f_D \left(\chi_D \right) d \chi_D
\label{eq:QD}
\end{equation}
...
\begin{equation}
\frac{\partial \ln(Q_D)}{\partial \ln(Q_D)} = -\frac{\alpha_D}{1 - h_D} - 1
\label{eq:PelasDemAgg}
\end{equation}

Which is the same as the price elasticity of demand at the level of individual consumers, derived in equation \ref{eq:PelasDem}.

Like the individual consumer demand curve, the aggregate demand curve is a power law. This becomes clearer if $Q_D$ is written more compactly as follows.
...
\begin{equation}
\theta_D = N_D (\alpha_D)^{\frac{\alpha_D}{1 - h_D} + 1} (y_0^D P_D \tilde{\beta}_w^{D - 1} \beta_x^D )^{\frac{1}{1 - h_D}} \mu_k^D
\end{equation}

And where the exponent $\eta(P, Q_D)$ is the price elasticity of demand (equation \ref{eq:PelasDemAgg}).

As with the consumer demand curve, the price elasticity of demand $\eta(P, Q_S)$ implies convexity for all possible demand side degrees of homogeneity (keeping in mind that Axioms 1-3 limit the domain of $h_D$ to $(0, 1)$). That is to say, for a given price decrease, the aggregate demand response is more than proportionate to the fall in price.
...
### The impact of new technology on equilibrium quantity and price

#### Basal yield enhancing technology
...
By Axioms 1-3, $\eta_S > 0$, $\eta_D < 0$, and $0 < h < 1$. The change in equilibrium supply given release of basal yield enhancing technology is thus positive ($\frac{\partial \ln(Q_e)}{\partial \ln(y_0)} > 0$) so long as
...
\begin{equation}
\begin{split}
\frac{\partial}{\partial \alpha_i} \left( \frac{\eta_D}{\eta_D - \eta_S} \right) &= \frac{\eta_D}{(\eta_D - \eta_S)^2} \frac{\partial \eta_S}{\partial \alpha_i} \:\:;\:\:\: \frac{\partial \eta_S}{\partial \alpha_i} = \frac{\partial}{\partial \alpha_i}\left( \frac{h}{1 - h} \right) = \frac{1}{(1 - h)^2} \\
&= \frac{\eta_D}{(\eta_D - \eta_S)^2 (1 - h)^2}
\end{split}
\end{equation}

\begin{equation}
\frac{\partial}{\partial \alpha_i} \left( \frac{\eta_S}{\eta_D - \eta_S} \right) = \frac{\partial}{\partial \alpha_i} \left( \frac{\eta_D}{\eta_D - \eta_S} \right)
\end{equation}
...
\begin{equation}
\begin{split}
\frac{\partial \ln(Q_e)}{\partial \alpha_i} &= \frac{\eta_D}{(\eta_D - \eta_S)^2 (1 - h)^2} \ln \left(\frac{\theta_S}{\theta_D} \right) + \frac{\eta_D}{\eta_D - \eta_S} \frac{\partial \ln(\theta_S)}{\partial \alpha_i} \\
&= \frac{\eta_D}{\eta_D - \eta_S} \left( \frac{\ln(P_e)}{(1 - h)^2} + \frac{\partial \ln(\theta_S)}{\partial \alpha_i} \right)
\end{split}
\end{equation}
...
\begin{equation}
P_e > e^{-(1 - h)^2 \frac{\partial \ln(\theta_S)}{\partial \alpha_i}}
\label{eq:QePoscondAi}
\end{equation}

Where the derivative $\frac{\partial \ln(\theta_S)}{\partial \alpha_i}$ works out to

\begin{equation}
\begin{split}
\frac{\partial \ln(\theta_S)}{\partial \alpha_i} &= \frac{1}{1 - h} \left[1 - \boldsymbol{\alpha} \cdot \frac{\partial \mathbf{w}}{\partial \alpha_i} + \ln(\frac{\alpha_i}{w_i}) + \ln(\theta_S) - \ln(\mu_k) - \ln(F(\bar{\chi})) \right] + \frac{\partial \ln(\mu_k)}{\partial \alpha_i} + \frac{\ln(F(\bar{chi}))}{\partial \alpha_i}
\label{eq:dLnThetaSdai}
\end{split}
\end{equation}

The percentage change in equilibrium price given a $1$% increase in the partial yield $y_i$, meanwhile, works out to

\begin{equation}
\begin{split}
\frac{\partial \ln(P_e)}{\partial \ln(y_i)} &= \frac{\partial \ln(P_e)}{\partial \alpha_i} \\
&= \frac{1}{\eta_D - \eta_S} \left( \frac{\ln(P_e)}{(1 - h)^2} + \frac{\partial \ln(\theta_S)}{\partial \alpha_i} \right) \\
&= \frac{1}{\eta_D} \frac{\partial \ln(Q_e)}{\partial \alpha_i}
\label{eq:dLnPedai}
\end{split}
\end{equation}
...
The aggregate change in net benefit resulting from new technology uptake is comprised of two components: 1) the aggregate change in producer net benefit, and 2) the aggregate change in demand side net benefit.
...
\begin{equation}
\psi^S = \int \frac{\partial \ln(\mathcal{L}^*)}{\partial \ln(y_0)} \mathcal{L}^* v \left( \frac{\partial \ln(\mathcal{L}^*)}{\partial \ln(y_0)} \mathcal{L}^* \right) d \left( \frac{\partial \ln(\mathcal{L}^*)}{\partial \ln(y_0)} \mathcal{L}^* \right)
\end{equation}

But $\frac{\partial \ln(\mathcal{L}^*)}{\partial \ln(y_0)} \mathcal{L}^* = \frac{\partial \mathcal{L}^*}{\partial \ln(y_0)}$, and so, recalling the expression for $\frac{\partial \mathcal{L}^*}{\partial \ln(y_0)}$ derived in  equation \ref{eq:prodNBy0}, this integral can be rewritten

\begin{equation}
\psi^S &= \left( 1 - \mathbf{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} + \frac{\partial \ln(P)}{\partial \ln(y_0)} \right) P \int y^* f(y^*) d y^*
\end{equation}

Where the law of the unconscious statistician has been invoked to replace $v \left( \frac{\partial \ln(\mathcal{L}^*)}{\partial \ln(y_0)} \mathcal{L}^* \right) d \left( \frac{\partial \ln(\mathcal{L}^*)}{\partial \ln(y_0)} \mathcal{L}^* \right)$ with $f(y^*) d y^*$.

Evaluating the integral, the aggregate change in producer net benefit reduces to

\begin{equation}
\psi^S = \left( 1 - \mathbf{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} + \frac{\partial \ln(P)}{\partial \ln(y_0)} \right) P Q
\end{equation}

At equlibrium, this becomes

\begin{equation}
\psi_e^S = \left( 1 - \mathbf{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} + \frac{\partial \ln(P_e)}{\partial \ln(y_0)} \right) P_e Q_e
\end{equation}
...
\begin{equation}
\begin{split}
\psi_e^S &= \left( 1 - \mathbf{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} + \frac{1 - \boldsymbol{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)}}{(\eta_D - \eta_S) (1 - h)} \right) P_e Q_e \\
&= \left(1 + \frac{1}{(\eta_D - \eta_S) (1 - h)} \right) \left( 1 - \boldsymbol{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} \right) P_e Q_e \\
&= \frac{-\alpha_D}{(\eta_D - \eta_S) (1 - h_D)} \left( 1 - \boldsymbol{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} \right) P_e Q_e
\end{split}
\end{equation}
...
\begin{equation}
\psi^D &= \int \frac{\partial \ln(\mathcal{L}_D^*)}{\partial \ln(y_0)} \mathcal{L}_D^* v_D \left( \frac{\partial \ln(\mathcal{L}_D^*)}{\partial \ln(y_0)} \mathcal{L}_D^* \right) d \left( \frac{\partial \ln(\mathcal{L}_D^*)}{\partial \ln(y_0)} \mathcal{L}_D^* \right)
\end{equation}
...
\begin{equation}
\psi^D &= -\alpha_D P_D \frac{\partial \ln(P)}{\partial \ln(y_0)} \int y_D^* f_D(y_D^*) d y_D^* \\
&= -\alpha_D P_D Q^D \frac{\partial \ln(P)}{\partial \ln(y_0)}
\end{equation}
...
\begin{equation}
\begin{split}
\psi_e^D &= -\alpha_D P_D Q^D \frac{\partial \ln(P_e)}{\partial \ln(y_0)} \\
&= \frac{-\alpha_D}{(\eta_D - \eta_S) (1 - h)} P_D Q^D \left( 1 - \boldsymbol{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} \right) P_D Q^D
\end{split}
\end{equation}
...
&= \frac{-\alpha_D }{\eta_D - \eta_S} \left( 1 - \boldsymbol{\alpha} \cdot \frac{\partial \ln(\mathbf{w})}{\partial \ln(y_0)} \right) \left(\frac{P_e Q_e}{1 - h_D} + \frac{P_D Q^D}{1 - h} \right)
\end{split}
\end{equation}
...
\begin{equation}
\frac{\psi_e^D}{\psi_e^D} = \frac{(1 - h_D) P_D Q^D}{(1 - h) P_e Q_e + (1 - h_D) P_D Q^D}
\end{equation}
