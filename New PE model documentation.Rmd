...
```{r, fig.show = 'hold', fig.width=6, fig.height=4, fig.align='center', fig.cap="\\label{fig:pricesVary}Maize and input prices by district in Kenya, 2007 USAID Tegemeo Agricultural Monitoring and Policy Analysis Project data.", echo = F}
...
To evaluate aggregate supply across all prices, then, it is necessary to integrate aggregate supply over the density of input and output prices. This can be done by defining a new variable $r$^[This may be thought of as the "technology adjusted output-input price ratio".],

\begin{equation}
r = \frac{P^h}{\prod_i w_i^{\alpha_i}}
\end{equation}

and integrating equation \ref{eq:QSeqP} over the density of $r$, giving
...
\begin{equation}
\mu_r = \int_0^{\infty} r^{\frac{1}{1 - h}} q(r) dr = E \left(r^{\frac{1}{1 - h}} \right)
\end{equation}

And $q()$ is the assumed density function of $r$ (Axiom 7).
