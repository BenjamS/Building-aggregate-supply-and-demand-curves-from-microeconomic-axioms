...
And where the $w_i$ are input prices. Equation \ref{eq:YdEnvCompact0} may also be written
...
Where $C^*$ is the optimal farm expenditure. This expression comes in handy for estimation purposes because it requires information only on cost and control variables, and not on input quantities.

Note that, if land area is included among the independent variables, then the yield function can be transformed into a production function ($q^*$) simply by adding $1$ to the estimated area elasticity of yield.

A corollary dervied in the Appendix that will soon come in handy is
...
Where $C^{\#}$ is the cost per unit output, i.e., $C^{\#} = C^* / y^*$. Axiom 3 may be formalized
...
These axiomatic bounds on cost per unit output indicate the domain of integration in the derivation of the aggregate supply curve farther below. Note, in passing, that, by Equation \ref{eq:lambda30}, the upper bound on cost per unit output implies the following lower bound on the budget shadow price.

\begin{equation}
\lambda > h
\end{equation}

### Geometric Brownian motion of prices and yield (Axiom 3) \label{sec:gBmAx}
...
The ugly term $E[E[q^*(T)]\bigr|_{t = 0}]$ represents the population mean of expected production at planting time. It is a double integral. By Fubini's theorem [], the order of integration may be swapped, such that the term may also be interpreted as the mean farm's expected production at planting time.

\begin{equation}
E[E[q^*(T)]\bigr|_{t = 0}] = E[E[q^*(T)]]\bigr|_{t = 0}
\label{eq:fubThm}
\end{equation}
<!-- $E[E[q^*(T)]\bigr|_{t = 0}] = E[\tilde{q}(T)]\bigr|_{t = 0}$, where the shorthand $\tilde{q}^*(T) = E[q^*(T)]\bigr|_{t=T}$ has been introduced. -->
<!-- In words, this says that the population mean of the expected value of $q^*(T)$ at planting time ($t = 0$) is equal to the expected value at planting time of the population mean of $q^*(T)$. -->

Introducing the shorthand $\tilde{q}^*(T) = E[q^*(T)]$ to reduce notational ugliness, the expected aggregate supply function can be rewritten as follows.
<!-- Likewise, -->
<!-- \begin{equation} -->
