...
To redress cases where implausibility is rooted in the assumptions and theoretical premises of the modeling, here we present an alternative aggregate supply model that prioritizes theoretical consistency and transparency, such that implausible results may be traced back to specific assumptions regarding economic behavior, and/or the estimation of specific parameter values that have a clear mapping to measurable empirical phenomena. Progress in this direction has long been discouraged by a general perception that the task is impossible. In their highly influential book, e.g., Alston et al. [-@alston1995science] write:
...
To be clear, we agree that the "precise econometric evidence or...detailed information" that the authors refer to is generally not available, but disagree with the suggestion that economic theory offers nothing in its stead. On the contrary, the whole point of having a theory is precisely to draw inference when information is scarce. If the inference proves wrong, then the theory is corrected. The authors themselves may have made little use of theory when they specified their linear aggregate supply and demand curves; but this does not mean that others are prohibited from doing so.
...
Four other desirable properties immediately follow from axioms 1 and 2 as corollaries, and need not be imposed as additional axioms: 1) a farm yield function that is homogeneous (and thus homothetic) and 2) concave in inputs, 3) upward sloping farm supply curves, and 4) downward sloping farm input demand curves. We also take a moment to demonstrate that the farm yield function agrees with Lipton's theory of technology adoption [].
...
The aggregate supply model that mathematically results from axioms 1-5 consists of terms that capture technology, agroclimatic conditions, input and output prices, and farm entry/exit. The term capturing farm entry/exit turns out to be surprisingly instructive and intuitive, as it is proportionate to the probability that output price will be greater than the mean farm's expected cost per unit output at harvest (i.e. the mean farm's probability of profit). The price elasticity of aggregate supply resulting from this functional form is likewise intuitive, consisting of farm level price elasticity of supply plus a term capturing farm entry/exit in response to price movements. The model also accounts for price volatility, such that the effect of price stabilization policies may be explicitly modeled. This gain in theoretical rigor and explanatory power comes at little or no cost to expedience on modern processors.

Derivation of the farm yield function from axioms 1 and 2 is review for professional economists, and is given in the Appendix. [rudimentary, but the fact that there is generally no appreciation of the contradiction between constant returns to scale and profit maximization would suggest that the "review" is new to many] The main sections of this article are reserved for the derivation of the aggregate supply model from the farm yield function and from axioms 3-5. We then walk through an illustrative use case, parameterizing the model based on Kenya maize farm data. Calculations of aggregate production and price elasticity of aggregate supply agree with empirical estimates. Food policy relevance, application in partial equilibrium analysis, and application in minimum data contexts are briefly considered in the Discussion.
...
Where $C^*$ is the optimal farm expenditure. This expression comes in handy for estimation purposes because it requires information only on cost and control variables, and not on input quantities. Another corollary that will soon come in handy is
...
Where $C^{\#}$ is the cost per unit output, i.e., $C^{\#} = C^* / y^*$.

upper and lower bounds important for the domain of profitable production farm entry/exit result from axiom (3)
...
By Equation \ref{eq:lambda30}, it follows that

\begin{equation}
\lambda > h
\end{equation}

lower bound defaults to physical lower bound of 0
...
Taking into account stochasticity of yield and price, the expression for aggregate supply given in Equation \ref{eq:sup1} may now be rewritten more accurately as follows. and given bounds of profitable production in inequality \ref{eq:upBnd2}.
...
\begin{equation}
E[E[q^*(T)]\bigr|_{t = 0}] = E[E[q^*(T)]\bigr|_{t = T}]\bigr|_{t = 0}
\label{eq:rubThm}
\end{equation}
<!-- $E[E[q^*(T)]\bigr|_{t = 0}] = E[\tilde{q}(T)]\bigr|_{t = 0}$, where the shorthand $\tilde{q}^*(T) = E[q^*(T)]\bigr|_{t=T}$ has been introduced. -->

In words, this says that the population mean of the expected value of $q^*(T)$ at planting time ($t = 0$) is equal to the expected value at planting time of the population mean of $q^*(T)$. Introducing the shorthand $\tilde{q}^*(T)$ to stand for $E[q^*(T)]\bigr|_{t = T}$, the aggregate supply can therefore be rewritten
<!-- \begin{equation} -->
