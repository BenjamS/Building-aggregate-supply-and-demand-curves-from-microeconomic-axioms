...
That is to say, if aggregate supply $Q$ in its most basic definition may be formulated as the definite integral of farm production function $q^*$ times the production density $\rho(q^*)$ over the domain of profitable production $(\underline{q}^*, \overline{q}^*)$,
...
In other words, mathematical descriptions of an assumed supply response to exogenous shock are of little use in the policy making process. What is required is an endogenous model of the supply response to exogenous shocks. For those interested in such modeling, here an aggregate supply curve function is derived from the following five non-controversial economic axioms.
...
Not at the cost of, then, but rather in addition to computational convenience, the axiomatic approach described here presents numerous methodological strengths not found in the conventional approach. In particular, the aggregation of farm level behavior up to the population level explicitly accounts for entry and exit (via axiom 3) and the shape of the distribution (axiom 4), resulting in an endogenous assessment of the farm employment rate as a byproduct. The price elasticity of supply is endogenous to the model . Because the model explicitly accounts for the yield distribution, which is in turn related theoretically to the expenditure distribution (via axioms 1 and 2), the proposed model is well equipped to address questions of distributional equity. The conventional approach, by contrast, readily admits that it cannot address such questions [@norton1992economic]. (In its defense, the conventional approach argues that distributional questions are irrelevant to agricultural research [@norton1992economic; @alston1995science], quite contrary to the modern consensus [@byerlee2009agriculture; @valdes2010reflections; @world2007world; @de2020using; @de2022agriculture].)

For purposes of impact assessment, the present work effectively replaces the exogenous guesswork of the conventional approach with endogenous modeling of the farm level supply and expenditure response to exogenous shock, and the aggregation of this response up to population level, based on axioms 1-4. There is no need for lengthy, abstract debates over which type of arbitrarily specified supply curve shift is best, nor any need to elicit the adoption rate from conflicted experts. The adoption rate, type of shift, and supply elasticity are all endogenous to the model; and thus take their values based on the exogenous shock to total factor productivity, any associated changes in input and output prices, and any subsidy accompanying the technology package. The expert input of agronomists, plant breeders, pathologists, geneticists, and any other agricultural scientists involved in the research is thereby confined to its proper domain---i.e. the agronomic question of expected change to total factor productivity---leaving to policy makers and stakeholders the socioeconomic dimensions of the assessment, particularly as regards policy levers (subsidies) and expected price changes.

Finally, the proposed model also explicitly accounts for the stochastic nature of farm supply, particularly as regards meteorological conditions and output price. Supply curves are notoriously sensitive to meteorological conditions (and this is reaffirmed in a pedagogical Kenya maize farm exercise included here), such that it is more accurate to speak of a supply "ribbon" bounded above and below by the best and worst case meteorological supply curves. Stochastically explicit accounting for output price under axiom 5, meanwhile, leads to a dual expression for aggregate supply in stochastic price parameter space, whereby a novel set of questions regarding price trend and volatility may be addressed. Axiom 5 is also shown to result in a more instructive interpretation of the supply curve equation in terms of the mean farm's probability of profit.
...
In words, the $\mathcal{P}$ term equals the probability that the harvest output price (raised to $\eta$) will be greater than the mean farm's expected unit expenditure (raised to $\eta$). The term may thus be considered a reflection (up to the adjustment $\eta$) of the mean probability of profit. To see this, start by taking the population mean of both sides of Equation \ref{eq:useful2}.
...
A model of agricultural supply and of the supply and welfare impacts of new technology (or any exogenous shock) is useful to agricultural policy makers insofar as it can map out the plausible outcome space for the range of assumptions and policy lever settings (e.g. subsidies) relevant to the policy discussion; and insofar as this mapping is transparently rooted in an empirically vetted, sensible theoretical foundation.
...
\begin{equation}
\begin{split}
-1/2[\zeta^2 - 2 \zeta s_{\chi} - 2 m_{\chi} ] \\
&= -1/2 [(\zeta - s_{\chi})^2 - 2 m_{\chi} - s^2_{\chi}] \\
...
\begin{equation}
\begin{split}
\frac{1}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\xi) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\zeta - s_{\chi} )^2}{2} + m_{\chi} + \frac{s^2_{\chi}}{2}} d \zeta \\
&= \frac{1}{\sqrt{2\pi}} e^{m_{\chi}  + \frac{s^2_{\chi}}{2} ^2} \int_{0}^{\frac{\ln(\xi) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\zeta - s_{\chi} )^2}{2}} d \zeta \\
...
gg <- gg + labs(title = "Table C1: Summary statistics for the 2007 Kenya long rains\nfarm level maize data.", subtitle = "N = 937")
gg <- gg + theme_classic() +
