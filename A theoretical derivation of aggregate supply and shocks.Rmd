...
With all the necessary parameters estimated, the theoretical expected aggregate supply curve is plotted at the four expected rainfall levels, with empirical midseason aggregate supply estimates overlaid, in the top panel of Figure \ref{fig:EaggQs}. The empirical estimates for the major drought years agree closely with the theoretical expected supply curve evaluated at the "dismal" rainfall level, while the estimates in normal or good rainfall years generally agree with the theoretical curve evaluated at the "normal" and "high" rainfall levels. The curve overshoots the 2010 estimate somewhat, possibly due to crop loss from flooding that year [@GIEWS5Aug2010]. The one major empirical divergence from the theoretical curve is 2009, an historic drought year [@MoApressRelease2009].
...
A model of agricultural supply and of the supply shift and welfare impacts of new technology (or any exogenous shock) is useful to agricultural policy makers insofar as it can map out the plausible outcome space for the range of assumptions and policy lever settings (subsidies) relevant to the policy discussion; and insofar as this mapping is transparently rooted in an empirically vetted, sensible theoretical foundation.
...
To more squarely meet the needs of agricultural policy makers, here an alternative modeling framework is theoretically derived from five non-controversial axioms and empirically vetted against Kenya farm data. The proposed framework can answer the usual set of policy questions, such as those just enumerated, as well as many others not often considered part of the purview of such models. The impact of changes in income inequality can be examined by varying the standard deviation of expenditure ($s_c$). Sensitivity to output price volatility can be examined by expressing the model in stochastic price parameter space and varying $s_p$.
...
In minimum data contexts, model parameters can still be estimated up to a respectable degree of precision because the model is expressed in terms of economic indicators and population parameters (the farm employment rate, PEFS-adjusted probability of profit, observed mean and standard deviation of yield and expenditure) that are often deductible from available information and expertise. Model parameters can also be estimated based on output price series, if available (as explained in section \ref{sec:acct4Stoch}).

The Kenya example above thus offers but a small glimpse into the model's full range of applicable use cases. The example focuses on a hypothetical classic yield enhancing SAI technology shock, but the model is equally at home in the "resilience enhancing" AEI paradigm. An AEI use case may involve assessment of, for example, mixed or complex (e.g. silvopastoril) systems, climate smart agriculture, agroclimatic advisory services, risk mitigating cooperative social mechanisms (including agtech based microfinance and insurance), payment for economic services, etc., and combinations thereof.

In the theoretical development thus far, the technology shock is introduced as an adjustment to TFP. In some cases, it may make more sense to represent the shock as a perturbation of the degree of homogeneity $h$ (i.e. a change in one or more of the underlying input efficiencies $\boldsymbol{\alpha}$). In the course of the present work, such "$h$-shocks" were briefly explored. The results of this exploration are not included in the final draft because they are so ambiguous as to require several pages of careful treatment unto themselves.
<!-- As mentioned in section \ref{sec:farmResp}, -->

The model developed thus far is limited to the evaluation of aggregate supply and producer side welfare impacts. Inclusion of consumer side impacts is fundamental to the technology investment case before donors because producer welfare alone may not offset the government's total subsidy cost (as in the Kenya example above). While this lies outside the scope of the present effort, it would be straightforward to extend the framework to include explicit modeling of aggregate demand and consumer side impacts---i.e. a full fledged partial equilibrium model by reinterpreting the input demand curve derived in section \ref{sec:axioms1and2} as a processing plant (e.g. miller, starch factory) feedstock demand curve. Aggregate demand is then evaluated by integrating this curve over the processor population density. Once aggregate demand is integrated, output price becomes endogenous to the model; and it would then be possible to evaluate consumer side impacts and producer-consumer tradeoffs (i.e. the food-price dilemma) in high fidelity. 
...
<!-- In many use cases, the number of processors may be quite small, resulting in a cartel or even monopsony demand curve, especially at subnational scales of analysis. The axiomatic approach would explicitly capture this and other demand side nuance, which is typically glossed over in the conventional treatment. -->
