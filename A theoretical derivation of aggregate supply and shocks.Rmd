---
title: |
 \singlespacing A theoretical derivation of the agricultural aggregate supply curve and research induced supply curve shift
author: |
 \small Benjamin Schiek (sole/corresponding author), b.schiek@cgiar.org\
 \small Performance, Innovation, and Strategic Analysis for Impact (PISA4Impact)\
 \small Alliance of Bioversity International and CIAT, Americas Hub, Km 17 Recta Cali-Palmira, Palmira, Colombia 763537\
# \singlespacing 
# date: "`r Sys.Date()`"
#abstract: |
#  \singlespacing Conventional models of the aggregate supply of agricultural commodities---specifically those used in ex-ante impact assessments of policy and research shocks---do not account for farm heterogeneity (or, worse still, assume identical marginal farms) and do not account for farm entry/exit. These models also require, on input, an empirical estimate of the price elasticity of supply, which is rarely available for the specific commodity and geographical focus in question. In impact assessment, these curves do not endogenously model the supply response to technology shocks, but rather 1) assume the farm level supply response as exogenously given based on agronomic information, and then 2) assume the mechanism by which the farm level response manifests at the aggregate level, i.e. the adoption rate and type of supply curve shift (pivot or parallel shift). To remove these methodological bottlenecks and/or potential sources of subjective bias, here a tractable aggregate supply curve is formally derived from five weak axioms. The derived form explicitly accounts for heterogeneously endowed farms and entry/exit. The price elasticity of supply is endogenous to the model and thus requires no external estimation. In the case of shocks, the endogenous supply response to the shock, adoption rate, and type of supply curve shift are likewise endogenous to the model, requiring no exogenous input apart from the expected change in total factor productivity, prices, and subsidy. The model also explicitly accounts for the stochastic nature of weather and output price. A pedagogical use case is presented using Kenya maize farm data.
header-includes:
  - \usepackage{geometry}
  ...
  - \usepackage{caption}
  - \captionsetup[figure]{font=scriptsize}  # Options: scriptsize, footnotesize, small
  - \usepackage{etoolbox}
...
---
<!-- main text: 10279 words, appendices+abstract~5000 words -->
...
#mainFolder <- "/home/ben/Documents/Theory of aggregate supply and shocks/"
...
\singlespacing Title: A theoretical derivation of the agricultural aggregate supply curve and research induced supply curve shift
...
That is to say, if aggregate supply $Q$ in its most basic definition may be formulated as the definite integral of farm production $q^*$ times the production density $\rho(q^*)$ over the domain of profitable production $(\underline{q}^*, \overline{q}^*)$,
...
then it is not readily apparent from conventional linear and CES specifications of $Q$ what the function $q^*$ is, nor whether it is consistent with microeconomic theory. Critically, the conventional models also do not explicitly account for heterogeneously endowed farm populations (captured in the farm production density $\rho$)---or, worse still, assume a population of homogeneously endowed identical marginal farms; and they neglect to account for the domain of profitable production $(\underline{q}^*, \overline{q}^*)$, thus precluding delineation of supply response in terms of yield and entry/exit components.

Such models also require, on input, an exogenously determined price elasticity of supply, the estimation of which is a paper-length research project unto itself. Because the time and resources required to empirically estimate the supply elasticity are generally not available for most impact assessments, the conventional approach depends critically upon previously published estimates, which rarely exist for the specific commodity and geographic focus in question.

Because these models are not formally derived from axioms, the mapping between their parameters and the relevant, measurable empirical phenomena (i.e., technology, input prices, agroclimatic conditions, demographic variables, etc.) is tenuous, such that it is not clear how a change in any of these phenomena might be accurately represented in terms of a change in parameter values, nor even which parameters might be affected by the change. This, in turn, precludes any endogenous modeling of economic decision making on the part of farmers in response to technology, or any other exogenous, shocks. Instead, the conventional models introduce farm level economic decision making as a calibrated assumption based primarily on agronomic data gathered from field trials, literature, and/or consultation with scientists involved in the research under evaluation.
<!-- When it comes to impact assessment, these models provide no practical, formal means of distinguishing between the exogenous shock, on the one hand, and the economically rational endogenous supply/expenditure/input mix response to the shock, on the other; and provide no method of calculating the latter from the former. -->
<!-- conflate the technology shock to total factor productivity (TFP) with the economic yield and/or expenditure response to the shock. That is to say, they  -->
<!-- The endogenous response to the shock is thus treated as though it were the exogenous shock. -->

This bears emphasis. The conventional aggregate supply models, specifically designed for ex-ante impact assessment of new technology, do not in fact assess the supply response to new technology, but rather present a mathematical description of a presumed supply response that has been predetermined outside of the model based primarily on agronomic information---often elicited from scientists involved in the research under evaluation---and then introduced into the model as an exogenous parameter. Model guidelines might offer some words of warning about the difference between economically and agronomically optimal yields/expenditures/input mixes [@alston1995science]; but no precise, practical means of distinguishing or relating the two is provided. Words of warning are also offered regarding the glaring conflict of interest inherent in the expert opinion of scientists closely involved in the research under evaluation; but again little in the way of actionable advice is actually given. Alston et al. [-@alston1995science], for example, prescribe the creation of
<!-- Alston et al. [-@alston1995science] offer a graphical illustration of the difference between the economically and agronomically optimal supply curves, but the illustration is conceptual, not practical. There is no clear, precise mapping between the displayed curves and the relevant, measurable empirical phenomena (technology, input prices, and agroclimatic conditions). -->
...
The subsequent step of translating the exogenously introduced farm level response into an aggregate supply response likewise involves no explicit, endogenous aggregation of microeconomic behavior, but rather exogenous guesswork translated into mathematical symbols. After a long and inconclusive debate in the 1970s over how the research induced supply curve shift should be represented [@alston1995science; @norton1981evaluating; @voon1991calculation]---i.e. whether it be a parallel shift or pivot or some combination thereof---the current default choice of parallel shift in most impact assessments comes down to us not on any theoretical or empirical grounds, but as a matter of fiat [@alston2009economics].^[The parallel shift also happens to be, surely by pure coincidence, the type of shift that results in the highest possible net benefit calculation, and that, by construction, precludes any possibility of uncomfortable producer-consumer tradeoffs (i.e. no food-price dilemma).] The aggregate assessment is also based upon an assumed adoption rate, again typically elicited from scientists closely involved in the research under evaluation, with little or no grounding in economic arguments.
<!-- over whether the aggregate supply response should be represented as a pivot or parallel shift of the curve -->
...
The final step of calculating net benefits in terms of discounted economic surplus, adjusted for probability of success, time lags in research and adoption, capital depreciation, and other exogenously collected parameters, bears some of the outward markings of a rigorous economic assessment. But insofar as the result of this calculation is prefigured by the initial, exogenously determined farm supply/expenditure response to technology change, the exogenously asserted adoption rate, and the exogenously asserted type of aggregate supply curve shift, then it is not so much an impact assessment as it is an impact assertion, i.e., an exercise in _petitio principii_.
...
Critics have long noted as much, albeit in a muted way. Rose [-@rose1980supply], for example, notes that
...
In other words, mathematical descriptions of an assumed supply response to exogenous shock are of little use to the policy making process. What is required is an endogenous model of the supply response to exogenous shocks. For those interested in such modeling, here an aggregate supply curve is derived from the following five non-controversial economic axioms.
...
In the course of the derivation, it becomes evident that five other desirable properties immediately follow from axioms 1-3 as corollaries, and need not be imposed as additional axioms: 1) The farm yield function is homogeneous (and thus homothetic) and 2) concave in inputs; 3) farm supply curves are upward sloping; 4) input demand curves are downward sloping; and 5) the derived yield function implies agreement with a consensus view of the economic rationale driving technology adoption, articulated by Lipton [-@lipton2005family].

On the other hand, two somewhat controversial corollaries also follow from this axiomatic approach: strictly decreasing returns to scale and constant marginal cost. The former contradicts the conventional wisdom handed down by Hicks [-@hicks1963theory] and Solow [-@solow1957technical]. The latter is well supported empirically [@lavoie2014post], but is eschewed in textbooks. Regardless of one's view on returns to scale and marginal cost curves, the very fact that it is possible to disagree with specific corollaries, and to trace the disagreement back to specific axioms, underscores the key strength of the derived model: its theoretical transparency. Under the theoretical opacity of the conventional approach, by contrast, it is not possible to know if one agrees or disagrees with the premises, or whether the premises contradict each other mathematically, since those premises are stated in literary form (if they are stated at all), and the model is not explicitly derived from such premises.

Subsequent research is free to modify the set of axioms. The point is to set out from axioms that sensibly and elegantly characterize the socioeconomic behavior in question; as opposed to setting out from a notion of what sort of functional form will facilitate computations. The present work effectively demonstrates that the presumption of a tradeoff between theoretical rigor and computational convenience, which presumably motivates the conventional approach, is fallacious. On the contrary, the theoretical derivation from axioms results in a tractable, closed form expression for aggregate supply, as well as similarly tractable expressions for the aggregate supply response to technology shocks and associated producer welfare impact.
<!-- The theoretical derivation should also be as elegant as possible. That is to say, it should strive to describe the greatest amount of empirical laws, stylized facts, possible based on the fewest possible axioms. -->

Not at the cost of, then, but in addition to computational convenience, the axiomatic approach described here presents numerous methodological advantages over the conventional approach. In particular, the aggregation of farm level behavior up to the population level explicitly accounts for entry and exit (via axiom 3) and the shape of the distribution (axiom 4). This results in endogenous assessment of the price elasticity of aggregate supply, thus removing a major methodological obstacle encountered in the conventional approach (i.e. the need for an empirical estimate of the supply elasticity). The distributionally explicit treatment here also results in an endogenous assessment of the farm employment rate, and generally equips the model to address important policy questions regarding distributional equity. The conventional approach, by contrast, readily admits that it is ill suited to such questions [@norton1992economic].^[In its defense, the conventional approach argues that distributional questions are irrelevant to agricultural research [@norton1992economic; @alston1995science], quite contrary to the modern consensus [@byerlee2009agriculture; @valdes2010reflections; @world2007world; @de2020using; @de2022agriculture]. See Appendix A for a deeper treatment.]
<!-- The Explicit treatment of the yield distribution, which is in turn related theoretically to the expenditure distribution (via axioms 1 and 2), -->

For purposes of impact assessment, the present work effectively replaces the exogenous guesswork of the conventional approach with endogenous modeling of the farm level supply and expenditure response to exogenous shock, and the aggregation of this response up to population level, based on axioms 1-4. That is to say, abstract debates over which type of arbitrarily specified supply curve shift is best, and elicitations of the adoption rate from potentially conflicted experts, are replaced by straightforward calculations based on the exogenous shock to total factor productivity (TFP), any associated changes in input and output prices, and any subsidy accompanying the technology package. The expert input of agronomists, plant breeders, pathologists, geneticists, and any other agricultural scientists involved in the research is thereby confined to its proper domain---i.e. the agronomic question of expected change to TFP---leaving to economists, policy makers, and stakeholders the socioeconomic dimensions of the assessment, particularly as regards policy levers (subsidies) and expected price changes.

Finally, the proposed model also explicitly accounts for the stochastic nature of meteorological conditions and output price. Supply curves can, of course, vary widely depending upon meteorological conditions (especially rainfall), such that it is more accurate to speak of a supply "ribbon" bounded above and below by the best and worst case meteorological supply curves. Stochastically explicit accounting for output price (under axiom 5) leads to a dual expression for aggregate supply in stochastic price parameter space, whereby a novel set of policy questions regarding price trend and volatility may be addressed. Axiom 5 is also shown to result in a more instructive interpretation of the supply curve equation in terms of the mean farm's probability of profit.
...
The theoretical derivation is presented in the Methodology section, immediately below. After the Methodology, the derived model is fit to Kenya maize farm data as a pedagogical use case. The fitted maize supply model is vetted against empirical estimates of maize supply and price elasticity of supply at different price points across seven years. A hypothetical, generic technology shock is then introduced in order to showcase the model's impact assessment capacity. The endogenously modeled aggregate supply response to the shock, and associated producer welfare outcomes, are evaluated at a range of different subsidy levels and assumed price shocks. Results are presented in a heatmap format to facilitate policy decision making. Extension of the framework to include aggregate demand and consumer side impacts is briefly outlined, along with other strengths, limitations, and possible next steps, in the Discussion and Conclusion.
<!-- The aggregate supply model is derived from axioms 1-5 in the Methodology section, immediately below. This includes derivation of equations for assessing the endogenous supply response and producer welfare impact given an exogenous technology shock, together with an endogenous model of the adoption rate. The stochasticity of meteorology and output price is accommodated by axiom 5 at the end of the section. All of the salient differences between the present work and the conventional modeling framework stem from this explicit, careful accounting of the endogenous response to exogenous shock, and of the stochastic nature of meteorology and output price. -->
<!-- The adoption rate model includes a term accounting for subsidies.  -->

The widespread use of the conventional modeling framework calls for a more detailed engagement with the literature than the cursory treatment offered above. To place the motivation for the present work on solid footing, this engagement is provided in Appendix A.
...
In sections \ref{sec:axioms1and2} and \ref{sec:axioms3and4}, a deterministic model of aggregate supply is derived from axioms 1-4, beginning with a derivation of the farm production function from axioms 1 and 2, which is then aggregated on the basis of axioms 3 and 4. (This is deterministic in the sense that it does not take into account the stochasticity of output price and weather.) An alternative form for aggregate supply is also derived based on the relation between conditional and unconditional means. Practical steps for evaluating curve parameters are briefly enumerated; and the importance of accounting for stratified (i.e. heterogeneous) TFP is addressed. In sections \ref{sec:endogResp} and \ref{sec:aggImpact}, the farm level endogenous supply and expenditure response to, and welfare impact of, exogenous technology shocks is formalized. The supply and welfare expressions are aggregated to population level, including endogenous modeling of the adoption rate. In section \ref{sec:axiom5}, the stochastic nature of weather and output price is formally accommodated, resulting in the final, stochastically explicit, functional form for expected aggregate supply. The introduction of axiom 5 permits an alternative or dual parsing of the curve in stochastic price parameter space, thus allowing for explicit modeling of changes in price volatility; and resulting in a novel interpretation of the curve in terms of the mean farm's probability of profit. An equation for the price elasticity of expected aggregate supply is presented at the end of the section, together with stochastically explicit versions of the equations for the aggregate supply response and producer welfare previously derived.
...
It is straightforward to see that equation \ref{eq:cobbDoug} is a homogeneous (and thus also homothetic) function with degree of homogeneity $h$ equal to $\sum_i \alpha_i$. There is no need, then, to impose homogeneity as an additional axiom. It follows from the axiom of diminishing marginal returns.
...
The constant $g$ is, of course, usually called the TFP, and understood to represent the yield per unit "all inputs" (although a more accurate description would be yield per unit "all input and control variables, raised to various powers"). The constant $g$ may be a sum of binary endowment terms ($b_i \beta_i$) that augment the TFP base constant ($\beta_0$) on farms where the endowment is present. That is to say, 

\begin{equation}
\begin{split}
\ln(g) &= \alpha_0 = \beta_0 + \sum_{i = 1}^{n_b} b_i \beta_i \\
& \text{(Where $b_i = 1$ if the endowment is present, else $b_i = 0$)}
\end{split}
\label{eq:g0Sum}
\end{equation}

In the Kenya maize farm exercise farther below, for example, there are four ($n_b = 4$) binary endowments---use of oxen in land preparation, use of tractor in land preparation, use of hybrid maize seed, and land tenure---resulting in 12 unique values of $g$ in the population, corresponding to 12 unique combinations of the four binary endowments plus the base endowment constant $\beta_0$.

To derive a more compact expression for yield, the parameter $g$ may be arbitrarily defined in terms of a new parameter ($u$).
...
This substitution will soon facilitate interpretation and manipulation of the optimal yield function. But first the optimal yield function must be derived from axiom 2 (profit maximization). Defining farm net benefit or profit ($\mathcal{L}$) as
...
Where $P$ is the output price, $C$ is expenditure ($C = \mathbf{w}'\mathbf{x}$, where $w_i \in\mathbf{w}$ is the $i^{th}$ input price), $B$ is the farm budget and $\lambda$ is the farm budget shadow price---that is, the marginal benefit to the farm of a marginal increase in budget ($\partial \mathcal{L} / \partial B = \lambda$)---then the first order conditions for a maximum,
...
Where $y^*$ is the optimal yield, i.e., the yield function in Equation \ref{eq:cobbDoug2} evaluated at the optimal input demand $x^*_i$. Multiplying this expression for $x^*_i$ by $w_i$ and summing both sides over $i$ then gives an expression for optimal expenditure ($C^*$).
...
Taking the mean of both sides and rearranging,
...
The expression can be simplified still further by taking the mean and solving for $u$.
...
This expression for optimal farm yield---henceforth referred to simply as the farm yield---is particularly useful as an econometric yield model, as it only requires information on the control variables $k_i$ and total cost $C^*$, and no information on individual input quantities, as would be required when estimating Equations \ref{eq:cobbDoug2} or \ref{eq:cobbDoug3}. Given data on total cost and control variables, one specifies the model

\begin{equation}
\begin{split}
y^* &= v k C^{*h}e^{\epsilon} \:\:;\:\:\: v = \frac{E[y^*]}{E[k] E[C^{*h}]} \\
&\text{(Where} \:\: \epsilon \:\: \text{is the residual)}
\end{split}
\label{eq:YdEnvReg1}
\end{equation}

Such that the estimated coefficient on logged farm cost may be directly interpreted as the degree of homogeneity for the population. When estimating the model in Equations \ref{eq:cobbDoug2} or \ref{eq:cobbDoug3}, by contrast, $h$ must be tallied by summing the estimated coefficients $\alpha_i$. The new term $v$, moreover, retains meaning as a measure of TFP---and arguably a more instructive one since all inputs are expressed in identical units of cost. It is henceforth referred to as the "expenditure TFP", and is related to conventional TFP as follows.
<!-- , and will likely vary across the population since not all farms necessarily apply all the inputs specified in the model -->

\begin{equation}
\begin{split}
v &= \frac{u}{E[k] E[C^*]^h} \\
&= g \frac{\prod_i^{n_x}E[x_i]^{\alpha_i}}{E[C^*]^h} \\
&= \frac{g}{h}\prod_i^{n_x} \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i}
\end{split}
\label{eq:v0forms}
\end{equation}
...
\frac{\partial \ln(y^*)}{\partial \ln(P)} &= \frac{h}{1 - h} \equiv \eta
\end{split}
\label{eq:PelastYstar}
\end{equation}

(Where Equation \ref{eq:cStar} is invoked in the second line.) Because returns to scale are strictly decreasing, then farm supply must be positively sloped. Moreover, farm supply is elastic for $h \in (1 / 2 , 1)$, inelastic for $h \in (0, 1 / 2)$, and linear when $h = 1 / 2$. Note that increasing returns to scale would imply negatively sloped supply curves. In Appendix B.1, the input own and cross price elasticities of demand are likewise derived and shown to be strictly negative only when returns to scale are decreasing.

Because $h$ is constant, the foregoing effectively demonstrates that axioms 1 and 2 lead to a CES farm level supply curve. However, below it is seen that the CES form is not preserved when production is aggregated up to population level.
...
## Aggregation of farm level supply based on axioms 3 and 4 \label{sec:axioms3and4}

The derivation of a compact, instructive expression for $y^*$ above brings us a step closer to parsing the aggregate supply function defined in Equation \ref{eq:aggSup0}. Denoting area $k_a$ such that $q^* = k_a y^*$,
<!-- (Equation \ref{eq:YdEnvReg1}) -->
<!-- Substituting Equation \ref{eq:qStar} for $q^*$ in Equation \ref{eq:aggSup0} gives -->

\begin{equation}
\begin{split}
Q &= N \int_{\underline{q}^*}^{\bar{q}^*} q^* \rho(q^*) dq^* \\
&= N \int_{0}^{\infty} k_a \int_{0}^{\bar{y}^*} y^* \rho(y^*) dy^* \rho(k_a) dk_a \\
...
And note that taking the mean of the last line and rearranging gives
...
To parse further, the upper bound on unit expenditure $\bar{C}^{\#}$ and density function $\rho(C^{\#})$ are resolved from axioms 3 and 4 below. The farm employment rate is also resolved as a byproduct.
...
Axiom 3 states that farms enter production only if they expect to make non-negative profit, i.e. if $C^* \leq R^*$, which simplifies to $C^{\#} \leq P$. (By Equation \ref{eq:cSharp} axiom 3 could also be expressed $\lambda \geq h$.) Note, in passing, that the farm employment rate ($\mathcal{E}$) may thus be expressed as follows.
...
Where $m_{\#} = E[\ln(C^{\#})]$, $s^2_{\#} = Var[\ln(C^{\#})]$, and $\Phi()$ is the standard normal cumulative distribution function.
...
&\text{(Where} \:\: m_{\chi} = E[\ln(\chi)] \:\:\:\:\:\:\:\:\: s^2_{\chi} = Var[\ln(\chi) \text{)}]
\end{split}
\label{eq:keyResult}
\end{equation}
...
Substituting the right hand side of this expression for the integral in Equation \ref{eq:aggSupStep1}, the expression for aggregate supply simplifies as follows.
...
Which, prima facie, is more instructive than the previous form since $\mathcal{E}$ has a straightforward interpretation as the farm employment rate, and more useful because $E[y^*|C^{\#} \leq P]$ can be estimated directly from the observed yield distribution. However, this form is inconvenient for purposes of drawing the entire aggregate supply curve, as the observed $E[y^*|C^{\#} \leq P]$ is valid only for the $P$ at which it is measured. The unconditional mean yield $E[y^*]$, on the other hand, is constant for all $P$. For this reason, Equation \ref{eq:aggSup1} is preferred for most purposes. The unconditional mean yield can be evaluated from observed cost data as shown below. Moreover, the term $\mathcal{P}(P^{\eta}; m_{\#}, s_{\#})$ turns out to have a meaningful economic interpretation that will become clear when the stochastic character of price is taken into account.

### Evaluating supply curve parameters in practice \label{sec:evalInPract}

Tractable expressions for unconditional mean expenditure and yield follow from Equations \ref{eq:fnCsharp} and from the lognormality of $y^*$ and $C^*$ (axiom 4).
...
This is preferred to calculating the parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ directly from observed $C^* / y^*$ because observed yield is harvest yield, whereas the $y^*$ appearing in all equations above refers not to harvest yield but to expected harvest yield at planting time. This distinction is made clearer when stochasticity is taken into account farther below.
...
\begin{equation}
\begin{split}
\bar{m}_{\#} &= m_{\#} - s_{\#} \frac{\phi(\zeta)}{\Phi(\zeta)} \\
\bar{s}_{\#}^2 &= s_{\#}^2 \left( 1 - \zeta \frac{\phi(\zeta)}{\Phi(\zeta)} - \left( \frac{\phi(\zeta)}{\Phi(\zeta)} \right)^2   \right) \:\:;\:\:\: \zeta = \frac{\ln(P) - m_{\#}}{s_{\#}}
\end{split}
...
As noted earlier (Equation \ref{eq:g0Sum}), binary endowments $\beta_i b_i$, and hence TFP, vary across the population. The distinct values that TFP takes in a given farm population, corresponding to the distinct number of combinations of binary endowments present in the population, are henceforth referred to as "TFP strata" and indexed by $\phi$. It follows that $E[C^*]$ and $E[y^*]$ can also be disaggregated by TFP stratum.
<!-- Wherever $g$ or $v$ appears above, then, what is really meant is $g_{\phi}$ and $v_{\phi}$, where $\ell$ indexes the values of TFP corresponding to each stratum ($\ell \in (1,12)$ in this case).  -->

\begin{equation}
\begin{split}
E[C^*]_{\phi} &= e^{m_{c \phi} + \frac{s^2_c}{2}} \\
\text{Where} &\:\:\: m_{c \phi} = \frac{1}{1 - h}(\ln(v_{\phi}) + m_k + m_{\#})
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
E[y^*]_{\phi} &= v_{\phi} E[k] E[C^{* h}]_{\phi} \\
&= v_{\phi} E[k] e^{h m_{c \phi} + h^2 \frac{s_c^2}{2}}
\end{split}
\end{equation}
<!-- To evaluate the stratified conditional (observed) means, replace $m_{c \phi}$, $s_c$ in these expressions with $\bar{m}_{c \phi}$, $\bar{s}_c$. -->

Parameters unindexed by $\phi$ refer to their values evaluated at the average expenditure TFP $v$.

\begin{equation}
v = \sum_{\phi} \frac{N_{\phi}}{N} v_{\phi}
\end{equation}

Where $N_{\phi} / N$ is the fraction of farms in the $\phi^{th}$ TFP stratum. That is to say, $E[y^*]$ and $E[C^*]$ refer to the unconditional mean yield and expenditure evaluated at $v$.

\singlespacing
## An endogenous model of supply response to and welfare impact of exogenous shock \label{sec:endogResp}
\doublespacing

Agronomic yields and expenditures observed on an experimental plot do not translate into the economic yield and expenditure response likely to occur in real farm populations. This economically obvious, empirically well documented fact of life [@herdt1988increasing; @balasubramanian2000yield] bears reiteration because confusion unfortunately still pervades the conversation in many areas of agricultural research impact assessment [@sumberg2012mind]. As noted in the Introduction, the authors of the conventional modeling framework are cognizant of the difference between economic yields and agronomic yields; and offer some words of warning on the matter [@alston1995science]. But they provide no practical, formal method to distinguish between the two or to explicitly relate the former to the latter, such that their model effectively blurs the technology shock and supply response together as a single, nebulous exogenous shock.

In this section, equations for the evaluation of the farm level endogenous yield and expenditure response to, and producer welfare impact of, exogenous technology shocks are derived within the proposed axiomatic framework. The yield and expenditure response models account not only for the shock itself, but also for any anticipated changes in input and output price associated with the shock. The welfare impact model additionally accounts for any subsidies included in the technology package. The farm level supply response and welfare impact models are then aggregated up to population level to generate expressions for total supply response and total producer welfare impact. The aggregation step involves endogenous modeling of the adoption rate based on whether or not the farmer expects adoption to increase their net benefit. Many of the salient differences between the present work and the conventional approach stem from this careful separation of exogenous from endogenous effects.

### The endogenous farm yield and expenditure response to exogenous shock \label{sec:farmResp}

The impact of yield scaling technologies and/or management practices on farm supply and expenditure can be represented through perturbations of the TFP parameter ($g_{\phi}$). This is tantamount to adding a binary endowment variable to the TFP sum in Equation \ref{eq:g0Sum}. The four binary endowments included in the Kenya maize model example may be considered yield scaling interventions that increase the value of $g_{\phi}$ on farms where the endowment is present. The percentage increase in TFP given the introduction of a new intervention $\beta_5 b_5$ on adopting farms (farms where $b_5 = 1$) may be calculated as

\begin{equation}
\begin{split}
\partial \ln(g_{\phi}) &\approx \frac{g_{\phi}' - g_{\phi}}{g_{\phi}} \:\:; \:\:\: g_{\phi}' = g_{\phi} e^{\beta_5 b_5} \\
&\approx e^{\beta_5} - 1
\end{split}
\end{equation}

Note that the percentage increase is the same across all adopting farms regardless of the status quo value $g_{\phi}$.

Ideally, the value of $\beta_5$ should be estimated directly from field trials. If this is not possible, then the estimate can be made on the basis of crop modeling and expert opinion. Regardless of the path taken in this exercise, it is critically important to keep in mind that the objective is to estimate the change in TFP ($\beta_5$ or $\partial \ln(g)$), not the change in economic yield itself ($\partial \ln(y^*)$). If information is gathered in terms of the change in agronomic yield and/or expenditure, then it must be converted into an estimate for $\partial \ln(g)$. The percentage changes in economic yield, expenditure, and expenditure TFP ($v$) then follow endogenously from Equations \ref{eq:fnCsharp} as a function of the change in TFP as well as any change in output and input prices.
...
\begin{equation}
\begin{split}
\frac{\partial \ln(y^*)}{\partial \ln(g)} &= \frac{1}{1 - h} \left( 1 + h \frac{\partial \ln(P)}{\partial \ln(g)} - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g)} \right) \\
\frac{\partial \ln(C^*)}{\partial \ln(g)} &= \frac{1}{1 - h} \left( 1 + \frac{\partial \ln(P)}{\partial \ln(g)} - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g)} \right) \\
...
The direction of the expected change in prices depends upon the type of research product. Most agricultural research currently falls into one of two broad paradigms: "sustainable agricultural intensification" (SAI) or "agroecological intensification" (AEI) [@mockshell2018beyond]. Briefly, the SAI paradigm focuses on genetic gain and high input, land sparing research stemming from the green revolution. The AEI paradigm, on the other hand, prioritizes low input, "land sharing" research and sustainability (or "socioecological resilience") over genetic gain [@dagunga2023agroecology; @MOCKSHELL201964; @altieri2020agroecology].

Widespread adoption of a new SAI technology typically results in increased output supply and input demand, and hence a lower output price and higher input prices. Widespread adoption of an AEI technology, on the other hand, can result in lower input demand and more sustainable, but not necessarily higher, output, in addition to environmental services. For pedagogical purposes, in this article the new technology is assumed to be an SAI research product, such that $\partial \ln(\mathbf{w}) / \partial \ln(g) > 0$ and $\partial \ln(P) / \partial \ln(g) < 0$.
<!-- \begin{equation} -->
<!-- \begin{split} -->
...
The welfare effect on a given farm adopting the new technology may be examined by differentiating $\mathcal{L}^*$ with respect to the change in TFP.
...
&= \frac{\lambda}{h} \left( C^* \left( 1 + \frac{\partial \ln(P)}{\partial \ln(g)} - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g)} \right) + h \frac{\partial B}{\partial \ln(g)} \right)
\end{split}
\label{eq:dLdlng}
\end{equation}
...
\begin{equation}
\partial \ln(P) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g) > 0
\label{eq:adoptCondSimp}
\end{equation}

As mentioned above, for SAI technologies the expectation is that $\partial \ln(P) < 0$ and $\partial \ln(\mathbf{w}) > 0$, and thus $\partial \ln(P) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) < 0$. In words, then, the adoption condition says that a farm stands to increase its net benefit through adoption of yield scaling technology only if the resulting percentage increase in TFP ($\partial \ln(g)$) is greater than the corresponding percentage decrease in output price minus the corresponding percentage increase in input prices scaled by their respective input elasticities ($\boldsymbol{\alpha}$). Note the resemblance between this and the "price/productivity tightrope" condition formulated by Lipton [-@lipton2005family].
...
The one substantive difference between Lipton's condition and the simplified condition in Inequality \ref{eq:adoptCondSimp} is that the percentage increases in input prices in the latter are scaled by their respective efficiency parameters ($\boldsymbol{\alpha}$). Note that, because $0 < h < 1$, then $0 < \alpha_i < 1$; and so this efficiency adjustment has a dampening effect on the input price increase. Note that the dampening is greater as the $\alpha_i$ (and thus the degree of homogeneity $h$) are lower.

The full version of the adoption condition in Inequality \ref{eq:adoptCond} also differs from Lipton by including the exogenous net subsidy term $h / C^* \partial B$. If the efficiency adjusted output-input price ratio decrease is greater than the increase in TFP ($g$), then the margin by which it is greater is the value this term must take in order to make adoption profitable for the farmer. Note that the required net subsidy $\partial B$ will vary across farms based on $C^*$. The factor $h / C^*$ (which may also be written $\lambda / R^*$) represents each farm's marginal valuation of the subsidy; and implies that the subsidy is relatively more valuable to farms with relatively lower status quo expenditure. Solving Inequality \ref{eq:adoptCond} for $C^*$ gives the maximum expenditure level for which adoption is profitable if the simple condition \ref{eq:adoptCondSimp} is not met.

\begin{equation}
C^* \leq \frac{-h \partial B}{\gamma} \:\:;\:\:\: \gamma = \partial \ln(P) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g)
\label{eq:cStarAdopt}
\end{equation}

If $\partial B = 0$, then Inequality \ref{eq:adoptCond} implies that adoption will be either $0\%$ or $100\%$. Because real adoption rates tend to fall somewhere between $0\%$ and $100\%$, the increased input demand associated with new technology adoption implies in and of itself that, without subsidies, $\partial B$ would be negative. One may also note that every major green revolution success story has involved subsidies of one kind or another to offset the required increase in input expenditure, and then some [@patel2013long; @pingali2012green]. In his Nobel lecture, Norman Borlaug said that "equally as important as" the research product itself was the "sound governmental economic policy which would assure the farmer a fair price for his grain, the availability of the necessary inputs...and the credit with which to buy them" [-@Borlaug1970]. Within the theoretical framework developed here, this means that the net subsidy $\partial B$ is generally positive.
...
Denoting the upper adoption bound on $C^*$ (Equation \ref{eq:cStarAdopt}) as $\hat{C^*}$, the adoption rate ($\mathcal{A}$) may be expressed as the integral of the density $\rho(C^*)$ over the domain $(0, \hat{C^*})$.

\begin{equation}
\begin{split}
\mathcal{A} &= \int_0^{\hat{C^*}} \rho(C^*) d C^* \\
&= \Phi \left( \frac{\ln(\hat{C^*}) - m_c}{s_c} \right) \\
&= \Phi\left( \frac{-h \partial B / \gamma - m_c}{s_c} \right)
\end{split}
\label{eq:AdoptRate}
\end{equation}
<!-- second line follows from lognormality of C^* -->

The aggregate net subsidy required to reach a given target adoption rate $\mathcal{A}'$ can then be computed as the subsidy per unit area times the total farm area times $\mathcal{A}'$.

\begin{equation}
\text{Aggregate net subsidy} = N E[k_a] \mathcal{A}' \partial B
\label{eq:aggSubsidy}
\end{equation}

The adoption rate within any particular TFP stratum can be calculated by evaluating this expression at the $v_{\phi}$ of interest.
<!-- aggSubsidy <- dB * Nf * muArea * adoptRateTotal * 10^-9 # Total subsidies in billions -->

<!-- \begin{equation} -->
<!-- A = \sum_{\phi} \frac{N_{\phi}}{N} A_{\phi} -->
<!-- \end{equation} -->
<!-- [[ -->
...
The total increase in aggregate supply due to a technology shock can be evaluated by integrating the change in farm production $\partial q^* = k_a \partial y^*$ over the density of adopting farms $(0, \hat{y}^*)$ where $\hat{y}^* = y^*(\hat{C}^*)$.
<!-- $\overset{\triangle}{y}$ -->

\begin{equation}
\begin{split}
\Delta Q &= N \int_0^{\hat{y}^*} k_a \partial y^* \rho(y^*) dy^* \\
&= N E[k_a] \int_0^{\hat{y}^*} y^* \partial \ln(y^*) \rho(y^*) dy^*
\end{split}
\end{equation}
...
\begin{equation}
\partial \ln(y^*) = \frac{\ell}{1 - h} \:\:;\:\:\: \ell = \partial \ln(g) + h \partial \ln(P) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})
\end{equation}
...
\begin{equation}
\begin{split}
\Delta Q &= N E[k_a] \frac{\ell}{1 - h} \int_0^{\hat{y}^*} y^* \rho(y^*) dy^* \\
&= N E[k_a] \frac{\ell}{1 - h} v E[k] \int_0^{\hat{C}^*} C^{*h} \rho(C^*) dC^* \\
&= N E[k_a] v E[k] E[C^{*h}] \frac{\ell}{1 - h} \Phi \left( \frac{\ln(\hat{C^*}) - m_c}{s_c} - h s_c  \right) \\
&= N E[k_a] v E[k] E[C^{*h}] \frac{\ell}{1 - h} \mathcal{P}(\hat{C}^{*h}; m_c, s_c)
\end{split}
\end{equation}
<!-- $$ -->
<!-- \Delta Q = N E[k_a] \frac{\ell}{1 - h} \int_0^{\hat{y^*}} y^* \rho(y^*) dy^* \\ -->
...
<!-- = N E[k_a] v E[k] E[C^{*h}] \frac{\ell}{1 - h} \mathcal{P}(\hat{C}^{*h}; m_c, s_c) -->
<!-- $$ -->
<!-- \Delta Q = N E[k_a] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \int_0^{\hat{y^*}} y^* \rho(y^*) dy^* \\ -->
<!-- = N E[k_a] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) v E[k] \int_0^{\hat{C^*}} C^{*h} \rho(C^*) dC^* \\ -->
<!-- = N E[k_a] v E[k] E[C^{*h}] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\hat{C^*}) - m_c}{s_c} - h s_c  \right) \\ -->
<!-- = N E[k_a] v E[k] E[C^{*h}] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \mathcal{P}(\hat{C}^{*h}; m_c, s_c) -->
<!-- $$ -->

Where the penultimate line follows from Equation \ref{eq:keyResult}. Recalling from Equation \ref{eq:v0forms} that $v = E[y^*] / (E[k] E[C^{*h}])$, this further reduces to

\begin{equation}
\Delta Q = N E[k_a] E[y^*] \frac{\ell}{1 - h} \mathcal{P}(\hat{C}^{*h}; m_c, s_c)
\label{eq:deltQdet}
\end{equation}
<!-- $$ -->
<!-- \Delta Q = N E[k_a] E[y^*] \frac{\ell}{1 - h}\mathcal{P}(\hat{C}^{*h}; m_c, s_c) -->
...
\begin{equation}
\partial \mathcal{L}^* = r ( C^* \gamma + h \partial B) \:\:; \:\:\: r = \frac{\lambda}{h} = \frac{P}{C^{\#}}
\end{equation}

And recall from Inequality \ref{eq:cStarAdopt} that $h \partial B = - \gamma \hat{C}^*$, such that the expression further reduces to

\begin{equation}
\partial \mathcal{L}^*(C^*, r) = r \gamma (C^* - \hat{C}^*)
\label{eq:dLstar}
\end{equation}

The aggregate change in net benefit ($\Delta \psi$) can then be evaluated by integrating $\partial \mathcal{L}^*(C^*, r)$ over the densities of farm expenditure ($C^* \in (0, \hat{C}^{*h})$), revenue to expenditure ratio ($r \in (1, \infty)$) and area ($k_a$). (The domain of $r$ follows from axiom 3.)

\begin{equation}
\begin{split}
\Delta \psi &= N \int_0^{\infty} \int_1^{\infty} \int_0^{\hat{C^*}} \partial \mathcal{L}^*(C^*, r) \rho(C^*) dC^* \rho(r) dr \rho(k_a) d k_a \\
&= N E[k_a] \int_1^{\infty} r \rho(r) dr  \int_0^{\hat{C^*}} \gamma (C^* - \hat{C}^*)  \rho(C^*) dC^* \\
&= \gamma N E[k_a] E[r] \Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} + s_{\#} \right) \left( E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c) - \hat{C}^* \mathcal{A} \right) \\
&= \gamma N E[k_a] E[r] \mathcal{P}(P^{-1}; m_{\#}, s_{\#}) \left( E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c) - \hat{C}^* \mathcal{A} \right)
\end{split}
\end{equation}

But note that, by Equation \ref{eq:keyResult2},

\begin{equation}
\begin{split}
E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c) &= E[C^*| C^* \leq R^*] \mathcal{A}  \\
E[r] \mathcal{P}(P^{-1}; m_{\#}, s_{\#}) &= E[r | r \geq 1] \mathcal{E}
\end{split}
\end{equation}

Such that the previous expression reduces to

\begin{equation}
\Delta \psi = \gamma N E[k_a] E[r | r \geq 1] \mathcal{A} \mathcal{E} (E[C^* | C^* \leq \hat{C}^*] - \hat{C}^*)
\label{eq:deltPsiDet}
\end{equation}

Which is both a more instructive and a more practical form, since it is expressed in readily interpretable terms $\mathcal{A}$ and $\mathcal{E}$, and in terms of conditional parameters ($E[C^* | C^* \leq \hat{C}^*]$ and $E[r | r \geq 1]$), which are calculable from observed sample data.

To calculate the change in aggregate supply and welfare for each TFP stratum ($\Delta Q_{\phi}$ and $\Delta \psi_{\phi}$), replace $v$, $N$, with $v_{\phi}$, $N_{\phi}$ in the equations above.
...
<!-- \end{split} -->
<!-- \end{equation} -->
<!-- Such that the previous equation can also be written as follows. -->
<!-- \begin{equation} -->
<!-- \Delta \psi = N E[k_a] E[r] \gamma (2 \mathcal{E} - \mathcal{P}(P; m_{\#}, s_{\#})) ( E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c)  - \hat{C}^* \mathcal{A} ) -->
<!-- \label{eq:dPsi} -->
<!-- \end{equation} -->
<!-- $$ -->
<!-- \Delta \psi = N \int_0^{\infty} \int_1^{\infty} \int_0^{\hat{C^*}} \partial \mathcal{L}^*(C^*, r) \rho(C^*) dC^* \rho(r) dr \rho(k_a) d k_a \\ -->
<!-- = N E[k_a] \int_1^{\infty} r \rho(r) dr \int_0^{\hat{C^*}} \gamma (C^* - \hat{C}^*)  \rho(C^*) dC^* \\ -->
<!-- = N E[k_a] E[r] \gamma \Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} + s_{\#} \right) \left( E[C^*] \Phi \left( \frac{\ln(\hat{C}^*) - m_c}{s_c} - s_c \right) - \hat{C}^* \mathcal{A} \right) \\ -->
<!-- = N E[k_a] E[r] \gamma (2 \mathcal{E} - \mathcal{P}(P; m_{\#}, s_{\#})) ( E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c)  - \hat{C}^* \mathcal{A}) -->
<!-- $$ -->

\singlespacing
## Accounting for stochasticity, derivation of the expected aggregate supply curve based on axiom 5 \label{sec:axiom5}
\doublespacing

The deterministic aggregate supply function derived in section \ref{sec:addSupDet} may be a suitable form for sectors characterized by deterministic production processes, negligible delays between input decisions and output, and negligible output price volatility. However, it is not suited to the primary sector, where production is partly deterministic but also partly a function of stochastic meteorological variables; where there is a significant delay between input decisions and output (i.e., the cropping season); and where output price volatility looms large in any management decision. In order to accommodate these hallmarks of agricultural supply, it is necessary to be more explicit in the treatment of yield and output price as stochastic variables.
<!-- Equation \ref{eq:aggSup1} -->

### Accounting for meteorological stochasticity

To begin with, yield is partly a function of stochastic meteorological variables unknown at planting time. The farm yield $y^*$ appearing in the equations above does not refer to harvest yield, but rather expected harvest yield at planting time. This is written $E[y^*(T)]|_{t = 0}$, where $T$ represents the number of time steps from planting ($t = 0$) until harvest. An expression for $E[y^*(T)]|_{t = 0}$ can be derived by first considering the expression for observed yield at harvest.
...
Where the stochastic meteorological component $k_m(T)^{\kappa_m}$ has been factored out of $k$. The component $k_m(T)$ may be a single stochastic meteorological component such as total rainfall by harvest time, or it may represent a product of cumulative meteorological variables raised to their respective elasticities. For example, if the key meteorological variables are cumulative rainfall ($k_r(T)$) and solar radiation ($k_s(T)$), then $k_m(T)^{\kappa_m} = k_r(T)^{\kappa_r} k_s(T)^{\kappa_s}$.

Taking the stochastic expectation of Equation \ref{eq:yStarT} then gives an expression for expected yield $E[y^*(T)]|_{t = 0}$.

\begin{equation}
E[y^*(T; k_m)]|_{t = 0} = E[\tilde{y}^*(T; k_m)]|_{t = 0} \left[\frac{k_m(T)^{\kappa_m}}{E[k_m(T)^{\kappa_m}]} \right]\Biggr|_{t = 0} \frac{k'}{E[k']} \frac{C^{*h}}{E[C^{*h}]} \:\:;\:\:\: \tilde{y}^*(T; k_m) = E[y^*(T; k_m)]
\label{eq:EyStarT0}
\end{equation}
...
Farm data reports harvest yield $y^*(T)$, but expected yield at planting time ($E[y^*(T; k_m)]|_{t = 0}$) depends upon farmer meteorological expectations and is generally not observable. Nonetheless, the implicit observed mean expected yield can be calculated for a given mean expected meteorological condition from fitted expenditure TFP ($v$), the mean observed exogenous continuous (non-meteorological) parameters raised to their fitted coefficients ($E[k']$), and mean observed $C^{* h}$. To see this, first note that, by Fubini's theorem, the mean expected meteorological condition may be expressed

\begin{equation}
E[E[k_m(T)^{\kappa_m}]|_{t = 0}] = E[\tilde{k_m}(T)^{\kappa_m}]|_{t = 0} \:\:;\:\:\: \tilde{k_m}(T)^{\kappa_m} = E[k_m(T)^{\kappa_m}]
\end{equation}

Such that the implicit observed mean expected yield (i.e. the stochastically explicit version of Equation \ref{eq:uncondMuYstar}) can be written
...
(Where $\theta$ is the stochastically explicit form of the condition $C^{\#} \leq P$. That is, $\theta= E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}$.)
...
Note that the optimal farm expenditure per acre $C^*$ remains a deterministic variable because it refers primarily to land preparation, seed, fertilizer and chemical pest control, the cost of which is typically known (executed or committed) at planting time.
<!-- Some formulations of farm cost might also include harvest post harvest labor costs (e.g., shelling, stooking, transport), but  -->
...
### Accounting for output price stochasticity \label{sec:acct4Stoch}

Absent a forward contract, the output sale price $P$ is a stochastic variable unknown to the farmer at the time of planting. Wherever $P$ appears in the equations above, then, what is really meant is $E[P(T)]|_{t = 0}$---i.e. the value that the farmer expects $P$ to take at harvest/sale, as evaluated at planting time. To parse $E[P(T)]|_{t = 0}$, a suitable stochastic model must be assumed for $P(t)$. Within a given season, prices often follow a geometric Brownian motion (gBm). That is to say, changes in $P$ over a small time interval $\Delta t$---i.e. $\Delta P = P(t + \Delta t) - P(t)$)---are often accurately modeled by
...
It is important to keep in mind that the price trajectory considered here is over a cropping season---from planting to harvest. When considered over multiple cropping seasons, commodity prices typically exhibit cyclic or mean reverting behavior, which requires a more sophisticated stochastic model.
...
\begin{equation}
\begin{split}
E[C^{\#}(T)]\bigr|_{t = 0} &= E[P(T)]\bigr|_{t = 0} \frac{h}{\lambda} \\
&= P(0) e^{m_p T} \frac{h}{\lambda}
\end{split}
\label{eq:useful2}
\end{equation}

Substituting the stochastic terms developed above for $y^*$, $P$, and $C^{\#}$ in the deterministic aggregate supply curve (Equation \ref{eq:aggSup1}), gives the stochastically explicit aggregate supply curve.

\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} &= N E[k_a] E[\tilde{y}^*(T; k_m(T))]|_{t = 0} \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right) \\
&= N E[k_a] E[\tilde{y}^*(T; k_m(T))]|_{t = 0} \mathcal{P}(E[P(T)^{\eta}], m_{\#}, s_{\#}) \\
\text{Where} \:\:\: m_{\#} &= E[\ln(E[C^{\#}(T)]|_{t = 0})] \:\:\:\: \text{ and } \:\:\:\: s^2_{\#} = Var\ln(E[C^{\#}(T)]|_{t = 0})
\end{split}
\label{eq:aggSup2}
\end{equation}

Equation \ref{eq:condMeanYd1}, relating the observed (conditional) mean and unconditional mean expected yield, can also be written in stochastically explicit form.

\begin{equation}
E[\tilde{y}^*(T; k_m(T))]|_{t = 0} \mathcal{P}(E[P(T)^{\eta}]|_{t = 0}, m_{\#}, s_{\#}) = E[\tilde{y}^*(T; k_m(T)) | \theta]|_{t = 0} \mathcal{E}
\label{eq:condMeanYd2}
\end{equation}
<!-- \begin{equation} -->
<!-- \begin{split} -->
...
By which the stochastically explicit alternative version of Equation \ref{eq:aggSup1Alt} follows.
<!-- Q = N E[k_a] E[y^*|C^{\#} \leq P] \mathcal{E} -->

\begin{equation}
E[Q(T)]|_{t = 0} = N E[k_a] E[\tilde{y}^*(T; k_m(T))|\theta]|_{t = 0} \mathcal{E}
\label{eq:aggSup2Alt}
\end{equation}
<!-- [have to change this as better method for deducing implicit expected yield given higher up: -->
<!-- -Above (in this same subsection) shown how to deduce implicit expected cond yield from the observed (cond) harvest yield. -->
<!-- -Now given the relation between the cond and uncond mean, the uncond mean can be derived from the cond mean as follows -->
<!-- (this can be addressed in the unconditional and cond mean section higher up, then repeated here just to give the stochastic expressions of the relations?) -->
<!-- Look, the two ways to deductively calculate expected unconditional mean yield are: -->
<!-- $muEyStarT0check <- dfAll(dollarSign)muEyStarT0trunc * EmploymentRate / ProbProfit $ -->
<!-- $muEyStarT0 <- mean(vStrat) * muKprime * muCstarh$ -->
<!-- The latter is more straightforward from data based on cost. The former can be used as a check. -->
<!-- So basically get rid of this part about deducing expected unconditional mean yield from employment rate and PEFS adj prob of profit--except maybe show that it could be used for back of envelope calcs in absence of data -->
<!-- The most relevant usage might be as a way to calculate PEFS adj prob of profit from emplyment rate and mean yield? -->
<!-- ] -->
...
### Expected aggregate supply in stochastic price parameter space

In the remainder of this section, it is shown that the $\mathcal{P}(E[P(T)^{\eta}]|_{t = 0}, m_{\#}, s_{\#})$ term has the following meaningful economic interpretation.

\begin{equation}
\mathcal{P}(E[P(T)^{\eta}]|_{t = 0}, m_{\#}, s_{\#}) = \text{Prob}(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0} \leq P(T)^{\eta}) \:\:;\:\:\: \tilde{C}^{\#}(T) = E[C^{\#}(T)]
\label{eq:probProfitResult}
\end{equation}
<!-- \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right) -->

In words, the $\mathcal{P}$ term equals the probability that the harvest output price (raised to $\eta$) will be greater than the mean farm's expected unit expenditure (raised to $\eta$). The term may thus captures (up to the adjustment $\eta$) the mean probability of profit. To see this, start by taking the population mean of both sides of Equation \ref{eq:useful2}.
...
But note also that at $t = T$,

\begin{equation}
E[C^{\#}(T)] = P(T) h E[\lambda^{-1}]
\end{equation}

And note that, taking the stochastic expectation,

\begin{equation}
E[E[C^{\#}(T)]]\bigr|_{t = 0} = E[P(T)]\bigr|_{t = 0} h E[\lambda^{-1}]
\end{equation}

Such that the left hand sides of this and Equation \ref{eq:fubThmStep} are equal.

<!-- E[E[C^{\#}(T)]\bigr|_{t = 0}] = E[E[C^{\#}(T)]]\bigr|_{t = 0} -->
<!-- \label{eq:fubThm} -->
<!-- C^{\#}(T)|_{\lambda^{-1} = E[\lambda^{-1}]} -->
\begin{equation}
\begin{split}
E[E[C^{\#}(T)]\bigr|_{t = 0}] &= E[E[C^{\#}(T)]]\bigr|_{t = 0} \\
&= E[\tilde{C}^{\#}(T)]\bigr|_{t = 0}
\end{split}
\label{eq:fubThm2}
\end{equation}

Which is a result that can again also be reached by invoking Fubini's theorem. To be clear, the result states that the population mean of the optimal expected unit expenditure ($E[C^{\#}(T)]\bigr|_{t = 0}$) is equal to the optimal expected unit expenditure on the mean farm, i.e. the farm where $\lambda^{-1} = E[\lambda^{-1}]$. It therefore maps the mean of all $C^{\#}(t)$ trajectories in the farm population to the mean of possible random trajectories on a single farm (the farm where $\lambda^{-1} = E[\lambda^{-1}]$).
...
Switching from population parameter space to stochastic price parameter space also requires a switch in variance from population variance to stochastic variance. This means replacing $Var[\ln(E[C^{\#}(T)]|_{t = 0})]$ (i.e. $s^2_{\#}$) with $Var[\ln(\tilde{C}^{\#}(T))]|_{t = 0}$. 

To parse $Var[\ln(\tilde{C}^{\#}(T))]|_{t = 0}$, note first that the optimal expected unit expenditure $E[C^{\#}(T)]\bigr|_{t = 0}$ is an abstract optimal target that farmers aim for, but that evolves before and after the actual farm expenditure that is "locked in" at planting based on the price information $P(0)$ available at that time. As an abstract target, then, optimal unit expenditure may be expressed
...
Because its evolution is proportional to output price, then, by Ito's Lemma, $C^{\#}(t)$ must also be a gBm with the same drift and volatility parameters as $P(t)$ (see Appendix B.4 for a proof). Therefore,

\begin{equation}
\begin{split}
Var[\ln(C^{\#}(T))]\bigr|_{t = 0} &= s^2_p T \\
\text{(and} \:\: Var[\ln(C^{\#}(T)^{\eta})]\bigr|_{t = 0} &= \eta^2 s^2_p T \: \text{)}
\end{split}
\label{eq:sPTresult}
\end{equation}
...
Substituting the dual expressions for $m_{\#}$ and $s^2_{\#}$ in the $\mathcal{P}$ term of Equation \ref{eq:probProfitResult}, and substituting $P(0)e^{m_p T}$ for $E[P(T)]|_{t = 0}$ (via Equation \ref{eq:EPT0}),

\begin{equation}
\begin{split}
\mathcal{P}(E[P(T)^{\eta}]|_{t = 0}, m_{\#}, s_{\#}) &= \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right) \\
&= \Phi \left( \frac{\ln(P(0)) + m_p T - \ln(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0}) + \eta \frac{s^2_p}{2} T}{s_p \sqrt{T}} - \eta s_p \sqrt{T} \right) \\
...
But recall from Equations \ref{eq:lnChngP} that $( m_p - \eta s^2_p/2) T$ and $s_p \sqrt{T}$ are the stochastic expectation and variance of $\ln(P(T)^{\eta})$, such that $z$ is just the standard score of $\ln(P(T)^{\eta})$ evaluated at the logged mean farm unit expenditure $\ln(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0})$.
...
<!-- &= N E[k_a] \times \text{Exp. uncond. mean yield} \times \text{PEFS-adj. mean probability of profit} -->
<!-- \end{split} -->
<!-- \label{eq:aggSup4} -->
<!-- \end{equation} -->

Regardless of the parameter space in which calculations are made, note that the relation between conditional and unconditional means (Equation \ref{eq:condMeanYd2}) may now be written

\begin{equation}
\begin{split}
E[\tilde{y}^*(T)]|_{t = 0} &= E[\tilde{y}^*(T) | \theta]|_{t = 0} \frac{\mathcal{E}}{\text{Prob}(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0} \leq P(T)^{\eta})} \\
\text{Exp. uncond. mean yield} &= \text{Exp. cond. mean yield} \times \frac{\text{Farm employment rate}}{\text{PEFS-adj. mean probability of profit}}
\end{split}
\label{eq:muEyStarT0frommuEyStarT0trunc}
\end{equation}
<!-- \text{Mean expected undonditional yield} &= \text{Mean expected conditional yield} -->

In the absence of both farm and price data, then, back of the envelope calculations can be made based on expert consensus regarding mean observed (conditional) expected yield, the farm employment rate, and the PEFS adjusted mean probability of profit.

\singlespacing
### Stochastically explicit expressions for price elasticity of aggregate supply, aggregate supply response, and producer welfare impact
\doublespacing

In the conventional approach described in the Introduction, the price elasticity of aggregate supply is estimated econometrically outside of the supply model, and then introduced into the model as an exogenous, constant parameter. The price elasticity of aggregate supply following from the theoretical derivation above, on the other hand, is
...
Contrary to the conventional approach, then, the theoretical derivation effectively demonstrates that, insofar as axioms 1-5 are true, the price elasticity of supply is endogenous to the supply model; and that it is not constant, but varies with price ($P(0)$, $m_p T$) and price volatility ($s_p \sqrt{T}$). It follows, moreover, that aggregate supply is elastic/inelastic if

\begin{equation}
s_p \sqrt{T} </> \frac{\phi(-z)}{\Phi(-z)}
\end{equation}

The stochastically explicit formulations for the research induced change in aggregate supply (Equation \ref{eq:deltQdet}) and producer welfare (Equation \ref{eq:deltPsiDet}) derived in section \ref{sec:aggImpact} follow as

<!-- \Delta Q = N E[k_a] E[y^*] \frac{\ell}{1 - h} \mathcal{P}(\hat{C}^{*h}; m_c, s_c) -->
\begin{equation}
\Delta E[Q(T; k_m(T))]|_{t = 0} = N E[k_a] E[\tilde{y}^*(T, k_m(T))]|_{t = 0} \frac{\ell}{1 - h} \mathcal{P}(\hat{C}^{*h}; m_c, s_c)
\label{eq:dEQT0}
\end{equation}
...
\begin{equation}
\begin{split}
\Delta \psi &= \gamma N E[k_a] E[\tilde{r}_{\theta}(T)]|_{t = 0} \mathcal{A} \mathcal{E} (E[C^* | C^* \leq \hat{C}^*] - \hat{C}^*) \:\:; \:\:\: \tilde{r}_{\theta} = E[r|r \geq 1] \\
&= \gamma N E[k_a] E \left[ \frac{P(T)}{E[C^{\#}(T) | \theta]} \right] \Biggr|_{t = 0} \mathcal{A} \mathcal{E} (E[C^* | C^* \leq \hat{C}^*] - \hat{C}^*)
\end{split}
\label{eq:dEPsi}
\end{equation}
<!-- \Delta \psi = \gamma N E[k_a] E[r | r \geq 1] \mathcal{A} \mathcal{E} (E[C^* | C^* \leq \hat{C}^*] - \hat{C}^*) -->
...
ft <- flextable(dfPoints_ft) %>% autofit() %>%
  add_header_lines(values = "Table 1. Empirical midseason forecasts of Kenya main season maize harvest 2006-2012") %>%
  fontsize(size = 7, part = "all") %>%
  line_spacing(space = 0, part = "all") %>%
  fit_to_width(7)
ft
ft %>% save_as_docx( path = paste0(mainFolder, "Table 1.docx"))
  # set_caption(as_paragraph(as_chunk("Table 1. Empirical midseason forecasts of Kenya main season maize harvest 2006-2012", props = fp_text_default(font.size = 8))), align_with_table = F) %>% autofit() %>% fit_to_width(7)
...
And recall that TFP may include binary endowment variables that augment the base constant on farms where the endowment is present (Equation \ref{eq:g0Sum}, replicated here for convenience).
...
As mentioned in section \ref{sec:axioms1and2}, in this exercise there are four binary endowment variables $b_i$ (which take a value of $1$ if the endowment is present and $0$ otherwise): use of oxen in land preparation, use of tractor in land preparation, use of hybrid seed, and land tenure. There are 12 unique combinations of these four endowments in the data. The control variables ($k_i$) are planting density (seed kg/acre), adult equivalent household size, farm size, and rainfall. The meteorological variable is 2007 main season rainfall divided by an historical benchmark, and so can be interpreted as the rainfall anomaly or departure from the historical benchmark. The farm expenditure $C^*$ includes the cost of synthetic fertilizer, land preparation, pest and disease control chemicals, and seed. The OLS regression results are presented in Table 2.
...
ftShow <- ft %>% autofit() %>%
  ...
  add_header_lines(values = c("Table 2. 2007 Kenya farm yield regression results",
                              "Dependent variable: ln[yield (kg/acre)]")) %>%
  fontsize(size = 7, part = "all") %>%
  line_spacing(space = 0, part = "all")

ftShow
ftShow %>% save_as_docx( path = paste0(mainFolder, "Table 2.docx"))
#------
...
The fitted binary endowment coefficients ($\beta_i$) indicate that hybrid maize seed is the most significant contributor to TFP, followed by land prep method and tenure. Interestingly, the oxen coefficient contributes slightly more than tractor. Based on these values, the expenditure TFPs are calculated for each of the 12 TFP strata ($v_{\phi}$), as well as the average across all strata ($v$). The implicit mean expected yield ($E[\tilde{y}^*(T;k_r) | \theta] |_{t = 0}$) is then calculated by Equation \ref{eq:impMuEyStarT0cond}, for $\tilde{k}_r^{\kappa_r} = 1$ (indicating rainfall expectation in line with the historical benchmark). These parameters are reported along with the mean observed cost per unit area and harvest yield, by TFP stratum, in Figure \ref{fig:v0strata}.

Note, in passing, that the stratified view reveals interesting information. As one might expect, hybrid maize yields are generally higher in tractor TFP strata than they are in oxen TFP strata. But the oxen systems generally exhibit lower unit cost and higher expenditure TFP, suggesting higher cost effectiveness. And note that the implicit expected yield at planting is lower than the actual harvest yield obtained. This makes sense because the implicit expected yield is calculated assuming rainfall in line with the historical trend ($\tilde{k}^{\kappa_r} = 1$); and rainfall largely surpassed the historical benchmark ($\tilde{k}^{\kappa_r} > 1$) in the data year (see Appendix C for a rainfall histogram).

\singlespacing
```{r Fig3, fig.show = "hold", fig.width = 6, fig.height=3, fig.align="center", fig.cap="\\label{fig:v0strata}(Left) The expenditure TFP, observed (i.e. conditional) mean cost/area, implicit conditional mean unit expenditure, mean observed harvest yield, and mean implicit conditional expected yield for each of the 12 TFP strata present in the Kenya farm data, and for the whole sample. The implicit expected yield assumes rainfall equal to the historical benchmark. (Right) TFP strata prevalance in the sample.", echo = FALSE}
#=============================================================================
...
# notThese <- c("Dist", "ln[Yield (kg/acre)]", "ln[Cost/acre (KES/acre)]", bin_vars, "ln[Rain anomaly]") 
# lnMod3Vars <- coefOrder
# kPrime = continuous control vars except for rainfall (and any other meteorological controls)
kPrimeVars <- c("ln[Farm size (acres)]",
                "ln[Planting density (kg seed/acre)]",
                "ln[Adult equivalent household size]")
...
dfx <- dfBins
dfx$vStrat <- c(vStrat)
dfx$lcStarTrunc <- dfMod3$`ln[Cost/acre (KES/acre)]`
...
dfStrat <- dfx %>% group_by(Type) %>% summarise(Type = unique(Type),
                                                ...
                                                mCstarTrunc = mean(lcStarTrunc),
                                                 sCstarTrunc = sd(lcStarTrunc),
                                                 muYdHarv = mean(ydHarv),
                                                 muKprimeStrat = mean(kPrime)) %>%
  mutate(muCstarTruncStrat = exp(mCstarTrunc + sCstarTrunc^2 / 2)) %>%
  mutate(muCstarHtruncStrat = exp(h * mCstarTrunc + h^2 * sCstarTrunc^2 / 2)) %>%
  mutate(muEyStarT0truncStrat = vStrat * muKprime * muCstarHtruncStrat) %>%
  mutate(mEcSharpT0truncStrat = (1 - h) * mCstarTrunc - mKprime - log(vStrat),
         sEcSharpT0truncStrat = sqrt((1 - h)^2 * sCstarTrunc^2 - sKprime^2)) %>%
...
dfAll <- dfx %>% summarise(v = mean(vStrat),
  ...
  mutate(muCstarTrunc = exp(mCstarTrunc + sCstarTrunc^2 / 2),
         muCstarHtrunc = exp(h * mCstarTrunc + h^2 * sCstarTrunc^2 / 2)) %>%
  mutate(muEyStarT0trunc = mean(vStrat) * muKprime * muCstarHtrunc) %>%
  mutate(mEcSharpT0trunc = (1 - h) * mCstarTrunc - mKprime - log(v),
         sEcSharpT0trunc = sqrt((1 - h)^2 * sCstarTrunc^2 - sKprime^2)) %>%
  mutate(muEcSharpT0trunc = exp(mEcSharpT0trunc + sEcSharpT0trunc^2 / 2)) %>%
  mutate(stratID = "All",
...
\singlespacing
```{r Fig2, fig.show = "hold", fig.width = 6, fig.height=3, fig.align="center", fig.cap="\\label{fig:mzP}Maize monthly price series during main season (March-August) at primary markets in Kenya. Price data are from the World Food Program (WFP 2025).", echo = FALSE}
#======================================================================
...
# Check that the two ways to deduce expected unconditional mean yield agree
...
The short rains harvest in 2007 was normal and so the "good short rains" estimate of $m_p$, and March 2007 price ($P(0)$), is used to compute $E[P(T)]|_{t = 0} = P(0) e^{m_p T}$, which then feeds into the calculation of the unconditional parameters $m_{\#}$ and $s_{\#}$ by simultaneous Equations \ref{eq:cSharpUncondParams}.

Once $m_{\#}$ and $s_{\#}$ are calculated, the unconditional parameters $m_c$, $s_c$ are calculated by Equation \ref{eq:mCsC}, and then the expected unconditional mean yield term ($E[\tilde{y}^*(T; k_r(T))]|_{t = 0}$) appearing in the expected aggregate supply function is calculated by Equation \ref{eq:impMuEyStarT0cond} at four different expected rainfall levels: $E[\tilde{k}_r(T)^{\kappa_r}]|_{t = 0} = 1.5^{\kappa_r}, \: 1^{\kappa_r}, \: 0.5^{\kappa_r}, \: 0.1^{\kappa_r}$. These values correspond to expected rainfall that is 150% of the historical benchmark ("high"), in line with the historical benchmark ("normal"), 50% of the historical benchmark ("low"), and 10% of the historical benchmark ("dismal"), respectively.
...
The two remaining parameters required to evaluate expected aggregate supply in Equation \ref{eq:aggSup2} are number of farms $N$ and mean farm size $k_a$. For mean farm size, the Kenya farm sample mean is used, giving $k_a =$ `r muAreaReport` acres. The number of maize farms is more difficult to assess. Nowadays, one sometimes sees the figure $3$ million mentioned [e.g. @njeru2022maize], but this appears to be unsupported by hard evidence; and, at any rate, would imply a farm population far lower than 3 million in 2007 if accurate. Here, a rough estimate is deduced by dividing planted area by mean farm size. The midseason reports examined for this exercise suggest that planted main season maize area during the period in question ranges between 3 and 4.5 million acres [@KFSSGJul2011]. Divided by $k_a$, this gives a range between $1.56$ and $2.34$ million farms. Because $N$ represents a ceiling near the top of the range, the figure $N =$ `r NfReport` million is used as a rough estimate.

With all the necessary parameters estimated, the theoretical expected aggregate supply curve is plotted at the four expected rainfall levels, with empirical midseason aggregate supply estimates overlaid, in the top panel of Figure \ref{fig:EaggQs}. The empirical estimates for the major drought years agree closely with the theoretical expected supply curve evaluated at the "dismal" rainfall level, while the estimates in normal or good rainfall years generally agree with the theoretical curve evaluated at the "normal" and "high" rainfall levels. In 2010, the empirical estimate was optimistic due to good rainfall; and so the theoretical curve overshoots this estimate somewhat, possibly due to crop loss from flooding resulting in a downward revision of the figure [@GIEWS5Aug2010]. The one major empirical divergence from the theoretical curve is 2009, an historic drought year [@MoApressRelease2009].
...
A key message emerging from these results is that aggregate supply varies widely with meteorological conditions. It may thus be more instructive to speak of a supply range or "ribbon", bounded by supply curves on top and bottom representing best and worst case meteorological scenarios.
...
gg <- gg + geom_point(data = dfPoints, aes(x = EPT0, y = `Estimate (million MT)`)) + ggrepel::geom_text_repel(data = dfPoints, aes(x = EPT0, y = `Estimate (million MT)`, label = `Estimate date`), size = 2, lineheight = 0.75, nudge_y = 0.3)
gg <- gg + theme_bw()
...
To illustrate the ex-ante impact assessment capacity of the model, consider a hypothetical scenario wherein the Kenya Ministry of Agriculture, at some point around 2008, is deciding whether or not, and at what level, to invest in release of a new maize technology package. Field trials suggest that the new technology results in a `r 100 * dlng0`% increase in TFP on adopting farms. As explained in section \ref{sec:farmResp}, this is tantamount to adding another binary endowment $b_5\beta_5 =$ `r round(betaNew, 2)` to the yield model. Say that the expected efficiency adjusted percentage increase in input prices ($\boldsymbol{\alpha} '\partial \ln(\mathbf{w})$) is `r aDotdlw`, and the expected percentage decrease in output price ($\partial \ln(P)$) is `r dlP`, such that $\partial \ln(P) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) =$ `r dlnNu`, which is greater than the percentage increase in TFP (and so $\gamma < 0$). Adoption would thus result in a `r 100 * dlv0dlg0`% increase in expenditure TFP ($v$), a `r -round(100 * dlMuCstar)`% reduction in expenditure, and a `r round(100 * dlMuEyStarT0)`% increase in yield on adopting farms (recall Equations \ref{eq:dldlg0}). However, by the criterion in Inequality \ref{eq:adoptCond}, adoption makes economic sense for the farmer only if a net subsidy is included in the technology package (i.e., only if $\partial B > 0$).

Say that the Ministry of Agriculture offers a net subsidy of $\partial B =$ `r dB` KES/acre to adopting farms, perhaps in the form of free seed and fertilizer. As explained in section \ref{sec:adopt}, this then implies the adoption expenditure per acre ceiling of $\hat{C}^* =$ `r round(cStarMaxAdopt)` KES (Equation \ref{eq:cStarAdopt}), and the corresponding adoption rate of $\mathcal{A} =$ `r round(100 * adoptRateTotal, 1)`% (Equation \ref{eq:AdoptRate}). The total net subsidy cost to the Ministry of Agriculture of achieving this adoption rate then follows from Equation \ref{eq:aggSubsidy} as `r round(aggSubsidyTot, 1)` million KES. The corresponding increase in expected aggregate supply and producer welfare follow from Equations \ref{eq:dEQT0} and \ref{eq:dEPsi} as `r round(dQtot, 1)` thousand metric tons and `r round(dPsiTot, 1)` million KES, respectively (assuming normal rainfall $\tilde{k}_r^{\kappa_r} = 1$). The breakdown by TFP stratum is given in the left panel of Figure \ref{fig:intervention}, accompanied by a plot of the corresponding supply ribbon shift in the right panel.
...
```{r Fig5, fig.show = "hold", fig.width = 7, fig.height=3, fig.align="center", fig.cap="\\label{fig:intervention}(Left) The adoption rate, change in supply, and change in welfare for each of the 12 TFP strata, for the technological intervention described in the text. (Right) The corresponding expected aggregate supply ribbon shift, where the dotted lines represent the new normal and dismal rainfall bounds of the aggregate supply ribbon.", echo = FALSE}
...
To stimulate policy discussion, it is helpful to consider prospective interventions in a sensitivity heatmap format, whereby a wide range of farm subsidy and price change scenarios can be examined at a single glance (Figure \ref{fig:decisionMaps}). These "policy decision maps" plot the adoption rate, supply change, and producer welfare outcome over a variety of different policy assumption "coordinates". In this case, the maps are expressed in coordinates of assumed efficiency adjusted price ratio change along the x-axis, and assumed subsidy per acre along the y-axis. The map contour lines represent outcome isoquants, in the same sense that contours on a topographical map represent lines of constant elevation. This facilitates evaluation of, for example, the sensitivity of expected adoption rate/supply/welfare outcomes to inaccuracy in the assumed price shock. Policy makers can then also evaluate, at a glance, the additional subsidy required to stay on the same outcome isoquant in the event of such inaccuracy. Additional maps may be made with one or both policy assumptions swapped out for other assumptions, such as assumed rainfall level or price volatility.

<!-- By illustrating the impact of changes in subsidy and assumed price shifts in a readily digestible manner, these maps facilitate intuition-building regarding the effectiveness of changes in subsidy policy and the implications of price changes associated with the intervention. Here  -->
...
\singlespacing
```{r Fig6, fig.show = "hold", fig.width = 7, fig.height=3, fig.align="center", fig.cap="\\label{fig:decisionMaps}Kenya maize farm subsidy policy heat maps of the (left to right) adoption rate, change in aggregate supply, and aggregate producer welfare given release of the technology package described in the text. Each coordinate on the map represents an assumed percentage decrease in the efficiency adjusted output-input price ratio and a net subsidy per acre. The scenario presented in the previous figure above is marked with a black dot. In these maps, the ratio of change in input price to change in output price is fixed at 0.75.", echo = FALSE}
...
gg <- gg + annotate("point", 
           x = dlNuPoint,  # Replace with your desired x-coordinate
           y = dBPoint,  # Replace with your desired y-coordinate
           color = "black",
           size = 2)
gg <- gg + labs(fill = "Adoption rate (%)", x = "Decrease in efficiency adjusted\noutput-input price ratio (%)",
...
gg <- gg + labs(fill = "Change in aggregate\nproducer welfare (million KES)", 
                x = "Decrease in efficiency adjusted\noutput-input price ratio (%)",
                y = "Net subsidy (KES/acre)")
gg <- gg + theme_bw()
gg <- gg + theme(axis.text = element_text(size = axisTextSize),
                 axis.title.x = element_text(size = axisTitleSize),
...
gg <- gg + annotate("point", 
           ...
           color = "black",
           size = 2)
gg <- gg + labs(fill = "Change in aggregate supply\n(1000 MT)",
                x = "Decrease in efficiency adjusted\noutput-input price ratio (%)",
                y = "Net subsidy (KES/acre)")
gg <- gg + theme_bw()
gg <- gg + theme(axis.text = element_text(size = axisTextSize),
...
gg <- gg + annotate("point", 
           ...
           color = "black",
           size = 2)
gg <- gg + labs(fill = "Welfare-tot net subsidy (billion KES)",
                x = "Decrease in efficiency adjusted\noutput-input price ratio (%)",
                y = "Net subsidy (KES/acre)")
gg <- gg + theme_bw()
gg <- gg + theme(axis.text = element_text(size = axisTextSize),
...
gg <- gg + annotate("point", 
           ...
           color = "black",
           size = 2)
gg <- gg + labs(fill = "ProbProfitAdopt",
                x = "Decrease in efficiency adjusted\noutput-input price ratio (%)",
                y = "Net subsidy (KES/acre)")
gg <- gg + theme_bw()
gg <- gg + theme(axis.text = element_text(size = axisTextSize),
...
The illustrative example presented above is an exercise in comparative statics. That is to say, it is a comparison of the "before new tech" (or status quo) and "after new tech" situations, for a single year, without accounting for the time it takes for release and adoption to occur. In a standard impact assessment, the static analysis would be converted into a temporally explicit assessment by introducing a sigmoidal adoption rate curve over time (as explained in Alston et al. [-@alston1995science] and elsewhere); and the welfare impact would then be calculated as the sum of discounted yearly net impacts out to a relevant cutoff year determined by the policy context.

# Discussion and conclusion

The model developed thus far is limited to the evaluation of aggregate supply and producer side welfare impacts. Inclusion of consumer side impacts is fundamental to the technology investment case before donors because producer welfare alone may not offset the government's total subsidy cost (as in the Kenya example above). While this lies outside the scope of the present effort, it would be straightforward to extend the present framework to include explicit modeling of aggregate demand and consumer side impacts---i.e. a full fledged partial equilibrium model---by reinterpreting the input demand curve derived in section \ref{sec:axioms1and2} as the wholesaler's or processing firm's (e.g. miller, starch factory) feedstock demand curve.

Aggregate demand could then be evaluated by integrating this curve over the firm population density, where the density function and domain of integration are determined by axioms 3 and 4. If the number of firms buying from the farm population is small (i.e. a cartel or monopsony, which is often the case at subnational scales), the integral is replaced by a discrete sum of the individual firm demand curves. Once aggregate demand is integrated, output price would become endogenous to the model; and it would then be possible to assess consumer side impacts and producer-consumer tradeoffs (the food-price dilemma).
<!-- As mentioned in section \ref{sec:farmResp}, -->
<!-- In the theoretical development thus far, the technology shock is introduced as an adjustment to TFP. In some cases, it may make more sense to represent the shock as a perturbation to the degree of homogeneity $h$---i.e. a change in one or more of the underlying input efficiencies $\boldsymbol{\alpha}$. A cursory exploration of the matter (not shared here), suggests that shocks to $h$ are considerably more complex than shocks to TFP, requiring a separate, probably lengthy, treatment unto themselves. And so, for now, they remain the subject of future work. -->

A model of agricultural supply and of the supply and welfare impacts of new technology (or any exogenous shock) is useful to agricultural policy makers insofar as it can map out the plausible outcome space for the range of assumptions and policy levers relevant to the discussion at hand; and insofar as this mapping is transparently rooted in an empirically vetted, sensible theoretical foundation.
<!-- (typically as regards price and meteorological conditions) -->

More specifically, this means that a useful model of aggregate supply must explicitly account for 1) the shape of the yield distribution, and hence also provide an endogenous accounting of farm entry and exit in response to price changes; 2) the endogenous farm level supply response to new technology and to any associated exogenous changes in input/output prices and subsidies; 3) the aggregation of farm level response and welfare impacts up to the population level---i.e. the supply curve shift---in a theoretically sensible and transparent way, including endogenous modeling of the adoption rate; and 4) the stochastic nature of price and meteorological conditions.

As mentioned in the Introduction, the conventional approach to modeling aggregate agricultural supply and technology shocks provides no such explicit, theoretically grounded, endogenous accounting. Instead, the shape of the distribution is unceremoniously ignored (or identically endowed marginal firms are assumed), the supply response and adoption rate are assumed as exogenous inputs into the model, essentially synonymous with the technology shock itself; the aggregation of farm level impacts up to the population level is neither empirically nor theoretically grounded, but rather introduced as a matter of fiat; and the stochastic dimension is ignored. As a result, the conventional model is ill suited to answer such basic policy questions such as:

* Upon what input price change assumptions is the assessment based? What is the assumed rainfall level? What is the assumed subsidy level? What happens if rainfall and/or prices are higher/lower than expected?

* By how much will the adoption rate/producer welfare/aggregate supply change if subsidies are increased/lowered, or if the change in TFP turns out to be $10$ percentage points higher/lower than expected?

* Given a certain change in TFP and input/output prices, what subsidy level is required to push adoption over $50$%?

To more squarely meet the needs of agricultural policy makers, here an alternative modeling framework was theoretically derived from five non-controversial axioms and empirically vetted against Kenya farm data. The proposed framework can answer the usual set of policy questions, such as those just enumerated, as well as many others not often considered part of the purview of such models. The impact of changes in income inequality can be examined by varying the standard deviation of expenditure ($s_c$). Sensitivity to output price volatility can be examined by expressing the model in stochastic price parameter space and varying $s_p$. The endogenously modeled economic results are transparently traceable to the five axioms and to the assumed exogenous factors---meteorological conditions and the change in prices and TFP.
 <!-- under different weather and price volatility settings -->

A useful model should also be highly adaptable to the data at hand. The conventional approach depends critically upon econometric estimates of the price elasticities of aggregate supply and demand. The first of these is rarely available for the commodity and geographic focus of the study at hand; and the second is never available. (To address the issue, practitioners are advised to set the to price elasticity of demand to $1/2$ as a matter of fiat [@alston1995science].) Note, moreover, that the present work effectively demonstrates that, under weak axioms, the price elasticity of aggregate supply is not an exogenous constant, but rather an endogenous variable that changes with price. (And one may expect the same result in the derivation of price elasticity of aggregate demand.)
<!-- Supply and demand elasticities aside, the availability of good farm data is of limited benefit in the conventional approach, as it is only used to estimate the status quo mean farm expenditure and yield. parameters of consequence are introduced based on consultations with scientists directly involved in the research under evaluation. And n -->

The model proposed here, on the other hand, is highly adaptable to the available data. When good farm data is available, as in the Kenya example above, results may be disaggregated by TFP strata to explicitly accommodate highly nuanced scenarios regarding technology packages tailored to specific farming systems. For example, for untenured farms, policy makers might consider a technology package bundled with legal services to formalize a title deed, such that TFP on such farms is augmented not just by the new techology coefficient ($\beta_{new}$), but also by the tenure coefficient $\beta_{new} + \beta_{tenure}$. The cost to government in legal services can be added to $\partial B$. If the technology shock is a new seed variety, then the model explicitly accounts for the replacement of any existing seed coefficients ($\beta_{hybrid}$ in hybrid maize systems) with the new seed coefficient ($\beta_{new}$).

When good farm data is unavailable, model parameters can still be estimated up to a respectable degree of precision because the model is expressed in terms of economic indicators and population parameters (the farm employment rate, PEFS-adjusted probability of profit, observed mean and standard deviation of yield and expenditure) that are often deductible from available information and expertise. No data are ever required to estimate supply and demand elasticities because these are endogenous to the model. Model parameters can also be estimated based on output price series, if available, as explained in section \ref{sec:acct4Stoch}.

For the pedagogical purpose of demonstrating the derived model's basic functionality, and for the sake of comparison with previous modeling frameworks, the Kenya example above focuses on a classic SAI yield enhancing technology shock. But the model is equally applicable to AEI shocks prioritizing socioecological resilience and distributional equality over genetic gain. In addition to the aforementioned extension to aggregate demand, an obvious next step for future work may be to test the model's strengths and limitations in these novel areas of application, including assessment of, for example, mixed or complex (e.g. silvopastoril) systems, climate smart agriculture, agroclimatic advisory services, price-risk policy, risk mitigating cooperative social mechanisms (including agtech based microfinance and insurance), payment for economic services, etc., and combinations thereof.
<!-- \ref{sec:aggSup1} -->
<!-- compare numbers of parameters? -->
<!-- \ref{sec:aggImpact} -->
<!-- [The model may used to determine whether there are other cheaper ways to achieve the same goal] -->
<!-- By including aggregate demand, the equilibrium output price becomes endogenous to the model, and consumer welfare outcomes may be evaluated. -->
<!-- ---by pairing the derivation above with a theoretical derivation of the demand curve from the same set of axioms -->
<!-- (Although it should be kept in mind that "consumer welfare" in such instances is, more accurately, processing plant welfare; and thus does not necessarily reflect the welfare that ultimately passes through to the end consumer.) -->
...
\begin{equation}
\begin{split}
y^* &= y|_{x_i = x^*_i} = g \prod_{i = 1}^{n_k} k_i^{\kappa_i} \prod_{i = 1}^{n_x} x_i^{* \alpha_i} \\
...
## B.2 Proof of Equation \ref{eq:keyResult} {-}

For lognormally distributed $\chi$, the probability density function $\rho(\chi)$ is defined
...
\begin{equation}
\int_{0}^{\xi} \chi \rho(\chi) d\chi = \frac{1}{\sqrt{2\pi} s_{\chi}} \int_{0}^{\xi} \chi e^{-\frac{(\ln(\chi) - m_{\chi})^2}{2 s^2_{\chi}}} d \ln(\chi)
\label{eq:proofStep1}
\end{equation}
...
\begin{equation}
\zeta = \frac{\ln(\chi) - m_{\chi}}{s_{\chi}} \: \rightarrow \: d \ln(\chi) = s_{\chi} d \zeta
\end{equation}
...
\begin{equation}
\begin{split}
\int_{0}^{\xi} \chi \rho(\chi) d\chi &= \frac{1}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\xi) - m_{\chi}}{s_{\chi}}} e^{(\zeta s_{\chi} + m_{\chi}) } e^{-\frac{\zeta^2}{2}} d \zeta \\
&= \frac{1}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\xi) - m_{\chi}}{s_{\chi}}} e^{-1/2[\zeta^2 - 2 \zeta s_{\chi} - 2 m_{\chi} ]} d \zeta
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
-1/2[\zeta^2 - 2 \zeta s_{\chi} - 2 m_{\chi} ] &= -1/2 [(\zeta - s_{\chi})^2 - 2 m_{\chi} - s^2_{\chi}] \\
&= -\frac{(\zeta - s_{\chi} )^2}{2} + m_{\chi}  + \frac{s^2_{\chi}}{2}
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\frac{1}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\xi) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\zeta - s_{\chi} )^2}{2} + m_{\chi} + \frac{s^2_{\chi}}{2}} d \zeta &= \frac{1}{\sqrt{2\pi}} e^{m_{\chi}  + \frac{s^2_{\chi}}{2}} \int_{0}^{\frac{\ln(\xi) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\zeta - s_{\chi} )^2}{2}} d \zeta \\
&= \frac{E[\chi]}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\xi) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\zeta - s_{\chi} )^2}{2}} d \zeta \\
...
\begin{equation}
\Phi(\mathcal{Z}) = \frac{1}{\sqrt{2\pi}} \int_{0}^{\mathcal{Z}} e^{-\frac{\psi^2}{2}} d \zeta
\end{equation}
...
\singlespacing
## B.3 Differentiation of expenditure TFP, yield, and expenditure, with respect to TFP {-}
\doublespacing
...
&\rightarrow \frac{\partial \ln(C^*)}{\partial \ln(g)} = \frac{1}{1 - h} \left( 1 + \frac{\partial \ln(P)}{\partial \ln(g)} - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g)} \right)
\end{split}
\end{equation}
...
\begin{split}
\frac{\partial \ln(y^*)}{\partial \ln(g)} &= \frac{\partial \ln(v)}{\partial \ln(g)} - h \frac{\partial \ln(C^*)}{\partial \ln(g)} \\
&= \frac{1}{1 - h} \left( 1 + h \frac{\partial \ln(P)}{\partial \ln(g)} - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g)} \right)
\end{split}
\end{equation}

\singlespacing
# B.4 Proof that optimal unit expenditure is a geometric brownian motion with the same parameters as output price {-}
\doublespacing
...
\begin{equation}
\Delta C^{\#} = \left(\frac{\partial C^{\#}}{\partial t} + m_p P(t) \frac{\partial C^{\#}}{\partial P} + \frac{1}{2} s_p^2 P(t)^2 \frac{\partial^2 C^{\#}}{\partial P^2}  \right) \Delta t + s_p P(t) \frac{\partial C^{\#}}{\partial P} \varepsilon \sqrt{\Delta t}
\label{eq:itoLem}
\end{equation}

Where $\varepsilon$ is a normally distributed random variable with mean $0$ and standard deviation $1$. (For a reference see Hull [-@hull9thEdition].)
...
\begin{equation}
\Delta C^{\#} = m_p C^{\#} \Delta t + s_p C^{\#} \varepsilon \sqrt{\Delta t}
\end{equation}
...
\begin{equation}
\begin{split}
E[\Delta C^{\#} / C^{\#}] &= m_p \Delta t \\
Var[\Delta C^{\#}] &= s^2_p \Delta t
\end{split}
\end{equation}
...
# Appendix C: Kenya farm survey data details and preparation {-}
...
\singlespacing
```{r Fig7, fig.show = "hold", fig.width = 4, fig.height=2, fig.align="center", fig.cap="\\label{fig:rainHist}Histogram of 2007 rainfall anomaly. Values close to 1 (marked by the black vertical line) represent rainfall in line with the historical benchmark. The histogram indicates that rainfall exceeded this benchmark in 2007 for most of the farm sample.", echo = FALSE}

gg <- ggplot(dfMod3, aes(x = exp(`ln[Rain anomaly]`)))
gg <- gg + geom_histogram(color = 4, fill = "white", bins = 15)
gg <- gg + geom_vline(xintercept = 1)
gg <- gg + labs(x = "2007 rain anomaly")
gg <- gg + theme(axis.title = element_text(size = axisTitleSize),
        axis.text = element_text(size = axisTextSize))
gg
...
```{r Fig8, fig.show = "hold", fig.width = 5, fig.height=2, fig.align="left", echo = FALSE}
...
dumVars <- c("Tenure - owned with title deed", "Landprep - oxen", "Landprep - tractor", "Hybrid seed")
ind <- which(!(dfPlot$var %in% dumVars))
dfPlot$val[ind] <- exp(dfPlot$val[ind])
dfPlot$var[-ind] <- paste(dfPlot$var[-ind], "(dummy)")
dumVars <- paste(dumVars, "(dummy)")
dfPlot$var[ind] <- gsub("ln\\[|\\]", "", dfPlot$var[ind])
continVars <- setdiff(unique(dfPlot$var), dumVars)
varOrder <- rev(c(continVars, dumVars))
...
dfPlot$`Stand. dev.`[grep("dummy", dfPlot$var)] <- NA
dfPlot <- dfPlot %>% gather(stat, val, Mean:Min)
gg <- ggplot(dfPlot, aes(x = stat, y = var))
gg <- gg + geom_tile(alpha = 0)
gg <- gg + geom_text(aes(label = val), size = labelSize) +
  scale_x_discrete(limits = c("Mean", "Stand. dev.", "Max", "Min"), position = "top")
gg <- gg + scale_y_discrete(limits = varOrder)
gg <- gg + labs(title = "Table C.1: Summary statistics for the 2007 Kenya long rains\nfarm level maize data.", subtitle = "N = 937")
gg <- gg + theme_classic() +
  theme(axis.title = element_blank(),
        axis.text = element_text(size = axisTextSize),
        plot.title = element_text(size = axisTitleSize),
        plot.subtitle = element_text(size = axisTextSize))
        #strip.text = element_text(size = facetTitleSize))
...
```
<!-- fig.cap="\\label{fig:sumStats}Summary statistics for the 2007 Kenya long rains farm level maize data.", -->
...
<!-- \begin{equation} -->
<!-- E[\ell|\ell < \gamma] = E[\ell] \frac{\Phi \left(\frac{\ln(\gamma) - m_{\phi}}{s_{\phi}} - s_{\phi}\right)}{F(\frac{\ln(\gamma) - m_{\phi}}{s_{\phi}} \right))} \:\:;\:\:\: m_{\phi} = E[\ln(\ell)] \:\: s^2_{\phi} = Var[\ln(\ell)] -->
<!-- \label{eq:condMean} -->
<!-- \end{equation} -->
...
<!-- Moreover, given conditional parameters $\hat{m}_{\phi}$ and $\hat{s}_{\phi}$ estimated from the observed truncated distribution,  -->
...
<!-- The disaggregated $\bar{m}_{c \phi}$ are also computed...and then the conditional mean cost and yield are also evaluated both in the aggregate and disaggregated by strata, as explained in section \ref{sec:TFPstrata}. -->
...
<!-- &= r \gamma (C^* - \hat{C}^*) \:\:;\:\:\: \gamma = \left(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \mathbf{w} \right) \\ -->
<!-- &= \gamma P (y^* - \hat{y^*}) \\ -->
<!-- &= \gamma P v k  (C^{*h} - \hat{C}^{*h}) -->
<!-- \end{split} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \Delta \psi &= N E[k_a] E[k] v P \gamma \int_0^{\bar{\bar{C}^{*}}} (C^{*h} - \hat{C}^{*h}) \rho (C^*) d C^* \\ -->
<!-- &= N E[k_a] E[k] v P \gamma (E[C^{*h}] \mathcal{P}(\hat{C^*}) - \hat{C}^{*h} \mathcal{A}) \:\:;\:\:\: \mathcal{P}(\bar{\bar{C^{\#}}}) = \Phi \left( \frac{\ln(\bar{\bar{C^{\#}}}) - m_{\#}}{s_{\#}} - \eta s_{\#}  \right) \\ -->
<!-- &= N E[k_a] P \gamma (E[y^*] \mathcal{P}(\bar{\bar{C^{\#}}}) - E[k] v \hat{C}^{*h} \mathcal{A}) \\ -->
...
<!-- \end{equation} -->
<!-- $$ -->
<!-- N E[k_a] E[k] v P \gamma (E[C^{*h}] \mathcal{P}(\hat{C^*}) - \hat{C}^{*h} \mathcal{A}) \:\:;\:\:\: \mathcal{P}(\hat{C^*}) = \Phi \left( \frac{\ln(\bar{\bar{C^{\#}}}) - m_{\#}}{s_{\#}} - \eta s_{\#}  \right) -->
<!-- $$ -->
<!-- &= N E[k_a] E[k] v P \gamma (E[C^{*h}] (1 - \Phi(\zeta)) - \hat{C}^{*h} \mathcal{A}) \\ -->
<!-- &= N E[k_a] P \gamma (E[y^*] (1 - \Phi(\zeta)) - E[k] v \hat{C}^{*h} \mathcal{A}) \\ -->
<!-- &= N E[k_a] P \gamma \left( E[y^*] (1 - \Phi(\zeta)) - E[\bar{\bar{y}^*}] \mathcal{A} \right) -->

<!-- By Equation \ref{eq:keyResult2} this can also be written -->

<!-- \begin{equation} -->
<!-- \Delta \psi = N E[k_a] P \gamma \mathcal{A} \left( E[y^*| C^* \leq \hat{C}^{*h}] -  E[\bar{\bar{y}^*}] \right) -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- E[y^*] \Phi \left( \frac{\ln(\hat{C^*}) - m_{c}}{s_c} - h s_c  \right) &= E[y^*| C^* \leq \hat{C^*}] \Phi \left( \frac{\ln(\hat{C^*}) - m_{c}}{s_c} \right) \\ -->
<!-- E[y^*] (1 - \Phi(\zeta)) &= E[y^*| C^* \leq \hat{C^*}] \mathcal{A} -->
<!-- \end{split} -->
<!-- \end{equation} -->
