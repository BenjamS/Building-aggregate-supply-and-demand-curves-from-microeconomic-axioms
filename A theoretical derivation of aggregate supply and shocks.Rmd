...
Parameters unindexed by $\ell$ refer to their values evaluated at the average expenditure TFP $v$.

\begin{equation}
v = \sum_{\ell} \frac{N_\ell}{N} v_{\ell}
\end{equation}

Where $N_{\ell} / N$ is the fraction of farms in the $\ell^{th}$ TFP stratum. That is to say, $E[y^*]$ and $E[C^*]$ refer to the unconditional mean yield and expenditure evaluated at $v$.
...
Denoting the upper adoption bound on $C^*$ (Equation \ref{eq:cStarAdopt}) as $\hat{C^*}$, the adoption rate ($\mathcal{A}$) when $\partial \ln(g) < -\partial \ln(\nu)$ may be expressed as the integral of the density $\rho(C^*)$ over the domain $(0, \hat{C^*})$.

\begin{equation}
\begin{split}
\mathcal{A} &= \int_0^{\hat{C^*}} \rho(C^*) d C^* \\
&= \Phi \left( \frac{\ln(\hat{C^*}) - m_c}{s_c} \right) \\
&= \Phi\left( \frac{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g)} - m_c}{s_c} \right)
\end{split}
\label{eq:AdoptRate}
\end{equation}
...
The adoption rate within a single TFP stratum can be calculated by evaluating this expression at the $v_{\ell}$ of interest.
...
\begin{equation}
\begin{split}
\partial \mathcal{L}^* &= \frac{\lambda}{h} ( C^* \gamma + h \partial B) \:\:; \:\:\: \gamma = \partial g - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w}) + \partial \ln(P) \\
&= r (C^* \gamma + h \partial B) \:\:; \:\:\: r = \frac{\lambda}{h} = \frac{P}{C^{\#}}
\end{split}
\end{equation}
...
To calculate the change in aggregate supply and welfare for each TFP stratum ($\Delta Q_{\ell}$ and $\Delta \psi_{\ell}$), replace $v$, $N$, with $v_{\ell}$, $N_{\ell}$ in the equations above.
...
To more squarely meet the needs of agricultural policy makers, here an alternative modeling framework is theoretically derived from five non-controversial axioms and empirically vetted against Kenya farm data. Because the model explicitly controls for meteorological conditions and price volatility, and disaggregates results by TFP stratum, the proposed model can answer the usual set of policy questions, such as those just enumerated, as well as many others not often considered part of the purview of such models. It can be used to examine highly nuanced scenarios regarding technology packages tailored to specific farming systems under different weather and price volatility settings.
