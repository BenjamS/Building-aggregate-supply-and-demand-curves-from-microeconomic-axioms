...
Given a lognormally distributed random variable $u$, such that $\ln(u)$ is normally distributed with mean $\nu$ and variance $\omega^2$, then the expected value of $u$ is
...
\begin{equation}
f(u) = \frac{1}{u \omega} \phi \left(\frac{\ln(u) - \nu}{\omega} \right)
\end{equation}
...
\begin{equation}
\frac{dp}{du} = \frac{1}{u \omega}\: \rightarrow \: d u = u \omega dp
\label{eq:subThis2}
\end{equation}
...
\begin{equation}
\begin{split}
\int_0^{\bar{u}} u f(u) \: du &= \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{\omega p + \nu} \phi(p) \: dp  \\
&= \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{\omega p + \nu} \frac{1}{\sqrt{2 \pi}} e^{\frac{p^2}{2}} \: dp \\
&= \frac{1}{\sqrt{2 \pi}} \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{p \omega + \nu -\frac{p^2}{2}} \: dp \\
&= \frac{1}{\sqrt{2 \pi}} \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{-1 / 2 (p^2 - 2 p \omega - 2 \nu)} \: dp \\
&= \frac{1}{\sqrt{2 \pi}} \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{-\frac{(p - \omega)^2}{2} + \nu + \frac{\omega^2}{2}} \: dp \\
&= \frac{1}{\sqrt{2 \pi}} \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{-\frac{(p - \omega)^2}{2}} e^{\nu + \frac{\omega^2}{2}} \: dp \\
&= e^{\nu + \frac{\omega^2}{2}} \frac{1}{\sqrt{2 \pi}} \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{-\frac{(p - \omega)^2}{2}} \: dp \\
&= \frac{E[u]}{\sqrt{2 \pi}} \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} e^{-\frac{(p - \omega)^2}{2}} \: dp \\
&= \frac{E[u]} \int_0^{\frac{\ln(\bar{u}) - \nu}{\omega}} \phi(p - \omega) \: dp \\
&= \frac{E[u]} \Phi \left(\frac{\ln(\bar{u}) - \nu}{\omega} - \omega \right)
\end{split}
\end{equation}

Finally, rearranging equation \ref{eq:muU} into an expression for $\nu$

\begin{equation}
\nu = \ln(E[u]) - \frac{\omega^2}{2}
\end{equation}

And substituting this in for $\nu$ in the previous expression gives

\begin{equation}
\begin{split}
\int_0^{\bar{u}} u f(u) \: du &= \frac{E[u]} \Phi \left(\frac{\ln(\bar{u}) - \ln(E[u]) + \frac{\omega^2}{2} }{\omega} - \omega \right) \\
&= \frac{E[u]} \Phi \left(\frac{\ln \left(\frac{\bar{u}}{E[u]} \right) - \frac{\omega^2}{2} }{\omega} \right)
\end{split}
\end{equation}

Since $\ln(\lambda)$ is normally distributed with mean $m_{\lambda}$ and variance $s_{\lambda}^2$, then $\ln(\lambda^{-\theta})$ is also normally distributed with mean $-\theta m_{\lambda}$ and variance $\theta^2 s_{\lambda}^2$. Since $\ln(\lambda^{-\theta})$ is normally distributed, \lambda^{-\theta} may be substituted for $u$, such that $\nu = -\theta m_{\lambda}$, $\omega^2 = \theta^2 s_{\lambda}^2$, and $\bar{u} = h^{-\theta}$, giving the following expression.

\begin{equation}
\int_0^{h^{-\theta}} \lambda^{-\theta} f(\lambda^{-\theta}) \: d \lambda^{-\theta} = E[\lambda^{-\theta}] \Phi \left(\frac{\ln \left(\frac{h^{-\theta}}{E[\lambda^{-\theta}]} \right) - \theta^2 \frac{s_{\lambda}^2}{2}}{\theta s_lambda} \right)
\end{equation}

Moreover, since $\lambda = E[b(T)]\bigr_{t = 0} h$, this can be rewritten

\begin{equation}
\int_0^{h^{-\theta}} \lambda^{-\theta} f(\lambda^{-\theta}) \: d \lambda^{-\theta} = h^{-\theta} E[E[b(T)]\bigr|_{t = 0}] \Phi \left(\frac{\ln \left(\frac{1}{E[E[b(T)]\bigr|_{t = 0}^{-\theta}]} \right) - \theta^2 \frac{s_{Eb(T)}^2}{2}}{\theta s_{Eb(T)}} \right)
\end{equation}

Where $s_{Eb(T)}^2 = Var[\ln(E[b(T)]\bigr|_{t = 0})]$.

Finally, by Rubini's theorem,
...
\begin{equation}
s_{Eb(T)}^2 = Var[\ln(\mu_b(T))]\bigr|_{t = 0}
\end{equation}
...
For an instantaneous (in practice, judiciously small) time step $\Delta t$. (For a reference on geometric Brownian motion, See Hull [-@].)

The mean and variance of the log "return" of $\mu_b(t)$ over period $T$, meanwhile, are
...
\begin{equation}
s_{\mu} = Var\left[\frac{\Delta \mu(T)}{\mu(T) \Delta t} \right]
\end{equation}
...
\begin{equation}
\begin{split}
\int_0^{h^{-\theta}} \lambda^{-\theta} f(\lambda^{-\theta}) \: d \lambda^{-\theta} &= h^{-\theta} E[\mu[b(T)]]\bigr|_{t = 0} \Phi \left(\frac{\ln \left(\frac{1}{E[\mu_b(T)]\bigr|_{t = 0}^{-\theta}]} \right) - \theta^2 \frac{s_{Eb(T)}^2}{2}}{\theta s_{Eb(T)}} \right) \\
&= h^{-\theta} E[\mu[b(T)]]\bigr|_{t = 0} \Phi \left(\frac{\ln \left(\frac{1}{E[\mu_b(T)]\bigr|_{t = 0}^{-\theta}]} \right) - \theta^2 \frac{s_{Eb(T)}^2}{2}}{\theta s_{Eb(T)}} \right)
\end{split}
\end{equation}
...
\begin{equation}
\int_0^{h^{-\theta}} \lambda^{-\theta} f(\lambda^{-\theta}) \: d \lambda^{-\theta} = h^{-\theta} E[E[b(T)]\bigr|_{t = 0}] \Phi \left(\frac{\ln \left(\frac{1}{E[\mu_b(T)]\bigr|_{t = 0}^{-\theta}]} \right) - \theta^2 \frac{s_{Eb(T)}^2}{2}}{\theta s_{Eb(T)}} \right)
\end{equation}
...
If $u$ is a random variable with lognormal density function $f(u)$, then

\begin{equation}
\int_{0}^{\bar{u}} u f(u) \: du = \mu_u \Phi(\bar{\nu}) - \Phi(\underline{\nu})
...
And where $\Phi(\zeta)$ is the standard normal cumulative distribution function, defined
...
\begin{equation}
\bar{\nu} = \frac{\ln \left(\frac{\bar{u}}{E[u]} \right) - \frac{s_u^2}{2}}{s_u}
\end{equation}
<!-- \begin{equation} -->
<!-- \bar{\nu} = \frac{\ln(\bar{u}) - m_u}{s_u} - s_u -->
<!-- \end{equation} -->
...
\begin{equation}
\frac{dv}{d \ln(u)} = \frac{1}{s_u}\: \rightarrow \: d \ln(u) = s_u dv
\label{eq:subThis2}
\end{equation}
