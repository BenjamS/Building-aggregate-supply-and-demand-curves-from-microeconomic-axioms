...
\begin{equation}
\begin{split}
y^*(T) &= y^*(T) = E[y^*(T)] \frac{k_m(T)^{\kappa_m}}{E[k_m(T)^{\kappa_m}]} \frac{k'}{E[k']} \frac{C^{*h}}{E[C^{*h}]} \:\:;\:\:\: k' = k_m(T)^{-\kappa_m} k \\
...
\begin{equation}
E[y^*(T)]|_{t = 0} = E[\tilde{y}^*(T)]|_{t = 0} \left[\frac{k_m(T)^{\kappa_m}}{\widetilde{k_m(T)^{\kappa_m}}} \right]\bigr|_{t = 0} \frac{k'}{E[k']} \frac{C^{*h}}{E[C^{*h}]} \:\:;\:\:\: \tilde{y}^*(T) = E[y^*(T)] \:\:\: \widetilde{k_m(T)^{\kappa_m}} = E[k_m(T)^{\kappa_m}]
\label{eq:EyStarT0}
\end{equation}
...
\begin{equation}
E[\tilde{y}^*(T)]|_{t = 0} = E[\tilde{y}^*(T) | E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}]|_{t = 0} \frac{\Phi\left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} \right)}{\Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)}
...
\begin{equation}
E[\tilde{y}^*(T)]|_{t = 0} = E[\tilde{y}^*(T) | E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}]|_{t = 0} \frac{1 - \Phi(z - \eta s_p \sqrt{T})}{1 - \Phi(z)}
...
\begin{equation}
\begin{split}
E[\tilde{y}^*(T)]|_{t = 0} &= E[y^*(T) | E[C^{\#}(T)]|_{t = 0} \leq P(T)]|_{t = 0} \frac{\text{Prob}(E[C^{\#}(T)]|_{t = 0} \leq P(T))}{\text{Prob}(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0} \leq P(T)^{\eta})} \\
...
# An illustrative use case: the maize aggregate supply curve in Kenya

## Parameterization of Kenya's maize aggregate supply curve and comparison to empirical estimates
...
(Where the last line follows from the input demand expression in Equation \ref{eq:inputDem2}.) The TFP constant $g_0$ may include endowment dummy variables that augment the constant on farms where the endowment is present.
...
To evaluate the expected unconditional mean yield term ($E[\tilde{y}^*(T)]|_{t = 0}$) in the expected aggregate supply expression (Equation \ref{eq:aggSup2}), the mean of the fitted values for yield are interpreted as the observed conditional mean yield. 

\begin{equation}
E[y^*(T)|E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}] = E[\hat{y}] \:\:;\:\:\: \hat{y} = y^*(T)e^{\epsilon}
\end{equation}

(Where $\epsilon$ is the regression residual.) The stochastic expectation of the conditional mean yield given expected meteorological condition $E[\widetilde{k_r(T)^{\kappa_r}}]|_{t = 0}$ can then be calculated as

\begin{equation}
E[\tilde{y}^*(T)|{E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}}]|_{t = 0} = \frac{E[\widetilde{k_r(T)^{\kappa_r}}]|_{t = 0}}{\widetilde{k_r(T)^{\kappa_r}}} \tilde{y}^*(T)|E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}
\label{eq:EyStarFromYT}
\end{equation}

Where the notation $\tilde{y}^*(T)|\{E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}\}$ has been introduced to stand for $E[y^*(T)|E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}]$.
<!-- $$ -->
<!-- E[\tilde{y}^*(T; E[\widetilde{k_r(T)^{\kappa_r}}]|_{t = 0})|{E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}}]|_{t = 0} = \frac{E[\widetilde{k_r(T)^{\kappa_r}}]|_{t = 0}}{\widetilde{k_r(T)^{\kappa_r}}} \tilde{y}^*(T)|E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0} -->
<!-- $$ -->
<!-- $$ -->
<!-- E[\tilde{y}^*(T)|{E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}}]\Bigr|_{\substack{\widetilde{k_m^{\kappa_r}} \\ t = 0}} = \frac{E[\widetilde{k_r(T)^{\kappa_r}}]|_{t = 0}}{\widetilde{k_r(T)^{\kappa_r}}} \tilde{y}^*(T)|E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0} -->
<!-- $$ -->

The observed optimal expected unit cost is then calculated as

\begin{equation}
E[C^{\#}(T)]|_{t = 0} = \frac{C^*}{E[y^*(T)|E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}]|_{t = 0}}
\end{equation}
E[\tilde{y}^*(T)|{E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}}]|_{t = 0}
As explained in \ref{sec:aggSupDeterm}, axiom 3 truncates the observed distribution of optimal expected unit cost at the expected output price. The conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ are calculated from this observed, truncated distribution; and then the unconditional parameters $m_{\#}$ and $s_{\#}$ are calculated by solving simultaneous Equations \ref{eq:cSharpUncondParams}. In addition to $\bar{m}_{\#}$ and $\bar{s}_{\#}$, this requires an estimate of expected harvest price at planting time ($E[P(T)]|_{t = 0}$).
...
Now, because the binary endowments $b_i$ vary across the population, the value of $v_0$ also varies. In the Kenya example, there are 14 distinct values of $v_0$, corresponding to 14 distinct combinations of the $\beta_i b_i$. The supply aggregation step in Equation \ref{eq:aggSupStep} effectively averages aggregate supply across all binary endowment strata. Denoting the value of each distinct combination as $v_{0i}$, the average $v_0$ is effectively a population weighted sum of the $v_{0i}$.
...
In passing, note that mean cost and yield disaggregated by stratum ($E[C_i^*]$, $E[y_i^*(T)]$), can be evaluated as follows.

\begin{equation}
\begin{split}
E[C_i^*] &= e^{m_{ci} + \frac{s^2_c}{2}} \:\:;\:\:\: m_{ci} = \frac{1}{1 - h}(\ln(v_{0i}) + m_k + m_{\#})  \\
...
\begin{equation}
E[y_i^*(T)] = v_{0i} E[k] E[C_i^{*h}]E[e^{\epsilon}]_i
\end{equation}

Where $E[e^{\epsilon}]_i$ refers to the regression errors of farms in the $i^{th}$ stratum. The ex-ante expected mean yield $E[\tilde{y}_i^*(T)]|_{t = 0}$ can also be deduced as in Equation \ref{eq:EyStarT0fromYhat}.
<!-- , since it is calculated based on the fitted value of $v_{0i}$ -->

\begin{equation}
E[\tilde{y}_i^*(T)]|_{t = 0} = \left( \frac{E[k_r(T)]|_{t = 0}}{E[k_r]_i} \right)^{\kappa_r} E[y_i^*(T) E[e^{-\epsilon}]_i
\end{equation}
...
# A = \Phi\left( \frac{\frac{-h (1 - h) \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} - (\ln(v_0) + m_k + m_{\#})}{\sqrt{s_k^2 + s_{\#}^2}} \right)
dfPlot$`Mean yield` <- round(muYstarT, 1)
dfPlot$`Mean cost/acre` <- round(muCstar, 1)
dfPlot$`Adoption rate` <- round(adoptRateStrat, 2)
dfPlot <- dfPlot %>% rename(`TFP (v0)` = v0) %>%
  relocate(`TFP (v0)`, `Mean cost/acre`, `Adoption rate`) %>%
...
gg <- ggplot(dfPlot, aes(x = var, y = Type, fill = val, label = val))
...
dfCheck <- data.frame(Type = endowVec, v0 = round(v0, 1),
                      yd = exp(dfMod3$`ln[Yield (kg/acre)]` - mod3$residuals - kRn * dfMod3$`ln[Rain anomaly]`),
