...
The fitted binary endowment coefficients ($\beta_i$) indicate that hybrid maize seed is the most significant contributor to TFP, followed by land prep method and tenure. Based on these values, the expenditure TFPs are calculated for each of the 14 binary endowment strata ($v_{\ell}$), and the average across all strata is also computed ($v$). The conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ are calculated from the fitted values $v$ and $h$, and the sample parameters $\bar{m}_c$, $\bar{s}_c$, $m_k$, and $s_k$, by Equation \ref{eq:mCshrpsCshrp}. The disaggregated $\bar{m}_{c \ell}$ are also computed...and then the conditional mean cost and yield are also evaluated both in the aggregate and disaggregated by strata, as explained in section \ref{sec:binEndStrat}.
...
#--------------------------------------------------------------------------
dfx$Type <- endowVec
dfStrat <- dfx %>% group_by(vStrat) %>% summarise(Type = unique(Type),
                                                mCstar = mean(lcStar),
                                                 sCstar = sd(lcStar),
  ...
  mutate(muEyStarT0truncStrat = vStrat * muK * muCstarHtruncStrat) %>%
...
dfAll <- dfx %>% summarise(v = mean(vStrat),
                              ...
                              muKstrat = mean(k)) %>%
  mutate(muCstarTruncStrat = exp(mCstar + sCstar^2 / 2),
         muCstarHtruncStrat = exp(h * mCstar + h^2 * sCstar^2 / 2)) %>%
  mutate(muEyStarT0truncStrat = mean(vStrat) * muK * muCstarHtruncStrat) %>%
  mutate(stratID = "All")
...
# benchmark
dfStratTab <- dfStrat %>% select(vStrat, muCstarTruncStrat, muYdHarv, muEyStarT0truncStrat)
tabHeaders <- c("Expenditure\nTFP",
                "Mean observed\ncost/area\n(KES/acre)",
                "Mean observed\nharvest yield\n(kg/acre)",
                "Mean implicit\nexpected yield\n(kg/acre)")
colnames(dfStratTab) <- tabHeaders
dfAllTab <- dfAll %>% select(v, muCstarTruncStrat, muYdHarv, muEyStarT0truncStrat)
colnames(dfAllTab) <- tabHeaders
#==========================================================================
...
dfStratTab$Type <- dfStratPct$Type
dfTab <- dfStrat %>% rename(`Expenditure\nTFP` = vStrat) %>%
  relocate(`Type`, `Expenditure\nTFP`, `Mean cost/area\n(KES/acre)`, `Mean yield\n(kg/acre)`, `Mean implicit\nexpected yield\n(kg/acre)`) %>%
  select(`Type`, `Expenditure\nTFP`, `Mean cost/area\n(KES/acre)`, `Mean yield\n(kg/acre)`, `Mean implicit\nexpected yield\n(kg/acre)`) %>% gather(var, val, `Expenditure\nTFP`:`Mean yield\n(kg/acre)`)
