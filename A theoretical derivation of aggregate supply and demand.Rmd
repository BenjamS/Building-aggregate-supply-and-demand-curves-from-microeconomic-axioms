...
Because of Axiom 4, the distribution of $C^{\#}$ is right truncated at $P$. Because $y^*$ is a function of $C^{\#}$, then $y^*$ is also truncated for given $k$. The population mean $E[y]$ is thus never observed except in the limit as $P$ becomes very large. The observed mean is the conditional mean $E[y^*|C^{\#} < P]$. For any random variable $\ell$ with probability density $f(\ell)$ and cumulative distribution function $F(\ell)$, the conditional mean of $\ell$ right truncated at $\gamma$ is
...
Moreover, given conditional parameters $\tilde{m}_{\ell}$ and $\tilde{s}_{\ell}$ estimated from the observed truncated distribution, the unconditional parameters can be deduced by solving the following system of equations.

\begin{equation}
\begin{split}
m_{\ell} &= \tilde{m}_{\ell} - \tilde{s}_{\ell} \frac{\phi(z)}{\Phi(z)} \\
s_{\ell}^2 &= \tilde{s}_{\ell}^2 \left( 1 - z \frac{\phi(z)}{\Phi(z)} - \left( \frac{\phi(z)}{\Phi(z)} \right)^2   \right) \:\:;\:\:\: \frac{\ln(\gamma) - m_{\ell}}{s_{\ell}} \:\:\:;\:\: z = \frac{\ln(\gamma) - m_{\ell}}{s_{\ell}}
\end{split}
\end{equation}
...
Substituting $C^{\# \eta}$ for $\ell$ and $P$ for $\gamma$,
...
The conditional mean of $y^*$ truncated at $C^{\#} < P$ can thus be parsed as follows.
