...
Because the adjustment $\eta$ is the output price elasticity of farm supply (PEFS), the probability term $1 - \Phi(z)$ is henceforth referred to as the PEFS-adjusted mean probability of profit. Substituting this for the $\Phi()$ term in Equation \ref{eq:aggSup2}, the aggregate supply function may be rewritten
...
Those familiar with financial analysis might notice a similarity between $\Phi(z)$ and the first $\Phi()$ term (sometimes referred to as the $N_1$ term) in the Black-Scholes equation []... similar to risky bet in the money, out of the money, at the money. Here it is tempting to introduce a sixth axiom saying that farmers will only pursue a risky bet if the probability of profit is greater than that of a coin toss (i.e., greater than $50\%$), and to thereby limit the domain of $z$ to $(-\infty, 0)$. But this is left to the discretion of the practitioner. [There are cases when rational economic agents pursue "out of the money" bets.]
