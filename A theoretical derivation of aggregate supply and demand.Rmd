...
Henceforth it is necessary to be more specific in the treatment of yield $y^*$ and output price $P$ as stochastic variables. Absent a forward contract, the output price at the time of harvest and sale $(t = T)$ is unknown to the farmer at the time of planting and input expenditures $(t = 0)$. Likewise, yield is largely dependent upon numerous stochastic meteorological processes occurring between planting and harvest---particularly temperature, solar radiation, and humidity---which are unknown to the farmer at the time of planting. Wherever $P$ and $y^*$ appear in the equations above, then, what is really meant is $E[P(T)]\bigr|_{t = 0}$ and $E[y^*(T)]\bigr|_{t = 0}$. That is, farmers make their management decisions at planting time ($t = 0$) based on what they expect the values of $y^*$ and $P$ to be at harvest ($t = T$).

The farm budget shadow price $\lambda$ is thus more accurately written

\begin{equation}
\lambda = h \frac{E[P(T)]\bigr|_{t = 0} E[y^*(T)]\bigr|_{t = 0}}{C^*} = h E\left[ \frac{P(T)]}{C^{\#}} \right]\Bigr|_{t = 0}
\label{eq:lambda4}
\end{equation}
...
```{r Fig1, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="center", fig.cap="\\label{fig:gBmEgs}(Left) Maize price time series from (top to bottom) Eldoret market, Kenya (downloaded using the FAO Food Price Monitoring and Analysis Tool), Johannesburg Stock Exchange, South Africa, and the Chicago Board of Trade, USA. The Kenya series is monthly, while the South African and US series are daily. (Right) The corresponding daily log returns (monthly log returns in the case of Kenya) exhibit the normal distribution typical of geometric Brownian motion.", echo = FALSE}
...
The log return of a geometric Browning motion is defined over finite time intervals $\ln(P(T) / P(\hat{t}))$ for some starting time $\hat{t}<T$, and is also normally distributed, with mean and variance
...
Examples of maize prices following a geometric Brownian motion are given from different parts of the world in Figure \ref{fig:gBmEgs}.

<!-- [atmospheric processes] -->
The modeling of stochastic meteorological processes is slightly less straightforward. Weather derivatives are increasingly priced based on    Geometric Brownian motion is also known to approximate the time evolution of certain meteorological variables such as temperature, solar radiation, and humidity, which, together with management decisions, drive production. Axiom 4 is the assumption that production and output price follow independent geometric Brownian motions.

<!-- Letting $b(T)$ stand for the ratio $P(T) / C^{\#}(T)$, this means that the log return $\ln(b(T) / b(\hat{t}))$ is normally distributed with mean and variance -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
