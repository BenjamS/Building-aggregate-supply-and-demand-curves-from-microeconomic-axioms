...
The key difference between the convention and the alternative approach developed here is that the present approach is built upon a transparent _endogenous_ accounting of the research induced supply and expenditure response to the exogenous technology shock. [and transparent aggregation]

ENDOGENOUS ADOPTION RATE
...
In the Kenya maize farm exercise farther below, for example, there are four ($n_b = 4$) binary endowment variables $b_i$ (which take a value of $1$ if the endowment is present and $0$ otherwise): use of oxen in land preparation, use of tractor in land preparation, use of hybrid maize seed, and land tenure. This means that $g$ can vary across the population. In the Kenya maize exercise, there are 14 unique values of $g$ in the population, corresponding to 14 unique combinations of the four binary endowments plus the regression constant.
...
Letting $C^{\#}$ denote optimal unit expenditure (also commonly referred to as the "average cost", $C^{\#} = C^* / y^*$), note that $C^*$ and $y^*$ can be rewritten in terms of $C^{\#}$ as follows.

\begin{equation}
\begin{split}
C^* = C^{\#} y^* &= C^{\#} v k C^{* h} \\
&rightarrow C^* = (v k C^{\#})^{\frac{1}{1 - h}} \\
&rightarrow y^* = (v k)^{\frac{1}{1 - h}} C^{\# \frac{h}{1 - h}}
\end{split}
\label{eq:fnCsharp}
\end{equation}
...
Assuming yield is lognormally distributed (axiom 4), then so must be $C^*$ and $C^{\#}$; and the expression reduces to
...
It is important to keep in mind that $E[y^*]$, $m_{\#}$, and $s_{\#}$ are unconditional population parameters. That is to say, $E[y^*]$ refers to the population mean yield; and $m_{\#}$ and $s_{\#}$ refer to the population mean and standard deviation of $\ln(C^{\#})$. But axiom 3 truncates the upper tail of the distribution of $C^{\#}$ at $P$. Within this axiomatic framework, then, sample parameters calculated from observational data must be interpreted as conditional parameters. That is to say, the sample mean of yield does not represent $E[y^*]$ but rather the conditional mean $E[y^*|C^{\#} \leq P]$; and likewise for the sample parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$,
...
Because $C^{\#}$ is lognormally distributed (axiom 4), then the unconditional parameters $m_{\#}$ and $s_{\#}$ can be deduced from the conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ by numerically solving the following system of equations.
...
Moreover, the conditional and unconditional means of a lognormally distributed variable $\chi$ with upper bound $\xi$ are related as follows.
...
\begin{equation}
E[C^{\# \eta} | C^{\#} \leq P] \mathcal{E} = E[C^{\# \eta}] \Phi \left(\frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#}\right)
\end{equation}

Considering that the conditional mean of $y^*$ follows from the yield expression in Equations \ref{eq:fnCsharp} as

\begin{equation}
\begin{split}
E[y^*|C^{\#} \leq P] &= E \left[ E[y^*] \frac{k^{\eta / h}}{E[k^{\eta / h}]}\right] \frac{E[C^{\# \eta} | C^{\#} \leq P]}{E[C^{\# \eta}]} \\
&= E[y^*] \frac{\Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)}{\mathcal{E}}
\end{split}
\end{equation}

Then a tractable expression for the unobservable, unconditional mean yield $E[y^*]$ follows from Equation \ref{eq:keyResult2} as a function of the observed or derived parameters $E[y^*|C^{\#} \leq P]$, $m_{\#}$, $s_{\#}$, and $\eta$.

\begin{equation}
E[y^*] = E[y^*|C^{\#} \leq P] \frac{\mathcal{E}}{\Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)}
...
\begin{equation}
E[C^*]  = E[C^*|C^{\#} \leq P] \frac{\Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} \right)}{\Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)}
\label{eq:condMeanYd1}
\end{equation}

Note differ by stratum,
...
The increase is thus the same across all adopting farms regardless of the starting value $g$. Ideally, the value of $\beta_5$ should be estimated directly from field trials. If this is not possible, then the estimate can be made on the basis of agronomic crop modeling and expert opinion. Regardless of the path taken in this exercise, it is critically important to keep in mind that the objective is to estimate the change in TFP ($\beta_5$ or $\partial \ln(g)$), not the change in economic yield itself ($\partial \ln(y^*)$). If information is gathered in terms of the change in agronomic yield and/or expenditure, then it must be converted into an estimate for $\partial \ln(g)$. The percentage change in economic yield, expenditure, and expenditure TFP ($v$) then follows from Equations \ref{eq:fnCsharp} as a function of the change in TFP as well as any change in output and input prices.
...
The key difference between the convention and the alternative approach developed here is that the present approach is built upon a transparent _endogenous_ accounting of the farm supply and expenditure response to the exogenous technology shock. [and transparent aggregation]
<!-- The key factor distinguishing the present work from conventional framework is then that the present work may be characterized as built around a careful endogeneous modeling of the yield and expenditure response to technology change. (And careful modeling of its aggregation up to a aggregate supply curve) and so transparent about the decomposition of the yield and expenditure shift into tfp and price, clearly listing the assumed price change and why it has certain aggregate impacts. -->

This is well documented and obvious from the theoretical framework developed thus far but bears emphasis as it is still commonplace in practice to hear talk of the "exogenous yield shock", or in discussions, for example, of yield gaps .
...
\begin{equation}
\begin{split}
\frac{\mathcal{L}^*}{\partial \ln(g)} &= \frac{\lambda}{h} \left((1 - h) \frac{\partial C^*}{\partial \ln(g)} + h \frac{\partial B}{\partial \ln(g)} \right) \\
...
Where $m_c$ and $s_c$ follow from the expenditure expression in Equations \ref{eq:fnCsharp}.

\begin{equation}
\begin{split}
m^i_c &= \frac{1}{1 - h}(\ln(v_i) + m_k + m_{\#}) \:\:;\:\:\: m_k = E[\ln(k)] \\
...
STRATIFICATION HERE
...
because the binary endowments $b_i$ vary across the population, the value of $g$ (and hence of $v$) also varies. In the present example, there are 14 distinct values of $v$, corresponding to 14 distinct combinations of the $\beta_i b_i$. The supply aggregation step in Equation \ref{eq:aggSupStep} averages aggregate supply across all binary endowment strata. Denoting the value of each distinct combination as $v_{0i}$, the average $v$ is effectively a population weighted sum of the $v_{0i}$.
<!-- Each value of $v$ effectively identifies a binary endowment stratum of the population. -->

\begin{equation}
v = \sum_i \frac{N_i}{N} v_{0i}
\end{equation}
...
&= N E[k_a] v^{\eta / h} \int_{0}^{\infty} k^{\eta / h} \int_{0}^{\bar{C}^{\#}} C^{\# \eta} \rho(C^{\#})  d C^{\#} \rho(k) dk \:\:;\:\:\: \eta = \frac{h}{1 - h} \\
&= N E[k_a] v^{\eta / h}  E[k^{\eta / h}] \int_{0}^{\bar{C}^{\#}} C^{\# \eta} \rho(C^{\#}) d C^{\#}
\end{split}
\label{eq:aggSupStep1}
\end{equation}
<!-- \frac{E[y^*]}{E[k] E[C^{* h}]} -->

Taking the expectation of the yield expression in Equations \ref{eq:fnCsharp} and rearranging,

\begin{equation}
v^{\eta/h} = \frac{E[y^*]}{E[k^{\eta / h}] E[C^{\# \eta}]}
\end{equation}

Such that the previous expression can be simplified to

\begin{equation}
Q = N E[k_a] \frac{E[y^*]}{E[C^{\# \eta}]} \int_{0}^{\bar{C}^{\#}} C^{\# \eta} \rho(C^{\#}) d C^{\#}
\label{eq:aggSupStep2}
\end{equation}

To resolve the integral on the right hand side, note that for any lognormally distributed variable $\chi$ and upper bound $\xi$,
...
Because $y^*$ is assumed to be lognormally distributed (axiom 4), then $C^{\# \eta}$ must also be lognormally distributed. Substituting $\chi = C^{\# \eta}$ and $\xi = \bar{C}^{\#}$, and recalling that $\bar{C}^{\#} = P$ (axiom 3),
...
The relation between conditional and unconditional mean yield (Equation \ref{eq:condMeanYd2}) can thus be rewritten as follows.
...
<!-- &= N E[k_a] E[y^*|C^{\#} \leq P] \mathcal{E} -->
<!-- \end{split} -->
<!-- \end{equation} -->

Which may be useful for point estimates of aggregate supply along the curve since $E[y^*|C^{\#} \leq P]$ can be estimated from the observed yield distribution. This form is also instructive because the $\Phi()$ term has a straightforward interpretation as the farm employment rate $\mathcal{E}$. However, it is inconvenient for purposes of drawing the entire aggregate supply curve, as the observed $E[y^*|C^{\#} \leq P]$ is valid only for the $P$ at which it is measured. The unconditional mean yield $E[y^*]$, on the other hand, is constant for all $P$. For this reason, Equation \ref{eq:aggSup1} is preferred. Moreover, the $\Phi()$ term in Equation \ref{eq:aggSup1} turns out to have a meaningful economic interpretation once the stochastic character of price is taken into account, as shown below.
<!-- \begin{equation} -->
...
$$
\ln(g) = \beta_0 + \sum_{i = 1}^{n_b} \beta_i b_i
$$
<!-- \begin{equation} -->
<!-- \ln(g) = \alpha_0 = \beta_0 + \sum_{i = 1}^{n_b} b_i \beta_i -->
<!-- \label{eq:g0Sum} -->
...
As mentioned in section \ref{sec:axioms1and2}, in this exercise the binary endowment variables $b_i$ (which take a value of $1$ if the endowment is present and $0$ otherwise) are: use of oxen in land preparation, use of tractor in land preparation, use of hybrid seed, and land tenure. There are 14 unique combinations of these four endowments in the data, representing different endowment strata.
...
The expenditure TFP, mean observed cost, and mean yield are presented in Table ... , disaggregated by...
...
\end{equation}
*--
...In Figure \ref{fig:v0strata}... MOVE strat TABLE HERE
[at what rain level?]
...
Estimation of the unconditional parameters $m_{\# i}$ and $s_{\#}$, requires the conditional parameters $\bar{m}_{\#i}$, $\bar{s}_{\#}$ computed from the observed farm data and expected harvest price at planting time for the year of the data. The conditional parameters $\bar{m}_{\#i}$ and $\bar{s}_{\#}$ can be computed from the farm expenditure ($C^*$) and exogenous factor ($k$) data based on Equations \ref{eq:mCsC}.
<!-- \begin{equation} -->
<!-- \begin{split} -->
...
\begin{equation}
\begin{split}
\bar{m}_{\#i} &= (1 - h) \bar{m}_c - m_k - \ln(v_i) \\
...
The unconditional parameters $m_{\#}$ and $s_{\#}$ can then be computed by solving simultaneous Equations \ref{eq:cSharpUncondParams} at $E[P(T)]|_{t = 0}$ (the expected price at planting time for 2007, the same year as the farm data used to estimate $\bar{m}_{\#}$ and $\bar{s}_{\#}$).
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \partial \ln(E[P(T)]|_{t = 0}) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g) + \frac{h}{C^*} \partial B &> 0 \\ -->
...
<!-- To parse $m_c$ and $s_c$, recall from Equation \ref{eq:mCk} that $C^* = (v k E[C^{\#}(T)]|_{t = 0})^{1 / (1 - h)}$, such that -->
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- m_c &= \frac{1}{1 - h}(\ln(v) + m_k + m_{\#}) \:\:;\:\:\: m_k = E[\ln(k)] \:\: s_k^2 = Var[\ln(k)] \\ -->
<!-- s_c^2 &= \frac{1}{(1 - h)^2} (s_k^2 + s_{\#}^2) -->
<!-- \end{split} -->
<!-- \label{eq:mCsC} -->
<!-- \end{equation} -->
<!-- Recalling from Equation \ref{eq:mCk} that $C^* = (v k C^{\#})^{1 / (1 - h)}$, then $m_c = 1 / (1 - h)(\ln(v) + m_k + m_{\#})$ and $s_c^2 = 1 / (1 - h)^2 (s_k^2 + s_{\#}^2)$ (where $m_k = E[\ln(k)]$ and $s_k^2 = Var[\ln(k)]$); and so the adoption rate can be written in terms of known parameters. -->
...
&= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \int_0^{\bar{\bar{y^*}}} y^* \rho(y^*) dy^* \\
&= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) v_{0i} E[k] \int_0^{\bar{\bar{C^*}}} C^{*h} \rho(C^*) dC^* \\
&= N_i E[k_a] v_{0i} E[k] E[C^{*h}]_i \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_c}{s_c} - h s_c  \right)
\end{split}
\end{equation}
<!-- $$ -->
<!-- N_i E[k_a] v_{0i} E[k] E[C^{*h}]_i \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_c}{s_c} - h s_c  \right) -->
...
\begin{equation}
\Delta Q_i = N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right)
\label{eq:deltQ}
\end{equation}
<!-- $$ -->
<!-- \Delta Q_i = N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right) -->
...
For purposes of evaluation of $\Delta Q_i$ based on farm data, this is a useful expression. Another instructive expression can be found by expressing the $\Phi()$ term in stochastic price parameter space. To do this, first recall that $C^* = (v k C^{\#})^{\eta / h}$ (Equation ...), such that the argument of $\Phi()$ can be rewritten
...
\begin{equation}
\begin{split}
\Delta Q_i &= N_i E[k_a] E[\tilde{y}^*(T, k_m(T))]_i|_{t = 0} \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) (1 - \Phi ( \zeta )) \\
&= N_i \mathcal{A}_i E[k_a] E[\tilde{y}^*(T, k_m(T))| C^* \leq \bar{\bar{C^*}}]_i|_{t = 0} \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w}))
\end{split}
\end{equation}
...
<!-- \begin{split} -->
<!-- \Delta Q_i &= N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right) \\ -->
<!-- &= N_i \mathcal{A}_i E[k_a] E[\tilde{y}^*(T, k_m(T))]_i \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) (1 - \Phi ( \zeta ) \:\:;\:\:\: \zeta = \frac{\ln \left( \frac{E[\tilde{C}^{\#}(T)]|_{t = 0}}{\bar{\bar{C^{\#}(0)}}} \right) - \left( m_p - \eta \frac{s_p^2}{2} \right) T}{s_p \sqrt{T}} -->
<!-- \end{split} -->
<!-- \end{equation} -->
<!-- \begin{equation} -->
<!-- \Delta Q_i &= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) E[\tilde{y}^*(T, k_m(T))| C^* \leq \bar{\bar{C^{\#}(0)}}]_i \mathcal{A}_i -->
<!-- \end{equation} -->
...
The marginal change in farm net benefit given a percentage change in $g$ follows as

\begin{equation}
\begin{split}
\frac{\partial \mathcal{L}^*}{\partial \ln(g)} &= r \left((1 - h) \frac{\partial C^*}{\partial \ln(g)} + h \frac{\partial B}{\partial \ln(g)} \right) \\
&= r \left( C^* \left(1 - \boldsymbol{\alpha} ' \frac{\partial \mathbf{w}}{\partial \ln(g)} \right) + h \frac{\partial B}{\partial \ln(g)} \right)
\end{split}
\end{equation}

The farm level change in net benefit per unit area given the intervention $\partial \ln(g)$ may thus be expressed

\begin{equation}
\begin{split}
\partial \mathcal{L}^* &= r ( C^* \left(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \mathbf{w} \right) + h \partial B ) \\
...
Specifically, then, the scaling shift can be modeled by shocking the parameter $u$. This results in an unambiguous increase in aggregate supply. That is to say, it is straightforward to see that
...
The adoption condition for a new scaling technology is $\frac{\partial \mathcal{L}^*}{\partial u}$.
...
<!-- \begin{equation} -->
<!-- E[\tilde{y}^*(T)]|_{t = 0} = u \frac{E[C^{*h}]}{E[C^*]^h} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- E[\tilde{y}^*(T)]|_{t = 0} &= u \frac{e^{hm_c + h^2 \frac{s^2_c}{2}}}{e^{hm_c + h \frac{s^2_c}{2}}} = u e^{\frac{s^2_c}{2}(h^2 - h)} \\ -->
<!-- &= u e^{\frac{1}{2} (\eta + 1)^2 (s^2_k + s^2_{\#}) (h^2 - h)} -->
<!-- \end{split} -->
<!-- \label{eq:muYdStoch1} -->
<!-- \end{equation} -->
