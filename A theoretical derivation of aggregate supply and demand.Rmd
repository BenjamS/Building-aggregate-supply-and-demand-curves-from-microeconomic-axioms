...
Parameterization of the aggregate supply curve derived above requires knowledge of the crop expected output price at time of planting $P(0)$, the price growth rate $m_p$ and volatility $s_p$, the length of the cropping season $T$, the number of farms in the population $N_f$, the output degree of homogeneity $h$, the expected mean cost per unit output as evaluated at planting time $E[C_0^{\#}(T)]|_{t = 0}$, and the expected mean harvested output as evaluated at planting time $E[\tilde{q}^*(T)]\bigr|_{t = 0}$. The last of these may of course be further decomposed as $E[A] E[\tilde{y}(T)]\bigr|_{t = 0}$, where $E[A]$ is the mean farm size and $\tilde{y}(T)$ is just the $\tilde{y}$ from Equation \ref{eq:yConst}, with the time of evaluation (harvest) made explicit.

When estimating $E[C_0^{\#}(T)]|_{t = 0}$ and $E[y^*(T)]\bigr|_{t = 0}$ based on farm survey data, it may be tempting to use the respective sample means. This would be erroneous in two ways: 1) The sample mean gives the actual values known post-harvest, whereas the model deduced above requires the farmers' expectation at planting time. And 2) recall that Axiom 3 implies that $C^{\#} < E[P(T)]\bigr|_{t = 0}$, which in turn implies that the sample means are over a truncated distribution.

Given the assumption of lognormally distributed yield (Axiom 4), then, by linear independence of the logged regressors in Equation \ref{eq:cobbDoug4}, $C^{\#}$ is also lognormally distributed, such that

\begin{equation}
C_0^{\#}(T) = e^{m_c + \frac{s_c^2}{2}}
\end{equation}

Where the unobserved mean and variance of the untruncated distribution ($m_c$, $s_c^2$) can be implicitly calculated from the observed (i.e. sample) mean and variance ($\overline{m}_c$, $\overline{s}_c^2$) by the following formulae.
...
If the available farm survey data contains control variables ($k_i$), such that the model in Equation \ref{eq:cobbDoug4} can be estimated (making sure to use the untruncated mean of $C^{\#}$), then the mean yield at harvest $\tilde{y}(T)$ can be calculated directly from the estimated regression constant.
