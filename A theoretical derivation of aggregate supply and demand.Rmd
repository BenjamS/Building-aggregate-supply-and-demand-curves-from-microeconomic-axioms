...
The total increase in aggregate supply can be evaluated by integrating the change in farm production over the density of adopting farms $(0, \bar{\bar{y^*}})$ where $\bar{\bar{y^*}} = y^*(\bar{\bar{C^*}})$.
...
&= N E[k_a] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \int_0^{\bar{\bar{y^*}}} y^* \rho(y^*) dy^* \\
...
Where the resolution of the integral in the last line follows from Equation \ref{eq:keyResult}. Recalling that $v = E[y^*] / (E[k] E[C^{*h}])$ (Equation \ref{eq:v0i}), this further reduces to
...
&= \gamma P (y^* - \bar{\bar{y^*}}) \\
\partial \mathcal{L}^*(C^*, k) &= \gamma P v k  (C^{*h} - \bar{\bar{C^{*h}}})
\end{split}
\end{equation}

The aggregate change in net benefit ($\Delta \psi$) for each stratum can be evaluated by integrating $\partial \mathcal{L}^*(C^*, k)$ over the farm expenditure ($C^* \in (0, \bar{\bar{C^{*h}}})$), exogenous factor ($k$), and area ($k_a$) densities.

\begin{equation}
\begin{split}
\Delta \psi &= N \int_0^{\infty} k_a \int_0^{\infty} k \int_0^{\bar{\bar{C}^{*}}} \partial \mathcal{L}^*(C^*, k) \rho(C^*) d C^* \rho(k) dk \rho(k_a) dk_a \\
N E[k_a] E[k] v P \gamma \int_0^{\bar{\bar{C}^{*}}} (C^{*h} - \bar{\bar{C^{*h}}}) \rho (C^*) d C^* \\
...
To calculate the change in aggregate supply and welfare for each stratum ($\Delta Q_{\ell}$ and $\Delta \psi_{\ell}$), in the equations above simply replace $v$, $N$, and $m_c$ with $v_{\ell}$, $N_{\ell}$, $m_{c \ell}$, as explained in section \ref{sec:binEndStrat}.
...
Expected yield not observed, but...THE IMPLICIT MEAN EXPECTED YIELD...
...in section \ref{sec:condMean}, tractable expression for mean yield, stochastic version

conditional mean

\begin{equation}
E[\tilde{y}^*(T; k_r) | \theta]|_{t = 0} = v E[E[k_r(T)^{\kappa_r}]|_{t = 0}] E[k'] E[C^{*h} | \theta]
...
\begin{equation}
E[\tilde{y}^*(T; k_r)]|_{t = 0} = v E[E[k_r(T)^{\kappa_r}]|_{t = 0}] E[k'] E[C^{*h}]
\label{eq:impMuEyStarT0uncond}
\end{equation}
...
\label{eq:dEPsi}
\end{equation}
<!-- \Delta E[Q(T; k_m(T))]|_{t = 0} &= N E[k_a] E[\tilde{y}^*(T, k_m(T))]|_{t = 0} \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) (1 - \Phi ( \zeta )) \\ -->
<!-- &= N \mathcal{A} E[k_a] E[\tilde{y}^*(T, k_m(T))| C^* \leq \bar{\bar{C^*}}]|_{t = 0} \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) -->
...
z <- (log(cStarMaxAdopt) - mCstarStrat) / sCstar
