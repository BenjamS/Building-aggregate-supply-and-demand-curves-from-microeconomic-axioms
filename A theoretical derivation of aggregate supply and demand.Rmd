...
The final step of calculating net benefits in terms of discounted economic surplus, adjusted for probability of success, expected adoption rate, time lags in research and adoption, capital depreciation, and other exogenously collected parameters, bears many of the markings of a rigorous economic assessment. But the result of this calculation is largely prefigured by the initial, exogenously determined farm supply/expenditure response to technology change, and to its assumed espression at the aggregate level, neither of which have a solid basis in economic theory. In this measure, then, the "impact assessment" is, more properly, an impact _assertion_ bordering on _petitio principii_, and very nearly a case in point of what Paul Romer calls "mathiness".
...
The present article is essentially one long disagreement with this passage. In particular, I disagree with the sweeping, and rather brazen, claim that economic theory does not tell us very much about the nature of aggregate supply curve shifts. On the contrary, economic theory provides a set of microeconomic principles or axioms whence a firm level yield model is derived, whence we can model "effects on individual agents", i.e., farm level response to exogenous shock. Insofar as the phrase "complete theory of aggregation" refers to the specification of aggregate supply and demand curves, this too can be derived from theory by integrating individual effects over a suitable density function (i.e., Equation \ref{eq:sup1}). The firm yield model implies bounds on the domain of optimal output, such that the "exit and entry of firms" is accounted for by integrating over the domain of optimal behavior. (Similar bounds can be determined for optimal input use, whence an aggregate demand curve could be derived; but the focus here is limited to aggregate supply.)

To be clear, I agree that the "precise econometric evidence or...detailed information" that the authors refer to is generally not available, but disagree with the suggestion that economic theory offers nothing in its stead. On the contrary, the whole point of having a theory is precisely to draw inference when available information provides no clear answer to our questions. I also agree that "assumptions are unavoidable" in the sense that the standard microeconomic axioms whence $q^*$ is derived, and plausible assumptions about the shape of the farm density $\rho(q^*)$ are unavoidable,   NOT of assuming what may then be endogenously modeled on this basis. tantamount to assuming what it is the job of economists to show.   [If the inference proves wrong, then the theory is corrected.] It is not theory, then, but rather models not explicitly built up from theory, that tell us "not very much" about the nature of supply curve shifts.  ...And  insofar as the 5 axioms are unavoidable, but assumption of the supply response to exogenous technology shock, which is nearly the same as assuming the result of the impact assessment, is not unavoidable. On the contrary, it is precisely the work of the economist to model the supply response to technology shocks endogenously.
It is not economic theory, then, but rather an expedient methodology which, by construction, precludes the possibility of explicit, endogenous modeling of the aggregate supply response to exogenous shocks, that tells us "not very much" about the the aggregate supply response to exogenous shocks. The present article, by contrast, explores what economic theory actually can tell us about the nature of the aggregate supply response.
...
Invoking the corollary in Equation \ref{eq:useful}, the function can be written more explicitly as a function of output price. 

\begin{equation}
Q = N \left( \frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E \left[\hat{\theta}_k^{\frac{\eta}{h}} \right] E[\lambda^{-\eta}] P^{\eta} \Phi(\gamma) \:\:;\:\:\: \gamma = \frac{\ln\left(\frac{P}{E \left[C^{\#} \right]} \right)^{\eta} - \frac{\eta^2}{2} Var \left[C^{\#} \right]}{\eta \sqrt{Var \left[C^{\#} \right]}}
\label{eq:sup2b}
\end{equation}
...
Henceforth it is necessary to be more specific in the treatment of output price $P$, yield $y^*$, and cost per unit output $C^{\#}$ as stochastic variables. Absent a forward contract, the output price at the time of harvest and sale $(t = T)$ is unknown to the farmer at the time of planting $(t = 0)$. Likewise, yield and cost per unit output are partly dependent upon stochastic processes occurring between planting and harvest (especially meteorological processes) which are unknown to the farmer at the time of planting. Wherever $P$, $y^*$, and $C^{\#}$ appear in the equations above, then, what is really meant is $E[P(T)]|_{t = 0}$, $E[y^*(T)]\bigr|_{t = 0}$, and $E[C^{\#}(T)]\bigr|_{t = 0}$---that is to say, what is meant is the values that the farmer expects $P$, $y^*$, and $C^{\#}$ to take at harvest/sale, as evaluated at planting time.
...
Making these substitutions for $P$ and $C^{\#}$ in Equation \ref{eq:sup2b}, then, the aggregate supply function becomes

\begin{equation}
\begin{split}
Q &= \beta E[P(T)^{\eta}]|_{t = 0} \Phi(\gamma) \:\:; \\ \gamma &= \frac{\ln\left(\frac{E[P(T)]|_{t = 0}}{E \left[ \tilde{C}^{\#}(T) \right]\bigr|_{t = 0}} \right)^{\eta} - \frac{\eta^2}{2} Var \left[\tilde{C}^{\#}(T) \right]}{\eta \sqrt{Var \left[\tilde{C}^{\#}(T) \right]}} \\
\beta &= N \left( \frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E \left[\hat{\theta}_k^{\frac{\eta}{h}} \right] E[\lambda^{-\eta}]
\end{split}
\label{eq:sup3}
\end{equation}
...
Now a stochastic model must be specified that reasonably describes the time evolution of $P(t)$. Within a given season, prices often follow a geometric Brownian motion (gBm). That is to say, changes in $P$ over a small time interval $\Delta t$ ($\Delta P = P(t + \Delta t) - P(t)$) are often accurately modeled by
...
Substituting the right hand side of Equation \ref{eq:EPT0} for $P$ in Equation \ref{eq:sup2},

\begin{equation}
Q = N \left( \frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E \left[\hat{\theta}_k^{\frac{\eta}{h}} \right] \frac{E \left[C^{\# \eta}\right]}{h^{\eta}} \Phi(\gamma) \:\:;\:\:\: \gamma = \frac{\ln\left(\frac{P(0)}{E \left[ \tilde C^{\#}(T) \right]\bigr_{t=0}} \right)^{\eta} - \eta^2 \frac{s_c^2}{2}}{\eta s_c \sqrt{T}}
\label{eq:sup4}
\end{equation}
