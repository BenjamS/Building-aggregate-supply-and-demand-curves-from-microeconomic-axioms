...
...in passing...diminishing returns to scale. Constant returns to scale ($h = 1$) imply the break-even point, while increasing returns to scale ($h > 1$) imply a loss. The farm yield function is thus globally concave in inputs. Constant and increasing returns, on the other hand, would imply global linearity and convexity, respectively. input price elasticity in appendix. constant increasing returns also imply flat/downward slpoing farm supply curves
...
## Integration of the farm production function based on axioms 3 and 4
...
Absent a forward contract, the output sale price $P$ is also a stochastic variable unknown to the farmer at the time of planting. Wherever $P$ appears in the equations above, then, what is really meant is $E[P(T)]|_{t = 0}$---the value that the farmer expects $P$ to take at harvest/sale, as evaluated at planting time. To parse $E[P(T)]|_{t = 0}$, a suitable stochastic model must be assumed for $P(t)$. Within a given season, prices often follow a geometric Brownian motion (gBm). That is to say, changes in $P$ over a small time interval $\Delta t$ ($\Delta P = P(t + \Delta t) - P(t)$) are often accurately modeled by
...
Substituting the stochastic terms developed above for $y^*$ and $P$ in the deterministic aggregate supply curve (Equation \ref{eq:aggSup1}), gives the _expected_ aggregate supply curve.

\begin{equation}
E[Q(T)]|_{t = 0} = N E[k_a] E[y^*(T)]|_{t = 0} \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)
\label{eq:aggSup2}
\end{equation}

This can be further refined into a more instructive expression by considering the stochastic character of optimal unit cost $C^{\#}$. Farm cost per acre $C^*$ typically includes land preparation, seed, fertilizer and chemical pest control, harvesting, and some post harvest activities (shelling, stooking, transport, etc.). The most significant of these costs are known (executed or planned) at planting time; and so $C^*$ can be treated deterministically. Optimal unit cost $C^{\#}$, on the other hand, must be treated as a stochastic variable. To see this, note that Equation \ref{eq:cStar} simplifies to

\begin{equation}
C^{\#} = \frac{P}{\lambda} h
\end{equation}

Taking into account the stochastic character of ouput price, this must now be rewritten more carefully as follows.
...
Note, moreover, that taking the expectation of of both sides gives the population mean of $E[C^{\#}(T)]\bigr|_{t = 0}$.
...
Which is a result that can also be reached by invoking Fubini's theorem. To be clear, this result states that the population mean of the optimal expected unit cost ($E[C^{\#}(T)]\bigr|_{t = 0}$) is equal to the optimal expected unit cost on the mean farm, i.e. the farm with $\lambda^{-1} = E[\lambda^{-1}]$. It therefore maps the mean of all $C^{\#}(t)$ trajectories in the farm population to the mean of possible random trajectories on a single farm (the farm where $\lambda^{-1} = E[\lambda^{-1}]$). An equivalent, perhaps clearer, way of writing the result is
<!-- [GRAPHIC showing equality of pop mean of expected future cSharp to expected cSharp on the mean farm (?)] -->

\begin{equation}
E[E[C^{\#}(T)]\bigr|_{t = 0}] = E[C^{\#}(T; E[\lambda^{-1}])]\bigr|_{t = 0}
\label{eq:fubThm2}
\end{equation}
...
When switching from population mean to stochastic mean, the variance likewise switches from a population variance to a stochastic variance. Most importantly for what follows, this means replacing $Var[\ln(E[C^{\#}(T)]|_{t = 0})]$ (i.e. $s^2_{\#}$) with $Var[\ln(C^{\#}(T; E[\lambda^{-1}]))]|_{t = 0}$. 

To parse $Var[\ln(C^{\#}(T; E[\lambda^{-1}]))]|_{t = 0}$, note first that the optimal expected unit cost $E[C^{\#}(T)]\bigr|_{t = 0}$ is an abstract optimal target that farmers aim for, but that evolves before and after the actual farm expenditure that is "locked in" at planting based on the price information $P(0)$ available at that time. As an abstract target, then, optimal unit cost may be expressed
...
And this is true for any value of $\lambda^{-1}$, including $\lambda^{-1} = E[\lambda^{-1}]$
Now, by definition, $m_{\#} = \ln(E[C^{\#}(T)]|_{t = 0}) - s^2_{\#} / 2$. Given the results in Equations \ref{eq:fubThm2} and \ref{eq:sPTresult}, this has the following dual expression in stochastic space.

\begin{equation}
m_{\#} = \ln(E[C^{\#}(T; E[\lambda^{-1}])]|_{t = 0}) - \frac{s^2_p}{2} T
\end{equation}

Substituting the righthand side of this in for $m_{\#}$ in Equation \ref{eq:aggSup2} gives

\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} &= N E[k_a] E[y^*(T)]|_{t = 0} \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - \ln(E[C^{\#}(T; E[\lambda^{-1}])]|_{t = 0}) + \frac{s^2_p}{2} T}{s_p \sqrt{T}} - \eta s_p \sqrt{T} \right) \\
&= N E[k_a] E[y^*(T)]|_{t = 0} \Phi \left( \frac{ \eta \ln \left(\frac{E[P(T)]|_{t = 0}}{E[C^{\#}(T; E[\lambda^{-1}])]|_{t = 0}} \right) - \eta \frac{s^2_p}{2} T}{\eta s_p \sqrt{T}} \right) \\
\end{split}
\end{equation}

By Equation \ref{eq:EPT0}, $P(0)e^{m_p T}$ may be substituted for $E[P(T)]|_{t = 0}$, resulting in a highly instructive form for expected aggregate supply.

\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} &= N E[k_a] E[y^*(T)]|_{t = 0} \Phi \left( \frac{ \ln \left(\frac{P(0)}{E[C^{\#}(T; E[\lambda^{-1}])]|_{t = 0}} \right) + m_p T - \frac{s^2_p}{2} T}{s_p \sqrt{T}} \right) \\
&= N E[k_a] E[y^*(T)]|_{t = 0} \Phi \left( - \frac{ \ln \left(\frac{E[C^{\#}(T; E[\lambda^{-1}])]|_{t = 0}}{P(0)} \right) - \left(m_p - \frac{s^2_p}{2} \right) T}{s_p \sqrt{T}} \right) \\
&= N E[k_a] E[y^*(T)]|_{t = 0} \text{Prob}(P(T) > E[C^{\#}(T)]|_{t = 0})
\end{split}
\label{eq:aggSup3}
\end{equation}

(To see how the last line follows from the previous line, revisit Equations \ref{eq:lnChngP}.) 

Expected aggregate supply is thus expressible in terms of (unconditional) mean yield, mean farm size, the number of farms, and the probability that output price will exceed unit cost on the mean farm---i.e. the mean farm's probability of profit.

...an instructive form for conditional mean yield... 

primal or dual...use the approach with higher resolution data...usually will be farm data. In practice, Equation \ref{eq:aggSup2} probably what gonna evaluate. but the dual is useful because it is a meaningful economic interpretation. lends itself to back of the envelope calculations in minimum data contexts since policy makers generally have or can reasonably formulate a rough idea of employment rate and probability of profit on the mean farm.

implies tradeoff map between employment rate and mean farm probability of profit [graph in kenya example]
...
<!-- unit cost $C^{\#}$ must likewise be replaced with $E\left[C^{\#}(T)\right]\biggr|_{t = 0}$, such that the population mean $E\left[C^{\#}\right]$ appearing in the aggregate supply function in Equation \ref{eq:sup2} must be replaced with -->
<!-- \begin{equation} -->
...
<!-- i.e., the log change in output price from its value at planting time ($P(0)$) to a value equal to the mean farm's expected unit cost, scaled by $\eta$. -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
...
<!-- \end{split} -->
<!-- \end{equation} -->
<!-- Because of the functional relation in Equation \ref{eq:lambda3}---which says that farms regulate their expenditure based on output price expectations---it can be shown by Ito's lemma that unit cost $C^{\#}$ is also a geometric Brownian motion with volatility $s_c$ equal to $s_p$. -->
...
<!-- \begin{equation} -->
<!-- y^* = (y_0 \theta)^{\eta + 1} \left( \frac{C^{\#}}{\eta} \right)^{\eta} \:\:;\:\:\: \eta = \frac{h}{1 - h} -->
