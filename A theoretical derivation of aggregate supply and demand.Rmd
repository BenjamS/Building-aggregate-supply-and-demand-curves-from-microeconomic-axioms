...
theseCols <- c("Cstar", "acres", "seed (kg/acre)", demog_vars)
...
```{r Fig4, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="center", fig.cap="\\label{fig:EyStarLambda} uyiijl", echo = FALSE}
...
kRn <- exp(lkRn)
cStar <- exp(df_mod2$Cstar)
# betaKnormdAdj <- betaKnormd / (cStar^hCoef)
#mean(betaKnormdAdj)
eps <- mod2$residuals
sErr <- sd(eps)
#----------------------------------------------------------------------
# y0 is EyStar*exp(-sErr^2 / 2) at mean(betaKnormdAdj*cStar^hCoef) and kRn = 1 (no rain anomaly)--i.e. basically y0 = mean(yStar)
y0 <- tfp * hCoef^hCoef
const <- y0 * betaK * exp(sErr^2 / 2)
#----------------------------------------------------------------------
EyStar1 <- const * kRn^kappaRn * (cStar / hCoef)^hCoef
EcSharp <- cStar / EyStar1
#---
# test
# tfp * muBetaK - y0
# EyStar - exp(mod2$fitted.values + sErr^2 / 2)
# EyStar - EyStar2
#---
# EyStarUp <- (y0 * betaKnormdAdj * kRn^kappaRn)^(etaCoef / hCoef) * EPT02007^etaCoef * exp(sErr^2 / 2)^(etaCoef / hCoef)
# EyStarUp <- (y0 * betaKnormdAdj * kRn^kappaRn)^(etaCoef / hCoef) * df_mod2$P^etaCoef * exp(sErr^2 / 2)^(etaCoef / hCoef)

# ERstar <- EyStar * EPT02007
lam <- EPT02007 / EcSharp * hCoef
# lam <- df_mod2$P / EcSharp * hCoef
...
#----------------------------------------------------------------------
EyStar2 <- (const * kRn^kappaRn)^(etaCoef / hCoef) * (EcSharp / hCoef)^etaCoef
#EyStar1 - EyStar2
EyStar3 <- (const * kRn^kappaRn)^(etaCoef / hCoef) * (EPT02007 / lam)^etaCoef
#EyStar1 - EyStar3
#----------------------------------------------------------------------
...
# #muEcSharpTrunc
#---
# x <- seq(0, 13, length = 200)
# y <- dlnorm(x, meanlog = mEcSharp, sdlog = sEcSharp)
...
#---------------------------------------------------------------------
# EyStarUpMu <- (mean(y0) * mean(kRn^kappaRn))^(etaCoef / hCoef) * (EPT02007 / hCoef)^etaCoef * exp(sErr^2 / 2)^(etaCoef / hCoef)
# mEyStarTrunc <- mean(log(EyStar))
# sEyStarTrunc <- sd(log(EyStar))
# muEyStarTrunc <- exp(mEyStarTrunc + sEyStarTrunc^2 / 2)
# xIn <- c(sEyStarTrunc, mEyStarTrunc)
# sol <- nleqslv::nleqslv(xIn, rootFnUp, jac = NULL,
#                         xUp = EyStarUpMu,
#                         sXtrunc = sEyStarTrunc,
#                         mXtrunc = mEyStarTrunc)
# sol
# sEyStar <- sol$x[1]
# mEyStar <- sol$x[2]
# muEyStarT0 <- exp(mEyStar + sEyStar^2 / 2)
# #---
# df_plot <- data.frame(EyStar, EyStarUp) %>% gather(var, val, EyStar:EyStarUp)
# gg <- ggplot(df_plot, aes(x = val, color = var))
# gg <- gg + geom_histogram(fill = "white")#, bins = 12)
# gg <- gg + geom_vline(xintercept = EyStarUpMu)
# gg
#----------------------------------------------------------------------
# cStarMuUp <- EyStarUpMu * EPT02007
# mCstarTrunc <- mean(log(cStar))
# sCstarTrunc <- sd(log(cStar))
# muCstarTrunc <- exp(mCstarTrunc + sCstarTrunc^2 / 2)
# xIn <- c(sCstarTrunc, mCstarTrunc)
# sol <- nleqslv::nleqslv(xIn, rootFnUp, jac = NULL,
#                         xUp = cStarMuUp,
#                         sXtrunc = sCstarTrunc,
#                         mXtrunc = mCstarTrunc)
...
#----------------------------------------------------------------------
# df_plot <- data.frame(EcSharp, lam)
# gg <- ggplot(df_plot, aes(x = log(EcSharp)))
# gg <- gg + geom_histogram(color = 4, fill = "white", bins = 12)
# gg <- gg + geom_vline(xintercept = log(EPT02007))
# gg1 <- gg
# 
# gg <- ggplot(df_plot, aes(x = log(lam)))
# gg <- gg + geom_histogram(color = 4, fill = "white", bins = 12)
# gg <- gg + geom_vline(xintercept = log(hCoef))
# gg2 <- gg
# gg1 + gg2 + plot_layout(ncol = 2)
#---------------------------------------------------------------------
mArea <- mean(df_mod2$acres)
sArea <- sd(df_mod2$acres)
...
QsFun <- function(EPT0vec, muKrn, kappaRn, muArea, muConst,
                  h, mLam, sLam,
                  muEcSharpT0, mP, sP, th, Nf){
  eta <- h / (1 - h)
  muEtaLamInv <- exp(-eta * mLam + eta^2 * sLam^2 / 2)
  muEyStarT0 <- (muConst)^(eta / h) * muKrn^kappaRn *  EPT0vec^eta
  muEqStarT0 <- muArea * muEyStarT0
...
dfQs2007 <- QsFun(EPT02007, muKrn = 1, kappaRn, muArea,
                  muConst = mean(const),
                  h = hCoef, mLam, sLam,
                  muEcSharpT0, mP = mP2006, sP = sP2006,
                  th, Nf = 1)
# dfQs2007 <- QsFun(EPT02007, muArea, g0, h = hCoef,
#                   muBetaRnEtaH, muBetaKetaH,
#                   muEtaLamInv, muEcSharpT0,
#                   mP = mP2006, sP = sP2006,
#                   th, Nf = 1)
Qs2007 <- dfQs2007$`Supply\n(million metric tons)`
Nf <- prodFAO2007 * 10^-6 / Qs2007
#Nf <- 1.7 * 10^6
# eta2007 <- dfQs2007$`Price elasticity\nof supply`
# pctDif2007 <- 100 * (10^6 * Qs2007 / prodFAO2007 - 1)
#----------------------------------------------------------------------
# Now have everything needed to parameterize 2009 expected aggregate supply curve (FPMA price info required for 2008 curve unavailable)
EPT0vec <- seq(1, 40, length.out = 60)
dfQs2009 <- QsFun(EPT0vec, muKrn = 1, kappaRn, muArea,
                  muConst = mean(const),
                  h = hCoef, mLam, sLam,
                  muEcSharpT0, mP = mP2008, sP = sP2008,
                  th, Nf)
# dfQs2009 <- QsFun(EPT0vec, muArea, g0, h = hCoef,
#                   muBetaRnAnomEtaH, muBetaKetaH,
#                   muEtaLamInv,
#               muEcSharpT0,
#               mP = mP2008, sP = sP2008,
#               th, Nf)
...
df_plot <- dfQs2009
gg <- ggplot(df_plot, aes(x = `Expected price (KES/kg)`, y = `Supply\n(million metric tons)`))
gg <- gg + geom_line()
gg <- gg + geom_vline(xintercept = EPT02009)
gg <- gg + theme_bw()
gg <- gg + theme(axis.title.x = element_blank())
ggQs <- gg
#---
gg <- ggplot(df_plot, aes(x = `Expected price (KES/kg)`, y = `Price elasticity\nof supply`))
gg <- gg + geom_line()
gg <- gg + theme_bw()
ggElast <- gg
df_inset <- subset(df_plot, `Expected price (KES/kg)` > 6.5 & `Expected price (KES/kg)` < 11)
...
ggPelastQs <- ggElast + annotation_custom(ggplotGrob(ggElastInset),
                            ...
                            ymin = 3, ymax = 10.5)
#----------
gg <- ggplot(df_plot, aes(x = `Expected price (KES/kg)`, y = `Participation (%)`))
gg <- gg + geom_line()
