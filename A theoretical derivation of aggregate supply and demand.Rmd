...
($y^*$ ultimately refers not to harvest yield but to expected harvest yield at planting time, which is not observed).
...
As noted in the introduction, conventional approach does not endogenously model the supply response to technology shock but rather assumes it. That is to say, it treats not only the technology shock, but also the supply response response itself as exogenously given. Proponents of the conventional approach are aware of the difference between agronomic outcomes at research stations and economically rational farm yield and expenditure decisions on real farms [@alston1995science]. However, beyond some words of warning about the difference, very little is offered in the way of a precise, practical method for accommodating the difference. One of the key features distinguishing the present work from the conventional approach is the explicit endogenous modeling of the farm level response to exogenous technology shock.]
...
\begin{equation}
Q = N E[k_a] E[y^*] \mathcal{P}(\bar{C}^{\#}) \:\:;\:\:\: \mathcal{P}(\bar{C}^{\#}) = \Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)
\label{eq:aggSup1}
\end{equation}

Or, by Equation \ref{eq:condMeanCshrp},
...
\begin{equation}
\partial \ln(y^*) = \frac{1}{1 - h} ( \partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w}) + h \partial \ln(P) )
\end{equation}
...
Where the last line follows from Equation \ref{eq:keyResult}. Recalling that $v = E[y^*] / (E[k] E[C^{*h}])$ (Equation \ref{eq:v0i}), this further reduces to

\begin{equation}
\Delta Q = N E[k_a] E[y^*] \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{c}}{s_c} - h s_c  \right)
\label{eq:deltQ}
\end{equation}
...
<!-- \end{split} -->
<!-- \label{eq:dLdlng} -->
<!-- From The farm level change in net benefit per unit area given the intervention $\partial \ln(g)$ may thus be expressed -->

The aggregate change in net benefit ($\Delta \psi$) can be similarly be evaluated by integrating $\partial \mathcal{L}^*(C^*, k)$ over the farm expenditure ($C^* \in (0, \bar{\bar{C^{*h}}})$), exogenous factor ($k$), and area ($k_a$) densities.
...
&= N E[k_a] P \gamma \left( E[y^*] \mathcal{P}(\bar{\bar{C^{\#}}}) - E[\bar{\bar{y}^*}] \mathcal{A} \right)
\end{split}
\end{equation}
<!-- $$ -->
<!-- N E[k_a] E[k] v P \gamma (E[C^{*h}] \mathcal{P}(\bar{\bar{C^*}}) - \bar{\bar{C^{*h}}} \mathcal{A}) \:\:;\:\:\: \mathcal{P}(\bar{\bar{C^*}}) = \Phi \left( \frac{\ln(\bar{\bar{C^{\#}}}) - m_{\#}}{s_{\#}} - \eta s_{\#}  \right) -->
...
<!-- as explained in section \ref{sec:binEndStrat} -->
...
Finally, the stochastically explicit formulations for the research induced change in aggregate supply and welfare derived in section \ref{sec:aggImpact} follow from exposition above as
...
# muEcSharpT0n <- exp(eta * mEcSharpT0 + eta^2 * sEcSharpT0^2 / 2)
# Now can calculate unconditional cStar parameters
mCstarStrat <- 1 / (1 - h) * (mean(beta0adj) + mK + mEcSharpT0stratVec)
sCstarStrat <- 1 / (1 - h) * sqrt(sK^2 + sEcSharpT0stratVec^2)
# muCstar <- exp(mCstar + sCstar^2 / 2)
# muCstarh <- exp(h * mCstar + h^2 * sCstar^2 / 2)
# muEyStarT0 <- mean(vStrat) * muK * muCstarh
# mLamba <- log(P02007) + (mPDown - sPDown^2 / 2) * timeHoriz + log(h) - mEcSharpT0
# mean(log(lamTrunc))
# log(P02007) + (mPDown - sPDown^2 / 2) * timeHoriz + log(h) #mEcSharpT0
ProbProfit <- pnorm((log(EPT02007) - mEcSharpT0stratVec) / (sEcSharpT0stratVec) - eta * sEcSharpT0stratVec)
EmploymentRate <- pnorm((log(EPT02007) - mEcSharpT0stratVec) / sEcSharpT0stratVec)
# muEyStarT0check <- dfAll$muEyStarT0truncStrat * EmploymentRate / ProbProfit
...
z <- (log(cStarMaxAdopt) - mCstarStrat) / sCstarStrat
adoptRateStrat <- pnorm(z)
adoptRateTotal <- sum(adoptRateStrat * stratPct)
# Total subsidies in billions
aggSubsidyStrat <- stratPct * dB * Nf * muArea * adoptRateStrat * 10^-9
aggSubsidy <- dB * Nf * muArea * adoptRateTotal * 10^-9
...
phiArg <- (log(cStarMaxAdopt) - mCstarStrat) / sCstarStrat - h * sCstarStrat
...
Say that the Ministry of Agriculture offers a net subsidy of $\partial B = $ `r dB` KES/acre to adopting farms, perhaps in the form of free seed and fertilizer. This then implies the adoption expenditure per acre ceiling of $\bar{\bar{C}^*} =$ `r round(cStarMaxAdopt)` KES (recall Equation \ref{eq:cStarAdopt}), and the corresponding adoption rate of $\mathcal{A} =$ `r round(100 * adoptRateTotal, 1)`% (recall Equation \ref{eq:AdoptRatei}). The aggregate net subsidy required from the Ministry of Agriculture to achieve this rate follows from Equation \ref{eq:aggSubsidy} as `r round(aggSubsidy, 2)` billion KES. The corresponding increase in aggregate supply and producer welfare follow from Equations \ref{eq:dEQT0} and \ref{eq:dEPsi} as `r round(dQ, 2)` 1000 MT and `r round(dPsi, 2)` billion KES, respectively. The breakdown by stratum is given in the left panel of Figure \ref{fig:intervention}, accompanied by a graphic of the corresponding supply ribbon shift in the right panel.
...
[at what rain level?... any impact assessment must be accompanied by a clear indication of the assumed meteorological conditions.]
...
dBvec <- seq(400, 2800, length.out = nRes)
...
for(i in 1:nRes){
  ...
  z <- (lCstarMaxAdopt - dfStrat$mCstar) / dfStrat$sCstar
  adoptRateStrat <- pnorm(z)
  adoptRateTotalCheck <- sum(adoptRateStrat * stratPct)
  # Total adopt rate
  z <- (lCstarMaxAdopt - dfAll$mCstar) / dfAll$sCstar
  adoptRateTotal <- pnorm(z)
...
phiArg <- (lCstarMaxAdopt - dfStrat$mCstar) / dfStrat$sCstar - h * dfStrat$sCstar
probProfAdoptStrat <- pnorm(phiArg)
dPsiStrat <- stratPct * Nf * muArea * dfStrat$muEyStarT0truncStrat * EPT02007 * (dlng0 + dlNu) * (probProfAdoptStrat - cStarMaxAdopt / dfStrat$muCstarHtruncStrat * adoptRateStrat)
dPsiStrat <- dPsiStrat * 10^-9 # in billions KES
dPsiTotCheck <- sum(dPsiStrat)
# Total welfare change
phiArg <- (lCstarMaxAdopt - dfAll$mCstar) / dfAll$sCstar - h * dfAll$sCstar
probProfAdoptTot <- pnorm(phiArg)
dPsiTot <- Nf * muArea * dfAll$muEyStarT0truncStrat * EPT02007 * (dlng0 + dlNu) * (probProfAdoptTot - cStarMaxAdopt / dfAll$muCstarHtruncStrat * adoptRateTotal)
dPsiTot <- dPsiTot * 10^-9 # in billions KES
# Check that the sum of stratified welfare changes matches the total
dPsiCheckVec[t] <- dPsiTot - dPsiTotCheck
#print(paste("Check dPsi", round(dPsi - dPsiCheck, 3)))
# dPsiNet <- dPsi - dB * adoptRateTotal * Nf * muArea * 10^-9
# dPsi / 67.32 # 2007 USD exchange rate
# Total welfare change
#---
# Stratified change in expected aggregate supply
dQStrat <- stratPct * Nf * muArea * dfStrat$muEyStarT0truncStrat * (dlng0 + dlnNuh) / (1 - h) * probProfAdoptStrat
# \Delta Q_i &= N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right)
dQStrat <- 10^-3 * dQStrat # in MT
dQStrat <- 10^-3 * dQStrat # in 1000 MT
dQTotCheck <- sum(dQStrat)
# Total change in expected aggregate supply
dQTot <- Nf * muArea * dfAll$muEyStarT0truncStrat * (dlng0 + dlnNuh) / (1 - h) * probProfAdoptTot
dQTot <- dQTot * 10^-6 # in 1000 MT
# Check that the sum of stratified supply changes matches the total
dQCheckVec[t] <- dQTot - dQTotCheck
#print(paste("Check dQ", round(dQTot - dQTotCheck, 3)))
#---
