...
To meet this demand, below we derive an aggregate supply curve from four Axioms. The derived curve explicitly accounts for farm entry/exit, such that supply response to shocks can be decomposed into yield and participation components. The parameters of the curve have a straightforward interpretation in terms of concrete, measurable phenomena, such that exogenous shocks may be modeled precisely and transparently. The farm participation term is surprisingly instructive, as it is proportionate to the probability that output price will be greater than marginal cost at the time of sale.
...
Four other desirable properties---a farm profit function convex in output price, a homogeneous (and thus homothetic) farm production function, upward sloping farm supply curves, and downward sloping farm input demand curves---follow as corollaries, and thus do not have to be imposed as additional Axioms. On the other hand, two somewhat controversial properties also follow: strictly decreasing returns to scale and constant marginal cost. (The latter is well supported empirically [@lavoie2014post], but is still eschewed in the textbooks.)

Regardless of one's view on returns to scale and marginal cost curves, the fact that it is possible to disagree with specific theoretical implications, and to trace disagreement back to specific offending Axioms, is a key strength of the derived curve relative to the expediently specified curve. The expedient method, by contrast, does not allow us to know whether we agree with their premises, because there is little clarity as to what those premises are in the first place. admit scrutiny of its premises, which precludes any possibility of knowing whether they are offensive or not.

...The model is of a comparable cost ...  The price paid in tractability is minimal. 
...
In the Discussion...Extensions to aggregate demand/PE analysis...application in min data contexts.
...
The solution for a given input $x_j^*$ works out to
...
With optimal yield then following as
...
Details of the derivation are omitted, as they can be found in any standard, rudimentary treatment of production functions (for e.g., see Heathfield and Wibe [-@heathfield1987introduction]). The second order conditions, likewise omitted, indicate that net benefit is maximized only over the domain $h \in (0, 1)$. Axioms 1 and 2, as formalized above, are thus inconsistent with either constant or increasing returns to scale.
...
...the farm output price elasticity of yield.
...
Textbook treatments do not always point out...

\begin{equation}
\frac{\partial \ln(y^*)}{\partial \ln(P)} \in (0, \infty)
\end{equation}

which in turn implies strictly decreasing returns to scale.

\begin{equation}
h \in (0, 1)
\end{equation}

Positively sloped supply curves also require decreasing returns to scale.

...some instructive, compact expressions which do not appear or are glossed over in the standard treatments. For example, the farm budget shadow price $\lambda$ can be written
...
which, when combined with Equation \ref{eq:inputDem1}, then gives
...
Combining Equations \ref{eq:lambda} and \ref{eq:YdEnvCompact} gives an alternative expression for $y^*$,

\begin{equation}
y^* = y_0 \beta \left( \frac{C^*}{h} \right)^h
\end{equation}

which in turn gives an alternative expression for $h$

\begin{equation}
\frac{\partial \ln(y^*)}{\partial \ln(C^*)} = h
\end{equation}

...isometry with axiom 1?...
