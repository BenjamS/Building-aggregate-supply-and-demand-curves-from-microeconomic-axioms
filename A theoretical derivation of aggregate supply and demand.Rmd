...
notThese <- c("Dist", "Yield (kg/acre)", "Cost/acre (KES/acre)", "Farm size (acres)", bin_vars)
...
#matK <- dfMod3b[-indRm, kVars] %>% as.matrix()
# lThetaKvec <- matK %*% kappaVec
# hist(lThetaKvec)
#matKa <- matK %*% diag(kappaVec)
mVec <- colMeans(matK)
sVec <- apply(matK, 2, sd)
#thetaKref <- exp(sum(c(mVec, abs(kappaVec) * sVec^2 / 2)))
# Note sample mean of thetaK very close to lognormal expectation, confirming lognormality of thetaK
...
# EyStar0 <- exp(mod3$fitted.values + sd(mod3$residuals)^2 / 2)
# #EyStar1 - EyStar0
# EcSharp <- exp(dfMod3$`Cost/acre`) / EyStar0
# lam <- EPT02007 / EcSharp * h
#=============================================================================
#---
...
# }
#----------------------------------------------------------------------
EyStarT0 <- exp(dfMod3$`Yield (kg/acre)`)# exp(mod3$fitted.values)# + sd(mod3$residuals)^2 / 2)
#mean(EyStarT0)
#EyStarT0 <- dfMod3$`Yield (kg/acre)`
EcSharp <- exp(dfMod3$`Cost/acre (KES/acre)`) / EyStarT0
#EcSharp <- exp(dfMod3b$`Cost/acre`) / EyStar0
lEcSharp <- log(EcSharp)
lam <- EPT02007 / EcSharp * h
sum(lam > h) / length(lam)
mEcSharpTrunc <- mean(lEcSharp); sEcSharpTrunc <- sd(lEcSharp)
...
muEcSharpT0n <- exp(eta * mEcSharp + eta^2 * sEcSharp^2 / 2)
#---
# Calculation of mean yield over whole population (y0 or muEyStarT0)
# (Sample mean of yield is mean over a truncated population)
# Use def of mean yield y0=v0*muThetaKnh*muEcSharpT0n
# Mean yield varies with binary controls. Here is the mean with just the base constant
# (i.e. all binaries = 0)
alpha0 <- coefVec[1]
v0base <- exp(alpha0)
y0base <- v0base * muThetaKnh * muEcSharpT0n
#y0base <- v0base * (muThetaK)^(eta / h) * muEcSharpT0n
#---
...
v0 <- exp(alpha0adj)
Eerr <- exp(sd(mod3$residuals)^2 / 2) #Don't forget to adjust for resids
y0 <- v0 * muThetaKnh * muEcSharpT0n * Eerr
muEcSharpTruncT0n <- exp(eta * mEcSharpTrunc + eta^2 * sEcSharpTrunc^2 / 2)
y0 <- v0 * muThetaKnh * muEcSharpTruncT0n * Eerr

mean(EyStarT0) #truncated sample mean
mean(y0)
# y0 <- g0 * EthetaK * EcStar * Eerr #Expected value of yStar
...
#gg <- gg + scale_y_discrete(limits = typeVec)
gg <- gg + geom_col()
gg <- gg + labs(title = "Pct of farms in smaple")
gg <- gg + theme_bw()
gg <- gg + theme(axis.title = element_blank(),
                 #axis.title.x = element_text(size = axisTitleSize),
...
#---------------------------------------------------------------------
#---
lEcSharpVec <- seq(-4, 7, length.out = length(lEcSharp))
f <- dnorm(lEcSharpVec, mEcSharp, sEcSharp)
dfPlot <- data.frame(f, lEcSharpVec)
gg <- ggplot() + geom_histogram(data = data.frame(lEcSharp), aes(x = lEcSharp, y = 0.7*after_stat(count / sum(count))), bins = 13, color = "black", fill = "white") #+ scale_y_continuous(labels = scales::percent)
gg <- gg + geom_line(data = dfPlot, aes(x = lEcSharpVec, y = f)) + geom_vline(xintercept = log(EPT02007), color = "red", lwd = 1) + theme_bw()
# ggplot() + geom_line(data = dfPlot, aes(x = lEcSharpVec, y = f)) + geom_vline(xintercept = log(EPT02007), color = "red", lwd = 1) + theme_bw()
EcSharpVec <- seq(0.0001, 40, length.out = length(EcSharp))
f <- dlnorm(EcSharpVec, mEcSharp, sEcSharp)
dfPlot <- data.frame(f, EcSharpVec)
gg <- ggplot() + geom_histogram(data = data.frame(EcSharp), aes(x = EcSharp, y = 0.7*after_stat(count / sum(count))), bins = 45, color = "black", fill = "white") #+ scale_y_continuous(labels = scales::percent)
