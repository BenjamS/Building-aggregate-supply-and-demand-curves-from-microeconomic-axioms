...
In the course of the derivation, it becomes evident that five other desirable properties immediately follow from axioms 1 and 2 as corollaries, and need not be imposed as additional axioms: 1) The farm yield function is homogeneous (and thus homothetic) and 2) concave in inputs; 3) farm supply curves are upward sloping; 4) input demand curves are downward sloping; and 5) the derived yield function implies agreement with the consensus understanding of economic incentives behind the decision to adopt or not adopt new technology (as articulated, for example, by Lipton [-@lipton2005family]).
...
In the first part of the derivation, immediately below, the farm yield function is derived from axioms 1-3. Most of these steps are elementary and thus presented in a highly summarized form, with details provided in the Appendix. However, a critical implication result overlooked by conventional models is that technology shocks cannot be represented by summarily shocking yield and/or expenditure based on agronomic trials. Agronomic trials may be useful to determine the shock to (total factor) productivity, but the yield response must then be evaluated endogenously based on the shock to productivity and expected change in input and output prices, plus any net subsidy offered as part of the technology package.   [enter lipton, adoption criterion here]

The first main contribution of the article comes in the subsequent derivation of the aggregate supply model from the farm yield function and from axioms 4 and 5. This is then followed by an illustrative use case based on Kenya maize farm data. The theoretical model's estimates of Kenyan aggregate maize supply and of the price elasticity of aggregate supply generally agree with empirical estimates.

The second main contribution of the article comes in the subsequent derivation and showcasing of equations for the evaluation of the _endogenous_ adoption rate, change in aggregate supply, and change in producer welfare in response to a hypothetical exogenous technology shock in the Kenya maize farm population. This is [underscores] the key result distinguishing the derived theoretical modeling framework from the conventional modeling approach: Whereas the conventional approach exogenously asserts not only the technology shock, but also the farm level production and/or expenditure response to the shock, the adoption rate, and the aggregate expression supply response and welfare outcome of the shock, the method derived here .

TFP, prices, and any net subsidy included in the package
...
Below, the theoretical aggregate supply curve is derived from axioms 1-5. First the farm production function is derived from axioms 1 and 2. Then a deterministic aggregate supply function is derived from the farm production function and axioms 3 and 4. This is deterministic in the sense that it does not take into account the stochasticity of output price and weather. The final form of the expected aggregate supply function then follows when stochasticity is taken into account.
...
Interpretation of the constant $g_0$ is as difficult as one wants to make it. It is, of course, usually called the "total factor productivity" (TFP), and understood to represent the yield per unit "all inputs"---although a more accurate description would be yield per unit "all input and control variables, raised to various powers". The TFP constant $g_0$ may in fact be a sum of binary endowments $b_i \beta_i$ that augment the base constant $\beta_0$ on farms where the endowment is present (where $b_i = 1$). That is to say, 

\begin{equation}
\alpha_0 = \beta_0 + \sum_{i = 1}^{n_b} b_i \beta_i
\end{equation}

In the Kenya maize farm exercise farther below, for example, there are four ($n_b = 4$) binary endowment variables $b_i$ (which take a value of $1$ if the endowment is present and $0$ otherwise): use of oxen in land preparation, use of tractor in land preparation, use of hybrid maize seed, and land tenure. This means that $g_0$ can vary across the population. In the Kenya maize exercise, there are 14 unique values of $g_0$ in the population, corresponding to 14 unique combinations of the four binary endowments plus the regression constant.

The parameter $g_0$ may also be arbitrarily defined in terms of a new parameter ($u_0$) combined with judiciously chosen reference terms such that the new parameter has a meaningful interpretation and/or results in a more useful expression for yield. For present purposes, $g_0$ is defined as follows
...
Where $u_0$ is a constant, such that $y$ may be rewritten
...
Where Euler's Theorem has been invoked in the second line of the condition. Axioms 1 and 2 thus require that yield exhibit decreasing returns to scale ($0 < h \leq 1$), whereby it also follows that yield is globally concave in inputs. (Increasing returns to scale would imply minimum profit and convexity in inputs, while constant returns would imply the break even point and linearity in inputs.)
...
<!-- \begin{equation} -->
<!-- y^* = \left( g_0 \prod_{i = 1}^{n_x} \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i} \right) \left( \frac{P}{\lambda} \right) -->
<!-- \end{equation} -->
...
### Deducing the upper bound on expenditure and farm employment rate from axioms 3 and 4

Letting $C^{\#}$ denote optimal unit expenditure (such that $C^{\#} = C^* / y^*$), note that farm expenditure and yield can be rewritten in terms of $C^{\#}$.

\begin{equation}
\begin{split}
C^* = C^{\#} y^* &= C^{\#} v_0 k C^{* h} \\
&= (v_0 k C^{\#})^{\frac{1}{1 - h}} \\
&rightarrow y^* = (v_0 k)^{\frac{1}{1 - h}} C^{\# \frac{h}{1 - h}}
\end{split}
\label{eq:fnCsharp}
\end{equation}

And note, moreover, that Equation \ref{eq:cStar} simplifies to
...
Axiom 3 states that farms enter production only if they expect to make non-negative profit, i.e. if $C^* \leq R^*$, which simplifies to $C^{\#} \leq P$. (It follows from Equation \ref{eq:cStar} or \ref{eq:cStar} that another way of expressing axiom 3 is $\lambda \geq h$.) The farm employment rate $\mathcal{E}$ may thus be expressed as

\begin{equation}
\mathcal{E} = \int_0^{P} \rho(C^{\#}) dC^{\#}
\end{equation}

If yield is lognormally distributed (axiom 4), then so must be $C^*$ and $C^{\#}$; and the expression reduces to
...
### The endogenous farm yield and expenditure response to exogenous technology change

The impact of yield scaling technologies and/or management practices on farm supply and expenditure can be represented through perturbations of the TFP parameter ($g_0$). This is tantamount to adding a binary endowment variable to the yield model in Equation \ref{eq:lYdEnvReg3}. The four binary endowments included in the Kenya maize model example may be considered yield scaling interventions that increase the value of $g_0$ on adopting farms. The percentage increase in TFP given the introduction of a new intervention $\beta_5 b_5$ on farms that implement the new technology (farms where $b_5 = 1$) may be calculated as

\begin{equation}
\begin{split}
\partial \ln(g_0) &\approx \frac{g_0' - g_0}{g_0} \:\:; \:\:\: g_0' = g_0 e^{\beta_5 b_5}
&\approx e^{\beta_5} - 1
\end{split}
\end{equation}

The increase is thus the same across all adopting farms regardless of the starting value $g_0$. Ideally, the value of $\beta_5$ should be estimated directly from field trials. If this is not possible, then the estimate can be made on the basis of agronomic crop modeling and expert opinion. Regardless of the path taken, it is critically important to keep in mind that the objective is to estimate the change in TFP ($\beta_5$ or $\partial \ln(g_0)$), not the change in economic yield itself ($\partial \ln(y^*)$). Information gathered in terms of the change in agronomic yield and/or expenditure must be converted into an estimate for $\partial \ln(g_0)$. The percentage change in economic yield, expenditure, and expenditure TFP ($v_0$) then follows from Equations \ref{eq:fnCsharp} as a function of the change in TFP as well as any change in output and input prices.
<!-- The percentage changes in expenditure TFP ($v_0$), farm expenditure, and yield given a 1% change in $g_0$ follow from Equations \ref{eq:cStar} and \ref{eq:YdEnvReg1}. -->

\begin{equation}
\begin{split}
\frac{\partial \ln(v_0)}{\partial \ln(g_0)} &= 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} \\
\frac{\partial \ln(C^*)}{\partial \ln(g_0)} &= \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} + \frac{\partial \ln(P)}{\partial \ln(g_0)} \right) \\
\frac{\partial \ln(y^*)}{\partial \ln(g_0)} &= \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} + h \frac{\partial \ln(P)}{\partial \ln(g_0)} \right)
\end{split}
\label{eq:dldlg0}
\end{equation}
...
The direction of the expected change in input price depends upon the type of research product. Most agricultural research currently falls into one of two broad paradigms: "sustainable agricultural intensifcation" (SAI) or "agroecological intensification" (AEI) [@mockshell2018beyond]. Widespread adoption of a new SAI technology typically results in increased output supply and input demand, and hence a lower output price and higher input prices. Widespread adoption of an AEI technology, on the other hand, typically results in lower input demand and emphasizes yield stability over quantity, hence lowering input prices and having little or uncertain impact on output price. For illustrative purposes, here the new technology is assumed to be an SAI research product, such that $\partial \ln(\mathbf{w}) / \partial \ln(g_0) > 0$ and $\partial \ln(P) / \partial \ln(g_0) < 0$).


The change in agronomic yield observed at research stations does not directly translate into the change in economic yield or farm expenditure likely to occur in a real farm population [@herdt1988increasing; @balasubramanian2000yield; @alston1995science].  This is well documented and obvious from the theoretical framework deveoped thus far but bears emphasis as it is still commonplace in practice to hear talk of the "exogenous yield shock", or in discussions, for example, of yield gaps [@sumberg2012mind].
The conventional modeling approach is cognizant of  words of warning. The key difference between the approach developed here and the conventional framework that precedes it is that the present approach goes beyond words of warning to provide a framework for explicitly accounting for the effect of price change, increased cost, and subsidies on economic yield and expenditure.

This is plain to see from the yield and expenditure expressions derived above; and is also just an empirically well documented lesson learned from decades of agricultural research .
As discussed in the Introduction, conventional ex-ante impact assessment models routinely neglect this well known fact. They do not endogenously model the supply and/or expenditure response to technology shocks, but rather exogenously assert the response, often on the presumption that agronomic results at research stations will translate into identical results in real farm populations.
Alston et al. note this is not the case...input mix, but in practice most impact studies do this
whether wittingly or unwittingly, presuming instead that research induced percentage changes in agronomic yield and expenditure are equivalent to the percentage changes likely to occur in a real farm population. That is to say, they .



\begin{equation}
\begin{split}
\frac{\partial \ln(y^*)}{\partial \ln(g_0)} &= \frac{\partial \ln(v_0)}{\partial \ln(g_0)} + h \frac{\partial \ln(C^*)}{\partial \ln(g_0)} \\
&= \frac{\partial \ln(v_0)}{\partial \ln(g_0)} + h \left( \frac{\partial \ln(y^*)}{\partial \ln(g_0)} + \frac{\partial \ln(C^{\#})}{\partial \ln(g_0)} \right) \\
&\rightarrow \frac{\partial \ln(y^*)}{\partial \ln(g_0)} = \frac{1}{1 - h} \left( \frac{\partial \ln(v_0)}{\partial \ln(g_0)} + h \frac{\partial \ln(P)}{\partial \ln(g_0)} \right)
\end{split}
\end{equation}
<!-- \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} + h \frac{\partial \ln(P)}{\partial \ln(g_0)} \right) -->
<!-- \frac{\partial \ln(v_0)}{\partial \ln(g_0)} + h \frac{\partial \ln(C^*)}{\partial \ln(g_0)} -->

To resolve $\partial \ln(v_0) / \partial \ln(g_0)$, 
...
<!-- &= \left( g_0 \prod_{i = 1}^{n_x} \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i} \right) \left( \frac{C^{\#}}{h} \right)^{\frac{h}{1 - h}} -->
...
<!-- \end{equation} -->
<!-- \begin{equation} -->
<!-- \frac{\partial \ln(C^{\#})}{\partial \ln(g_0)} = \frac{\partial \ln(P)}{\partial \ln(g_0)} -->
<!-- \label{eq:cSharp} -->
<!-- \end{equation} -->
]]
<!-- The change in yield is positive only if $\partial \ln(g_0) > \boldsymbol{\alpha} ' \partial \ln(\mathbf{w}) + h \partial \ln(P)$. -->
<!-- Change in expenditure is positive if $\partial \ln(g_0) > (1 - h) (\boldsymbol{\alpha} ' \partial \ln(\mathbf{w}) + \partial \ln(P))$. -->

### The endogenous adoption rate
...
The welfare effect on a given farm of adopting the new technology can be examined by differentiating optimal net benefit with respect to the change in TFP.

\begin{equation}
\begin{split}
\frac{\mathcal{L}^*}{\partial \ln(g_0)} &= \frac{\lambda}{h} \left((1 - h) \frac{\partial C^*}{\partial \ln(g_0)} + h \frac{\partial B}{\partial \ln(g_0)} \right) \\
&= \frac{\lambda}{h} \left( C^* \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} + \frac{\partial \ln(P)}{\partial \ln(g_0)} \right) + h \frac{\partial B}{\partial \ln(g_0)} \right)
\end{split}
\end{equation}

Assuming farmers adopt only if $\partial \mathcal{L}^*/\partial \ln(g_0) > 0$ (this is implicit in axiom 2), then the adoption condition is

\begin{equation}
\begin{split}
\partial \ln(P) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g_0) + \frac{h}{C^*} \partial B &> 0 \\
\partial \ln(\nu) + \partial \ln(g_0) + \frac{h}{C^*} \partial B &> 0 \:\:;\:\:\: \partial \ln(\nu) = \partial \ln(P) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w})
\end{split}
\label{eq:adoptCond}
\end{equation}
...
\begin{equation}
\partial \ln(g_0) > -\partial \ln(\nu)
\label{eq:adoptCondSimp}
\end{equation}

As mentioned above, for SAI technologies the expectation is that $\partial \ln(P) < 0$, $\partial \ln(\mathbf{w}) > 0$, and thus $\partial \nu < 0$. In words, then, the adoption condition says that a farm stands to increase its net benefit through adoption of yield scaling technology only if the resulting percentage increase in TFP ($g_0$) is greater than the corresponding percentage decrease in output price minus the corresponding percentage increase in input prices scaled by their respective input elasticities $\alpha_i$. Note the resemblance between this and the "price/productivity tightrope" condition formulated by Lipton [-@lipton2005family].
...
Denoting the upper adoption bound on $C^*$ (Equation \ref{eq:cStarAdopt}) as $\bar{\bar{C^*}}$, the adoption rate $A$ when $\partial \ln(g_0) < -\partial \ln(\nu)$ can be expressed as the integral of the density $\rho(C^*)$ over the domain $(0, \bar{\bar{C^*}})$.

\begin{equation}
\mathcal{A} = \int_0^{\bar{\bar{C^*}}} \rho(C^*) d C^*
\end{equation}

If $y^*$ is lognormally distributed (axiom 4), then so is $C^*$, and the expression resolves as follows.

\begin{equation}
\begin{split}
\mathcal{A} &= \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_c}{s_c} \right) \\
& m_c = E[\ln(C^*)] \:\:\:\: s^2_c = Var[\ln(C^*)]
\end{split}
\end{equation}
...
\phi \left(\frac{\ln(C^*) - m_c}{s_c} \right) d \ln(C^*) \:\:;\:\:\: \bar{\bar{C^*}} = \frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} \\
&= \Phi \left( \frac{\bar{\bar{C^*}} - m_c}{s_c} \right)
\end{split}

To parse $m_c$ and $s_c$, recall from Equation \ref{eq:mCk} that $C^* = (v_0 k E[C^{\#}(T)]|_{t = 0})^{1 / (1 - h)}$, such that
<!-- note that Equations \ref{eq:paramsCondCsharp} can be rearranged into expressions for $\bar{m}_c$ and $\bar{s}_c$, whence the expression for the unconditional parameters follows immediately. -->

\begin{equation}
\begin{split}
m_c &= \frac{1}{1 - h}(\ln(v_0) + m_k + m_{\#}) \:\:;\:\:\: m_k = E[\ln(k)] \:\: s_k^2 = Var[\ln(k)] \\
s_c^2 &= \frac{1}{(1 - h)^2} (s_k^2 + s_{\#}^2)
\end{split}
\label{eq:mCsC}
\end{equation}


## Integration of the farm production function based on axioms 3 and 4 \label{sec:aggSupDeterm}

The farm production function is just farm yield times area $k_a$.
...
<!-- To develop an expression for aggregate supply, it only remains to substitute $q^*$ into Equation \ref{eq:aggSup0}, to specify a yield density function, and to integrate over the domain of profitable production. The domain of profitable production is given by axiom 3: farms enter production only if they expect to make non-nonegative profit. Mathematically, this means $C^* \leq R^*$. The yield density function is given by axiom 4, which states that yield is lognormally distributed. -->
...
&= N E[k_a] \frac{E[y^*]}{E[k] E[C^{* h}]} \int_{0}^{\infty} k \int_{0}^{\bar{C}^{*}} C^{*h} \rho(C*) dC^* \rho(k) dk \\
&= N E[k_a] \frac{E[y^*]}{E[C^{* h}]} \int_{0}^{\bar{C}^*} C^{* h} \rho(C^*)
\end{split}
\label{eq:aggSupStep}
\end{equation}
...
\begin{equation}
\bar{C}^* = R^* = P y^*|_{C^* = \bar{C}^*} = P v_0 k R^{*h}
\end{equation}
...
\begin{equation}
\begin{split}
\ln(\bar{C}^*) = \ln(\bar{R}^*) &= \ln(P y^*|_{C^* = R^*}) = \ln(P v_0 k) + h \ln(R^*) \\
&= \ln(P v_0 k) + h(\ln(P v_0 k) + h\ln(R^*)) \\
&= \ln(P v_0 k) + h(\ln(P v_0 k) + h(\ln(P v_0 k) + h\ln(R^*))) \\
&= \cdots \\
&= \ln(P v_0 k) \sum_{j = 0}^{\infty} h^{j} \\
&= \frac{1}{1 - h} \ln(P v_0 k) = (\frac{\eta}{h}) \ln(P v_0 k)
\end{split}
\end{equation}
...
\begin{equation}
\bar{C}^*(k) = (P v_0 k)^{\frac{\eta}{h}}
\end{equation}

And so aggregate supply for given $k$ can be expressed

\begin{equation}
Q(k) = N E[k_a] \frac{E[y^*]}{E[C^*(k)^h]} \int_{0}^{(P v_0 k)^{\frac{\eta}{h}}} C^*(k)^h \rho(C^*(k)) dC^*(k)
\label{eq:aggSupStep}
\end{equation}

If $y^*$ is lognormally distributed (axiom 4), then $C^*$ is also lognormally distributed. For any lognormally distributed variable $\chi$ and upper bound $\xi$, it can be shown that
...
Where $\Phi()$ is the standard normal cumulative distribution function. (See Appendix A.4 for a proof.)

Substituting $\chi = C^{* h}$ and $\xi = \bar{C}^*$, 

\begin{equation}
\begin{split}
\int_{0}^{(P v_0 k)^{\frac{\eta}{h}}} C^{* h} \rho(C^*) d C^* &= E[C^{* h}] \Phi\left( \frac{\eta \ln(P v_0 k) - h m_c}{h s_c} - h s_c \right) \\
& m_c = E[\ln(C^*)] \:\:\:\: s^2_c = Var[\ln(C^*)]
\end{split}
\label{eq:integralStep}
\end{equation}
...
\begin{split}
C^* = C^{\#} y^* &= C^{\#} v_0 k C^{*h} \\
&\rightarrow C^* = (C^{\#} v_0 k)^{\frac{1}{1 - h}} \\
&\Rightarrow E[\ln(C^*(k))] = \frac{1}{1 - h} (E[\ln(C^{\#})] + \ln(v_0 k)) \\
& m_c = (\frac{\eta}{h}) (m_{\#} + \ln(v_0 k))  \:\:;\:\:\: m_{\#} = E[\ln(C^{\#})]
\end{split}
\label{eq:mCk}
\end{equation}
...
\begin{equation}
\begin{split}
\int_{0}^{(P v_0 k)^{\eta / h}} C^{* h} \rho(C^*) d C^* &= E[C^{* h}] \Phi \left( \frac{\eta \ln(P v_0 k) - \eta m_{\#}}{\eta s_{\#}} - \eta s_{\#} \right) \\
&= E[C^{* h}] \Phi \left( \frac{\ln(P v_0 k) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)
\end{split}
\end{equation}
...
\begin{equation}
Q = N E[k_a] E[y^*|C^{\#} \leq P] \Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} \right)
\end{equation}

Which may be useful for point estimates of aggregate supply along the curve since $E[y^*|C^{\#} \leq P]$ can be estimated from the observed yield distribution. This form is also instructive because the $\Phi()$ term has a straightforward interpretation as the fraction of the population participating in production, i.e. the farm employment rate. However, it is inconvenient for purposes of drawing the entire aggregate supply curve, as the observed $E[y^*|C^{\#} \leq P]$ is valid only for the $P$ at which it is measured. The unconditional mean yield $E[y^*]$, on the other hand, is constant for all $P$. For this reason, Equation \ref{eq:aggSup1} is preferred. Moreover, the $\Phi()$ term in Equation \ref{eq:aggSup1} turns out to have a meaningful economic interpretation once the stochastic character of price is taken into account, as shown below.
<!-- \begin{equation} -->
...
&= v_0 \frac{k_m(T)^{\kappa_m}}{E[k_m(T)^{\kappa_m}]} k' C^{*h} \:\:;\:\:\: v_0 = \frac{E[y^*(T)]}{E[k'] E[C^{*h}]}
\end{split}
\label{eq:yStarT}
\end{equation}
...
<!-- &= v_0 k' \frac{E[k_m(T)]|_{t = 0}}{E[\tilde{k}_m(T)]|_{t = 0}} C^{*h} \:\:;\:\:\: v_0 = \frac{E[\tilde{y}^*(T)]|_{t = 0}}{E[k'] E[C^{*h}]} -->
<!-- \end{split} -->
<!-- \label{eq:stochYd2} -->
<!-- \end{equation} -->

Note that optimal farm expenditure per acre $C^*$ remains a deterministic variable. This is because $C^*$ primarily refers to land preparation, seed, fertilizer and chemical pest control. Some formulations of farm cost might also include some harvest post harvest labor costs (e.g., shelling, stooking, transport), but the main cost components are typically known (executed or committed) at planting time.
...
<!-- \end{equation} -->
<!-- \begin{equation} -->
<!-- E[y^*(T)]|_{t = 0} = v_0 E[k_m(T)]|_{t = 0} \hat{k} C^{*h} \:\:;\:\:\: v_0 = \frac{E[\tilde{y}^*(T)]|_{t = 0}}{E[k] E[C^{*h}]} -->
<!-- \label{eq:YdEnvReg2} -->
<!-- \end{equation} -->
<!-- \:\:;\:\:\: v_0 = \frac{E[y^*]}{E[k] E[C^{*h}]}  -->
...
Farm cost per acre is deterministic, but optimal unit expenditure $C^{\#}$ is proportional to price, and must therefore also be stochastic.
**--
To see this, note that Equation \ref{eq:cStar} simplifies to
...
Which, taking into account the stochastic character of price, must now be more carefully written as follows.
...
\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} &= N E[k_a] E[\tilde{y}^*(T)]|_{t = 0} \text{Prob}(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0} \leq P(T)^{\eta}) \\
...
To parameterize the expected aggregate supply curve, Equation \ref{eq:yStarT} (in logs) is fit to the farm data. Here it is important to note that $v_0$ may be expressed in terms of $g_0$.

**--
\begin{equation}
\begin{split}
v_0 &= \frac{E[y^*(T)]}{E[k] E[C^{* h}]} \\
&= \frac{u_0}{E[k] E[C^*]^h} \\
&= g_0 \frac{\prod_i^{n_x}E[x_i]^{\alpha_i}}{E[C^*]^h} \\
&= g_0 \frac{1}{h}\prod_i^{n_x} \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i}
\end{split}
\label{eq:v0forms}
\end{equation}
...
The TFP constant $g_0$ may include binary endowment variables that augment the constant on farms where the endowment is present.

\begin{equation}
\ln(y^*(T)) &= \ln(v_0) + \sum_i^{n_k} \kappa_i \ln(k_i) + h \ln(C^*) + \epsilon
\label{eq:lYdEnvReg3}
\end{equation}
<!-- \:\:;\:\:\: v_0 = \frac{E[y^*]}{E[k] E[C^{*h}]}  -->

Where $\epsilon$ is the regression residual, and

$$
\ln(v_0) = \sum_i^{n_x} \ln \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i} - \ln(h) + \ln(g_0) \\
\ln(g_0) = \beta_0 + \sum_{i = 1}^{n_b} \beta_i b_i
$$
In this exercise, the binary endowment variables $b_i$ (which take a value of $1$ if the endowment is present and $0$ otherwise) are: use of oxen in land preparation, use of tractor in land preparation, use of hybrid seed, and land tenure. The control variables ($k_i$) are planting density (seed kg/acre), adult equivalent household size, farm size, and rainfall. The rainfall variable is 2007 main season rainfall divided by an historical benchmark, and so can be interpreted as the rainfall anomaly or departure from the historical benchmark. The farm expenditure $C^*$ includes the cost of synthetic fertilizer, land preparation, pest and disease control chemicals, and seed. The OLS regression results are presented in Table 2.
**--
\pagebreak
...
The estimated coefficient on logged cost per acre is statistically significant and indicates that the population yield degree of homogeneity is $h =$ `r hReport`, by which it follows immediately that the price elasticity of farm supply is $\eta = h / (1 - h) =$ `r etaReport`. The coefficients on the other regressors are also statistically significant and take values and signs that agree with intuition. Note that the coefficient on the farm size regressor is negative, affirming the inverse relation between farm size and yield found in numerous other studies [@muyanga2019revisiting; @larson2014should; @wassie2019revisiting; @gautam2019too; @kimhi2006plot].

To evaluate the expected unconditional mean yield term ($E[\tilde{y}^*(T)]|_{t = 0}$) and parameters $m_{\#}$, $s_{\#}$ appearing in the expected aggregate supply expression (Equation \ref{eq:aggSup2}), first the mean fitted values for yield ($\hat{y}^*$) are interpreted as the observed conditional mean yield.
...
To estimate $m_{\#}$ and $s_{\#}$, note that the second line of Equations \ref{eq:mCk} (rewritten in stochastic terms) can be rearranged as an expression for $E[C^{\#}(T)]|_{t = 0}$.

\begin{equation}
E[C^{\#}(T)]|_{t = 0} = \frac{C^{* 1 - h}}{v_0 k}
\end{equation}

Such that the conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ can be estimated from the data as follows.
<!-- To parse $m_c$ and $s_c$, recall from Equation \ref{eq:mCk} that $C^* = (v_0 k C^{\#})^{1 / (1 - h)}$, such that -->

\begin{equation}
\begin{split}
\bar{m}_{\#} &= (1 - h) \bar{m}_c - m_k - \ln(v_0) \\
\bar{s}_{\#}^2 &= (1 - h)^2 \bar{m}_c^2 - m_k^2 \\
& \bar{m}_c = E[\ln(C^*)| \theta] \:\:\:\: \bar{s}_c^2 = Var[\ln(C^*)| \theta]
\end{split}
\label{eq:paramsCondCsharp}
\end{equation}

The unconditional parameters $m_{\#}$ and $s_{\#}$ are then calculated by solving simultaneous Equations \ref{eq:cSharpUncondParams}. In addition to $\bar{m}_{\#}$ and $\bar{s}_{\#}$, this requires an estimate of expected harvest price at planting time ($E[P(T)]|_{t = 0}$).
...
#v0 %>% unique() %>% length # 14 endowment combos present
# dfPlot <- data.frame(County = dfMod3$Dist, v0)
...
kVars <- setdiff(lnMod3Vars, notThese)
...
mKvec <- matK %*% diag(kappaVec) %>% colMeans()
sKvec <- matK %*% diag(kappaVec) %>% apply(2, sd)
mK <- sum(mKvec)
sK <- sqrt(sum(sKvec^2))
...
mCstar <- 1 / (1 - h) * (mean(beta0adj) + mK + mEcSharpT0)
sCstar <- 1 / (1 - h) * sqrt(sK^2 + sEcSharpT0^2)
muCstar <- exp(mCstar + sCstar^2 / 2)
muCstarh <- exp(h * mCstar + h^2 * sCstar^2 / 2)
muK <- exp(mK + sK^2 / 2)
muEyStarT0check <- mean(v0 * muK) * muCstarh
...
# EcSharpT0nTrunc <- (exp(dfMod3$`ln[Cost/acre (KES/acre)]`) / EyStarT0trunc)^eta
...
Calculation of $m_{\#}$ and $s_{\#}$ requires the expected price at planting for 2007, the same year as the farm data used to estimate $\bar{m}_{\#}$ and $\bar{s}_{\#}$. The short rains harvest in 2007 was normal and so the "good short rains" estimate of $m_p$, and March 2007 price is used to calculate $E[P(T)]|_{t = 0} = P(0) e^{m_p T}$ (where $P(0)$ is the price at planting in March).

Once $m_{\#}$ and $s_{\#}$ are calculated, the expected unconditional mean yield ($E[\tilde{y}^*(T; k_m(T))]|_{t = 0}$) follows from Equation \ref{eq:condMeanYd2}. In the process, the farm employment rate is calculated as `r round(EmploymentRate, 2)` and the PEFS adjusted mean probability of profit is calculated as `r round(ProbProfit, 2)`. The expected unconditional mean yield is evaluated at four different expected rainfall levels: $E[E[k_r(T)^{\kappa_r}]]|_{t = 0} = 1.5^{\kappa_r}, \: 1^{\kappa_r}, \: 0.5^{\kappa_r}, \: 0.1^{\kappa_r}$. These values correspond to expected rainfall that is 150% of the historical benchmark ("high"), in line with the historical benchmark ("normal"), 50% of the historical benchmark ("low"), and 10% of the historical benchmark ("dismal"), respectively.

Note in passing that, because the binary endowments $b_i$ vary across the population, the value of $g_0$ (and hence of $v_0$) also varies. In the present example, there are 14 distinct values of $v_0$, corresponding to 14 distinct combinations of the $\beta_i b_i$. The supply aggregation step in Equation \ref{eq:aggSupStep} averages aggregate supply across all binary endowment strata. Denoting the value of each distinct combination as $v_{0i}$, the average $v_0$ is effectively a population weighted sum of the $v_{0i}$.
<!-- Each value of $v_0$ effectively identifies a binary endowment stratum of the population. -->

\begin{equation}
v_0 = \sum_i \frac{N_i}{N} v_{0i}
\end{equation}

Where $N_i / N$ is the fraction of farms in the $i^{th}$ binary endowment stratum.

The conditional mean cost and yield disaggregated by stratum ($E[C^*| \theta]_i$, $E[y^*(T)| \theta]_i$), can be evaluated as follows.

\begin{equation}
\begin{split}
E[C^*| \theta]_i &= e^{\bar{m}_{ci} + \frac{\bar{s}^2_c}{2}} \:\:;\:\:\: \bar{m}_{ci} = \frac{1}{1 - h}(\ln(v_{0i}) + m_k + \bar{m}_{\#})  \\
&= v_{0i}^{\frac{1}{1 - h}} e^{\frac{1}{1 - h}(m_k + \bar{m}_{\#}) + \frac{1}{(1 - h)^2} \frac{s^2_k + \bar{s}_{\#}^2}{2}}
\end{split}
\end{equation}
...
\begin{equation}
E[\hat{y}^*(T, k_m(T))]^i|_{t = 0} = v_{0i} E[k] E[C^{*h}| \theta]_i
\label{eq:v0i}
\end{equation}
<!-- E[e^{\epsilon}]_i -->
<!-- Where $E[e^{\epsilon}]_i$ is the mean of the exponential function of the regression error of farms in the $i^{th}$ stratum. -->

The stratified ex-ante expected mean yield $E[\tilde{y}^*(T)]|_{t = 0}^i$ can also be deduced as in Equation \ref{eq:EyStarT0fromYhat}.
...
```{r Fig3, fig.show = "hold", fig.width = 6, fig.height=3, fig.align="center", fig.cap="\\label{fig:v0strata}(Left) The mean observed cost, yield, and TFP (v0) for each of the 14 different combinations of binary endowments present in the Kenya farm data; and (right) their respective prevalance in the population.", echo = FALSE}
#---------------------------------------------------------------------
# Graphic of prevalence of each endowment combo and its value
cellText_size <- 2.5
dfStrat <- data.frame(Type = endowVec, v0 = round(v0, 1)) %>%
  mutate(xx = 1) %>% group_by(Type) %>%
  summarise(`Number of farms` = sum(xx, na.rm = T), v0 = unique(v0)) %>%
  as.data.frame() %>%
  mutate(`Pct of farms in sample` = `Number of farms` / sum(`Number of farms`) * 100) %>%
  arrange(v0) %>% mutate(Type = factor(Type, Type))
stratPct <- dfStrat$`Pct of farms in sample` / 100
v0strat <- dfStrat$v0
muV0 <- sum(v0strat * stratPct)
...
#-----------------------------------------------------------------------------
typeVec <- dfStrat$Type
mCstari <- 1 / (1 - h) * (log(v0strat) + mEcSharpT0)
#mCstari <- 1 / (1 - h) * (log(v0strat) + mK + mEcSharpT0)
muK <- exp(mK + sK^2 / 2)
muCstari <- exp(mCstari + sCstar^2 / 2)
muCstarih <- exp(h * mCstari + h^2 * sCstar^2 / 2)
muYstarTicheck <- mean(v0strat * muK) * muCstarih
muYstarTi <- v0strat * muK * muCstarih
muYstarT <- sum(muYstarTi * stratPct)
muCstar <- sum(muCstari * stratPct)
#---
# dfStrat$`Mean yield\n(kg/acre)` <- round(c(muYstarTi, muYstarT), 1)
# dfStrat$`Mean cost/area\n(KES/acre)` <- round(c(muCstari, muCstar), 1)
dfStrat$`Mean yield\n(kg/acre)` <- round(muYstarTi, 1)
dfStrat$`Mean cost/area\n(KES/acre)` <- round(muCstari, 1)
dfTab <- dfStrat %>% rename(`TFP (v0)` = v0) %>%
  relocate(`Type`, `TFP (v0)`, `Mean cost/area\n(KES/acre)`, `Mean yield\n(kg/acre)`) %>%
  select(`Type`, `TFP (v0)`, `Mean cost/area\n(KES/acre)`, `Mean yield\n(kg/acre)`) %>% gather(var, val, `TFP (v0)`:`Mean yield\n(kg/acre)`)
...
ggEndow1 <- gg
#---
gg <- ggplot(dfStrat, aes(x = `Pct of farms in sample`, y = Type))
#gg <- gg + scale_y_discrete(limits = typeVec)
gg <- gg + geom_col()
...
(ggEndow1 / ggEndowPop + plot_layout(heights = c(1, 1 / 14))) | (ggEndow2 / ggBlank + plot_layout(heights = c(1, 1 / 14)))
...
A key message emerging from these results is that aggregate supply varies widely with meteorological conditions. It is thus highly misleading to speak of a single supply "curve". Rather, individual supply curves represent specific cases within a broad range of meteorological scenarios. It is thus more accurate to speak of a supply range or "ribbon" bounded by supply curves on top and bottom representing best and worst case meteorological scenarios, respectively.
...
Note that the increase in TFP is the same regardless of the starting value of $g_0$, which varies across the population because the binary endowments $b_{i \in [1,4]}$ vary.

The resulting percentage changes in farm level expenditure TFP ($v_0$), expenditure per acre, and yield given a 1 percentage change in $g_0$ follow from Equations \ref{eq:v0forms}, \ref{eq:mCk}, and \ref{eq:yStarT}, respectively.
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \frac{\partial \ln(C^*)}{\partial \ln(g_0)} &= \frac{\partial \ln(E[C^*])}{\partial \ln(g_0)} = \frac{\partial \ln(y^*(T))}{\partial \ln(g_0)} = \frac{\partial \ln(E[\tilde{y}^*(T; k_m(T))]|_{t = 0})}{\partial \ln(g_0)} \\ -->
<!-- &= \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} \right) -->
<!-- \end{split} -->
<!-- \label{eq:ddlng0} -->
<!-- \end{equation} -->

<!-- Where the last equality follows noting that $u_0 = v_0 E[k] E[C^*]^h$. -->
<!-- -- -->

(Note from Equation \ref{eq:useful2} that expected optimal unit expenditure does not vary with changes in $g_0$.)

The aggregate impact of such interventions hinges critically on the adoption rate. In Appendix C it is shown that axioms 1 and 2 imply the following technology adoption condition.

<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \partial \ln(E[P(T)]|_{t = 0}) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g_0) + \frac{h}{C^*} \partial B &> 0 \\ -->
<!-- \partial \ln(\nu) + \partial \ln(g_0) + \frac{h}{C^*} \partial B &> 0 \:\:;\:\:\: \partial \ln(\nu) = \partial \ln(E[P(T)]|_{t = 0}) - \boldsymbol{\alpha} {'} \partial \ln(\mathbf{w}) -->
<!-- \end{split} -->
<!-- \label{eq:adoptCond} -->
<!-- \end{equation} -->
...
<!-- Widespread adoption of yield scaling enhancements tends to boost production and thus lower output price while at the same time increasing input demand and thus input prices. Hence the expectation is that $\partial \ln(E[P]|_{t = 0}) < 0$, $\partial \ln(\mathbf{w}) > 0$, and thus $\partial \nu < 0$. In words, then, the adoption condition says that a farm stands to increase its net benefit through adoption of yield scaling technology only if the resulting percentage increase in TFP ($g_0$) is greater than the corresponding percentage decrease in output price minus the corresponding percentage increase in input prices scaled by their respective input elasticities $\alpha_i$. -->

<!-- Note the resemblance between the adoption condition in Inequality \ref{eq:adoptCondSimp} and the "price/productivity tightrope" condition formulated by Lipton [-@lipton2005family]. -->

<!-- >\singlespacing "New science usually raises farm supply of outputs and demand for inputs. That makes outputs cheaper and inputs more expensive; hence the ratio of farm output prices to input prices falls. Do small and poor farmers gain? If, and only if, this science-induced fall in their relative farm prices is slower than the science-induced rise in their conversion ratio of physical inputs into physical outputs (that is, TFP)." -->

<!-- \doublespacing -->

<!-- The only substantive difference between Lipton's condition and the condition in Inequality \ref{eq:adoptCondSimp} is that the percentage increases in input prices in the latter are scaled by their respective elasticities ($\alpha_i$). And of course the more complete version of the adoption condition in Inequality \ref{eq:adoptCond} differs from Lipton by also including the exogenous net subsidy term. -->

<!-- If the efficiency adjusted output-input price ratio decrease is greater than the increase in TFP ($g_0$), then the margin by which it is greater is the net subsidy term $h / C^* \partial B$ required to make adoption profitable. Note that the required net subsidy $\partial B$ will vary across farms based on $C^*$. The factor $h / C^*$, which may also be written $\lambda / E[R^*(T)]|_{t = 0}$, represents each farm's marginal valuation of the subsidy; and implies that the subsidy is relatively more valuable to farms with relatively lower expenditure. Solving Inequality \ref{eq:adoptCond} for $C^*$ gives the maximum expenditure level for which adoption is profitable if the (absolute value of the) efficiency adjusted output-input price ratio decrease is greater than the increase in TFP ($|\partial \ln(\nu)| > \partial \ln(g_0)$). -->
...
<!-- If $\partial B=0$, then Inequality \ref{eq:adoptCond} implies that adoption will be either $0\%$ or $100\%$. Empirically, this is almost never observed. Adoption rates tend to fall somewhere between $0\%$ and $100\%$. The increased input demand associated with new technology adoption implies in and of itself that, without subsidies, $\partial B < 0$. And, of course, every major green revolution success story has involved subsidies of one kind or another to offset the required increase in input expenditure, and then some []. Within the theoretical framework developed here, this means that the net subsidy $\partial B$ is generally positive. -->
<!-- <!-- Assuming farms spend their whole farm budget ($B = C^*$), then the subsidy term simplifies to $h \partial \ln(C^*)$ and the adoption condition can be rewritten --> -->

<!-- Denoting the upper adoption bound on $C^*$ (Equation \ref{eq:cStarAdopt}) as $\bar{\bar{C^*}}$ and recalling that axiom 4 implies lognormally distributed $C^*$, the adoption rate $A$ when $\partial B > 0$ and $|\partial \ln(\nu)| > \partial \ln(g_0)$ can be calculated as -->

<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \mathcal{A} &= \int_0^{\bar{\bar{C^*}}} \phi \left(\frac{\ln(C^*) - m_c}{s_c} \right) d \ln(C^*) \:\:;\:\:\: \bar{\bar{C^*}} = \frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} \\ -->
<!-- &= \Phi \left( \frac{\bar{\bar{C^*}} - m_c}{s_c} \right) -->
<!-- \end{split} -->
<!-- \end{equation} -->
...
<!-- \end{equation} -->
**--
<!-- Recalling from Equation \ref{eq:mCk} that $C^* = (v_0 k C^{\#})^{1 / (1 - h)}$, then $m_c = 1 / (1 - h)(\ln(v_0) + m_k + m_{\#})$ and $s_c^2 = 1 / (1 - h)^2 (s_k^2 + s_{\#}^2)$ (where $m_k = E[\ln(k)]$ and $s_k^2 = Var[\ln(k)]$); and so the adoption rate can be written in terms of known parameters. -->
<!-- ; and from Equation \ref{eq:useful2} that $m_{\#} = \ln(E[P(T)]|_{t = 0}) - m_{\lambda}$ (where $m_{\lambda} = E[\ln(\lambda)]$) -->
<!-- \begin{equation} -->
<!-- A = \Phi\left( \frac{\frac{-h (1 - h) \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} - (\ln(v_0) + m_k + m_{\#})}{\sqrt{s_k^2 + s_{\#}^2}} \right) -->
<!-- \label{eq:adoptRate} -->
<!-- \end{equation} -->

<!-- Note that this is the adoption rate of farms already participating in production. The number of adopting farms is thus -->

<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- N_{adopt} &= \text{Adoption rate} \times \text{Farm employment rate} \times N \\ -->
<!-- &= A \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right) N -->
<!-- \end{split} -->
<!-- \end{equation} -->

Because the adoption rate is partly a function of $v_{0i}$, it varies across strata. The adoption rate for the whole population is then

\begin{equation}
\begin{split}
\mathcal{A} = \sum_i \frac{N_i}{N} \mathcal{A}_i \\
&\text{where} \\
&\mathcal{A}_i = \Phi\left( \frac{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} - m_{ci}}{s_c} \\
&m_{ci} = \frac{1}{1 - h}(\ln(v_{0i}) + m_k + m_{\#})
\end{split}
\label{eq:AdoptRatei}
\end{equation}
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \frac{\partial \ln(E[Q(T)]|_{t = 0})}{\partial \ln(g_0)} = \frac{\partial \ln E[\tilde{y}^*(T; k_m(T))]|_{t = 0}}{\partial \ln(g_0)} \\ -->
<!-- &= \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} \right) -->
<!-- \end{split} -->
<!-- \end{equation} -->

The total increase in aggregate supply for each stratum ($\Delta Q_i$) can be evaluated by integrating the change in farm production over the density of adopting farms $(0, \bar{\bar{y^*}})$ where $\bar{\bar{y^*}} = y^*(\bar{\bar{C^*}})$.
...
\begin{equation}
\begin{split}
\Delta Q_i &= N_i \int_0^{\bar{\bar{y^*}}} k_a \partial \y^* \rho(y^*) dy^* \\
&= N_i E[k_a] \int_0^{\bar{\bar{y^*}}} y^* \partial \ln(y^*) \rho(y^*) dy^* \\
&= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \int_0^{\bar{\bar{y^*}}} y^* \rho(y^*) dy^* \\
&= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) v_{0i} E[k] \int_0^{\bar{\bar{C^*}}} C^{*h} \rho(C^*) dC^* \\
&= N_i E[k_a] v_{0i} E[k] E[C^{*h}]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_c}{s_c} - h s_c  \right)
\end{split}
\end{equation}
<!-- $$ -->
<!-- N_i E[k_a] v_{0i} E[k] E[C^{*h}]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_c}{s_c} - h s_c  \right) -->
...
Where the resolution of the integral in the last line follows from Equation \ref{eq:keyResult}. Recalling that $v_{0i} = E[y^*]_i / (E[k] E[C^{*h}]_i)$ (Equation \ref{eq:v0i}), this further reduces to

\begin{equation}
\Delta Q_i = N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right)
\label{eq:deltQ}
\end{equation}
<!-- $$ -->
<!-- \Delta Q_i = N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right) -->
...
For purposes of evaluation of $\Delta Q_i$ based on farm data, this is a useful expression. Another instructive expression can be found by expressing the $\Phi()$ term in stochastic price parameter space. To do this, first recall that $C^* = (v_0 k C^{\#})^{\eta / h}$ (Equation ...), such that the argument of $\Phi()$ can be rewritten

\begin{equation}
\frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c = \frac{\ln(\bar{\bar{C^{\#}}}) - m_{\#}}{s_{\#}} - \eta s_{\#}
\end{equation}
...
\begin{equation}
\begin{split}
E[y^*]_i \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right) &= E[y^*| C^* \leq \bar{\bar{C^*}}]_i \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} \right) \\
E[y^*]_i (1 - \Phi(\zeta)) &= E[y^*| C^* \leq \bar{\bar{C^*}}]_i \mathcal{A}_i
\end{split}
\end{equation}

Such that the expression for $\Delta Q_i$ may be rewritten as follows.

\begin{equation}
\begin{split}
\Delta Q_i &= N_i E[k_a] E[\tilde{y}^*(T, k_m(T))]_i|_{t = 0} \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) (1 - \Phi ( \zeta )) \\
&= N_i \mathcal{A}_i E[k_a] E[\tilde{y}^*(T, k_m(T))| C^* \leq \bar{\bar{C^*}}]_i|_{t = 0} \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w}))
\end{split}
\end{equation}
<!-- For purposes of evaluation given farm data, the left hand side is   Phi term better evaluated as in Equation \ref{eq:deltQ}. -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \Delta Q_i &= N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right) \\ -->
<!-- &= N_i \mathcal{A}_i E[k_a] E[\tilde{y}^*(T, k_m(T))]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w})) (1 - \Phi ( \zeta ) \:\:;\:\:\: \zeta = \frac{\ln \left( \frac{E[\tilde{C}^{\#}(T)]|_{t = 0}}{\bar{\bar{C^{\#}(0)}}} \right) - \left( m_p - \eta \frac{s_p^2}{2} \right) T}{s_p \sqrt{T}} -->
<!-- \end{split} -->
<!-- \end{equation} -->
<!-- \begin{equation} -->
<!-- \Delta Q_i &= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) E[\tilde{y}^*(T, k_m(T))| C^* \leq \bar{\bar{C^{\#}(0)}}]_i \mathcal{A}_i -->
<!-- \end{equation} -->

The total change in expected aggregate supply across all strata then follows as
...
In Appendix ... it is shown that farm net benefit per unit area may be expressed

\begin{equation}
\begin{split}
\mathcal{L}^* &= P y^* + \lambda (B - C^*) \\
&= r ((1 - h) C^* + h B ) \:\:;\:\:\: r = \frac{\lambda}{h} = \frac{P}{C^{\#}}
\end{split}
\end{equation}

The marginal change in farm net benefit given a percentage change in $g_0$ follows as

\begin{equation}
\begin{split}
\frac{\partial \mathcal{L}^*}{\partial \ln(g_0)} &= r \left((1 - h) \frac{\partial C^*}{\partial \ln(g_0)} + h \frac{\partial B}{\partial \ln(g_0)} \right) \\
&= r \left( C^* \left(1 - \boldsymbol{\alpha} ' \frac{\partial \mathbf{w}}{\partial \ln(g_0)} \right) + h \frac{\partial B}{\partial \ln(g_0)} \right)
\end{split}
\end{equation}

The farm level change in net benefit per unit area given the intervention $\partial \ln(g_0)$ may thus be expressed

\begin{equation}
\begin{split}
\partial \mathcal{L}^* &= r ( C^* \left(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \mathbf{w} \right) + h \partial B ) \\
&= r \gamma (C^* - \bar{\bar{C}^*}) \:\:;\:\:\: \gamma = \left(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \mathbf{w} \right) \\
&= \gamma P (y^* - \bar{\bar{y^*}}) \\
\partial \mathcal{L}^*(C^*, k)_i &= \gamma P v_{0i} k  (C^{*h} - \bar{\bar{C^{*h}}})
\end{split}
\end{equation}

The aggregate change in net benefit ($\Delta \psi_i$) for each stratum can then be evaluated by integrating $\partial \mathcal{L}^*(C^*, k)_i$ over the farm expenditure ($C^* \in (0, \bar{\bar{C^{*h}}})$), exogenous factor ($k$), and area ($k_a$) densities.

\begin{equation}
\begin{split}
\Delta \psi_i &= N_i \int_0^{\infty} k_a \int_0^{\infty} k \int_0^{\bar{\bar{C}^{*}}} \partial \mathcal{L}^*(C^*, k)_i \rho(C^*) d C^* \rho(k) dk \rho(k_a) dk_a \\
N_i E[k_a] E[k] v_{0i} P \gamma \int_0^{\bar{\bar{C}^{*}}} (C^{*h} - \bar{\bar{C^{*h}}}) \rho (C^*) d C^* \\
&= N_i E[k_a] E[k] v_{0i} P \gamma (E[C^{*h}]_i (1 - \Phi(\zeta)) - \bar{\bar{C^{*h}}} \mathcal{A}_i) \\
&= N_i E[k_a] P \gamma (E[y^*]_i (1 - \Phi(\zeta)) - E[k] v_{0i} \bar{\bar{C^{*h}}} \mathcal{A}_i) \\
&= N_i E[k_a] P \gamma \left( E[y^*]_i (1 - \Phi(\zeta)) - E[\bar{\bar{y}^*}] \mathcal{A}_i \right)
\end{split}
\end{equation}
...
\begin{equation}
\Delta \psi_i = N_i E[k_a] P \gamma \mathcal{A}_i \left( E[y^*| C^* \leq \bar{\bar{C^{*h}}}]_i -  E[\bar{\bar{y}^*}] \right)
\end{equation}

The total change in aggregate net benefit summed across all strata then follows as

\begin{equation}
\Delta \psi = \sum_i \Delta \psi_i
\label{eq:dPsi}
\end{equation}
...
dlMuYstarT <- 1 / (1 - h) * (dlnNuh + dlng0)
dB <- 550
#---
# Save to mark on heat maps below
dlNuPoint <- dlnNu
dBPoint <- dB
#---
cStarMaxAdopt <- -h * dB / (dlnNu + dlng0)
z <- (log(cStarMaxAdopt) - mCstari) / sCstar
adoptRateStrat <- pnorm(z)
adoptRateTotal <- sum(adoptRateStrat * stratPct)
aggSubsidyStrat <- stratPct * dB * Nf * muArea * adoptRateStrat * 10^-9 # Total subsidies in billions
aggSubsidy <- dB * Nf * muArea * adoptRateTotal * 10^-9 # Total subsidies in billions
#sum(aggSubsidyStrat) - aggSubsidy # Check if zero
aggSubsidyUSD <- aggSubsidy / 67 * 10^3 #in million USD
aggSubsidyStratMilln <- aggSubsidyStrat * 10^3
# A = \Phi\left( \frac{\frac{-h (1 - h) \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} - (\ln(v_0) + m_k + m_{\#})}{\sqrt{s_k^2 + s_{\#}^2}} \right)
phiArg <- (log(cStarMaxAdopt) - mCstari) / sCstar - h * sCstar
probProfAdopt <- pnorm(phiArg)
dPsiStrat <- stratPct * Nf * muArea * muYstarTi * EPT02007 * (dlnNu + dlng0) * (probProfAdopt - cStarMaxAdopt / muCstarih * adoptRateStrat)
dPsiStrat <- dPsiStrat * 10^-9 # in billions KES
dPsi <- sum(dPsiStrat)
#dPsi / 67.32 # 2007 USD exchange rate
dQStrat <- stratPct * Nf * muArea * muYstarTi * (dlnNuh + dlng0) / (1 - h) * probProfAdopt
# \Delta Q_i &= N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right)
...
Returning to the Kenya maize data, say a new technology package is released in Kenya that results in a `r 100 * dlng0`% increase in TFP on adopting farms. This is tantamount to adding another binary endowment $b_5\beta_5 =$ `r round(betaNew, 2)` to the yield model. Say that the expected $\boldsymbol{\alpha}$-adjusted percentage increase in input prices ($\boldsymbol{\alpha} '\partial \ln(\mathbf{w})$) is `r aDotdlw`, and the expected percentage decrease in output price ($\partial \ln(P)$) is `r dlP`, such that $\partial \ln(\nu) =$ `r dlnNu`, which is greater than the percentage increase in TFP. Adoption would thus result in a `r 100 * dlv0dlg0`% increase in expenditure TFP ($v_0$), a `r -round(100 * dlMuCstar)`% reduction in expenditure, and a `r round(100 * dlMuYstarT)`% increase in yield on adopting farms (recall Equations \ref{eq:dlv0dlg0}, \ref{eq:dlyStardlg0}, and \ref{eq:dlCstardlg0}). However, by the criterion in Inequality \ref{eq:adoptCondSimp}, adoption makes economic sense for the farmer only if a net subsidy is included in the technology package (i.e., only if $\partial B > 0$).

Say that the Ministry of Agriculture offers a net subsidy of $\partial B = $ `r dB` KES/acre to adopting farms, perhaps in the form of free seed and fertilizer. This then implies the adoption expenditure per acre ceiling of $\bar{\bar{C}^*} =$ `r round(cStarMaxAdopt)` KES (recall Equation \ref{eq:cStarAdopt}), and the corresponding adoption rate of $\mathcal{A} =$ `r round(100 * adoptRateTotal, 1)`% (recall Equation \ref{eq:AdoptRatei}). The aggregate net subsidy required from the Ministry of Agriculture to achieve this rate follows from Equation \ref{eq:aggSubsidy} as `r round(aggSubsidy, 2)` billion KES. The corresponding increase in aggregate supply and producer welfare follow from Equations \ref{eq:dQ} and \ref{eq:dPsi} as `r round(dQ, 2)` 1000 MT and `r round(dPsi, 2)` billion KES, respectively. The breakdown by stratum is given in the left panel of Figure \ref{fig:intervention}, accompanied by a graphic of the corresponding supply ribbon shift in the right panel.
<!-- By the criterion in Inequality \ref{eq:adoptCondSimp}, adoption in the absence of exogenous subsidies/costs will increase farm net benefit only if the the percentage increase in TFP $\partial \ln(g_0) =$ `r 100 * dlng0`% is greater than the absolute value of the percentage decrease in output price plus the $\alpha_i$-scaled increase in input prices (i.e. only if $\partial \ln(g_0) > |\partial \ln(\nu)|$).  -->
[at what rain level?]

\singlespacing

```{r Fig5, fig.show = "hold", fig.width = 7, fig.height=3, fig.align="center", fig.cap="\\label{fig:intervention}(Left) The adoption rate, change in welfare, and change in supply for each of the 14 binary endowment strata, for the technological intervention described in the text. (Right) The corresponding expected aggregate supply ribbon shift, where the dotted lines represent the new normal and dismal rainfall bounds of the aggregate supply ribbon.", echo = FALSE}
...
ggShift <- gg
#---
dfPopRow <- data.frame(Type = "Population total",
                       var = unique(dfTab$var),
                       val = c(round(100 * adoptRateTotal, 1), round(dPsi, 1), round(dQ, 1)))
gg <- ggplot(dfPopRow, aes(x = var, y = Type, fill = val, label = val))
gg <- gg + geom_tile(alpha = 0)
gg <- gg + theme_bw()
gg <- gg + theme(axis.title = element_blank(),
...
(ggShift / ggShiftPop + plot_layout(heights = c(1, 1 / 14))) | (ggEQsT0Shift)
...
For the purpose of policy formation, it is instructive to consider, at a glance, multiple farm subsidy and price change scenarios, and their supply and welfare outcomes, in a heat map format (Figure \ref{fig:decisionMaps}). These heat maps facilitate digestion of the possible supply and welfare outcomes of a wide range of subsidy policy options and assumptions regarding input and output price change. The intervention scenario considered a point on a map of possible supply shift and welfare outcomes given the expected 
[at what rain level?]
...
# Adoption rate map
nRes <- 20
# dlAw <- seq(0.05, 0.35, length.out = nRes)
# dlP <- seq(-0.01, -0.35, length.out = nRes)
# dlNuVec <- dlP / dlAw
aDotdlwVec <- seq(0.18, 0.45, length.out = nRes)
dlP <- -nuRatio * aDotdlwVec
#dlNuVec <- seq(-0.65, -0.36, length.out = nRes)
...
for(i in 1:nRes){
  ...
  z <- (lCstarMaxAdopt - mCstari) / sCstar
  adoptRateStrat <- pnorm(z)
  adoptRateTotal <- sum(adoptRateStrat * stratPct)
#---
phiArg <- (log(cStarMaxAdopt) - mCstari) / sCstar - h * sCstar
probProfAdopt <- pnorm(phiArg)
dPsiStrat <- stratPct * Nf * muArea * muYstarTi * EPT02007 * (dlng0 + dlNu) * (probProfAdopt - cStarMaxAdopt / muCstarih * adoptRateStrat)
dPsiStrat <- dPsiStrat * 10^-9 # in billions KES
dPsi <- sum(dPsiStrat)
# dPsiNet <- dPsi - dB * adoptRateTotal * Nf * muArea * 10^-9
# dPsi / 67.32 # 2007 USD exchange rate
#---
dQStrat <- stratPct * Nf * muArea * muYstarTi * (dlng0 + dlnNuh) / (1 - h) * probProfAdopt
# \Delta Q_i &= N_i E[k_a] E[y^*]_i \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right)
dQStrat <- 10^-3 * dQStrat # in MT
dQStrat <- 10^-3 * dQStrat # in 1000 MT
dQ <- sum(dQStrat)
#---
matDq[i, j] <- round(dQ, 3)
matDpsi[i, j] <- round(dPsi, 3)
...
gg <- gg + labs(fill = "Change in aggregate\nproducer welfare (billion KES)", x = "Decrease in output input price ratio (%)",
                y = "Net subsidy (KES/acre)")
gg <- gg + theme_bw()
gg <- gg + theme(axis.text = element_text(size = axisTextSize),
                 axis.title.x = element_text(size = axisTitleSize),
...
The theoretical supply curve and imact assessment framework presented above... to solve for equilibrium price and consumer welfare impacts require demand curve specificatiobn...which extends beyond the scope of this article but you can guess what it would be...input demand integrated over density...or monopsony...a single firm or cartel of firms buying all the production....so consumer welfare is actually firm welfare, to get at consumers have to go beyond a bit
this is comparative statics. welfare has to be discounted according to release/adoption period. the change in prices, especially output price, is not immediate, lag... the adoption rate refers to the economically rational level of adoption once prices adjust to the intervention. many might adopt at first but then find out that it is not profitable and back out as prices adjust. welfare and subsidy incur year by year
...
Do not exogenously impose a change in expenditure and/or yield by fiat, but rather endogenously compute the change in expenditure and yield given an exogenous change in TFP and prices.
can disaggregate results (adoption rate, supply change, and welfare change) by stratum
another way is to shock h, much more ambiguous results.
distributional questions
also can look at shifts resulting from changes to k. also shifts due to changes in input price multiple effects at the same time.... mean prob of profit vs. farm employment
can also look at shocks to continuous endowments k, rainfall etc., many of these simultaneously...
...
A great deal of nuance may be introduced. For example, if the technology package intervention explored above represents improved seed, then this implies that the strata planting hybrid seed do not simply add the new binary endowment $b_5\beta_5$ to their yield models, but also remove the hybrid seed endowment. The theoretical modeling framework developed here can account for such nuance in a straightforward and transparent manner. The conventional ex-ante impact assessment frameworks cannot.
can do same analysis focusing on specific strata.
could examine the supply and welfare impact if hybrid maize adopted in strata where not yet adopted

have not considered the role of price volatility

all the things we couldn't get to

the percentage shift $\partial \nu$ will ultimately depend upon the change in equilibrium price, which requires the demand curve... no space here but demand curve will look like an aggregate input demand curve.

Conventional approach is exogenously determine adoption rate by expert opinion. Here adoption rate is endogenous to the theoretical framework.

minimum data contexts

Have accumulated many departures from the conventional wisdom along the way that are important to point out. (and yet few would argue with axioms 1 and 2)
Take a moment to take stock of the many contradictions between the statements generated so far by these two very uncontroversial axioms and the conventional wisdom.
--strictly decreasing returns to scale, else downward sloping supply, breakeven or loss, and unbounded optimal expenditure and constant 100% employment regardless of price, no market exit or entry in response to price
--cost increasing in tech (above)
--yield increasing in cost (above)
--farms do not have identical marginal cost (key assumption underlying the linear aggregate supply function)
--identical marginal cost implies constant returns to scale and perfectly inelastic aggregate supply
--marginal cost constant
--marginal cost relation to unit cost

map between employment rate and PEFS adj mean prob of profit
...
Specifically, then, the scaling shift can be modeled by shocking the parameter $u_0$. This results in an unambiguous increase in aggregate supply. That is to say, it is straightforward to see that



Evaluating the lagrangian at $y^*$ gives the optimal farm profit or net benefit.


The adoption condition for a new scaling technology is $\frac{\partial \mathcal{L}^*}{\partial u_0}$.

In the theoretical framework derived here, the adoption rate follows endogenously from the farm net benefit (profit) envelope.

<!-- To model the effect of technologies that increase input efficiency, the     Another approach is to introduce a  -->

<!-- Substituting $E[C^{\#}(T)]|_{t = 0} E[y^*(T)]|_{t = 0}$ for $C^*$ in the expected farm yield (Equation \ref{eq:stochYd2}) and solving for $E[y^*(T)]|_{t = 0}$ gives the expected yield in terms of unit cost. -->

<!-- \begin{equation} -->
<!-- E[y^*(T)]|_{t = 0} = E[\tilde{y}^*(T)]|_{t = 0} \left( \frac{\hat{k}}{E[k]} \right)^{\eta + 1} \frac{E[C^{\#}(T)]|_{t = 0}^{\eta}}{E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0}} -->
<!-- \label{eq:stochYd3} -->
<!-- \end{equation} -->

<!-- <!-- (Detailed steps given in Appendix 2C.) -->
<!-- An expression for $s^2_c$ then follows as -->

<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- C^* &= E[C^{\#}(T)]|_{t = 0} E[y^*(T)]|_{t = 0} \\ -->
<!-- &= E[\tilde{y}^*(T)]|_{t = 0} \left( \frac{\hat{k}}{E[k]} \right)^{\eta + 1} \frac{E[C^{\#}(T)]|_{t = 0}^{\eta + 1}}{E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0}} -->
<!-- &\rightarrow s^2_c = Var[\ln(C^*)] = (\eta + 1)^2 (s^2_k + s^2_{\#}) \:\:;\:\:\: s^2_k = Var[\ln(\hat{k})] -->
<!-- \end{split} -->
<!-- \end{equation} -->

<!-- Now, rewriting the unconditional mean expected yield (Equation \ref{eq:muYstarU0}) in stochastic terms, -->

<!-- \begin{equation} -->
<!-- E[\tilde{y}^*(T)]|_{t = 0} = u_0 \frac{E[C^{*h}]}{E[C^*]^h} -->
<!-- \end{equation} -->

<!-- And recalling that $E[y^*(T)]|_{t = 0}$ and thus $C^*$ are assumed to be lognormally distributed, this reduces to -->

<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- E[\tilde{y}^*(T)]|_{t = 0} &= u_0 \frac{e^{hm_c + h^2 \frac{s^2_c}{2}}}{e^{hm_c + h \frac{s^2_c}{2}}} = u_0 e^{\frac{s^2_c}{2}(h^2 - h)} \\ -->
<!-- &= u_0 e^{\frac{1}{2} (\eta + 1)^2 (s^2_k + s^2_{\#}) (h^2 - h)} -->
<!-- \end{split} -->
<!-- \label{eq:muYdStoch1} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \frac{\partial \ln(E[\tilde{y}^*(T)]|_{t = 0})}{\partial h} = s^2_c\left(h - \frac{1}{2} \right) -->
<!-- \begin{cases} -->
<!-- < 0, & \text{if $h$ < 1 / 2} \\ -->
<!-- = 0, & \text{if $h$ = 1 / 2} \\ -->
<!-- > 0, & \text{if $h$ > 1 / 2} -->
...
<!-- The unconditional mean yield increases with technology only if $h > 1 / 2$. Implies a critical threshold on one side of which a subsitence or cost saving mentality predominates---the farmer reasons that they can achieve the same level of income by producing, and thus spending, less---whereas for $h > 1 / 2$ the entrepreneurial mentality takes hold---the farmer reasons that they can raise their income level  -->
<!-- the marginal value of less effort, more free time is greater than that of increased income. Beyond the critical value of 1/2 the opposite is true. -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \frac{\partial \ln(E[Q(T)]|_{t = 0})}{\partial h} &= \frac{\partial \ln(E[y^*(T)]|_{t = 0})}{\partial h} + \frac{\partial \ln(\Phi(-z))}{\partial h} \\ -->
<!-- &= s^2_c \left(h - \frac{1}{2} \right) + \frac{\phi(-z)}{\Phi(-z)} \frac{\partial (-z)}{\partial h} \\ -->
<!-- &= s^2_c \left(h - \frac{1}{2} \right) - \frac{\phi(-z)}{\Phi(-z)} \frac{\partial (-z)}{\partial h} s_{\#} (\eta + 1)^2 -->
<!-- \end{split} -->
<!-- \end{equation} -->

<!-- Now, recalling Equation \ref{eq:stochYd2}, and substituting $E[C^{\#}(T)]|_{t = 0} E[y^*(T)]|_{t = 0}$ for $C^*$, note that stochastic yield resolves as a function of $E[C^{\#}(T)]|_{t = 0}$. -->

<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- E[y^*(T)]|_{t = 0} &= E[\tilde{y}^*(T)]|_{t = 0} E[k_m(T)]|_{t = 0} \frac{\hat{k}}{E[k]} \frac{(E[C^{\#}(T)]|_{t = 0} E[y^*(T)]|_{t = 0})^{h}}{E[\tilde{C}^{\#}(T)]|_{t = 0} E[\tilde{y}^*(T)]|_{t = 0}^{*h}} \:\:;\:\:\: \tilde{y}^*(T) = y^*(T)\bigr|_{\hat{k} = E[\hat{k}], C^{*h} = E[C^{*h}]} \:\: \tilde{C}^{\#}(T) = E[C^{\#}(T)] \\ -->
<!-- E[y^*(T)]|_{t = 0}^{1 - h} &= E[\tilde{y}^*(T)]|_{t = 0}^{1 - h} \frac{\hat{k}}{E[k]} \frac{E[y^*(T)]|_{t = 0}}{E[\tilde{C}^{\#}(T)]|_{t = 0}} \\ -->
<!-- E[y^*(T)]|_{t = 0} &= E[\tilde{y}^*(T)]|_{t = 0} \left( \frac{\hat{k}}{E[k]} \right)^{1/(1 - h)} \frac{E[C^{\#}(T)]|_{t = 0}}{E[\tilde{C}^{\#}(T)]|_{t = 0}} -->
<!-- \end{split} -->
<!-- \end{equation} -->

<!-- C^* = E[\tilde{y}^*(T)]|_{t = 0} \frac{k}{E[k]} -->

<!-- It follows that -->

<!-- \begin{equation} -->
<!-- \frac{\ln(\partial E[Q(T)]|_{t = 0})}{\partial h} \begin{cases} -->
<!-- < 0, & \text{if $h$ < 1 / 2} \\ -->
<!-- = 0, & \text{if $h$ = 1 / 2} \\ -->
<!-- > 0, & \text{if $h$ > 1 / 2} -->
<!-- \end{cases} -->
...
<!-- In the conventional approach to ex-ante impact assessment, the technology adoption rate is exogenously specified based on expert opinion. -->
...
\begin{equation}
\begin{split}
y^* &= y|_{x_i = x^*_i} = g_0 \prod_{i = 1}^{n_k} k_i^{\kappa_i} \prod_{i = 1}^{n_x} x^*_i^{\alpha_i} \\
&= g_0 \prod_{i = 1}^{n_k} k_i^{\kappa_i} \prod_{i = 1}^{n_x} \left( \frac{y^* P \alpha_i}{\lambda w_i} \right)^{\alpha_i}
&\rightarrow y^* = \left( g_0 \prod_{i = 1}^{n_k} k_i^{\kappa_i} \prod_{i = 1}^{n_x} \frac{\alpha_i}{w_i} \right)^{1 / (1 - h)} \left( \frac{P}{\lambda} \right)^{h / (1 - h)}
\end{split}
\label{eq:yStarAlt}
\end{equation}
...
Note that the expenditure TFP $v_0$ can be expressed

\begin{equation}
\begin{split}
v_0 &= \frac{u_0}{E[k] E[C^*]^h} \\
&= g_0 \frac{\prod_i^{n_x}E[x_i]^{\alpha_i}}{E[C^*]^h} \\
&= g_0 \frac{1}{h}\prod_i^{n_x} \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i}
\end{split}
\label{eq:v0forms}
...
The derivatives $\partial \ln (C^*) / \partial \ln(g_0)$ and $\partial \ln(y^*) / \partial \ln(g_0)$ then follow from the expressions in Equation \ref{eq:fnCsharp} as

\begin{equation}
\begin{split}
\frac{\partial \ln(C^*)}{\partial \ln(g_0)} &= \frac{1}{1 - h} \left( \frac{\partial \ln(v_0)}{\partial \ln(g_0)} + \frac{\partial \ln(C^*)}{\partial \ln(g_0)} \right)  \\
&\rightarrow \frac{\partial \ln(C^*)}{\partial \ln(g_0)} = \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} + \frac{\partial \ln(P)}{\partial \ln(g_0)} \right)
\end{split}
\end{equation}
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- y^* &= y|_{x_i = x^*_i} = u_0 \frac{k}{E[k]} \prod_{i = 1}^{n_x} \left( \frac{x^*_i}{E[x_i]} \right)^{\alpha_i} \:\:;\:\:\: k = \prod_{i = 1}^{n_k} k_i^{\kappa_i} \\ -->
<!-- &= u_0 \frac{k}{E[k]} \prod_{i = 1}^{n_x} \left( \frac{x^*_i}{E[x_i]} \right)^{\alpha_i} -->
<!-- \end{split} -->
<!-- \end{equation} -->
<!-- \begin{equation} -->
<!-- y = u_0 \frac{k}{E[k]} \prod_{i = 1}^{n_x} \left( \frac{x_i}{E[x_i]} \right)^{\alpha_i} \:\:;\:\:\: k = \prod_{i = 1}^{n_k} k_i^{\kappa_i} -->
...
Assuming that farms will adopt a new yield scaling technology only if they expect it to increase their net benefit, the adoption condition is $\partial \mathcal{L}^*/\partial \ln(g_0) > 0$. That is,

\begin{equation}
\frac{\partial \mathcal{L}}{\partial \ln(g_0)} = E[R^*(T)]|_{t = 0} (1 - h) \left( \frac{\partial \ln(P)}{\partial u_0} + \frac{\partial \ln(y^*)}{\partial u_0} \right) + \lambda \frac{\partial B}{\partial u_0} > 0
\end{equation}

The yield derivative resolves by substituting $P y^* h/\lambda$ for $C^*$ in Equation \ref{eq:YdEnvNeat0} and differentiating with repect to $\ln(g_0)$.

\begin{equation}
\begin{split}
\frac{\partial \ln(y^*)}{\partial \ln(g_0)} &= 1 - \mathbf{\alpha} {'} \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} + h \frac{\partial \ln(P)}{\partial \ln(g_0)} + h \frac{\partial \ln(y^*)}{\partial \ln(g_0)} \\
&\rightarrow \frac{\partial \ln(y^*)}{\partial \ln(g_0)} = \frac{1}{1 - h} \left( 1 - \mathbf{\alpha} {'} \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} + \frac{\partial \ln(P)}{\partial \ln(g_0)} \right)
\end{split}
\end{equation}
...
\begin{equation}
\frac{\partial \mathcal{L}}{\partial \ln(g_0)} = \frac{1 - \mathbf{\alpha} {'} \frac{\partial \ln(\mathbf{w})}{\partial u_0}} + \partial \ln(P)}{\partial \ln(g_0)} \right) + \frac{\lambda}{R^*} \frac{\partial B}{\partial u_0} > 0
\end{equation}

Which further reduces to

\begin{equation}
\partial \ln(P) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g_0)  + \frac{\lambda}{R^*} \partial B > 0
\end{equation}

Or, substituting $h / C^*$ for $\lambda / R^*$,

\begin{equation}
\partial \ln(P) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g_0)  + \frac{h}{C^*} \partial B > 0
\end{equation}

And, in stochastic terms,

\begin{equation}
\partial \ln(E[P(T)]|_{t = 0}) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g_0)  + \frac{h}{C^*} \partial B > 0
\end{equation}
