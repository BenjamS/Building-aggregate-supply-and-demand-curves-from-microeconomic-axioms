---
title: A theoretical derivation of the aggregate agricultural supply curve
...
Where the term $\partial B$ represents the marginal subsidies, costs, or any other exogenous mitigating circumstances, per unit area, entailed in the adoption of the technology package. In the absence of such mitigating circumstances, the condition simplifies to
...
Say that the Ministry of Agriculture offers a net subsidy of $\partial B = $ `r dB` KES/acre to adopting farms, perhaps in the form of free seed and fertilizer. This then implies the adoption expenditure per acre ceiling of $\bar{\bar{C}^*} =$ `r round(cStarMaxAdopt)` KES (recall Equation \ref{eq:cStarAdopt}), and the corresponding adoption rate of $\mathcal{A} =$ `r round(100 * adoptRateTotal, 1)`% (recall Equation \ref{eq:AdoptRatei}). The aggregate net subsidy required from the Ministry of Agriculture to achieve this rate follows from Equation \ref{eq:aggSubsidy} as `r round(aggSubsidy, 2)` billion KES. The corresponding increase in aggregate supply and producer welfare follow from Equations \ref{eq:dQ} and \ref{eq:dPsi} as `r round(dQ, 2)` 1000 MT and `r round(dPsi, 2)` billion KES, respectively. The breakdown by stratum is given in the left panel of Figure \ref{fig:intervention}, accompanied by a graphic of the supply ribbon shift in the right panel.
...
policymakers may disagree over details regarding the expected rise in input costs and decrease in output price, and face a changing landscape of fiscal constraints that require consideration of multiple net subsidy scenarios.


the intervention scenario considered just a point on a map of possible supply shift and welfare outcomes given the expected 
can then do the same analysis for a wide range of price and subsidy scenarios

address the alpha in aDotW

```{r Fig5, fig.show = "hold", fig.width = 7, fig.height=3, fig.align="center", fig.cap="\\label{fig:decisionMaps}Decision support heat maps of the (left to right) adoption rate, change in aggregate supply, and aggregate producer welfare, for different 'coordinates' of net subsidy and output-input price decrease given the release of a technology package in Kenya that increases maize TFP by 20%. The ratio of change in input to change in output price is fixed at 0.75 in these maps.", echo = FALSE}
...
# dlNuVec <- dlP / dlAw
aDotdlwVec <- seq(0.15, 0.45, length.out = nRes)
...
dfAdopt <- as.data.frame(matAdopt)
colnames(dfAdopt) <- round(dBvec, 2) # paste("dB =", round(dBvec, 2))
gatherCols <- colnames(dfAdopt)
dfAdopt$dlNu <- round(dlNuVec, 2) #paste("dlnNu =", round(dlnNuVec, 2))
dfPlot <- dfAdopt %>% gather_("dB", "adoptRate", gatherCols)
dfPlot$dB <- as.numeric(dfPlot$dB)
gg <- ggplot(dfPlot, aes(x = dlNu, y = dB, z = adoptRate, fill = adoptRate, group = 1))
gg <- gg + geom_tile(alpha = 1) + geom_contour()
gg <- gg + metR::geom_text_contour(aes(z = adoptRate), stroke = 0.15)
gg <- gg + scale_fill_gradient(low = "coral", high = "cyan")
...
dfPlot <- dfMat %>% gather_("dB", "adoptRate", gatherCols)
dfPlot$dB <- as.numeric(dfPlot$dB)
gg <- ggplot(dfPlot, aes(x = dlNu, y = dB, z = adoptRate, fill = adoptRate, group = 1))
gg <- gg + geom_tile(alpha = 1) + geom_contour()
gg <- gg + metR::geom_text_contour(aes(z = adoptRate),
...
this a direct measure of welfare.
...
have to think of supply not as curve but as a supply channel or ribbon or interval.
