...
However, computational expedience is no longer of pressing concern in this regard; and popularity can be a misleading indicator of quality. It could merely reflect the low cost of implementing the model, for example, and/or the lack of comparably low cost alternatives. A more telling indicator of model quality, we submit, is the growing "disconnect" between the actual level of funding for agricultural research and the level of funding that would be justified by impact assessments conducted on the basis of conventional aggregate supply models [@hurley2016returns]. There is good reason to suspect that this disconnect is due to the implausibly high---and sometimes absurdly high---internal rates of return reported in conventional impact assessments of agricultural R&D [@hurley2014re; @alston2001attribution; @hurley2016returns]. In a meta-analysis of 492 such impact assessments, for example, Hurley et al. [@hurley2016] note that the average reported internal rate of return (IRR) implies that the USDA's 2000 R&D investment of \$4.1 billion should be worth \$56.3 quintillion in 2050, or 2.3 million times projected world GDP at that time. "[I]t is not difficult to see," the authors conclude, "how policy makers may question the credibility of such evidence" [@hurley2016].
...
Hurley et al. [@hurley2014re] respond to the incredulity of policy makers by replacing IRR with their "modified" IRR or "MIRR", whereby implausibly high projected returns are deflated to plausible levels. This is a suitable response insofar as the implausibility in question is rooted in the discounting method (although see the debate between Ohemke [] and Hurley et al. 2014). The present work is motivated by cases where implausible ex ante impact assessment results are rooted in the modeling of exogenous shocks on farm populations---particularly the modeling of aggregate supply and supply curve shifts. In such cases, application of MIRR does not resolve but rather exacerbates implausibility, by masking it and thus complicating identification of the real source of implausibility.
...
To redress cases where implausibility is rooted in the assumptions and theoretical premises and opacity of the modeling, here we present an alternative aggregate supply model that prioritizes theoretical consistency and transparency, such that implausible results may be traced back to specific assumptions regarding economic behavior, and/or the estimation of specific parameter values that have a clear mapping to measurable empirical phenomena. Progress in this direction has long been discouraged by a widespread perception that the task is impossible. In their highly influential book, e.g., Alston et al. [-@alston1995science] write:
...
3) Farms enter production only if they expect to make positive profit

4) Output price and yield follow independent geometric Brownian motions

5) Farm yield is lognormally distributed

Five other desirable properties immediately follow from axioms 1 and 2 as corollaries, and need not be imposed as additional axioms: 1) The farm yield function is homogeneous (and thus homothetic) and 2) concave in inputs; 3) farm supply curves are upward sloping; and 4) input demand curves are downward sloping. We also take a moment to demonstrate 5) that the derived yield function agrees with Lipton's theory of technology adoption [].
...
The aggregate supply model that mathematically results from axioms 1-5 consists of terms that capture technology, agroclimatic conditions, input and output prices, and farm entry/exit. The term capturing farm entry/exit turns out to be surprisingly instructive and intuitive, as it is proportionate to the probability that output price will be greater than the mean farm's expected cost per unit output at harvest---i.e. the mean farm's probability of profit. The resulting expression for price elasticity of aggregate supply is likewise intuitive, consisting of farm level price elasticity of supply plus a term capturing farm entry/exit in response to price movements. The model also accounts for price volatility, such that the effect of price stabilization policies may be explicitly modeled. These gains in theoretical rigor and explanatory power come at little or no cost to expedience on modern processors.  ...it turns out that the supply response to changes in technology is quite complex, and can be negative under certain circumstances...
...
\begin{equation}
Q = N \left( frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E[\theta_k^{\frac{\eta}{h}}] h^{-\eta} \int_{\underline{C}^{\#}}^{\bar{C}^{\#}} C^{\#}^{\eta} f(C^{\#}) \: dC^{\#}
\end{equation}

Where the farm density $\rho(C^{\#})$ that appears in Equation \ref{eq:sup1} has been rewritten in terms of the probability density $f(C^{\#})$ and the number of farms in the population $N$.

4) Assuming that farms enter production only if they expect to make a profit (axiom 4), then cost per unit output cannot exceed price, thus resolving the upper bound of this integral. Assuming the lower bound is $0$, then the domain of integration is $0 < C^{\#} < P$.

\begin{equation}
Q = N \left( frac{\theta_x}{h^{\eta} \theta_w} \right)^{\frac{\eta}{h}} E[\theta_k^{\frac{\eta}{h}}] \int_{0}^{P} C^{\#}^{\eta} f(C^{\#}) \: dC^{\#}
\end{equation}
...
\begin{equation}
\int_{0}^{P} C^{\#}^{\eta} f(C^{\#}) \: dC^{\#} = E[C^{\# \eta}] \Phi \left(\frac{\ln\left(\frac{P}{E[C^{\#}]} \right)^{\eta} - \frac{\eta^2}{2} Var[C^{\#}]}{\eta \sqrt{Var[C^{\#}]}} \right)
\end{equation}
...
\begin{equation}
Q = N \left( frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E[\theta_k^{\frac{\eta}{h}}] \frac{E[C^{\# \eta}]}{h^{\eta}}
\end{equation}

Axiom 3 (farms only enter production if they expect to make a profit) imposes the following domain on $C^{\#}$.
...
Which in turn defines the domain of integration (farm entry/exit) in Equation \ref{eq:sup1}.

An alternative 

Moreover, by Equation \ref{eq:useful1}, the upper bound on cost per unit output implies the following lower bound on the budget shadow price.
