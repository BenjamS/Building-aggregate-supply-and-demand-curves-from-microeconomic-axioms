...
To evaluate this expression, a suitable stochastic model must be assumed for $P(t)$ and $C^{\#}(t)$. Within a given season, prices often follow a geometric Brownian motion (gBm). That is to say, changes in $P$ over a small time interval $\Delta t$ ($\Delta P = P(t + \Delta t) - P(t)$) are often accurately modeled by
...
# Control vars
controlVarsVec <- c("Seed (kg/acre)", "Adult equivalent household size", "Rain anomaly", "Child family labor, planting (person-hrs/acre)", "Cost/acre", "Farm size (acres)")
...
The binary control variables included in this exercise are land tenure (whether or not the head of household holds a title deed to the land they are farming), adoption of hybrid maize, and land preparation endowments (oxen, tractor, or none). There are 14 distinct combinations of these four endowments occurring in the sample, including the "base" case of zero binary endowments. The base case, the ...ytilde  The prevalence of these 14 endowment configurations is summarized in the right panel of Figure ....



<!-- Apart from the usual inputs and outputs (fertilizer, P&D control chemicals, and hired labor), the data also include detailed information on family labor, agroclimatic factors, demography, and infrastructure. -->

Descriptive histograms of the continuously valued variables used in this exercise are presented in Figure ... 

The fertilizer and P&D chemical input "prices" are calculated at farm level as the cost of each input divided by its quantity. The fertilizer data details several different mixtures within three main categories: 
The maize price and input prices are 


Prior to estimation all continuously valued variables except for rainfall anomaly are divided by their respective sample means and the natural log is taken. As explained in ... this transformation means that the mean logged yield is equivalent to the constant plus the binary endowments.

[show equation?]

Continuous independent variables are divided by their sample mean, such that their mean is 0.

Several other input, infrastructural, demographic, and agroclimatic variables, including tribal affiliation, were explored in this exercise, but ultimately discarded due either 
