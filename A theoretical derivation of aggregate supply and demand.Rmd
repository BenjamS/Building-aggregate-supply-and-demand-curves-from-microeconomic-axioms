...
### Conditional and unconditional means \label{sec:condMean}
...
THE IMPLICIT MEAN EXPECTED YIELD...
...in section \ref{sec:condMean}, tractable expression for mean yield, stochastic version

\begin{equation}
E[\tilde{y}^*(T; k_r) | \theta]|_{t = 0} = v k_r^{\kappa_r} E[k'] E[C^{*h} | \theta]
\label{eq:impMuEyStarT0}
\end{equation}
...
dfStrat <- dfx %>% group_by(Type) %>% summarise(Type = unique(Type),
  ...
  mutate(mEcSharpT0 = (1 - h) * mCstar - mK - log(vStrat),
         sEcSharpT0 = sqrt((1 - h)^2 * sCstar^2 - sK^2)) %>%
  mutate(muEcSharpT0truncStrat = exp(mEcSharpT0 + sEcSharpT0^2 / 2)) %>%
...
dfAll <- dfx %>% summarise(v = mean(vStrat),
  ...
  mutate(mEcSharpT0 = (1 - h) * mCstar - mK - log(v),
         sEcSharpT0 = sqrt((1 - h)^2 * sCstar^2 - sK^2)) %>%
  mutate(muEcSharpT0truncStrat = exp(mEcSharpT0 + sEcSharpT0^2 / 2)) %>%
  mutate(stratID = "All",
         Type = "All") %>% relocate(Type)
...
dfAllTab <- dfAll %>% select(Type, v, muCstarTruncStrat, muEcSharpT0truncStrat,
                             muYdHarv, muEyStarT0truncStrat)
...
To evaluate the expected unconditional mean yield term ($E[\tilde{y}^*(T)]|_{t = 0}$) and parameters $m_{\#}$, $s_{\#}$ appearing in the expected aggregate supply expression (Equation \ref{eq:aggSup2}), first the conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ are calculated from the fitted values $v$, $h$, and the sample parameters $\bar{m}_c$, $\bar{s}_c$, $m_k$, and $s_k$, by Equation \ref{eq:mCshrpsCshrp}.
<!-- The disaggregated $\bar{m}_{c \ell}$ are also computed...and then the conditional mean cost and yield are also evaluated both in the aggregate and disaggregated by strata, as explained in section \ref{sec:binEndStrat}. -->
...
The unconditional parameters $m_{\#}$ and $s_{\#}$ can then be computed by solving simultaneous Equations \ref{eq:cSharpUncondParams} at $E[P(T)]|_{t = 0}$ (the expected price at planting time for 2007, the same year as the farm data used to estimate $\bar{m}_{\#}$ and $\bar{s}_{\#}$). [This requires EPT02007].
