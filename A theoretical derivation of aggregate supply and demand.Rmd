...
In July of 2008, after a poor mar-may main rains season the Ministry of Agriculture estimated that the main season harvest would be 2.16 million MT (GIEWS 2008 brief).




The data comes from... Cannot validate against FAO as FAO is post hoc, have to compare to the GIEWS 2008 June forecast.
...
df <- do.call(rbind, listDf) %>% as.data.frame() %>% group_by(Mkt) %>%
  ...
  mutate(year = year(Date),
         month = month(Date)) %>% subset(year <= 2012 & month %in% c(2:9))
#-------------------------------------------------------------------------------------
theseColors <- distinctColorPalette(length(mktVec))
gg <- ggplot(df, aes(x = factor(month.abb[month], levels = month.abb), y = Price, group = Mkt, color = Mkt))
gg <- gg + geom_line(lwd = 0.75)
...
dfHistUp <- df %>% subset(year %in% upYrs)
...
gg <- gg + theme(axis.title = element_blank(),
                 ...
                 strip.text = element_blank(),
                 plot.title = element_text(size = titleSize))
dfHistDown <- df %>% subset(!(year %in% upYrs))
...
dfPT <- df %>% subset(month %in% c("7", "8", "9")) %>% select(Mkt, year, Price, month) %>%
...
EQsFun <- function(EPT0, muEyStarT0, mEcSharpT0, sEcSharpT0, h, muArea, Nf, kappaRn){
  ...
  EQsT0LoRn <- muArea * Nf * 0.5^kappaRn * muEyStarT0 * ProbProfitMeanFarm #Expected rainfall 50% lower than benchmark
  #------------Expected agg supply in million MT
  EQsT0 <- EQsT0 * 10^-9
  EQsT0HiRn <- EQsT0HiRn * 10^-9
  EQsT0LoRn <- EQsT0LoRn * 10^-9
  ...
  #------------
  dfOut <- data.frame(EPT0, EQsT0, EQsT0HiRn, EQsT0LoRn, etaP0EQsT0, ProbProfitMeanFarm,
                      EmploymentRate)# etaP0EQsT0, etaSpEQsT0)
...
#-------------------------------------------------------------------------------------
Nf <- 2 * 10^6; muArea <- mean(exp(dfMod3$`Farm size (acres)`))
EPT0vec <- seq(0.1, 35, length.out = 50)
...
gg <- gg + geom_ribbon(aes(x = `Expected price (KES/kg)`,
                       ...
                       fill='blue',alpha=0.2)
dfLine <- dfOut[, 1:4]; colnames(dfLine)[-1] <- c("Normal rainfall", "High rainfall", "Low rainfall")
dfLine <- dfLine %>% gather(rainLev, `Expected supply\n(million metric tons)`, `Normal rainfall`:`Low rainfall`)
