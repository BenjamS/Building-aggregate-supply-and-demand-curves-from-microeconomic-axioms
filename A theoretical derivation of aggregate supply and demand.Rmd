...
...in passing...diminishing returns to scale. Constant returns to scale ($h = 1$) imply the break-even point, while increasing returns to scale ($h > 1$) imply a loss. The farm yield function is thus globally concave in inputs. Constant and increasing returns, on the other hand, would imply global linearity and convexity, respectively. \eta (PEFS) explained here constant increasing returns also imply flat/downward slpoing farm supply curves...input price elasticity in appendix.
...
This expression is particularly useful as an econometric yield model, as it only requires information on the control variables $k_i$ and total cost $C^*$, and no information on individual input quantities, as would be required when estimating Equation \ref{eq:cobbDoug3}. Given farm data, one specifies the model
...
\bar{s}_{\#}^2 &= s_{\#}^2 \left( 1 - z \frac{\phi(z)}{\Phi(z)} - \left( \frac{\phi(z)}{\Phi(z)} \right)^2   \right) \:\:;\:\:\: z = \frac{\ln(\gamma) - m_{\#}}{s_{\#}}
\end{split}
...
\begin{equation}
E[Q(T)]|_{t = 0} = N E[k_a] E[\tilde{y}^*(T)]|_{t = 0} \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right) \:\:;\:\:\: m_{\#} = E[\ln(E[C^{\#}(T)]|_{t = 0})] \:\:\: s^2_{\#} = Var\ln(E[C^{\#}(T)]|_{t = 0})
\label{eq:aggSup2}
\end{equation}
...
Substituting the dual expressions for $m_{\#}$ and $s^2_{\#}$ in the $\Phi()$ term of Equation \ref{eq:probProfResult}, and substituting $P(0)e^{m_p T}$ for $E[P(T)]|_{t = 0}$ (via Equation \ref{eq:EPT0}), resolves the $\Phi()$ term as follows.
...
The empirical estimates of aggregate supply for the years 2006-2012 are mostly taken from FAO Global Information and Early Warning System (GIEWS) reports published in the middle or towards the end of the main maize cropping season in each respective year---prior to harvest but after planted area and main season rainfall are known. These estimates are summarized in Table 1.
...
EQsT2006Jun14MoA <- 2.7 #GIEWS
#EQsT2007May15MoA <- 1.04 * 2.7 # GIEWS 3 total (short and main), up 4% from prev yr
EQsT2007Sep11usda <- 2.6 # USDA FAS
EQsT2008Jul15MoA <- 2.16 # MoA forecast (GIEWS 2008 report), also reported in USDA FAS Commod Intel Report - 100,000 ha not planted due to post election violence, west received normal rainfall but rest of main maize growing area received 20-80% of normal. https://ipad.fas.usda.gov/highlights/2008/09/kenya/
EQsT2008sep17usda <- 2.55 # USDA FAS Commod Intel Report https://ipad.fas.usda.gov/highlights/2008/09/kenya/
EQsT2009AugMoA <- 1.84 # GIEWS Severe drought, crop failure
EQsT2010Aug5MoA <- 2.6 # GIEWS Good rain, started 2 weeks early
EQsT2011Nov3MoA <- 2.34 # KFSSG Dismal rainfall outside of western Kenya
EQsT2012Jul16MoA <- 2.4 # MLND outbreak, late start to rains
#EQsT2013Jul16MoA <- 1.62 # GoK KFSSG Poorly distributed low rainfall, early cessation
EQsTvec <- c(EQsT2006Jun14MoA, EQsT2007Sep11usda, EQsT2008Jul15MoA,
             EQsT2009AugMoA, EQsT2010Aug5MoA, EQsT2011Nov3MoA, EQsT2012Jul16MoA)
reportDate <- c("14 Jun 2006 (GIEWS)", "11 Sept 2007 (USDA)",
                "15 Jul 2008 (GIEWS)\n(Post-election violence, drought)",
                "Aug 2009\n(Severe drought, crop failure)",
                "5 Aug 2010 (GIEWS)",
                "3 Nov 2011 (KFSSG)\n(Drought)",
                "16 Jul 2012 (GIEWS)")
citeVec <- c("FAO GIEWS 2006", "USDA 2007", "FAO GIEWS 2008", "FAO GIEWS 2009",
             "FAO GIEWS 2010", "KFSSG 2011", "FAO GIEWS 2012")
commentVec <- c("Normal rainfall",
                "Normal rainfall",
                "Post-election violence, drought",
                "Severe drought, crop failure",
                "Good rains that came early, but some crops loss due to flooding",
...
knitr::kable(dfPoints)#, format = "html")

```


To parameterize the expected aggregate supply curve, Equation \ref{eq:lYdEnvReg2} is fit to the data. In this exercise, the constant $v_0$ includes endowment dummy variables that augment the constant on farms where the endowment is present.
...
```{r, fig.show = "hold", fig.width = 5, fig.height=5, fig.align="center", fig.cap="\\label{fig:lognormalYd}", echo = FALSE}
...
modelsummary(mod3, output = "latex", statistic = "p.value",
...
As explained in section \ref{sec:acct4Stoch}, the expected yield $E[y^*(T)]|_{y = 0}$ is calculated as $y^*(T) e^{-\epsilon} k_r^{-\kappa_r}$. The observed optimal expected unit cost is then calculated as $E[C^{\#}(T)]|_{t = 0} = C^* / E[y^*(T)]|_{y = 0}$. As explained in \ref{sec:aggSupDeterm}, axiom 3 truncates the distribution of (expected) optimal unit cost at the (expected) output price. The conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ are calculated from this observed, truncated distribution; and then the unconditional parameters $m_{\#}$ and $s_{\#}$ are calculated by solving simultaneous Equations \ref{eq:cSharpUncondParams}. In addition to $\bar{m}_{\#}$ and $\bar{s}_{\#}$, this requires an estimate of expected harvest price at planting time ($E[P(T)]|_{t = 0}$).

The Kenya farm data includes the harvest output price as observed at harvest $P(T)$, but not $E[P(T)]|_{t = 0}$. Recall from Equation \ref{eq:EPT0} that calculation of $E[P(T)]|_{t = 0}$ requires the maize price at planting $P(0)$, the drift parameter $m_p$, and the time from planting to harvest $T$. In the present exercise, this information is taken from the World Food Program main season monthly price series plotted in Figure \ref{fig:mzP} [@WFPpriceData]. The price at planting in March $P(0)$ is calculated as the average across the main maize markets. The drift parameter $m_p$ is then calculated as the mean log change from one month to another. Missing values are omitted from these calculations.

When calculating $m_p$, it is important to take note of price dynamics peculiar to the market in question. In Kenya, the short rains maize crop is harvested about a month before the main season crop is planted. If the short rains harvest is good, then the maize price tends to remain flat during the main season or decrease a little. If the short rains harvest is poor, then the price tends to trend upwards during the main season and to be a little more volatile. This is reflected in Figure \ref{fig:mzP}, where the price series in 2008, 2009, 2011, and 2012---all drought years---are upward trending, following a poor short rains season; and where the series are flat during the other years, when rainfall was normal. To accommodate this dichotomy, $m_P$ is calculated separately for bad short rain years and good short rain years. For years beginning with a bad short rains harvest, $E[P(T)]|_{t = 0}$ is calculated using the $m_p$ calculated over the price series in bad short rains years. For years beginning with a good short rains harvest, $E[P(T)]|_{t = 0}$ is calculated using the $m_p$ calculated over the series in good short rains years.
<!-- Farmers base expectations of harvest price on most recent price movement, adaptive expectation [@onono2013response].  -->
the time horizon is T = 6; muArea is...
...
c("Kenya MoA 14 Jun 2006",
                         "Kenya MoA 15 May 2007",
                         "USDA 11 Sept 2007",
                         "Kenya MoA 15 Jul 2008 (Post-election violence)")

The data comes from... Cannot validate against FAO as FAO is post hoc. It's expected supply so have to compare to the GIEWS midseason forecast--although keep in mind the midseason update knows the rainfall.

Price controls came into play during the 2009-2012 crisis
Gov gave fert to some farmers

```{r Fig1, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="left", fig.cap="\\label{fig:mzP}Maize monthly price series during main season (March-August) at primary markets in Kenya. Price data are from the World Food Program.", echo = FALSE}
#======================================================================
...
No good info on the number of maize farms, can deduce from planted area. A common figure seen nowadays is 3 million [e.g. ...], extrapolating back to 2007, this  but if you look at planted area, it's more like 2 million.

The expected aggregate supply curve is evaluated at different expected rainfall levels in Figure \ref{fig:EaggQs}, top. The rainfall levels "high", "normal", "low", and "dismal" correspond to values of $k_r = 1.5$, $k_r = 1$, $k_r = 0.5$, and $k_r = 0.1$, respectively; which in turn represent expected rainfall that is 150% of the historical benchmark, in line with the historical benchmark, 50% of the historical benchmark, and 10% of the historical benchmark, respectively. The empirical estimates for the major drought years agree closely with the dismal rainfall expected aggregate supply curve, while the estimates in normal or good rainfall years generally agree with the theoretical expected aggregate supply curve evaluated at the normal and high rainfall levels. The curve overshoots the 2010 estimate somewhat, possibly due to crop loss that resulted from flooding that year [2010giews]. The one major empirical divergence from the theoretical curve is 2009, an historic drought year [2009giews].
...
This is the short run curve, assumes mean farm size, number of farms, tech stay the same. 
...
```{r Fig2, fig.show = "hold", fig.width = 5, fig.height=4, fig.align="left", fig.cap="\\label{fig:EaggQs}Expected aggregate supply", echo = FALSE}
...
EQsT2012Jul16MoA <- 2.4 # MLND outbreak, late start to rains
#EQsT2013Jul16MoA <- 1.62 # GoK KFSSG Poorly distributed low rainfall, early cessation
dfPoints <- data.frame(`Estimate entity\nand date` = c("14 Jun 2006 (GIEWS)",
                         "11 Sept 2007 (USDA)",
                         "15 Jul 2008 (GIEWS)\n(Post-election violence, drought)",
                         "Aug 2009\n(Severe drought, crop failure)",
                         "5 Aug 2010 (GIEWS)",
                         "3 Nov 2011 (KFSSG)\n(Drought)",
                         "16 Jul 2012 (GIEWS)"),
                       Val = c(EQsT2006Jun14MoA, #EQsT2007May15MoA,
                               EQsT2007Sep11usda, EQsT2008Jul15MoA,
                               EQsT2009AugMoA, EQsT2010Aug5MoA,
                               EQsT2011Nov3MoA, EQsT2012Jul16MoA),
                       EPT0 = c(EPT02006, EPT02007, EPT02008, EPT02009, EPT02010,
                                EPT02011, EPT02012))
colnames(dfPoints)[1] <- "Estimate entity\nand date"
...
gg <- gg + scale_color_manual(values = c("red", "green", "orange", "black"))
#gg <- gg + geom_vline(xintercept = EPT02007)
gg <- gg + geom_point(data = dfPoints, aes(x = EPT0, y = Val)) + ggrepel::geom_text_repel(data = dfPoints, aes(x = EPT0, y = Val, label = `Estimate entity\nand date`), size = 3, lineheight = 0.75)
gg <- gg + theme_bw()
...
The fubini thm result could be powerful as it maps what is happening across the whole population to what is happening on a single farm i.e. the mean farm with \lambda = E[\lambda]--- a dual space in price, but should use the space for which have higher resolution data. in the Kenya maize farm example the farm data is higher res than the available price data. the stachastic price space doesn't reflect the farm population space

T potentially very useful as some crops like cassava have highly flexible harvest dates, can be left in ground until price is right.
...
Letting $C^{\#}$ denote the unit cost (often called the "average cost of production"),
...
And the marginal cost and unit cost are thus related as follows.
...
That is to say, the farm's unit cost is equal to its marginal cost scaled by $h$. The standard assertion that profit-maximizing farms produce such that marginal cost equals average cost thus implies that $h = 1$ (constant returns to scale). (Which in turn implies perfectly inelastic farm supply curve.)
...
<!-- \begin{equation} -->
<!-- y^* < (g_0 \theta)^{\eta + 1}  \left(\frac{P}{h} \right)^{\eta} \:\:;\:\:\: \eta = \frac{h}{1 - h} -->
<!-- \label{eq:ydUpBound} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \bar{y}^*(k) = E[y^*] \frac{k^{\eta + 1}}{E[k^{\eta + 1}]} \frac{P^{\# \eta}}{E[C^{\# \eta}]} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- q^* = k_a^{1 + \kappa_a \eta + 1} E[y^*] \frac{\tilde{k}^{\eta + 1}}{E[k^{\eta + 1}]} \frac{C^{\# \eta}}{E[C^{\# \eta}]} \:\:;\:\:\: \tilde{k} = \prod_{i \neq a}^{n_k} -->
<!-- \end{equation} -->
...
<!-- <!-- The last of these may of course be further decomposed as $E[A] E[\tilde{y}(T)]\bigr|_{t = 0}$, where $E[A]$ is the mean farm size and $\tilde{y}(T)$ is just the $\tilde{y}$ from Equation \ref{eq:yConst}, with the time of evaluation (harvest) made explicit. --> -->

<!-- In the present example, both farm data and some price series data from GIEWS is available. The price data is monthly data, with just 5 observations prior to 2008, and so is quite low resolution for purposes of estimating supply in 2008. The available farm data is comparatively high resolution, with 942 observations across most of the maize growing regions in the country. Therefore gonna estimate the PEFS-adj. mean probability of profit and h and unconditional yield from unconditional mSharp and sSharp  in farm population space. -->

<!-- First estimate the unconditional means based on the observed conditional means cSharp -->
...
<!-- cite the GIEWS 2007 forecast!!! validate against GIEWS forecast not FAO because the model is for EXPECTED agg supply, FAO is post harvest info. -->
...
<!-- <!-- Evaluation of Equation \ref{eq:sup4}, requires estimation of the parameters $\theta_x$, $\theta_w$, $\hat{\theta}_k$, the farm level output price elasticity of supply $\eta$, the farm production degree of homogeneity $h$, the mean farm's expected unit cost $E\left[\tilde{C}^{\#}(T)\right]\biggr|_{t = 0}$,  --> -->
