...
```{r Fig4, fig.show = "hold", fig.width = 4, fig.height=3, fig.align="center", fig.cap="\\label{fig:EyStarLambda} uyiijl", echo = FALSE}
...
anomVec <- c()
for(i in 1:nYr){
  thisDate <- as.Date(paste0(as.character(2005 + i), "-03-01"))
...
mP <- mVec[1]
sP <- sVec[1]
EPT02007 <- floor(P02007 * exp(mP * th))
#----------------------------------------------------------------------
# coefsNames <- gsub("`", "", names(mod2$coefficients))
# coefsNames <- coefsNames[-c(1, 2)]
# #kVec <- mod1$coefficients[-which(coefsNames %in% input_vars[-c(1, 3:4)])]
kVec <- mod2$coefficients[-c(1, 2)]
names(kVec) <- gsub("`", "", names(kVec))
lkMat <- as.matrix(df_mod[, names(kVec)])
betaK <- exp(lkMat %*% kVec)
#hist(betaK)
...
a0 <- mod2$coefficients[1]
a0 <- a0 + hCoef * log(hCoef)
g0 <- exp(a0)
#betaK * g0
# cStar <- exp(df_mod2$Cstar)
# EyStar <- g0 * betaK * (cStar / hCoef)^hCoef
# exp(mod2$fitted.values) - EyStar
...
#----
EPT0vec <- seq(1, 25, length.out = 60)
Nf <- 1.7 * 10^6
...
QsFun <- function(EPT0vec, muArea, g0, h, muBetaKetaH, muEtaLamInv,
                  muEcSharpT0, mP, sP, th, Nf){
  eta <- h / (1 - h)
  muEyStarT0 <- (g0)^(eta / h) * muBetaKetaH * EPT0vec^eta * muEtaLamInv
...
dfQs <- QsFun(P0vec, muArea, g0, h = hCoef, muBetaKetaH, muEtaLamInv,
                  muEcSharpT0, mP, sP, th, Nf)
