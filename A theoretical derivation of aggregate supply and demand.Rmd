...
then it is not readily apparent from expedient specifications of $Q$ what the functions $q^*$ and $\rho(q^*)$ are, nor whether they are plausible vis-a-vis microeconomic theory and empirical evidence. It is no secret, for example, that the widely used linear aggregate supply curve implies that all farms are equally endowed and produce the same output quantity (in which case $\rho(q^*)$ would have to be the Dirac delta density function). Critically, expedient specifications of $Q$ also omit or gloss over farm entry and exit---i.e. the domain of profitable production $(\underline{q}^*, \bar{q}^*)$---such that it is not possible to decompose changes in aggregate supply into yield and farm entry/exit components. Wohlgenant [-@wohlgenant1997nature] demonstrates analytically that impact assessment results are highly sensitive to both heterogeneous endowments and entry/exit.
<!-- [...holding multiple axioms without knowing if they perhaps contradict each other...] -->

Expedient functional forms are also _empirically_ opaque, in the sense that the parameterization of such forms is highly abstract, with no clear mapping to the relevant measurable, real world phenomena---i.e. technology, input prices, agroclimatic conditions, demographic variables, etc. This leaves very little basis upon which to assess the plausibility of different parameterizations of a given exogenous shock, or to attribute modeled impacts to the exogenous shock with confidence (the attribution problem). Some expedient forms also introduce awkward artifacts---such as the possibility of negative supply quantities and/or of farms producing for free, in the case of the linear aggregate supply curve---which further limit the model's explanatory power.
...
5) Output price and yield follow independent geometric Brownian motions
...
The question remains as to what kind of stochastic process $y^*(t)$ and $P(t)$ follow. We first address price movements. Within a given season, prices often follow a geometric Brownian motion. That is to say, changes in $P$ over a small time interval $\Delta t$ ($\Delta P = P(t + \Delta t) - P(t)$) are often accurately modeled by
...
For what follows, it is also important to note that the log change of a geometric Brownian motion $\ln(P(T) / P(\hat{t}))$ is defined over finite time intervals for some starting time $\hat{t}<T$, and is also normally distributed, with mean and variance
...
The ugly term $E[E[q^*(T)]\bigr|_{t = 0}]$ represents the population mean of expected production at planting time. It is a double integral. By Fubini's theorem, the order of integration may be swapped, such that the term may be interpreted as the mean farm's expected production at planting time.
