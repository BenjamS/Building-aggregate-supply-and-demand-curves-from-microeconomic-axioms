...
Standard textbook treatments of Cobb-Douglas firm behavior focus on profit maximization without a budget constraint, and limit consideration to the two factor case. It is not readily apparent how the treatment extends to  therefore necessary to take a moment here to extend the textbook treatment to the more general case at hand. The first order conditions for a maximum of $\mathcal{L}$ are
...
The first order conditions may be solved for optimal input $x_j^*$ as follows.
...
The degree of homogeneity $h$ is thus also the scale elasticity of yield ($\partial \ln(y^*) / \partial \ln(C^*) = h$) as noted in the textbooks. But note also the similarity to Equation \ref{eq:integrate_lny}. And note that

\begin{equation}
\frac{\partial \ln(y^*)}{\partial C^*} \sim \frac{1}{C^*}
\end{equation}

Which is congruent to Axiom 1.
...
farm output price elasticity of yield.
...
Because $0 < h < 1$, the farm supply curve is always positively sloped. [might mention elasticity critical point at h = 1/2] Increasing returns to scale imply negatively sloped supply curve.

Combining Equations \ref{eq:inputDem1} and \ref{eq:YdEnvCompact} gives an alternate expression for input demand.
...
thus the own and cross input price elasticities of demand are, respectively,
...
whereby it is clear that the domain $0 < h < 1$ implies negatively sloped own and cross price demand curves, and increasing returns to scale would imply positively sloped own and cross price demand curves.
