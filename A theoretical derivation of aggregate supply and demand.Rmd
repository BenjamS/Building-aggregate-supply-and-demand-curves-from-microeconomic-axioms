...
This expression is particularly useful for estimation, as it only requires information on the control variables $k_i$ and total cost $C^*$, and no information on individual input quantities, as would be required when estimating Equation \ref{eq:cobbDoug3}. Moreover, the estimated coefficient on logged farm cost may be directly interpreted as the degree of homogeneity for the population. When estimating the model in Equation \ref{eq:cobbDoug3}, by contrast, $h$ must be tallied by summing the estimated coefficients $\alpha_i$, and will likely vary across the population since not all farms necessarily apply all the inputs specified in the model.
...
Noting that $C^* = C^{\#} y^*$, where $C^{\#}$ is defined as the cost per unit output, another useful expression is obtained by substituting $C^{\#} y^*$ for $C^*$ and solving for $y^*$, giving
...
This expression is useful for integrating farm production over the production density to arrive at aggregate supply because the argument $C^{\#}$ has a readily defined upper bound of integration at $P$. (By axiom 2, clearly $0 < C^{\#} < P$.) 

This means that not all farms in the population participate in production. he population means $E[y^*]$ and $E[C^{\#}]$ do not correspond to the observed sample means. 

area and yield not independent
...
[[By axioms 1 and 2, then we get corrollaries: upward sloping farm supply, downward sloping input demand, concavity in inputs, marginal cost equals P/lambda, ]]
...
Likewise, "farm production function" henceforth refers to the budget constrained, profit maximizing production function $q^*$ obtained when $y^*$ is multiplied by the cultivated area $k_a$.
...
EQsFun <- function(EPT0, muEcSharpT0, h, muY0, muArea, mP, sP, th, Nf){
  eta <- h / (1 - h)
  PT0 <- EPT0 * exp(-mP * th)
  ...
  EQsT0 <- muArea * Nf * muY0 * ProbProfitMeanFarm #Expected agg supply in kg
  ...
  #------------
  dfOut <- data.frame(EPT0, EQsT0, ProbProfitMeanFarm, etaP0EQsT0, etaSpEQsT0)
  return(dfOut)
}
#----------------------------------------------------------------------
...
#   return(outVec)
# }
EcSharpT0 <- exp(dfMod3$`Cost/acre (KES/acre)`) / EyStarT0
lEcSharpT0 <- log(EcSharpT0)
lam <- EPT02007 / EcSharpT0 * h
sum(lam > h) / length(lam)
mEcSharpT0trunc <- mean(lEcSharpT0); sEcSharpT0trunc <- sd(lEcSharpT0)
...
#======================================================================
#======================================================================
lEcSharpT0vec <- seq(-4, 7, length.out = length(lEcSharpT0))
f <- dnorm(lEcSharpT0vec, mEcSharpT0, sEcSharpT0)
dfPlot <- data.frame(f, lEcSharpT0vec)
gg <- ggplot() + geom_histogram(data = data.frame(lEcSharpT0), aes(x = lEcSharpT0, y = 0.7*after_stat(count / sum(count))), bins = 13, color = "black", fill = "white") #+ scale_y_continuous(labels = scales::percent)
gg <- gg + geom_line(data = dfPlot, aes(x = lEcSharpT0vec, y = f)) + geom_vline(xintercept = log(EPT02007), color = "red", lwd = 1) + theme_bw()
...
A.1
## Derivation of the optimal yield function
...
By definition, for a random variable $v$ and a constant $M$,

\begin{equation}
E[v | v < M] = \int_{-\infty}^{M} v p(v) \: dv
\label{ev:def}
\end{equation}

Where $p(v)$ is the probability density function of the random variable $v$. If $\ln(v)$ is normally distributed with mean $\nu$ and variance $\omega^2$, then
...
\begin{equation}
p(v) = \frac{1}{v \omega} \phi \left(\frac{\ln(v) - \nu}{\omega} \right)
\end{equation}
...
\begin{equation}
\frac{du}{d v} = \frac{1}{v \omega}\: \rightarrow \: d v = v \omega du
\label{ev:subThis2}
\end{equation}

The definition in equation \ref{ev:def} resolves as follows.

\begin{equation}
\begin{split}
E[\max(v - M, 0)] &= \int_{\frac{\ln(M) - \nu}{\omega}}^{\infty} (e^{u \omega + \nu} - M) \phi(u) \: du \\
&= \int_{\frac{\ln(M) - \nu}{\omega}}^{\infty} e^{u \omega + \nu} \phi(u) \: du - M \int_{\frac{\ln(M) - \nu}{\omega}}^{\infty} \phi(u) \: du \\
&= \frac{1}{\svrt{2 \pi}} \int_{\frac{\ln(M) - \nu}{\omega}}^{\infty} e^{u \omega + \nu -\frac{u^2}{2}} \: du - M \Phi \left(-\frac{\ln(M) - \nu}{\omega} \right) \\
&= \frac{1}{\svrt{2 \pi}} \int_{\frac{\ln(M) - \nu}{\omega}}^{\infty} e^{-\frac{(u - \omega)^2}{2} + \nu + \frac{\omega^2}{2}} \: du - M \Phi \left(-\frac{\ln(M) - \nu}{\omega} \right) \\
&= e^{\nu + \frac{\omega^2}{2}} \frac{1}{\svrt{2 \pi}} \int_{\frac{\ln(M) - \nu}{\omega}}^{\infty} e^{-\frac{(u - \omega)^2}{2}} \: du - M \Phi \left(-\frac{\ln(M) - \nu}{\omega} \right) \\
&= E[v] \Phi \left(- \frac{\ln(M) - \nu}{\omega} + \omega \right) - M \Phi \left(-\frac{\ln(M) - \nu}{\omega} \right)
\end{split}
\end{equation}

Note that \ref{ev:muXT} can be rearranged into an expression for $\nu$.
...
\begin{equation}
E[\max(v - M, 0)] = E[v] \Phi \left(\frac{\ln \left( \frac{E[v]}{M} \right) + \frac{\omega^2}{2}}{\omega} \right) - M \Phi \left(-\frac{\ln \left( \frac{E[v]}{M} \right) - \frac{\omega^2}{2}}{\omega} \right)
\end{equation}
