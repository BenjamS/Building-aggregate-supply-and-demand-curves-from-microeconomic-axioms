...
$$
\text{kg.} \times \: \text{acres}^{-1} \times \: \text{liters}^{-\alpha_{pest}} \times \: \text{kg.}^{-\alpha_{fert}} \times \: \text{manhours}^{-\alpha_{labor}} \times\: \\ \text{mm.}^{-\alpha_{rain}} \times\: \text{num. persons}^{-\alpha_{HHsize}} \:;\:\: \alpha_{(var)} \in \mathbb{R}
$$
...
(Where $\epsilon$ is the error term.) Such that the estimated coefficient on logged farm cost may be directly interpreted as the degree of homogeneity for the population. When estimating the model in Equation \ref{eq:cobbDoug3}, by contrast, $h$ must be tallied by summing the estimated coefficients $\alpha_i$, and will likely vary across the population since not all farms necessarily apply all the inputs specified in the model.
...
\begin{equation}
\bar{C}^*(k) = (P v_0 k)^{\eta / h}
\end{equation}

\begin{equation}
Q = N E[k_a] \frac{E[y^*]}{E[C^{* h}]} \int_{0}^{(P v_0 k)^{\eta / h}} C^{* h} \rho(C^*) dC^*
\label{eq:aggSupStep}
\end{equation}
...
\begin{equation}
\int_{0}^{\zeta} \chi \rho(\chi) d\chi = E[\chi] \Phi\left( \frac{\ln(\zeta) - m_{\chi}}{s_{\chi}} - s_{\chi} \right) \:\:;\:\:\: m_{\chi} = E[\ln(\chi)] \:\: s^2_{\chi} = Var[\ln(\chi)]
\label{eq:keyResult}
\end{equation}
...
&\Rightarrow E[\ln(C^*)] = \frac{1}{1 - h} (E[\ln(C^{\#})] + \ln(v_0 k)) \\
...
To parameterize the expected aggregate supply curve, Equation \ref{eq:lYdEnvReg2} is fit to the farm data. In this exercise, the constant $v_0$ includes endowment dummy variables that augment the constant on farms where the endowment is present.

\begin{equation}
\ln(y^*(T)) = \ln(v_0) + \sum_i^{n_k} \kappa_i \ln(k_i) + h \ln(C^*) + \epsilon \:\:;\:\:\: \ln(v_0) = \beta_0 + \sum_{i = 1}^{n_b} \beta_i b_i
\label{eq:lYdEnvReg3}
\end{equation}
<!-- \:\:;\:\:\: v_0 = \frac{E[y^*]}{E[k] E[C^{*h}]}  -->
...
\begin{equation}
R^* ( \partial \ln(E[P(T)]|_{t = 0}) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g_0) ) + \lambda \partial B > 0
\label{eq:adoptCond}
\end{equation}

Where the term $\partial B / \partial u_0$ represents any subsidies or fixed costs entailed in the technology package. In the absence of subsidies/fixed costs, the condition simplifies to
...
Widespread adoption of yield scaling enhancements tends to boost production and thus lower output price while at the same time increasing input demand and thus increasing input prices. Hence the expectation is that $\partial \ln(E[P]|_{t = 0}) < 0$ and $\partial \ln(\mathbf{w}) > 0$. In words, then, the adoption condition says that a farm stands to increase its net benefit through adoption of yield scaling technology only if the resulting percentage increase in TFP ($g_0$) is greater than the corresponding percentage decrease in output price minus the corresponding percentage increase in input prices scaled by their respective input elasticities $\alpha_i$.

Note the resemblance between this and the "price/productivity tightrope" argument formulated by Lipton.
...
The percentage increase in TFP due to adoption varies with the binary endowments already present on the farm. In the example above, a farm with just the basal endowment $b_0$ that adopts hybrid maize stands to increase TFP by $100\times(e^{b_0} + e^{b_{hybrid}}) / e^{b_0} =$ `r `.

This means that the adoption rate is endogenously calculated .

introducing technology shock by perturbing h is harder and ambiguous.
...
\begin{equation}
\frac{\partial \mathcal{L}}{\partial u_0} = R^* (1 - h) \left( \frac{\partial \ln(P)}{\partial u_0} + \frac{\partial \ln(y^*)}{\partial u_0} \right) + \lambda \frac{\partial B}{\partial u_0} > 0
\end{equation}

Considering the conventional expression for optimal yield (Equation \ref{eq:yStarAlt}), the yield derivative resolves as follows.

\begin{equation}
\frac{\partial \ln(y^*)}{\partial u_0} = \frac{h}{1 - h} \frac{\partial \ln(P)}{\partial u_0} + \frac{1}{1 - h} \left(\frac{\partial \ln(g_0)}{\partial u_0 } - \mathbf{\alpha} {'} \frac{\partial \ln(\mathbf{w})}{\partial u_0} \right)
\end{equation}

Substituting this into the adoption condition and simplifying,

\begin{equation}
\frac{\partial \mathcal{L}}{\partial u_0} = R^* \left( \frac{\partial \ln(P)}{\partial u_0} - \mathbf{\alpha} {'} \frac{\partial \ln(\mathbf{w})}{\partial u_0} + \frac{\partial \ln(g_0)}{\partial u_0 } \right) + \lambda \frac{\partial B}{\partial u_0} > 0
\end{equation}
...
Where $P$ has been replaced with its stochastic expression.
