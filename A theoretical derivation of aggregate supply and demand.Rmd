...
To parameterize the expected aggregate supply curve, Equation \ref{eq:lYdEnvReg2} is fit to the farm data. In applied cases such as this, it is helpful to note that $v_0$ may be expressed
...
notThese <- c("Dist", "ln[Yield (kg/acre)]", "ln[Cost/acre (KES/acre)]", "ln[Rain anomaly]", bin_vars)
...
The impact of yield scaling technologies and/or management practices on aggregate supply can be represented through perturbations of the TFP parameter ($g_0$). This is tantamount to adding a dummy endowment variable to the yield model in Equation \ref{eq:lYdEnvReg3}. In the pedagogical example above, the use of tractors during land preparation and adoption of hybrid maize seed may be considered yield scaling interventions that increase the value of $g_0$ on adopting farms.
...
Where the term $\partial B$ represents subsidies, switching costs, or any other exogenous mitigating circumstances entailed in the adoption of the technology package. In the absence of such circumstances, the condition simplifies to

\begin{equation}
\partial \ln(g_0) > \mathbf{\alpha} {'} \partial \ln(\mathbf{w}) - \partial \ln(E[P(T)]|_{t = 0})
\label{eq:adoptCondSimp}
\end{equation}

Widespread adoption of yield scaling enhancements tends to boost production and thus lower output price while at the same time increasing input demand and thus input prices. Hence the expectation is that $\partial \ln(E[P]|_{t = 0}) < 0$ and $\partial \ln(\mathbf{w}) > 0$. In words, then, the adoption condition says that a farm stands to increase its net benefit through adoption of yield scaling technology only if the resulting percentage increase in TFP ($g_0$) is greater than the corresponding percentage decrease in output price minus the corresponding percentage increase in input prices scaled by their respective input elasticities $\alpha_i$. Note the resemblance between this and the "price/productivity tightrope" argument formulated by Lipton [-@lipton2005family].
...
The only substantive difference between Lipton's argument and the condition in Equation \ref{eq:adoptCondSimp} is that the percentage increases in input prices in the latter are scaled by their respective elasticities ($\alpha_i$). And of course the more complete version of the adoption condition in Equation \ref{eq:adoptCond} differs from Lipton by also including the subsidy/fixed cost term.

The alpha_i can be calculated from cost shares
...
Say $\beta_5 = 0.3$. Then $\partial \ln(g_0) = 0.35$, and so, by the criterion in Equation \ref{eq:adoptCondSimp} adoption in the absence of subsidies will increase farm net benefit if the associated percentage decrease in output price plus the $\alpha_i$-scaled increase in input prices is less than $35\%$. If the $\alpha_i$-adjusted output-input price ratio decrease is greater than $35\%$, then the subsidy term $h / C^* \partial B$ required to make adoption profitable is the margin by which it is greater. Note that the required subsidy $\partial B$ will vary across farms based on $C^*$. The factor $h / C^*$, which may also be written $\lambda / E[R^*(T)]|_{t = 0}$, represents each farm's marginal valuation of the subsidy. This implies that the subsidy is relatively more valuable to farms with relatively lower expenditure, and is thus generally consistent with the law of diminishing marginal returns.

The Lipton condition formalized in Equation \ref{eq:adoptCondSimp} implies that adoption will be either $0\%$ or $100\%$. Empirically, this is almost never observed. Adoption rates tend to fall somewhere between $0\%$ and $100\%$. Within the theoretical framework developed here, this means that the subsidy/cost term is important. The increased input demand associated with adoption implies in and of itself that, without subsidies, $\partial B < 0$. And, of course, every major green revolution success story has involved subsidies of one kind or another to offset the required increase in input expenditure, and then some, such that $\partial B > 0$ [].
...
Solving Inequality \ref{eq:adoptCond} for $C^*$ gives the upper bound on


For positive values of $\partial B$, Equation \ref{eq:adoptCond} implies that the adoption rate $A$ may be calculated as
<!-- \begin{equation} -->
<!-- \partial \ln(C^*) < \frac{1}{h} (\partial \ln(g_0) + \partial \ln(E[P(T)]|_{t = 0}) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w})) -->
<!-- \label{eq:adoptCond2} -->
<!-- \end{equation} -->
...
Recalling from Equation \ref{eq:mCk} that $C^* = (v_0 k C^{\#})^{1 / (1 - h)}$, then $m_c = 1 / (1 - h)(\ln(v_0) + m_k + m_{\#})$ and $s_c^2 = 1 / (1 - h)^2 (s_k^2 + s_{\#}^2)$ (where $m_k = E[\ln(k)]$ and $s_k^2 = Var[\ln(k)]$), then this reduces to
<!-- ; and from Equation \ref{eq:useful2} that $m_{\#} = \ln(E[P(T)]|_{t = 0}) - m_{\lambda}$ (where $m_{\lambda} = E[\ln(\lambda)]$) -->

\begin{equation}
\begin{split}
A = \Phi \left(\frac{(1 - h) / h (\partial \ln(g_0) + \partial \ln(E[P(T)]|_{t = 0}) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w})) - (\ln(v_0) + m_k + m_{\#})}{\sqrt{s_k^2 + s_{\#}^2}}\right)
\end{split}
\end{equation}


When binary endowments vary across the population, as in the Kenya example above, the value of $g_0$ varies from farm to farm. Each value of $g_0$ effectively identifies a binary endowment stratum of the population. In such cases, the supply aggregation step in Equation \ref{eq:aggSupStep} effectively averages aggregate supply across all strata. In the Kenya example, there are 14 distinct values of $g_0$, corresponding to 14 distinct combinations of the four binary endowment variables with the regression constant $\beta_0$ (Figure ...).
...
beta0adj <- beta0 + as.matrix(dfBins) %*% kappaBins
v0 <- exp(beta0adj)
...
Denoting each distinct value of $g_0$ as $g^i_0$, the average of $g_0$ is effectively a population weighted sum of the $g^i_0$.

\begin{equation}
E[g_0] = \sum_i \frac{N_i}{N} g^i_0
\end{equation}

Where $N_i$ is the number of farms in the $i^{th}$ binary endowment stratum.





This means that the adoption rate is endogenously calculated .
...
can also look at shocks to continuous endowments k, rainfall etc.
