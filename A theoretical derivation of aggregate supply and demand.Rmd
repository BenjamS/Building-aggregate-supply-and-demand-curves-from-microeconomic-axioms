...
The unconditional parameters $m_{\#}$ and $s_{\#}$ can be deduced from the conditional parameters $\hat{m}_{\#}$ and $\hat{s}_{\#}$ by numerically solving the following system of equations.

\begin{equation}
\begin{split}
m_{\#} &= \hat{m}_{\#} - \hat{s}_{\#} \frac{\phi(z)}{\Phi(z)} \\
s_{\#}^2 &= \hat{s}_{\#}^2 \left( 1 - z \frac{\phi(z)}{\Phi(z)} - \left( \frac{\phi(z)}{\Phi(z)} \right)^2   \right) \:\:;\:\:\: \frac{\ln(\gamma) - m_{\#}}{s_{\#}} \:\:\:;\:\: z = \frac{\ln(\gamma) - m_{\#}}{s_{\#}}
\end{split}
...
\begin{equation}
y^*(T) = \ln(v_0) + \sum_i^{n_k} \kappa_i \ln(k_i) + h \ln(C^*) + \epsilon \:\:;\:\:\: \ln(v_0) = \beta_0 + \sum_{i = 1}^{n_b} \beta_i b_i
\label{eq:lYdEnvReg3}
\end{equation}
...
As explained in section \ref{sec:acct4Stoch}, the expected yield $E[y^*(T)]|_{y = 0}$ is calculated as $y^*(T)e^{-\epsilon}$....also rainfall

$y^*(T) \frac{E[k_r^{\kappa_r}]}{k_r^{\kappa_r}} e^{-\epsilon}$

As discussed in section \ref{sec:acct4Stoch}, Equation \ref{eq:lYdEnvReg2} is fit to the data,
and then the expected yield is estimated as $E[y^*(T)]|_{t = 0} = y^*(T) - \epsilon$. 



\ln(v_0) = \beta_0 + \sum_{i = 1}^{n_b} \beta_i b_i

To estimate the degree of homogeneity $h$, Equation \ref{eq:YdEnvReg2} is estimated in logs. 
