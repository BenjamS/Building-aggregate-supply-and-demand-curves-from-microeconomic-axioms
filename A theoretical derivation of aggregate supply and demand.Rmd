...
The price elasticity of expected aggregate supply curve is presented in Figure \ref{fig:EaggQs}, bottom. Empirical estimates of the maize price elasticity of supply in Kenya generally range from $0.1$ to $0.365$ [@abodi2021supply; @onono2013response; @olwande2009supply], which is roughly the range of elasticities indicated by the theoretical curve for the years examined here.
<!-- In one study, Onono et al.'s estimate of price elasticity of supply: 0.365 [@onono2013response] -->

Given the sensitivity of aggregate supply to meteorological conditions, the notion of a single supply "curve" is misleading. It is more accurate to a speak of a supply bracket or "ribbon" bounded on top and bottom by the best and worst case meteorological scenarios---in this case the curves corresponding to high and dismal rainfall.

The expected aggregate supply and price elasticity curves parameterized in this exercise are "short run" curves. That is to say, they are valid only insofar as the values of their fitted parameters---$h$, $N$, $k_a$, $k$, $m_{\#}$, $s_{\#}$, $m_p$---are accurate. Empirical estimates for years well before or well after the year of the data used to estimate the parameters will thus diverge from the curve.
...
Say $\beta_5 = 0.3$. Then $\partial \ln(g_0) = 0.35$, and so, by the criterion in Inequality \ref{eq:adoptCondSimp} adoption in the absence of exogenous subsidies/costs will increase farm net benefit if the associated percentage decrease in output price plus the $\alpha_i$-scaled increase in input prices (i.e. $\partial \ln(\nu)$) is less than $35\%$. 
The adoption rate for the hypothetical new technology described above ($\beta_5 = 0.3$), assuming a 40% decrease in the $\alpha_i$-adjusted output-input price ratio, with a net subsidy per acre of $\partial B = 2.5$, is presented in Figure \ref{fig:v0strata} alongside the mean yield, cost, and TFP ($v_0$), disaggregated by stratum.

The pct change in v0 TFP, yield, and expenditure...

\singlespacing

```{r Fig5, fig.show = "hold", fig.width = 6, fig.height=3, fig.align="center", fig.cap="\\label{fig:v0strata}(Left) The adoption rate, change in welfare, and change in supply for each of the 14 different combinations of binary endowments present in the Kenya farm data; and (right) the research induced expected aggregate supply ribbon shift, where the dotted lines represent the new normal and dismal rainfall bounds of the aggregate supply ribbon.... given subsidy x and price ration decrease y.", echo = FALSE}

# betaNew <- 0.2
# dlng0 <- exp(betaNew) - 1
dlng0 <- 0.2
betaNew <- log(dlng0 + 1)
aDotdlw <- 0.2; dlP <- -0.20
dlnNu <- dlP - aDotdlw
dB <- 800
...
#---
# dfPlot <- data.frame(Type = endowVec, v0 = round(v0, 1)) %>%
#   mutate(xx = 1) %>% group_by(Type) %>%
#   summarise(`Number of farms` = sum(xx, na.rm = T), v0 = unique(v0)) %>%
#   as.data.frame() %>%
#   mutate(`Pct of farms in sample` = `Number of farms` / sum(`Number of farms`) * 100) %>%
...
# dlNuVec <- dlP / dlAw
nuRatio <- 0.75
aDotdlwVec <- seq(0.25, 0.45, length.out = nRes)
