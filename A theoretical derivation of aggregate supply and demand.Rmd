...
Now, because the binary endowments $b_i$ vary across the population, the value of $g_0$ (and hence of $v_0$) also varies. In the Kenya example, there are 14 distinct values of $v_0$, corresponding to 14 distinct combinations of the $\beta_i b_i$. The supply aggregation step in Equation \ref{eq:aggSupStep} averages aggregate supply across all binary endowment strata. Denoting the value of each distinct combination as $v_{0i}$, the average $v_0$ is effectively a population weighted sum of the $v_{0i}$.
...
Where $N_i / N$ is the fraction of farms in the $i^{th}$ binary endowment stratum. The conditional mean cost and yield disaggregated by stratum ($E[C^*| \theta]_i$, $E[y^*(T)| \theta]_i$), can be evaluated as follows.
...
\begin{equation}
E[\hat{y}^*(T, k_m(T))]^i|_{t = 0} = v_{0i} E[k] E[C^{*h}]_i
\label{eq:v0i}
\end{equation}
<!-- E[e^{\epsilon}]_i -->
...
```{r Fig4, fig.show = "hold", fig.width = 6, fig.height=3, fig.align="center", fig.cap="\\label{fig:v0strata}(Left) The mean cost, mean yield, and TFP (v0) for each of the 14 different combinations of binary endowments present in the Kenya farm data; and (right) their respective prevalance in the population.", echo = FALSE}
#-----------------------------------------------------------------------
...
\singlespacing
```{r Fig3, fig.show = "hold", fig.width = 5, fig.height=5, fig.align="center", fig.cap="\\label{fig:EaggQs}(Top) The theoretical expected aggregate maize supply curve fitted to 2007 Kenya long rains maize season data, plotted against empirical estimates of expected aggregate supply for the years 2006-2012. (Bottom) The corresponding theoretical price elasticity of expected aggregate supply curve.", echo = FALSE}
#----------------------------------------------------------------------------------------
...
## The research induced supply curve shift
...
# stratPct <- dfPlot$`Pct of farms in sample` / 100
# typeVec <- dfPlot$Type
# v0strat <- dfPlot$v0
# mCstari <- 1 / (1 - h) * (log(v0strat) + mEcSharpT0)
# #mCstari <- 1 / (1 - h) * (log(v0strat) + mK + mEcSharpT0)
...
# muYstarT <- sum(muYstarTi * stratPct)
# muCstar <- sum(muCstari * stratPct)
# # EcSharpT0 DOESN"T CHANGE WITH g0, v0!!! Changes with h. EcSharpT0 = EPT0 * h / lambda
# # mEcSharpT0trunc <- (1 - h) * mCstarTrunc - mK - beta0adj
# # sEcSharpT0trunc <- sqrt((1 - h)^2 * sCstarTrunc^2 - sK^2)
# # muEcSharpT0Trunc <- exp(mEcSharpT0trunc + sEcSharpT0trunc^2 / 2)
# dfPlot$`Mean yield\n(kg/acre)` <- round(muYstarTi, 1)
# dfPlot$`Mean cost/acre\n(KES)` <- round(muCstari, 1)
# dfPlot1 <- dfPlot %>% rename(`TFP (v0)` = v0) %>%
#   relocate(`Type`, `TFP (v0)`, `Mean cost/acre\n(KES)`, `Mean yield\n(kg/acre)`) %>%
#   select(`Type`, `TFP (v0)`, `Mean cost/acre\n(KES)`, `Mean yield\n(kg/acre)`) %>% gather(var, val, `TFP (v0)`:`Mean yield\n(kg/acre)`)
# gg <- ggplot(dfPlot1, aes(x = var, y = Type, fill = val, label = val))
# gg <- gg + scale_y_discrete(limits = typeVec)
# gg <- gg + geom_tile(alpha = 0)
# #gg <- gg + labs(title = expression(v[0]))
# gg <- gg + scale_x_discrete(position = "top")
...
dfStrat$`Adoption rate\n(%)` <- 100 * round(adoptRateStrat, 3)
dfStrat$`Change in welfare\n(Billion KES)` <- round(dPsiStrat, 2)
dfStrat$`Change in supply\n(1000 MT)` <- round(dQStrat, 2)
dfTab <- dfStrat %>% rename(`TFP (v0)` = v0) %>%
  select(`Type`, `Adoption rate\n(%)`, `Change in welfare\n(Billion KES)`, `Change in supply\n(1000 MT)`) %>%
  relocate(`Type`, `Adoption rate\n(%)`, `Change in welfare\n(Billion KES)`, `Change in supply\n(1000 MT)`) %>% gather(var, val, `Adoption rate\n(%)`:`Change in supply\n(1000 MT)`)
...
ggShift / ggShiftPop + plot_layout(heights = c(1, 1 / 14))
...
Say that the new binary endowment is targeted at 

```{r Fig5, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="center", fig.cap="\\label{fig:adoptMap}....", echo = FALSE}
...
dBvec <- seq(100, 1600, length.out = nRes)
...
matAdopt[i, j] <- round(sum(adoptRateStrat * stratPct), 3) * 100
}
}
rm(cStarMaxAdopt, adoptRateStrat)
#image(matAdopt)
...
dfPlot$dB <- as.numeric(dfPlot$dB)
gg <- ggplot(dfPlot, aes(x = dlNu, y = dB, z = adoptRate, fill = adoptRate, group = 1))
gg <- gg + geom_tile(alpha = 1) + geom_contour(binwidth = 1000)
gg <- gg + metR::geom_text_contour(aes(z = adoptRate),
                                   #breaks = seq(100, 3000, length.out = 6),
                                   stroke = 0.15)
gg <- gg + scale_fill_gradient(low = "coral", high = "cyan")
#gg <- gg + geom_text(aes(label = adoptRate), size = labelSize)
gg <- gg + labs(fill = "Change in welfare (Billion KES)", x = "Decrease in output input price ratio (%)",
                y = "Net subsidy (KES/acre)")
gg <- gg + theme_bw()
gg <- gg + theme(axis.text = element_text(size = axisTextSize),
                 axis.title = element_text(size = axisTitleSize),
...
dlEPT0 <-  -0.15
aDotdlW <- dlEPT0 - dlNu
dlmuEyStarT0 <- 1 / (1 - h) * (dlng0 - aDotdlW)
dB <- 2000
cStarMaxAdopt <- -h * dB / (dlNu + dlng0)
lCstarMaxAdopt <- log(cStarMaxAdopt)
z <- (lCstarMaxAdopt - mCstari) / sCstar
adoptRateStrat <- pnorm(z)
adoptRate <- sum(adoptRateStrat * stratPct)
#dfOut <- EQsFun(EPT0vec, muEyStarT0, mEcSharpT0, sEcSharpT0, h, muArea, Nf, kappaRn)
gatherCols <- colnames(dfPlot)[-1]
dfPlot <- dfOut %>% select(`Expected price (KES/kg)`, `Expected supply\n(million MT)`) %>%
  mutate(newCurve = `Expected supply\n(million MT)` * (1 + adoptRate * dlmuEyStarT0)) %>%
  gather_("curve", "val", gatherCols)

gg <- ggplot(dfPlot, aes(x = `Expected price (KES/kg)`, y = val, color = curve, group = curve))
gg <- gg + geom_line()
gg
