...
aggSubsidyMillnUSD <- aggSubsidy / 67 * 10^3 #in million USD
aggSubsidyStratMillnKES <- aggSubsidyStrat * 10^3 #in million KES
#--------------------------------------------------------------------------
# Welfare assessment
...
dPsiTotCheck <- Nf * muArea * muEyStarT0 * EPT02007 * (dlnNu + dlng0) * (probProfAdoptTot - cStarMaxAdopt / muCstarh * adoptRateTotal) * 10^-9
...
for(i in 1:nRes){
  ...
  z <- (lCstarMaxAdopt - mCstarStrat) / sCstar
  adoptRateStrat <- pnorm(z)
  adoptRateTotalCheck <- sum(adoptRateStrat * stratFrac)
  # Total adopt rate
  z <- (lCstarMaxAdopt - mCstar) / sCstar
...
phiArgStrat <- (lCstarMaxAdopt - mCstarStrat) / sCstar - h * sCstar
probProfAdoptStrat <- pnorm(phiArgStrat)
dPsiStrat <- stratFrac * Nf * muArea * muEyStarT0strat * EPT02007 * (dlng0 + dlNu) * (probProfAdoptStrat - cStarMaxAdopt / muCstarhStrat * adoptRateStrat)
dPsiStrat <- dPsiStrat * 10^-9 # in billions KES
dPsiTot <- sum(dPsiStrat)
# Total welfare change
phiArgTot <- (lCstarMaxAdopt - mCstar) / sCstar - h * sCstar
probProfAdoptTot <- pnorm(phiArgTot)
dPsiTotCheck <- Nf * muArea * muEyStarT0 * EPT02007 * (dlng0 + dlNu) * (probProfAdoptTot - cStarMaxAdopt / muCstarh * adoptRateTotal)
dPsiTotCheck <- dPsiTotCheck * 10^-9 # in billions KES
...
dQTotCheck <- Nf * muArea * muEyStarT0strat * (dlng0 + dlnNuh) / (1 - h) * probProfAdoptTot
dQTotCheck <- dQTotCheck * 10^-6 # in 1000 MT
...
any impact assessment must be accompanied by a clear indication of the assumed meteorological conditions.
