...
# u <- dfMod3$`Child family labor, planting (person-hrs/acre)`
# dfMod3$`Child family labor, planting (person-hrs/acre)` <- ifelse(u == 0, NA, u)
# notThese <- c("Dist", "Yield (kg/acre)", bin_vars, "Rain anomaly")
# contIndVars <- setdiff(mod3Vars, notThese)
# thisFun <- function(x){out <- x - log(mean(exp(x))); return(out)}
# dfMod3[, contIndVars] <- as.data.frame(apply(dfMod3[, contIndVars], 2, thisFun))
# #PerformanceAnalytics::chart.Correlation(dfMod3[, contIndVars])
# colMeans(exp(dfMod3[, contIndVars]))
mod3 <- lm(`Yield (kg/acre)`~., dfMod3[, -1])
summ(mod3)
#car::vif(mod3)
plot(mod3$fitted.values, mod3$residuals)
indRm <- which(abs(mod3$residuals) > 1.5)
#View(dfMod3[indRm,]); length(indRm)
mod3 <- lm(`Yield (kg/acre)`~., dfMod3[-indRm, -1])
summ(mod3)
plot(mod3$fitted.values, mod3$residuals)
notThese <- c("Dist", "Yield (kg/acre)", "Cost/acre", bin_vars, "Rain anomaly")
kVars <- setdiff(mod3Vars, notThese)
names(mod3$coefficients) <- gsub("`", "", names(mod3$coefficients))
alphaKvec <- mod3$coefficients[which(names(mod3$coefficients) %in% kVars)]
matK <- dfMod3[, kVars] %>% as.matrix()
lThetaKvec <- matK %*% alphaKvec
hist(lThetaKvec)

matKa <- matK %*% diag(alphaKvec)
mVec <- colMeans(matKa)
sVec <- apply(matKa, 2, sd)
# Sample mean of thetaK very close to lognormal expectation, confirming lognormality of thetaK
EthetaK <- exp(sum(c(mVec, sVec^2 / 2)))
EthetaKsamp <- matK %*% alphaKvec %>% exp() %>% mean()
#EthetaK/EthetaKsamp
#EthetaKsamp2 <- matK %*% diag(alphaKvec) %>% colMeans() %>% sum() %>% exp()
alphaCstar <- mod3$coefficients[which(names(mod3$coefficients) == "Cost/acre")]
# Sample mean of Cost/acre very close to lognormal expectation
EcStar <- exp(mean(alphaCstar * dfMod3$`Cost/acre`) + sd(alphaCstar * dfMod3$`Cost/acre`)^2 / 2)  
EcStarSamp <- exp(alphaCstar * dfMod3$`Cost/acre`)  %>% mean()
beta0 <- mod3$coefficients[1]
g0 <- exp(beta0)
y0 <- g0 * EthetaK * EcStar



%>% colMeans() %>% sum() %>% exp()

EthetaK <- mean(exp(lThetaKvec))
EThetaK2 <- matK %*% diag(alphaKvec) %>% colMeans() %>% sum() %>% exp()
kVecTilde <- exp(matK) %*% diag(1 / (colMeans(exp(matK))))
colMeans(kVecTilde)
hist(rowSums(log(kVecTilde)))
hist(log(kVecTilde) %*% alphaKvec)
mean(exp(log(kVecTilde) %*% alphaKvec))
#exp(mean(log(kVecTilde) %*% alphaKvec))
#======================================================================
#======================================================================
#======================================================================
