...
The degree of homogeneity $h$ is thus also the scale elasticity of yield ($d \ln(y^*) / d \ln(C^*) = h$) as noted in the textbooks. This form is particularly useful for estimation, as it requires only the control variables $k_i$ and input cost $C^*$, as opposed to detailed information on all of the input quantities. The constant $g_0 \beta$ is estimated as the y intercept of the model. That is, instead of estimating the model in Equation \ref{eq:cobbDoug3}, one estimates

\begin{equation}
y = \hat{g}_0 C^{*h} \prod_{i = 1}^{n_k} \left( \frac{k_i}{\tilde{k}_i} \right)^{\kappa_i} \prod_{i = 1}^{n_x}
\label{eq:cobbDoug4}
\end{equation}

Where the estimated constant $\hat{g}_0$ may then be interpreted as $\hat{g}_0 \approx g_0 \beta_{\alpha} / \beta_w h^{-h}$.

the fitted coefficient on the $\ln(C^*)$ term may be interpreted as $h$. Note also the similarity to Equation \ref{eq:integrate_lny}. And note that
...
Below, an aggregate supply curve is parameterized based on Kenya farm data for the 2007 main cropping season [@maizeData]. Summary statistics can be found in the Appendix.

The yield model is specified as a function of cost ($C^*$) and control variables ($k_i$), and no management variables, in accordance with Equation \ref{eq:yStarC}. The cost variable (cost per acre) covers the cost of inputs and land preparation. The control variables are farm size (acres), seed quantity per acre, child family labor (person-hours/acre) during planting, adult-equivalent household size (aehh07), and the rainfall anomaly (2007 main season rainfall divided by the historical average). Except for rainfall anomaly, all of these variables are known to the farmer ex-ante, i.e. at planting time. As discussed in section \ref{sec:ax12}, the ex-ante control variables are divided by their respective sample means $\tilde{k}_i$ in order to facilitate interpretation of the constant $g_0 = e^{\alpha_0}$. That is to say, the constant $g_0$ is replaced by a new constant $y_0 = g_0 \prod_i \tilde{k}_i$, such that $y_0$ may be interpreted as yield evaluated at the control variable sample means, and at the historical average rainfall (i.e. at a rainfall anomaly of $1$). Henceforth this yield is referred to as the "sample average yield".

In addition the continuous control variables, four binary control variables are included in the model, covering endowments in terms of land tenure, adoption of hybrid maize seed, and land preparation technique (oxen, tractor, or none). These take a value of $e$ in the affirmative case and $1$ otherwise, such that the coefficients on the binary terms are added to the intercept term $\alpha_0$ on farms that have the endowment, and disappear from the equation on farms that do not have the endowment. The sample average yield is thus not limited to the base case of $y_0$, but varies across farms depending on binary endowments. The base case represents the sample average yield on farms with none of the binary endowments, i.e. farms without land tenure, which do not use hybrid seed, and which use neither oxen nor tractors for land preparation.

Letting the notation $\tilde{y}$ refer more generally to sample average yields that may or may not include one or more of these endowments, in addition to the base endowment, Equation \ref{eq:yStarC} implies

\begin{equation}
\tilde{y} = \frac{\beta_{\alpha}}{\beta_w} h^h
\end{equation}

The input variable elasticities $\alpha_i$ and input prices $w_i$ are thus implicit in the constant $\tilde{y}$, without requiring detailed information about either.

Models including farmer age, education, gender, maximum and minimum temperature, other types of family labor, and distance to market variables were also explored. But the coefficients on these variables were either statistically insignificant, introduced multicollinearity issues, or introduced missing data issues.
...
```{r Fig4, fig.show = "hold", fig.width=5, fig.height=3, fig.align="center", fig.cap="\\label{fig:y0}Yield evaluated at the sample means of the control variables, and with rainfall anomaly set to 1 (no divergence from historical rainfall), for farms with different endowments. The 'Base' endowment is just the exponential of the regression constant.", echo = FALSE}
