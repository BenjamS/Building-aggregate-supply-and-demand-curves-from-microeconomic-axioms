...
Note that Equation \ref{eq:cStar} simplifies to

\begin{equation}
C^{\#} = \frac{P}{\lambda} h
\end{equation}

Taking into account the stochastic character of ouput price and optimal unit cost formalized above, this must be rewritten more carefully as follows.
...
This equation reminds us that optimal expected unit cost represents an optimal target that continues to evolve past $t = 0$ (planting), regardless of the actual farm expenditure that is "locked in" at $t = 0$ based on the price information available at that time. As an optimal target, then, $E[C^{\#}(T)]\bigr|_{0 < t < T}$ is itself a stochastic variable following a trajectory proportional to the price trajectory. By Ito's Lemma, this stochastic trajectory must also be a gBm with the same drift and volatility parameters as $P(t)$. (A proof is given in Appendix B.)

\begin{equation}
\begin{split}
Var[\ln(E[C^{\#}(T)]_{0 < t < T})] &= Var[E[P(T)]\bigr|_{0 < t < T}] \\
s^2_{\#} &= s^2_p (T - t)
\end{equation}


Taking the expectation of Equation \ref{eq:useful2} gives the population mean of $E[C^{\#}(T)]\bigr|_{t = 0}$.
...
\begin{equation}
E[E[C^{\#}(T)]]|_{t = 0} = E[P(T)]\bigr|_{t = 0} h E[\lambda^{-1}]
\end{equation}

Such that

\begin{equation}
E[E[C^{\#}(T)]\bigr|_{t = 0}] = E[E[C^{\#}(T)]]|_{t = 0}
\label{eq:fubThm}
\end{equation}

Which is a result that can also be reached by invoking Fubini's theorem. To be clear, this result states that the population mean of the optimal expected unit cost ($E[C^{\#}(T)]\bigr|_{t = 0}$) is equal to the optimal expected unit cost on the mean farm, i.e. the farm with $\lambda = E[\lambda]$. This is a useful insight because it effectively maps the trajectories of $E[C^{\#}(T)]\bigr|_{0 < t < T}$ on all farms in the population to the trajectory on a single farm (the mean farm).

[GRAPHIC showing equality of pop mean of expected future cSharp to expected cSharp on the mean farm (?)]


Substituting the stochastic terms developed above for $y^*$, $C^{\#}$, and $P$ in the deterministic aggregate supply curve (Equation \ref{eq:aggSup1}), gives the _expected_ aggregate supply curve.

\begin{equation}
E[Q(T)]|_{t = 0} = N E[k_a] E[y^*] \Phi\left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)
\label{eq:aggSup1}
\end{equation}
