...
\begin{equation}
\begin{split}
A &= \int_0^{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)}} \phi \left(\frac{\ln(C^*) - m_c}{s_c} \right) d \ln(C^*) \\
...
& m_{ci} = \frac{1}{1 - h}(\ln(v_0^i) + m_k + m_{\#})
\end{equation}

The percentage change in aggregate supply given a 1% change in TFP, assuming 100% adoption, follows from Equation \ref{eq:aggSup2}.

\begin{equation}
\frac{\partial \ln(E[Q(T)]|_{t = 0})}{\partial \ln(g_0)} = \frac{\partial \ln E[y^*(T; k_m(T))]|_{t = 0}}{\partial \ln(g_0)} = \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} \right)
\end{equation}

The change given adoption rate $A$ between $0-100$% is

\begin{equation}
\frac{\partial \ln(E[Q(T)]|_{t = 0})}{\partial \ln(g_0)} = A \frac{\partial \ln E[y^*(T; k_m(T))]|_{t = 0}}{\partial \ln(g_0)} = \frac{A}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} \right)
\end{equation}

The welfare impact change in net benefit...
...
z <- (cStarMaxAdopt - mCstari) / sCstar
...
# dlNuVec <- dlP / dlAw
dlNuVec <- seq(-0.65, -0.35, length.out = nRes)
dBvec <- seq(1, 15, length.out = nRes)
...
for(i in 1:nRes){
  ...
  z <- (cStarMaxAdopt - mCstari) / sCstar
adoptRateStrat <- pnorm(z)
matAdopt[i, j] <- sum(adoptRateStrat * stratPct)
}
}
...
#Works:
# nRes <- 30
# dlnNuVec <- seq(-0.5, -0.35, length.out = nRes)
# dBvec <- seq(2, 3.5, length.out = nRes)
# matAdopt <- matrix(NA, nRes, nRes)
# for(j in 1:nRes){
#     dlnNu <- dlnNuVec[j]
# for(i in 1:nRes){
...
# }
# dfAdopt <- as.data.frame(matAdopt)
# colnames(dfAdopt) <- paste("dB =", round(dBvec, 2)); gatherCols <- colnames(dfAdopt)
# dfAdopt$dlnNu <- paste("dlnNu =", round(dlnNuVec, 2))
# dfPlot <- dfAdopt %>% gather_("dB", "adoptRate", gatherCols)
# gg <- ggplot(dfPlot, aes(x = dB, y = dlnNu, fill = adoptRate))
# gg <- gg + geom_tile()
# gg
...
#EPT0vec <- seq(0.1, 60, length.out = 50)
aDotdlW <- 0.15
muEyStarT0new <- 1 / (1 - h) * (dlng0 - aDotdlW)
dfOut <- EQsFun(EPT0vec, muEyStarT0new, mEcSharpT0, sEcSharpT0, h, muArea, Nf, kappaRn)
EPT0EmpirVec = c(EPT02006, EPT02007, EPT02008, EPT02009, EPT02010, EPT02011, EPT02012)
dfPoints$EPT0 <- EPT0EmpirVec
