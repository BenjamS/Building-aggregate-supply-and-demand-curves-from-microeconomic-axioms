...
a0adj <- a0 + as.matrix(df_mod2[, bin_vars]) %*% kBinVec
tfp <- exp(a0adj)
kNames <- c("Cstar", "acres", "seed (kg/acre)",
            "planting fam lab child (pers-hrs/acre)",
            demog_vars)
kVec <- mod2Coefs[which(coefsNames %in% kNames)]
lkMat <- as.matrix(df_mod2[, kNames])
betaK <- exp(lkMat %*% kVec)
mBetaK <- mean(log(betaK))
sBetaK <- sd(log(betaK))
muBetaK <- exp(mBetaK + sBetaK^2 / 2)
betaKnormd <- betaK / muBetaK
# mBetaKnormd <- mean(log(betaKnormd))
# sBetaKnormd <- sd(log(betaKnormd))
# muBetaKnormd <- exp(mBetaKnormd + sBetaKnormd^2 / 2)
# mean(betaKnormd)
#kMat <- exp(lkMat)
...
# muCstarTruncH <- exp(hCoef * mCstarTrunc + hCoef^2 * sCstarTrunc^2 / 2)
#muCstarTruncH <- mean(cStar^hCoef)
betaKnormdAdj <- betaKnormd / cStar^hCoef
#mean(betaKnormdAdj)
sErr <- sd(mod2$residuals)
#----------------------------------------------------------------------
# y0 is EyStar*exp(-sErr^2 / 2) at mean(betaKnormdAdj*cStar^hCoef) and kRn = 1 (no rain anomaly)--i.e. basically y0 = mean(yStar)
y0 <- tfp * muBetaK
#----------------------------------------------------------------------
EyStar <- y0 * betaKnormdAdj * kRn^kappaRn * cStar^hCoef * exp(sErr^2 / 2)
EcSharp <- cStar / EyStar
EyStar2 <- (y0 * betaKnormdAdj * kRn^kappaRn)^(etaCoef / hCoef) * EcSharp^etaCoef * exp(sErr^2 / 2)^(etaCoef / hCoef)
#---
# test
# tfp * muBetaK - y0
...
#----------------------------------------------------------------------
# EyStarUpMu <- mean(y0^(etaCoef / hCoef)) * EPT02007^etaCoef * exp(sErr^2 / 2)^(etaCoef / hCoef)
# df_plot <- data.frame(EyStar, EyStarUp)# %>% gather(var, val, EyStar:EyStarUp)
# #gg <- ggplot(df_plot, aes(x = val))
# gg <- ggplot(df_plot, aes(x = EyStar))
# gg <- gg + geom_histogram(fill = "white", color = "orange", bins = 12)
# gg <- gg + geom_vline(xintercept = mean(EyStarUp))
# # gg
#----------------------------------------------------------------------
...
QsFun <- function(EPT0vec, muArea, g0, h, muBetaRnEtaH,
                  muBetaKetaH, muEtaLamInv,
                  muEcSharpT0, mP, sP, th, Nf){
  eta <- h / (1 - h)
  muEyStarT0 <- (g0)^(eta / h) * muBetaRnEtaH * muBetaKetaH * EPT0vec^eta * muEtaLamInv
...
# prodFAO2008 <- 2367237
#----------------------------------------------------------------------
mRnAnom <- 0 #mean(log(rainAnom))
sRnAnom <- sd(log(rainAnom))
