...
This is preferred to calculating the parameters directly from observed $C^* / y^*$ because observed yield is harvest yield, whereas the $y^*$ appearing in all of the equations above refers not to harvest yield but to expected harvest yield at planting time. This becomes clear when stochasticity is taken into account.
...
\begin{equation}
\begin{split}
\mathcal{A} &= \Phi \left( \frac{\ln(\hat{C^*}) - m_c}{s_c} \right) \\
...
The total increase in aggregate supply due to a technology shock can be evaluated by integrating the change in farm production $\partial y^*$ over the density of adopting farms $(0, \hat{y^*})$ where $\hat{y^*} = y^*(\hat{C^*})$.
<!-- $\overset{\triangle}{y}$ -->
...
&= r (C^* \gamma + h \partial B) \\
& \text{Where} \:\:\: \gamma = \partial g - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w}) + \partial \ln(P), \:\:\: r = \frac{\lambda}{h} = \frac{P}{C^{\#}}
\end{split}
\end{equation}

But recall from Equation \ref{eq:cStarAdopt} that $h \partial B = - \gamma \hat{C}^*$, such that the expression reduces to
...
The aggregate change in net benefit ($\Delta \psi$) can be evaluated by integrating $\partial \mathcal{L}^*(C^*, r)$ over the farm expenditure ($C^* \in (0, \hat{C}^{*h})$), revenue to expenditure ratio ($r \in (1, \infty)$) and area ($k_a$) densities. (The domain of $r$ follows from axiom 3.)
...
&= N E[k_a] E[r] \gamma \Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} + s_{\#} \right) \left( E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c) - \hat{C}^* \mathcal{A} \right) \\
&= N E[k_a] E[r] \gamma \mathcal{P}(P^{-1}; m_{\#}, s_{\#}) \left( E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c) - \hat{C}^* \mathcal{A} \right)
\end{split}
\end{equation}
...
Such that the previous equation could also be written as follows.
...
Or, by Equation \ref{eq:keyResult2},

\begin{equation}
\Delta \psi = N E[k_a] \gamma E[r | r \geq 1] \mathcal{A} \mathcal{E} (E[C^* | C^* \leq \hat{C}^*] - \hat{C}^*)
\end{equation}

To calculate the change in aggregate supply and welfare for each stratum ($\Delta Q_{\ell}$ and $\Delta \psi_{\ell}$) in the equations above replace $v$, $N$, with $v_{\ell}$, $N_{\ell}$.
...
By which the stochastically explicit version of aggregate supply (Equation \ref{eq:aggSup1Alt}) follows.
<!-- Q = N E[k_a] E[y^*|C^{\#} \leq P] \mathcal{E} -->
...
In the remainder of this section, it is shown that the $\mathcal{P}(E[P(T)]|_{t = 0})$ term appearing on the left hand side of Equation \ref{eq:condMeanYd2}, and in the expected aggregate supply function, has a meaningful economic interpretation. Specifically,
...
The stochastically explicit formulations for the research induced change in aggregate supply and producer welfare derived in section \ref{sec:aggImpact} follow from the exposition above as
...
\begin{equation}
\begin{split}
\Delta \psi &= N E[k_a] E[\tilde{r}(T)]|_{t = 0} \gamma (2 \mathcal{E} - \mathcal{P}(E[P(T)]|_{t = 0}; m_{\#}, s_{\#})) (E[C^*] \mathcal{P}(\hat{C}^*; m_c, s_c)  - \hat{C}^* \mathcal{A}) \\
& \text{Where} \:\:\: \tilde{r}(T) = E[r(T)] = E\left[ \frac{P(T)}{C^{\#}(T)} \right] \Biggr|_{t = 0} =  \frac{E[\lambda]}{h}
\end{split}
\label{eq:dEPsi}
\end{equation}
...
For policy formation, it is helpful to consider prospective interventions in a sensitivity heatmap format, whereby multiple farm subsidy and price change scenarios can be examined at a single glance (Figure \ref{fig:decisionMaps}). These "policy decision maps" plot the adoption rate, supply change, and producer welfare outcome over a range of different lever-assumption "coordinates". The maps here are expressed in coordinates of subsidy level (y-axis), and assumed price ratio change (x-axis). The policy assumption may be swapped out for other assumptions, such as assumed rainfall level.
...
gg <- gg + annotate("point", 
           ...
           color = "black",
           size = 2)
gg <- gg + labs(fill = "Change in aggregate supply (1000 MT)",
                x = "Decrease in efficiency adjusted\noutput-input price ratio (%)",
                y = "Net subsidy (KES/acre)")
gg <- gg + theme_bw()
gg <- gg + theme(axis.text = element_text(size = axisTextSize),
