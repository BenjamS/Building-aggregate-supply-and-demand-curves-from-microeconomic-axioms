...
Letting $C^{\#}$ denote optimal unit expenditure ($C^{\#} = C^* / y^*$), note that $C^*$ and $y^*$ can be rewritten in terms of $C^{\#}$ as follows.
...
Assuming farmers adopt only if $\partial \mathcal{L}^*/\partial \ln(g) > 0$ (this is implicit in axiom 2), then the adoption condition is
...
By axiom 3 and the expression for $C^*$ in Equations \ref{eq:fnCsharp},
...
And noting that for any lognormally distributed variable $\chi$ and upper bound $\xi$,
...
\begin{equation}
\begin{split}
\int_{0}^{(P v k)^{\frac{\eta}{h}}} C^{* h} \rho(C^*) d C^* &= E[C^{* h}] \Phi\left( \frac{\eta \ln(P v k) - h m_c}{h s_c} - h s_c \right) \\
& m_c = E[\ln(C^*)] \:\:\:\: s^2_c = Var[\ln(C^*)]
\end{split}
...
To parameterize the expected aggregate supply curve, Equation \ref{eq:yStarT} (in logs) is fit to the farm data. Here it is important to note that $v$ may be expressed in terms of $g$.

**--
\begin{equation}
\begin{split}
v &= \frac{E[y^*(T)]}{E[k] E[C^{* h}]} \\
&= \frac{u}{E[k] E[C^*]^h} \\
...
The TFP constant $g$ may include binary endowment variables that augment the constant on farms where the endowment is present.
...
$$
\ln(v) = \sum_i^{n_x} \ln \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i} - \ln(h) + \ln(g) \\
...
Note that the expenditure TFP $v$ can be expressed
...
Then the derivative of expenditure TFP with respect to TFP is
