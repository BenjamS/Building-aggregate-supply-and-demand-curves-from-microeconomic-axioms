...
Where the geometric series converges to the last equation only because axioms 1 and 2 constrain $h$ to $(0,1)$ (decreasing returns ot scale). Assuming constant or increasing returns to scale ($h \in (1, \infty)$) would result in a divergent infinite sum and thus unbounded $C^*$.

Given the domain of homogeneity $h \in (0,1)$, the upper bound on cost for given $k$ can thus be expressed

\begin{equation}
\bar{C}^*(k) = (P v_0 k)^{\eta + 1}
\end{equation}

And so aggregate supply for given $k$ can be expressed

\begin{equation}
Q(k) = N E[k_a] \frac{E[y^*]}{E[C^*(k)^h]} \int_{0}^{(P v_0 k)^{\eta + 1}} C^*(k)^h \rho(C^*(k)) dC^*(k)
\label{eq:aggSupStep}
\end{equation}

If $y^*$ is lognormally distributed (axiom 4), then $C^*$ is also lognormally distributed. For any lognormally distributed variable $\chi$ and upper bound $\zeta$, it can be shown that

\begin{equation}
\begin{split}
\int_{0}^{\zeta} \chi \rho(\chi) d\chi &= E[\chi] \Phi\left( \frac{\ln(\zeta) - m_{\chi}}{s_{\chi}} - s_{\chi} \right) \:\:; \\ 
...
Substituting $\chi = C^{* h}$ and $\zeta = \bar{C}^*$, 

\begin{equation}
\int_{0}^{(P v_0 k)^{\eta / h}} C^{* h} \rho(C^*) d C^* = E[C^{* h}] \Phi\left( \frac{\eta \ln(P v_0 k) - h m_c}{h s_c} - h s_c \right) \:\:;\:\:\: m_c = E[\ln(C^*)] \:\: s^2_c = Var[\ln(C^*)]
\label{eq:integralStep}
\end{equation}

But note that $C^*$ can be expressed $C^* = C^{\#} y^*$, where $C^{\#}$ is the optimal cost per unit output (also commonly referred to as the "average cost"). Then,
...
\begin{equation}
\begin{split}
\int_{0}^{(P v_0 k)^{\eta / h}} C^{* h} \rho(C^*) d C^* &= E[C^{* h}] \Phi \left( \frac{\eta \ln(P v_0 k) - \eta m_c}{\eta s_c} - \eta s_c \right) \\
&= E[C^{* h}] \Phi \left( \frac{\ln(P v_0 k) - m_c}{s_c} - \eta s_c \right)
\end{split}
\end{equation}

Substituing the right hand side of this equation for the integral in Equation \ref{eq:aggSupStep} results in the following expression for aggregate supply.
...
Where $\phi()$ is the standard normal probability density function. The unconditional mean yield $E[y^*]$ can then be deduced considering that
...
\begin{equation}
\begin{split}
E[y^*|C^{\#} \leq P] = E[y^*] &= E \left[ E[y^*] \frac{k^{\eta + 1}}{E[k^{\eta + 1}]}\right] \frac{E[C^{\# \eta} | C^{\#} \leq P]}{E[C^{\# \eta}]} \\
...
Whence the unknown $E[y^*]$ follows as a function of the observed or derived parameters $E[y^*|C^{\#} \leq P]$, $m_{\#}$, and $s_{\#}$.
...
To begin with, yield is partly a function of stochastic meteorological variables unknown at planting time. Wherever $y^*$ appears in the equations above, what is really meant is $E[y^*(T)]|_{t = 0}$---that is to say, the farm's expectation of what yield will be at harvest time ($t = T$) as evaluated at planting time ($t = 0$).

\begin{equation}
E[y^*(T)]|_{t = 0} = E[E[y^*(T)]|_{t = 0}] E[k_m(T)]|_{t = 0} \frac{\hat{k}}{E[k]} \frac{C^{*h}}{E[C^{*h}]} \:\:;\:\:\: \hat{k} = \frac{k}{E[k_m(T)]|_{t = 0}}
\label{eq:stochYd1}
\end{equation}
<!-- \:\:\: k_r(T) = k_{r1}^{\kappa_{r1}} k_{r2}^{\kappa_{r2}} \dots k_{rn_r}^{\kappa_{rn_r}} -->

Where the stochastic meteorological component $k_m(T)$ has been factored out of $k$ and made explicit. The component $k_m(T)$ is the product at harvest of any cumulative meteorological variables specified in the yield model, raised to their respective elasticities. For example, if the key meteorological variables are rainfall and solar radiation, then $k_m(T) = k_r(T)^{\kappa_r} k_s(T)^{\kappa_s}$, where $k_r(T)$ and $k_s(T)$ are cumulative rainfall and solar radiation at harvest.

The term $E[E[y^*(T)]|_{t = 0}]$ is the population mean of the expectation of the stochastic process $y^*(t)$. This can be rewritten in more expedient notation by evaluating the population mean of the expected yield equation at $t = T$, 

\begin{equation}
y^*(T) = E[y^*(T)] k_m(T) \frac{\hat{k}}{E[k]} \frac{C^{*h}}{E[C^{*h}]}
\end{equation}

And then taking the stochastic expectation

\begin{equation}
E[y^*(T)]|_{t = 0} = E[E[y^*(T)]]|_{t = 0} E[k_m(T)]|_{t = 0} \frac{\hat{k}}{E[k]} \frac{C^{*h}}{E[C^{*h}]}
\end{equation}
...
Which is a result that can also be reached by invoking Fubini's theorem. The result states that the population mean of the expectation of $y^*(T)$ is equal to the expectation of $y^*(T)$ on the mean farm---i.e., the farm where $\hat{k} = E[\hat{k}]$ and $C^{*h} = E[C^{*h}]$. The stochastic formulation of farm yield may thus be more clearly written
...
\begin{equation}
E[y^*(T)]|_{t = 0} = E[\tilde{y}^*(T)]|_{t = 0} E[k_m(T)]|_{t = 0} \frac{\hat{k}}{E[k]} \frac{C^{*h}}{E[C^{*h}]} \:\:;\:\:\: \tilde{y}^* = y^*(T)\bigr|_{\hat{k} = E[\hat{k}], C^{*h} = E[C^{*h}]}
\label{eq:stochYd2}
\end{equation}

Note that optimal farm expenditure per acre $C^*$ remains a deterministic variable. This is because $C^*$ primarily refers to land preparation, seed, fertilizer and chemical pest control. Some formulations of farm cost might also include some harvest post harvest labor costs (e.g., shelling, stooking, transport), but the main cost components are typically known (executed or planned) at planting time.

For purposes of parameter estimation, expected yield is generally not observable. However, expected yield can be deduced from observed yield $y^*(T)$ by estimating Equation \ref{eq:stochYd2} at $t = T$ (in logs).
...
And then interpreting the fitted values for yield as the logged expected yield. That is to say,

\begin{equation}
\ln(E[y^*(T)]|_{t = 0}) = \ln(y^*(T)) - \epsilon
\end{equation}
<!-- \begin{equation} -->
...
\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} = N E[k_a] E[\tilde{y}^*(T)]|_{t = 0} \Phi \left( \frac{\ln(E[P(T)]|_{t = 0}) - m_{\#}}{s_{\#}} - \eta s_{\#} \right) \\
...
\begin{equation}
E[\tilde{y}^*(T)]|_{t = 0} = E[\tilde{y}^*(T) | E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}]|_{t = 0} \frac{\Phi\left( \frac{\ln(P) - m_{\#}}{s_{\#}} \right)}{\Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)}
\label{eq:condMeanYd2}
\end{equation}
<!-- \begin{split} -->
<!-- E[E[y^*(T)]|_{t = 0}] &= E[E[y^*(T)]|_{t = 0} | E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|_{t = 0}] -->
<!-- \frac{\Phi\left( \frac{\ln(P) - m_{\#}}{s_{\#}} \right)}{\Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)} \\ -->

It was noted earlier that the $\Phi()$ term in the numerator on the righthand side of this expression may be interpreted as the farm employment rate. In the remainder of this section, it is shown that the $\Phi()$ term appearing in the denominator, and in the expected aggregate supply function, also has a meaningful economic interpretation. Specifically,
...
Where the unconditional mean yield in stochastic price parameter space is

\begin{equation}
E[\tilde{y}^*(T)]|_{t = 0} = E[\tilde{y}^*(T) | E[C^{\#}(T)]|_{t = 0} \leq P(T)]|_{t = 0} \frac{1 - \Phi(z - \eta s_p \sqrt{T})}{1 - \Phi(z)}
...
\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} &= N E[k_a] E[y^*(T)]|_{t = 0} \text{Prob}(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0} \leq P(T)^{\eta}) \\
&= N E[k_a] E[y^*(T)]|_{t = 0} \times \text{PEFS-adj. mean probability of profit}
\end{split}
\label{eq:aggSup4}
\end{equation}
...
\begin{equation}
\begin{split}
E[\tilde{y}^*(T)]|_{t = 0} &= E[y^*(T) | E[C^{\#}(T)]|_{t = 0} \leq P(T)]|_{t = 0} \frac{\text{Prob}(E[C^{\#}(T)]|_{t = 0} \leq E[P(T)]|{t = 0})}{\text{Prob}(E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0} \leq P(T)^{\eta})} \\
\text{Mean expected yield} &= \text{Mean observed expected yield} \times \frac{\text{Farm employment rate}}{\text{PEFS-adj. mean probability of profit}}
\end{split}
\end{equation}
...
ft <- flextable(dfPoints_ft) %>% 
  set_caption("Empirical estimates of main season maize harvest 2006-2012", align_with_table = F) %>% autofit() %>% fit_to_width(7)
...
To parameterize the expected aggregate supply curve, Equation \ref{eq:lYdEnvReg2} is fit to the farm data. In applied cases such as this, it is helpful to note that $v_0$ may be expressed in a variety of ways.
...
Where the last line follows from the input demand expression in Equation \ref{eq:inputDem2}. In this exercise, moreover, the TFP constant $g_0$ includes endowment dummy variables that augment the constant on farms where the endowment is present.

\begin{equation}
\begin{split}
\ln(y^*(T)) &= \ln(v_0) + \sum_i^{n_k} \kappa_i \ln(k_i) + h \ln(C^*) + \epsilon \:\:;\:\:\: \ln(v_0) = \sum_i^{n_x} \ln \left( \frac{\alpha_i}{w_i} \right)^{\alpha_i} - \ln(h) + \ln(g_0) \\
...
The dummy endowment variables $b_i$ (which take a value of $1$ if the endowment is present and $0$ otherwise) are: use of oxen in land preparation, use of tractor in land preparation, adopter of hybrid seed, and land tenure. The control variables ($k_i$) are planting density (seed kg/acre), adult equivalent household size, farm size, and rainfall. The rainfall variable is 2007 main season rainfall divided by an historical benchmark, and so can be interpreted as the rainfall anomaly or departure from the historical benchmark. The farm expenditure $C^*$ includes the cost of synthetic fertilizer, land preparation, pest and disease control chemicals, and seed. The regression results are presented in Table 2.
...
indDum <- which(colnames(dfMod3) %in% c("Tenure: owned with title deed", "Landprep: tractor", "Landprep: oxen", "Adopter"))
notThese <- c(1, indDum)
...
Because the binary endowments $b_i$ vary across the population, the value of $v_0$ also varies. Each value of $v_0$ effectively designates a binary endowment stratum of the population. In the present example, there are 14 distinct values of $v_0$, corresponding to 14 distinct combinations of the $\beta_i b_i$. The supply aggregation step in Equation \ref{eq:aggSupStep} effectively averages aggregate supply across all strata. Denoting each distinct value of $v_0$ as $v^i_0$, the average $v_0$ is effectively a population weighted sum of the $v^i_0$.

\begin{equation}
E[v_0] = \sum_i \frac{N_i}{N} v^i_0
\end{equation}

Where $N_i / N$ is the fraction of farms in the $i^{th}$ binary endowment stratum.

The mean expenditure per acre and harvest yield depend on $v^i_0$ and thus likewise vary across binary endowment strata. Because $C^*$ is lognormally distributed (by axiom 4), then the mean expenditure for the $i^{th}$ stratum may be calculated as $E[C^*]_i = e^{m^i_c + \frac{s^2_c}{2}}$, where $m^i_c$ and $s_c$ can be parsed by recalling from Equation \ref{eq:mCk} that $C^* = (v_0 k C^{\#})^{1 / (1 - h)}$, such that

\begin{equation}
\begin{split}
m^i_c &= \frac{1}{1 - h}(\ln(v^i_0) + m_k + m_{\#}) \:\:;\:\:\: m_k = E[\ln(k)] \:\: s_k^2 = Var[\ln(k)] \\
s_c^2 &= \frac{1}{(1 - h)^2} (s_k^2 + s_{\#}^2)
\end{split}
\label{eq:mCsC}
\end{equation}

The mean harvest yield for each stratum then follows from the first of Equations \ref{eq:v0forms} as $E[y^*(T)]_i = v^i_0 E[k] E[C^{*h}]_i$ (where $E[C^{*h}]_i = e^{h m^i_c + h^2\frac{s^2_c}{2}}$). The TFP parameter for each stratum ($g^i_0$) can likewise be calculated from the last of Equations \ref{eq:v0forms} given the input elasticities $\alpha_i$.
...
&= e^{\frac{1}{1 - h}(\ln(v_0) + m_k + m_{\#}) + \frac{1}{(1 - h)^2} \frac{s^2_k + s_{\#}^2}{2}}
\end{equation}


3,   and so the adoption rate can be written in terms of known parameters.



(Figure \ref{fig:v0strata}).
...
# vHybrid <- exp(kappaBins["Adopter"])
# vTractor <- exp(kappaBins["Landprep - tractor"])
# vTenure <- exp(kappaBins["Tenure - owned with title deed"])
# round((v0base + vHybrid) / v0base * 100, 2)
# round((v0base + vTractor + vHybrid) / (v0base + vTractor) * 100, 2)
# round((v0base + vTenure + vTractor + vHybrid) / (v0base + vTenure) * 100, 2)
...
cellText_size <- 2.5
df_plot <- data.frame(Type = endowVec, v0 = round(v0, 1)) %>%
...
sK <- sqrt(sum(sKvec^2))
v0strat <- df_plot$v0
mC <- log(v0strat) + mK + mEcSharpT0
sC <- 1 / (1 - h) * sqrt(sK^2 + sEcSharpT0^2)
Ek <- exp(mK + sK^2 / 2)
muCstar <- exp(mC + sC^2 / 2)
EcStarh <- exp(h * mC + h^2 * sC^2 / 2)
muYstarT <- v0strat * Ek * EcStarh
df_plot$`Mean yield` <- round(muYstarT, 1)
df_plot$`Mean cost/acre` <- round(muCstar, 1)
df_plot <- df_plot %>% rename(`TFP (v0)` = v0) %>%
  relocate(`TFP (v0)`, `Mean cost/acre`, `Mean yield`) %>%
  gather(var, val, `TFP (v0)`:`Mean yield`)

#df_plot$Type <- factor(df_plot$Type, levels = df_plot$`TFP (v0)`[order(df_plot$`TFP (v0)`)], ordered = T)
#gg <- ggplot(df_plot, aes(x = Element, y = Type, fill = y0, label = y0))
gg <- ggplot(df_plot, aes(x = var, y = Type, fill = val, label = val))
...
As explained in section \ref{sec:acct4Stoch}, the expected yield is calculated as $E[y^*(T)]|_{y = 0} = y^*(T) e^{-\epsilon} k_r^{-\kappa_r}$. The observed optimal expected unit cost is then calculated as $E[C^{\#}(T)]|_{t = 0} = C^* / E[y^*(T)]|_{y = 0}$. As explained in \ref{sec:aggSupDeterm}, axiom 3 truncates the distribution of (expected) optimal unit cost at the (expected) output price. The conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ are calculated from this observed, truncated distribution; and then the unconditional parameters $m_{\#}$ and $s_{\#}$ are calculated by solving simultaneous Equations \ref{eq:cSharpUncondParams}. In addition to $\bar{m}_{\#}$ and $\bar{s}_{\#}$, this requires an estimate of expected harvest price at planting time ($E[P(T)]|_{t = 0}$).
...
The two remaining parameters required are number of farms $N$ and mean farm size $k_a$. For mean farm size, the Kenya farm sample mean is used, giving $k_a =$ `r muAreaReport` acres. The number of maize farms is more difficult to assess. One sometimes sees the figure $3$ million mentioned [e.g. @njeru2022maize], but this appears to be unsupported by any hard evidence. Here, a rough estimate is deduced by dividing planted area by mean farm size. The midseason reports examined for this exercise suggest that planted maize area during the period in question ranges between 3 and 4.5 million acres [@KFSSGJul2011]. Divided by $k_a$, this gives a range between $1.56$ and $2.34$ million farms. The figure $N = 2$ million is used as a rough estimate.

The expected aggregate supply curve is evaluated at different expected rainfall levels in Figure \ref{fig:EaggQs}, top. The rainfall levels "high", "normal", "low", and "dismal" correspond to values of $k_r = 1.5$, $k_r = 1$, $k_r = 0.5$, and $k_r = 0.1$, respectively; which in turn represent expected rainfall that is 150% of the historical benchmark, in line with the historical benchmark, 50% of the historical benchmark, and 10% of the historical benchmark, respectively. The empirical supply estimates for the major drought years agree closely with the theoretical expected supply curve evaluated at the dismal rainfall level, while the estimates in normal or good rainfall years generally agree with the theoretical curve evaluated at the normal and high rainfall levels. The curve overshoots the 2010 estimate somewhat, possibly due to crop loss from flooding that year [@GIEWS5Aug2010]. The one major empirical divergence from the theoretical curve is 2009, an historic drought year [@MoApressRelease2009].
...
EcStarT0trunc <- exp(dfMod3$`ln[Cost/acre (KES/acre)]`)
EcSharpT0trunc <- EcStarT0trunc / EyStarT0trunc
lEcSharpT0trunc <- log(EcSharpT0trunc)
#EPT02007 <- mean(df_mod$`Maize price (KES/kg)`) * exp(-mP * timeHorizon)
...
The impact of yield scaling technologies and/or management practices on aggregate supply can be represented through perturbations of the TFP parameter ($g_0$). This is tantamount to adding a dummy endowment variable to the yield model in Equation \ref{eq:lYdEnvReg3}. The four endowmwnt dummies included in the pedagogical example above may be considered yield scaling interventions that increase the value of $g_0$ on adopting farms.
<!-- unconditional mean expected yield term $E[\tilde{y}^*(T)]|_{t = 0}$. Recall from Equation \ref{eq:muYstarU0} that the unconditional mean expected yield can be defined (in stochastic terms), -->
<!-- \begin{equation} -->
<!-- E[\tilde{y}^*(T)]|_{t = 0} = u_0 \frac{E[C^{*h}]}{E[C^*]^h} -->
<!-- \end{equation} -->
<!-- Yield scaling interventions can thus be represented by perturbations of $u_0$.  -->
...
Where the term $\partial B$ represents subsidies, costs, or any other exogenous mitigating circumstances, per unit area, entailed in the adoption of the technology package. In the absence of such mitigating circumstances, the condition simplifies to
...
Widespread adoption of yield scaling enhancements tends to boost production and thus lower output price while at the same time increasing input demand and thus input prices. Hence the expectation is that $\partial \ln(E[P]|_{t = 0}) < 0$ and $\partial \ln(\mathbf{w}) > 0$. In words, then, the adoption condition says that a farm stands to increase its net benefit through adoption of yield scaling technology only if the resulting percentage increase in TFP ($g_0$) is greater than the corresponding percentage decrease in output price minus the corresponding percentage increase in input prices scaled by their respective input elasticities $\alpha_i$.

Note the resemblance between the adoption condition in Inequality \ref{eq:adoptCondSimp} and the "price/productivity tightrope" argument formulated by Lipton [-@lipton2005family].
...
The only substantive difference between Lipton's condition and the condition in Inequality \ref{eq:adoptCondSimp} is that the percentage increases in input prices in the latter are scaled by their respective elasticities ($\alpha_i$). And of course the more complete version of the adoption condition in Inequality \ref{eq:adoptCond} differs from Lipton by also including the exogenous subsidy/cost term.

Now, given a new yield enhancing binary endowment $\beta_5$ (representing, for example, improved seed and practices), the percentage change in TFP on farms that implement the new technology may be calculated as

\begin{equation}
\begin{split}
\partial \ln(g^i_0) \approx \frac{g^i_0' - g^i_0}{g^i_0} \:\:; \:\:\: g^i_0' = g^i_0 e^{\beta_5 b_5} \:\: \forall i
&= e^{\beta_k} - 1
\end{split}
\end{equation}

Say $\beta_5 = 0.3$. Then $\partial \ln(g_0) = 0.35$, and so, by the criterion in Inequality \ref{eq:adoptCondSimp} adoption in the absence of exogenous subsidies/costs will increase farm net benefit if the associated percentage decrease in output price plus the $\alpha_i$-scaled increase in input prices is less than $35\%$. If the $\alpha_i$-adjusted output-input price ratio decrease is greater than $35\%$, then the subsidy term $h / C^* \partial B$ required to make adoption profitable is the margin by which it is greater. Note that the required subsidy $\partial B$ will vary across farms based on $C^*$. The factor $h / C^*$, which may also be written $\lambda / E[R^*(T)]|_{t = 0}$, represents each farm's marginal valuation of the subsidy; and implies that the subsidy is relatively more valuable to farms with relatively lower expenditure. Solving Inequality \ref{eq:adoptCond} for $C^*$ gives the maximum expenditure level for which adoption is profitable if the (absolute value of the) $\alpha_i$-adjusted output-input price ratio decrease is greater than the increase in TFP ($|\partial \ln(\nu)| > \partial \ln(g_0)$).
...
If $\partial B=0$, then Inequality \ref{eq:adoptCond} implies that adoption will be either $0\%$ or $100\%$. Empirically, this is almost never observed. Adoption rates tend to fall somewhere between $0\%$ and $100\%$. Within the theoretical framework developed here, this means that the subsidy/cost term $\partial B$ is generally not zero. The increased input demand associated with adoption implies in and of itself that, without subsidies, $\partial B < 0$. And, of course, every major green revolution success story has involved subsidies of one kind or another to offset the required increase in input expenditure, and then some, such that $\partial B > 0$ [].
...
Given the upper bound on $C^*$ for adoption (Equation \ref{eq:cStarAdopt}) and recalling that axiom 4 implies lognormally distributed $C^*$, the adoption rate $A$ when $\partial B > 0$ and $|\partial \ln(\nu)| > \partial \ln(g_0)$ can be calculated as

\begin{equation}
\begin{split}
A &= \int_0^{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)}} \phi(\frac{\ln(C^*) - m_c}{s_c}) d \ln(C^*) \\
&= \Phi\left( \frac{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} - m_c}{s_c} \right)
\end{split}
\end{equation}
<!-- \partial \ln(E[P(T)]|_{t = 0}) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w}) + \partial \ln(g_0) -->
<!-- A &= \int_0^{1 / h (\partial \ln(g_0) + \partial \ln(E[P(T)]|_{t = 0}) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w}))} \phi \left(\frac{\lnC^* - m_c}{s_c} \right) d \ln(C^*) \\ -->
<!-- &= \Phi \left(\frac{1 / h (\partial \ln(g_0) + \partial \ln(E[P(T)]|_{t = 0}) - \mathbf{\alpha} {'} \partial \ln(\mathbf{w})) - m_c}{s_c}\right) -->
...
<!-- &= \Phi\left( \frac{\frac{-h (1 - h) \partial B}{\partial \ln(P/\mathbf{w}^{\boldsymbol{\alpha}}) + \partial \ln(g_0)} - (\ln(g_0) + \sum_i^{n_x} \alpha_i\ln(\alpha_i / w_i) - h\ln(h) + m_k + m_{\#})}{\sqrt{s_k^2 + s_{\#}^2}} \right) -->
<!-- $$ -->
<!-- \Phi\left( \frac{\frac{-h (1 - h) \partial B}{\partial \ln(\mathcal{\nu}) + \partial \ln(g_0)} - (\ln(v_0) + m_k + m_{\#})}{\sqrt{s_k^2 + s_{\#}^2}} \right) -->
<!-- $$ -->


Because the adoption rate is partly a function of $v_0$, and because $v_0$ varies across strata, then the adoption rate must also vary across strata. Equation \ref{eq:adoptRate} indicates that the adoption rate will be higher in strata with lower starting $v_0$, _ceteris paribus_.
...
\begin{equation}
\frac{\partial E[Q(T)]|_{t = 0}}{\partial u_0} = \frac{E[Q(T)]|_{t = 0}}{u_0} > 0
\end{equation}
...
So for a given constant upper bound $\chi \leq \zeta$,

\begin{equation}
\int_{0}^{\zeta} \chi \rho(\chi) \: d\chi = \frac{1}{\sqrt{2\pi} s_{\chi}} \int_{0}^{P} \chi e^{-\frac{(\ln(\chi) - m_{\chi})^2}{2 s^2_{\chi}}} d \ln(\chi)
\label{eq:proofStep1}
\end{equation}
...
\begin{equation}
\begin{split}
\int_{0}^{\zeta} \chi \rho(\chi) \: d\chi &= \frac{1}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\zeta) - m_{\chi}}{s_{\chi}}} e^{(\gamma s_{\chi} + m_{\chi}) } e^{-\frac{\gamma^2}{2}} d \gamma \\
&= \frac{1}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\zeta) - m_{\chi}}{s_{\chi}}} e^{-1/2[\gamma^2 - 2 \gamma s_{\chi}  - 2 m_{\chi} ]} d \gamma
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\frac{1}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\zeta) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\gamma - s_{\chi} )^2}{2} + m_{\chi}  + \frac{s^2_{\chi}}{2} ^2} d \gamma \\
&= \frac{1}{\sqrt{2\pi}} e^{m_{\chi}  + \frac{s^2_{\chi}}{2} ^2} \int_{0}^{\frac{\ln(\zeta) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\gamma - s_{\chi} )^2}{2}} d \gamma \\
&= \frac{E[\chi]}{\sqrt{2\pi}} \int_{0}^{\frac{\ln(\zeta) - m_{\chi}}{s_{\chi}}} e^{-\frac{(\gamma - s_{\chi} )^2}{2}} d \gamma \\
&= E[\chi] \Phi \left(\frac{\ln(\zeta) - m_{\chi}}{s_{\chi}} - s_{\chi}  \right)
\end{split}
\end{equation}
