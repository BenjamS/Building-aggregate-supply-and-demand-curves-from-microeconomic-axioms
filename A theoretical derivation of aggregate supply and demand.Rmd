...
...in passing...diminishing returns to scale. Constant returns to scale ($h = 1$) imply the break-even point, while increasing returns to scale ($h > 1$) imply a loss. The farm yield function is thus globally concave in inputs. Constant and increasing returns, on the other hand, would imply global linearity and convexity, respectively. input price elasticity in appendix.
...
To develop an expression for aggregate supply, it is useful to express $y^*$ in terms of optimal cost per unit output or "average cost", henceforth referred to as the optimal unit cost or unit expenditure. Denoting this $C^{\#}$, then by definition,
...
While this is a poor choice of econometric yield model (because unit cost is calculated as a function of yield, and would thus introduce an endogeneity problem if used in an OLS regression), it will soon be seen to facilitate the derivation of the aggregate supply function. But first, it is important to point out that the introduction of $C^{\#}$ in Equation \ref{eq:cStar} results in the following endogenous cost model.
...
&= N E[k_a] \frac{E[y^*]}{E[k^{\eta + 1}] E[C^{\# \eta}]} \int_{0}^{\infty} k^{\frac{\eta}{h}} \int_{0}^{\bar{C}^{\#}} C^{\# \eta} \rho(C^{\#}) dC^{\#} \rho(k) dk \\
&= N E[k_a] \frac{E[y^*]}{E[C^{\# \eta}]} \int_{0}^{\bar{C}^{\#}} C^{\# \eta} \rho(C^{\#})
\end{split}
\end{equation}

Axiom 3 states that farms enter production only if they expect to make positive profit. This condition may be formalized $R^* > C^*$, which in turn implies $P > C^{\#}$. By axiom 3, then, the upper bound of integration $\bar{C}^{\#}$ must be $P$.
...
In the evaluation of $Q$, it is important to keep in mind that $E[y^*]$, $m_{\#}$, and $s_{\#}$ are unconditional population parameters. That is to say, $E[y^*]$ refers to the mean yield given the participation of all farms in the population; and $m_{\#}$ and $s_{\#}$ refer to the mean and standard deviation of $\ln(C^{\#})$ given full participation, i.e. as $C^{\#}$ goes to infinity. But axiom 3 truncates the upper tail of the distribution of $C^{\#}$ at $P$. The observed parameters $\tilde{m}_{\#}$ and $\tilde{s}_{\#}$ are thus conditional.
...
Given this formalization of the stochastic character of ouput price and optimal unit cost, the relation between the two (Equation \ref{eq:useful}) can now be rewritten more carefully as follows.
...
Which is a result that can also be reached by invoking Fubini's theorem. To be clear, this result states that the population mean of the optimal expected unit cost ($E[C^{\#}(T)]\bigr|_{t = 0}$) is equal to the optimal expected unit cost on the mean farm, i.e. the farm with $\lambda = E[\lambda]$.
