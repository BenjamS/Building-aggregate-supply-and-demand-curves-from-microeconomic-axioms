...
(Where $\epsilon$ is the error term.) Such that the estimated coefficient on logged farm cost may be directly interpreted as the degree of homogeneity for the population. When estimating the model in Equations \ref{eq:cobbDoug2} or \ref{eq:cobbDoug3}, by contrast, $h$ must be tallied by summing the estimated coefficients $\alpha_i$, and will likely vary across the population since not all farms necessarily apply all the inputs specified in the model. The new term $v_0$, moreover, is arguably a more meaningful measure of TFP than $g_0$, since inputs are expressed in identical units of cost; and is henceforth referred to as the "expenditure TFP".

### Important corollaries: returns to scale, cancavity, price elasticities of farm supply and input demand

In Appendices A.2 and A.3 it is shown that axioms 1 and 2 imply the degree of homogeneity $h$ must exhibit strictly decreasing returns to scale ($0 < h < 1$), whereby it also follows that farm yield is globally concave in inputs, and that the farm supply curve is positively sloped. It is also shown that the output price elasticity of farm supply ($\eta$) is

\begin{equation}
\eta = \frac{h}{1 - h}
\end{equation}

Whereby the domains of $h$ over which farm supply is elastic or inelastic immediately follow. The input own and cross price elasticities of demand are likewise derived and shown to be strictly negative when returns to scale are decreasing.
...
### The farm yield and expenditure response to technology change
...
<!-- \label{eq:cSharp} -->
<!-- \end{equation} -->
Noting that $v_0$ can also be expressed
\begin{equation}
\begin{split}
...
<!-- However, the agronomic yields obtained at research stations are not indicative of economic yields ($y^*$) or farm expenditures ($C^*$) in a real farm population. This is plain to see from the yield and expenditure expressions derived above; and is also just an empirically well documented lesson learned from decades of agricultural research [@herdt1988increasing; @balasubramanian2000yield; @sumberg2012mind]. -->
...
### The technology adoption decision is endogenous
...
this is comparative statics. welfare has to be discounted according to release/adoption period. welfare and subsidy incur year by year
...
Decreasing returns to scale also imply strictly downward sloping input demand curves. To see this, consider the derivative of input demand (Equation \ref{eq:inputDem1}) with respect to input own price.
