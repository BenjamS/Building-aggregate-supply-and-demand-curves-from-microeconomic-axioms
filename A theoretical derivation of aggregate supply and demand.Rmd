...
\begin{equation}
g_0 = \tilde{y} \prod_{i = 1}^{n_k} \tilde{k}_i^{-\kappa_i} \prod_{i = 1}^{n_x} \tilde{x}_i^{-\alpha_i}
\end{equation}

Such that the yield function may be rewritten

\begin{equation}
y = \tilde{y} \prod_{i = 1}^{n_k} \left( \frac{k_i}{\tilde{k}_i} \right)^{\kappa_i} \prod_{i = 1}^{n_x} \left( \frac{x_i}{\tilde{x}_i} \right)^{\alpha_i}
\label{eq:cobbDoug3}
\end{equation}

Because the ratios in parentheses are dimensionless, $\tilde{y}$ must have the same units as $y$; and $\tilde{y}$ may be interpreted as the value of $y$ when all management and control variables equal their reference values. In the empirical study included here, for example, the reference values are set equal to the sample means, such that $y_0$ is the value of $y$ evaluated at the sample means of the input and control variables.
...
\begin{equation}
y^* = y(\mathbf{x}^*; \tilde{y}, \mathbf{k}, \boldsymbol{\alpha}, \boldsymbol{\kappa}) \:;\:\: x^* = \text{argmax}_{\mathbf{x}}(P y - \lambda(C - B))
\end{equation}
...
\begin{equation}
y^* =  \left(\tilde{y} \prod_i \left(\frac{\alpha_i}{w_i}\right)^{\alpha_i} \prod_i k_i^{\kappa_i} \right)^{\frac{1}{1 - h}} \left(\frac{P}{\lambda}\right)^{\frac{h}{1 - h}}
\label{eq:YdEnv1}
\end{equation}
...
\begin{equation}
y^* = (\tilde{y} \theta)^{\frac{1}{1 - h}} \left( \frac{P}{\lambda} \right)^{\frac{h}{1 - h}}
...
\begin{equation}
y^* = \tilde{y} \theta \left( \frac{C^*}{h} \right)^h
\label{eq:yStarC}
\end{equation}

The degree of homogeneity $h$ is thus also the scale elasticity of yield ($d \ln(y^*) / d \ln(C^*) = h$) as noted in the textbooks. This form is particularly useful for estimation, as it requires only the control variables $k_i$ and input cost $C^*$, as opposed to detailed information on all of the input quantities. That is, instead of estimating the model in Equation \ref{eq:cobbDoug3}, one estimates

\begin{equation}
y = \hat{y} C^{*h} \prod_{i = 1}^{n_k} \left( \frac{k_i}{\tilde{k}_i} \right)^{\kappa_i}
\label{eq:cobbDoug4}
\end{equation}

Where the estimated constant $\hat{y}$ may then be interpreted as 

\begin{equation}
\hat{y} = \tilde{y} \frac{\theta_{\alpha}}{\theta_w} h^{-h}
\label{eq:yConst}
\end{equation}

Such that the input elasticities $\alpha_i$ and prices $w_i$ are implicit in the fitted constant $\hat{y}$ without requiring detailed information about input types and quantities. Moreover, the fitted coefficient on $\ln(C^*)$ may be interpreted as $h$.

Note also the similarity to Equation \ref{eq:integrate_lny}. And note that
...
Below, an aggregate supply curve is parameterized based on Kenya farm data for the 2007 main cropping season [@maizeData]... multiple family labor types, fert/chem types, also county level historical rainfall data and 2007 rainfall data at.... (graphics here or in Appendix) Summary statistics can be found in the Appendix. lots of management variables available, but not required because of Equation \ref{eq:yStarC}.

The yield model is specified in logs as a function of cost ($C^*$), control variables ($k_i$), and binary endowment variables ($b_i$), plus an error term $\epsilon$ assumed to be normally distributed with mean 0.

\begin{equation}
\ln(y) = \ln(\hat{y}) + \sum_{i = 1}^{n_k} \kappa \ln \left(\frac{k_i}{\tilde{k}_i}\right) + h \ln \left( \frac{C^*}{\tilde} \right) + \epsilon \:\:;\;\;\; \ln(\hat{y}) = \beta_0 + \sum_i \beta_i \ln(b_i)
\label{eq:regMod}
\end{equation}

This is just the yield function in Equation \ref{eq:yStarC} in logs, and adjusted for estimation error $\epsilon$ and binary endowments $b_i$. The binary endowments in the model are land tenure (whether or not the farmer owns their land), hybrid maize seed, oxen, and tractor. These variables take a value of $e$ if the endowment is present and $1$ otherwise, such that the coefficients $\beta_i$ are added to the intercept term $\beta_0$ on farms that have the endowment, and vanish otherwise.
...
The continuous control variables $k_i$ are farm size (acres), seed quantity per acre, child family labor during planting (person-hours/acre), adult-equivalent household size (aehh07), and the 2007 main season rainfall. The cost per acre $C^*$ is the sum of all land prep and input (fertilizer, hired labor, and pesticide) costs. The OLS estimation results are presented in Table 1.

Models including farmer age, education, gender, maximum and minimum temperature, other types of family labor, and distance to market variables were also explored. But the coefficients on these variables were either statistically insignificant or introduced multicollinearity and/or missing data issues.

As discussed in section \ref{sec:ax12}, the independent continuous variables $k_i$ and $C^*$ are divided by their respective sample means $\tilde{k}_i$ and $\tilde{C}^*$ in order to facilitate interpretation of the constant $\hat{y}$. (In the case of rainfall, $\tilde{k}_i$ is the historical average, not the sample average). The constant $\hat{y}$ may then be interpreted as the "sample average yield", that is, the farm yield evaluated at the sample mean of the independent variables (historical mean in the case of rainfall).
...
Note that the constant $\hat{y}$ is not the same for all farms, but varies depending upon how many binary endowments the farm has. The "base case" of $\hat{y} = e^{\beta_0}$, represents the sample average yield on farms with neither land tenure nor hybrid maize nor use of oxen or tractors for land preparation. In the sample examined here, there are 14 unique values for $\hat{y}$, summarized in Figure \ref{fig:ySampAvg}.

Equation \ref{eq:regMod} represents the real yield $y^*(T)$, known post harvest. Assuming that farmers generally expect rainfall to adhere to the historical average, then the expected yield at planting time ($E[y^*(T)]\bigr|_{t = 0}$) may reckoned from the available post hoc information as follows.

\begin{equation}
E[y^*(T)]\bigr|_{t = 0} = y^*(T) \frac{k_r}{\tilde{k}_r}^{-\kappa_r} e^{\frac{s_{\epsilon}^2}{2}}
\end{equation}

Where $s_{\epsilon}^2$ is the variance of the error term $\epsilon$.


The coefficients and intercept of the logged model are estimated via OLS and presented in Table 1. Because the model is estimated in logs, the coefficients on the continuous variables may be interpreted as the control variable elasticities ($\kappa_i$). Note that the coefficient on acres is negative, affirming the inverse relation between farm size and yield found in numerous other studies [@muyanga2019revisiting; @larson2014should; @wassie2019revisiting; @gautam2019too; @kimhi2006plot]. The coefficient on child family labor during planting is also negative, confirming that this variable is better thought of as a hard constraint rather than a discretionary management variable. There are 14 unique sample average yields in the present sample, summarized in Table 2.
...
By Equation \ref{eq:yStarC}, the coefficient of `r round(hCoef, 3)` on cost/acre can be interpreted as the yield degree of homogeneity $h$. By Equation \ref{eq:PelastYstar}, the farm level price elasticity of supply then follows as `r round(etaCoef, 3)`.

The fitted values $\hat{y}$ may be expressed as follows.

\begin{equation}
\hat{y} = \tilde{y} \tilde{\theta}_k \frac{C^*}{\tilde{C}^*} e^{\epsilon}
\end{equation}

Expected yield

\begin{equation}
E[\hat{y}] = E[\tilde{y}] E[\tilde{\theta}_k] E\frac{C^*}{\tilde{C}^*} e^{\frac{s_{\epsilon}^2}{2}}
\end{equation}
...
#=====================================================================
# Get FAO Kenya maize data for calibration of Nf / validation of Qs
thisFolder
