...
Substituing the right hand side of this equation for the integral in the previous equation results in the following closed form expression for aggregate supply.

\begin{equation}
Q = N E[k_a] E[y^*] \Phi\left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)
\end{equation}

Here it is important to note that E[y] is not observed. The conditional mean E[y|c<p] is observed. The unconditional mean does not change with price but with technology or lambda. The conidtional mean changes with price, in addition to technology and lambda. So, given the relation between conditional and unconditional mean of a lognormally distributed variable, 
...
maizeProvs <- c("Uasin Gishu", "Narok", "Nakuru", "Trans Nzoia", "Kakamega", "Bungoma", "Nandi")
...
# kVars <- c("Seed (kg/acre)", "Farm size (acres)", "Rain anomaly", "Adult equivalent household size")
# dfMod3$k <- dfMod3 %>% select(all_of(kVars)) %>% rowSums
# dfMod3 <- dfMod3 %>% select(-all_of(kVars))
...
dfMod3 <- dfMod3 %>% subset(cSharp < 2.5)
...
anomVec <- c()
for(i in 1:(nYr - 1)){
  thisDate <- as.Date(paste0(as.character(yrVec[i]), "-04-01"))
...
#=============================================================================
ProbProfit <- pnorm((log(EPT02007) - mP2006 * th) / (sP2006 * sqrt(th)) - eta * sP2006 * sqrt(th))
EmploymentRate <- pnorm(log(EPT02007), mP2006 * th, sP2006 * sqrt(th))
EyStarT0 <- EyStarT0trunc * EmploymentRate / ProbProfit

#muEcStarT0hSamp <- mean(exp(dfMod3$`Cost/acre (KES/acre)`)^h)
EcStarT0Samp <- exp(dfMod3$`Cost/acre (KES/acre)`)
ErStarT0Dedu <- EyStarT0 * EPT02007
EcSharpT0 <- EcStarT0Samp / EyStarT0
lambda <- EPT02007 / EcSharpT0 * h
...
EcStarT0base <- EyStarT0 / (muThetaK * v0base)
...
EcSharpT0trunc <- exp(dfMod3$`Cost/acre (KES/acre)`) / EyStarT0trunc
lEcSharpT0trunc <- log(EcSharpT0trunc)
lam <- EPT02007 / EcSharpT0trunc * h
