...
However, it is important to keep in mind that these are _unconditional_ means. Because axiom 3 truncates the upper tail of the distribution of $C^{\#}$ at $P$, then sample parameters calculated from observed data must be interpreted as conditional parameters. That is to say, the sample means of expenditure and yield do not represent $E[C^*]$ and $E[y^*]$, but rather the conditional means $E[C^* | C^{\#} \leq P]$ and $E[y^* | C^{\#} \leq P]$.

The conditional means can be evaluated by replacing $m_c$, $s_c$, $m_{\#}$, and $s_{\#}$ in the expressions above with the sample parameters $\bar{m}_c$, $\bar{s}_c$, $\bar{m}_{\#}$, and $\bar{s}_{\#}$, defined
...
This is preferred to calculating the mean and standard deviation directly from observed $y^*/C^*$ for reasons that will become clear when stochasticity is taken into account farther below.
(It will turn out that $y^*$ refers not to harvest yield but to expected harvest yield at planting time, which is not observed.)
...
Note, moreover, that the conditional and unconditional means of any lognormally distributed variable $\chi$ with upper bound $\xi$ are related as follows.
...
\begin{equation}
E[C^{\# \eta}] \Phi \left(\frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#}\right) = E[C^{\# \eta} | C^{\#} \leq P] \mathcal{E}
...
\begin{equation}
E[y^*] \Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right) = E[y^* | C^{\#} \leq P] \mathcal{E}
...
Where $N_{\ell} / N$ is the fraction of farms in the $\ell^{th}$ binary endowment stratum.

## The endogenous farm level response to exogenous technology shocks
...
By Equation \ref{eq:condMeanCshrp},
...
The fitted binary endowment coefficients ($\beta_i$) indicate that hybrid maize seed is the most significant contributor to TFP, followed by ... and ..., with farm tenure also playing a role... Based on this, the expenditure TFPs are calculated for each of the 14 binary endowment strata ($v_{\ell}$), and the average across all strata is also computed ($v$). The conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ are calculated from the fitted values $v$ and $h$, and the sample parameters $\bar{m}_c$, $\bar{s}_c$, $m_k$, and $s_k$, by Equation \ref{eq:mCshrpsCshrp}. And the disaggregated $\bar{m}_{c \ell}$ are also computed...and then the conditional mean cost and yield are also evaluated both in the aggregate and disaggregated by strata, as explained in section \ref{sec:binEndStrat}

presented in Table 2 along with the corresponding conditional mean cost and yield, which are evaluated as explained in section \ref{sec:binEndStrat}.
...
The expenditure TFP,  and presented in Table 2, disaggregated by binary endowment stratum.
...
equations x in section x can be rearranged into expressions for msharp ssharp
