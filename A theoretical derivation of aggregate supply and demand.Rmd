...
\begin{equation}
y^* = (y_0 \theta)^{\frac{\eta}{h}} \left( \frac{P}{\lambda} \right)^{\eta} \:\:;\:\:\: \eta = \frac{h}{1 - h}
...
Where the $w_i$ are input prices. Note that $\eta$ is the farm level output price elasticity of supply; and that it is endogenously determined by the technology $h$. The yield envelope---i.e., the budget constrained, profit maximizing farm yield in Equation \ref{eq:YdEnvCompact0}---is henceforth referred to simply as the farm yield. Likewise, "farm production function" henceforth refers to the budget constrained, profit maximizing production function $q^*$ obtained when $y^*$ is multiplied by the cultivated area $k_a$.

\begin{equation}
q^* = k_a y^* = k_a (y_0 \theta)^{\frac{\eta}{h}} \left( \frac{C^{\#}}{h} \right)^{\eta}
\end{equation}
...
4) Assuming that farms enter production only if they expect to make a profit (axiom 4), then cost per unit output cannot exceed price, thus resolving the upper bound of this integral. Since cost cannot be negative, the lower bound is $0$. The domain of integration must then be $0 < C^{\#} < P$.
...
\begin{equation}
Q = N \left( \frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E \left[\hat{\theta}_k^{\frac{\eta}{h}} \right] \frac{E \left[C^{\# \eta}\right]}{h^{\eta}} \Phi(\gamma) \:\:;\:\:\: \gamma = \frac{\ln\left(\frac{P}{E \left[C^{\#} \right]} \right)^{\eta} - \frac{\eta^2}{2} Var \left[\ln \left(C^{\#} \right) \right]}{\eta \sqrt{Var \left[\ln \left( C^{\#} \right) \right]}}
\label{eq:sup2a}
\end{equation}

By making substitutions based on the corollary in Equation \ref{eq:useful}, this can be rewritten in a variety of ways, in accordance with the objectives and information available in a given use case. For the rest of the article, the focus is on the classical form below, where the fraction $C^{\#} / h$ has been replaced with $P / \lambda$.

\begin{equation}
Q = N \left( \frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E \left[\hat{\theta}_k^{\frac{\eta}{h}} \right] E[\lambda^{-\eta}] P^{\eta} \Phi(\gamma) \:\:;\:\:\: \gamma = \frac{\ln\left(\frac{P}{E \left[C^{\#} \right]} \right)^{\eta} - \frac{\eta^2}{2} Var \left[ \ln \left(C^{\#} \right) \right]}{\eta \sqrt{Var \left[\ln \left(C^{\#} \right) \right]}}
\label{eq:sup2b}
\end{equation}

This is "classical" in the sense that makes aggregate supply as explicit a function of output price as possible.
...
\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} &= \beta E[P(T)^{\eta}]|_{t = 0} \Phi(\gamma) \:\:; \\
...
\begin{equation}
\begin{split}
E[Q(T)]|_{t = 0} &= \beta P(0)^{\eta} e^{\eta m_p T} \Phi(-z) \:\:; \\
...
The term $z$ is the standard score of the hypothetical log change in output price $\ln \left(E\left[\tilde{C}^{\#}(T)\bigr|_{t = 0} \right] / P(0) \right)$, scaled by the farm level price elasticity $\eta$---i.e., the log change in output price from its value at planting time ($P(0)$) to a value equal to the mean farm's expected cost per unit output, scaled by $\eta$. The term $\Phi(z)$ is thus the probability that output price will fall from its value at planting time to or below the mean farm's expected cost per unit output (scaled by $\eta$). And the term $\Phi(-z)$, it follows, is the probability that output price will remain above the mean farm's expected cost per unit output (scaled by $\eta$). In simple terms, $\Phi(-z)$ is roughly the probability that the mean farm will be profitable.
<!-- \begin{equation} -->
<!-- \begin{split} -->
...
The output price elasticity of aggregate supply is thus the farm level elasticity plus the effect of farm entry/exit in response to the price movement. As farm entry approaches $100\%$, the price elasticity of aggregate supply approaches $\eta$, the price elasticity of farm level supply. Aggregate supply is inelastic/elastic if
...
A novelty of the form is that aggregate supply sensitivity to price volatility may also be inspected.
...
z is negative (?) therefore $\eta^2 > z / (s_p \sqrt{T})$ therefore $\frac{\partial \ln(E[Q(T)]|_{t = 0})}{\partial (s_p \sqrt{T})} < 0$


<!-- Note, moreover, that elasticity is inversely proportionate to the price risk ($s_p \sqrt{T}$). -->

The effect of price risk on supply can be examined more explicitly as follows.
...
# An illustrative use case: Parameterization of the aggregate supply curve using Kenya farm survey data
