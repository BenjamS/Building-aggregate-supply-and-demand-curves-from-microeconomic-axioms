...
To redress cases where implausibility is rooted in the assumptions and theoretical premises of the modeling---which we suspect account for the majority of cases---here we present an alternative aggregate supply model that prioritizes theoretical consistency and transparency, such that implausible results may be traced back to specific assumptions regarding economic behavior, and/or the estimation of specific parameter values with a clear mapping to measurable empirical phenomena. Progress in this direction has long been discouraged by a general perception that the task is impossible. In their highly influential book, e.g., Alston et al. [-@alston1995science] write:
...
The aggregate supply model that mathematically results from axioms 1-5 consists of terms that capture technology, agroclimatic conditions, input and output prices, and farm entry/exit. The term capturing farm entry/exit turns out to be surprisingly instructive and intuitive, as it is proportionate to the probability that output price will be greater than the mean farm's expected cost per unit output at harvest (i.e. the mean farm's probability of profit). The price elasticity of aggregate supply resulting from this functional form is likewise intuitive, consisting of farm level price elasticity of supply plus a term capturing farm entry/exit in response to price movements. The model also accounts for price volatility, such that the effect of price stabilization policies may be explicitly modeled. This gain in theoretical rigor and explanatory power comes at little or no cost to tractability on modern processors.

Derivation of the farm yield function from axioms 1 and 2 is review for professional economists, and is given in the Appendix. The main sections of this article are reserved for the derivation of the aggregate supply model from the farm yield function and from axioms 3-5. With the main task out of the way, we then walk through an illustrative use case, parameterizing the model based on Kenya maize farm data. The parameterized model generates calculations of aggregate production and  price elasticity of aggregate supply that agree with empirical estimates. Food policy relevance, application in partial equilibrium analysis, and application in minimum data contexts are briefly considered in the Discussion.
<!-- ----------------------------------- -->
...
## Derivation of aggregate supply from Axioms 3-5

### Geometric Brownian motion of prices and yield (Axiom 3) \label{sec:gBmAx}

price might be mean reverting over the course of multiple seasons, but follows a trend with any given season.
...
<!-- However, the specific question of whether or not $y^*(t)$ follows a geometric Brownian motion can be assessed in another way: Insofar as the stochastic movement of the meteorological variables on which yield depends follows a geometric Brownian motion (as in Figure \ref{fig:gBmWeather}), then it can be shown by Ito's lemma that $y^*(t)$ must also follow a geometric Brownian motion. -->
...
...It is important to consider these time series over a relevant time period, i.e. the growing season. Beyond that the motion may exhibit cyclic or mean reverting behavior, which diverges from gBm..
