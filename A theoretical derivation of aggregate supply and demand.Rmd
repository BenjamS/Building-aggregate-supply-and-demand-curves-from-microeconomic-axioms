...
thisFolder <- "D:/OneDrive - CGIAR/Documents 1/CIAT 2/A theory of aggregate supply and demand/Price data/"
...
dfHist <- df %>% subset(year %in% c(2008, 2011, 2012))
gg <- ggplot(dfHist, aes(x = `Log change`))
...
#-------------------------------------------------------------------------------------
#shapiro.test(df$`Log change`)
sum(is.na(dfHist$`Log change`) == F)
mP <- mean(dfHist$`Log change`, na.rm = T)
sP <- sd(dfHist$`Log change`, na.rm = T)
timeHoriz <- 
mP * timeHoriz
sP * sqrt(timeHoriz)
dfP0 <- df %>% subset(Date %in% as.Date(c("2008-03-01", "2008-02-01", "2008-04-01")))
P0 <- mean(dfP0$Price, na.rm = T)
EPT0 <- P0 * exp(mP * timeHoriz)
...
lamTrunc <- EPT0 / EcSharpT0trunc * h
...
mLamba <- log(P02007) + (mP - sP^2 / 2) * timeHoriz + log(h) - mEcSharpT0
mean(log(lamTrunc))

ProbProfit <- pnorm((log(EPT0) - mEcSharpT0) / (sEcSharpT0) - eta * sEcSharpT0)
