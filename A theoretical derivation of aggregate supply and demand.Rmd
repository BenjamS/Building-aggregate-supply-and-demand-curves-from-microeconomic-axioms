...
Four other desirable properties---a farm profit function convex in output price, a homogeneous (and thus homothetic) farm production function, upward sloping farm supply curves, and downward sloping farm input demand curves---follow as corollaries, and thus do not have to be imposed as additional Axioms. [The form also agrees with Lipton's theory of technology adoption.]
...
## Derivation of the farm production function from Axioms 1-3
...
It is straightforward to see that equation \ref{eq:cobbDoug2} is a convex, homogeneous (and thus also homothetic) function with degree of homogeneity $h$ equal to $\sum_i \alpha_i$. There is no need, then, to impose these properties as additional Axioms. They follow from the Axiom of diminishing marginal returns.
...
Where Euler's Theorem has been invoked in the second line of the condition. It follows that Axiom 2 requires $0 < h < 1$, i.e. diminishing returns to scale. Constant returns to scale ($h = 1$) imply the break-even point, while increasing returns to scale ($h > 1$) imply a loss.
...
To reduce notational clutter, $y^*$ is henceforth rewritten as follows.
...
Axioms 1 and 2 thus imply constant marginal cost.
...
\begin{equation}
\begin{split}
\frac{\partial \ln(x_j^*)}{\partial \ln(w_j)} &= - 1 - \frac{1}{1 - h} \frac{\partial \ln(\beta_w)}{\partial \ln(w_v)} \\
&= \frac{-\alpha_v}{1 - h} - 1
\end{split}
...
Yield and output price are, of course, stochastic variables. Henceforth it is necessary to introduce more precision in their treatment as such. Yield and output price In the absence of forward contracts, output price $P$ is likewise unknown at the time of planting. The farmer makes planting decisions based on input prices, the yield they _expect_ to achieve, and the price they _expect_ their product to fetch in the market at harvest.


That is to say, $\lambda$ is more properly defined
...
Necessary to be more precise in language...P is expected price at time of sale, as evaluated at the time of planting (the time of expenditures on farm inputs). This is henceforth written as
