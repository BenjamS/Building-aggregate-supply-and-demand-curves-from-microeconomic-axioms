...
\begin{equation}
y^* = y(\mathbf{x}^*; y_0, \mathbf{k}, \boldsymbol{\alpha}, \boldsymbol{\kappa}) \:;\:\: x^* = \text{argmax}_{\mathbf{x}}(P y - \lambda(C - B))
\end{equation}
...
Where the estimated constant $\tilde{y}$ may then be interpreted as 

\begin{equation}
\tilde{y} = y_0 \frac{\theta_{\alpha}}{\theta_w} h^{-h}
\label{eq:yConst}
\end{equation}

Such that the input elasticities $\alpha_i$ and prices $w_i$ are implicit in the fitted constant $\tilde{y}$ without requiring detailed information about input types and quantities. A further advantage of this form is that the fitted coefficient on $\ln(C^*)$ may be directly interpreted as $h$, valid for all farms in the sample. Estimation of the model in Equation \ref{eq:cobbDoug3}, on the other hand, requires the extra step of summation of the input elasticities $\alpha_i$ in order to arrive at $h$. This results in varying $h$ across the sample, as not all farms apply all the inputs considered in the model.
...
The aggregate supply curve derived from Axioms 1-5 above is parameterized based on Kenya farm data. This sort of a validation but not so much a validation...

## Data, specification, estimation

The farm data used below cover the 2007 main maize season in all maize producing regions of Kenya. The data are a subset of a much larger panel covering the main and short seasons of multiple crops for the years 2000, 2004, 2007, and 2010, originally gathered by the Egerton University Tegemeo Institute of Agricultural Policy and Development in partnership with Michigan State University, under the auspices of a USAID project [@maizeData]. The data include use of x organic and chemical fertilizers, x pest/plague control chemicals. The data also include detailed information on quantities of adult and child family labor expended on x different farm labor activities. In the case of 2007 main season maize, only xx fertilizers and xx pest/plague chemicals were used in significant quantities; and family labor was concentrated on xx activities (Figure \ref{xx}). Numerous demographic and infrastructural control variables are also included, as well as 2007 rainfall data... as well as the historical average rainfall. ...binary endowment variables...
...
As discussed in section \ref{sec:ax12}, the independent continuous variables $k_i$ and $C^*$ are divided by their respective sample means $\tilde{k}_i$ and $\tilde{C}^*$ in order to facilitate interpretation of the constant $\hat{y}$. (In the case of rainfall, $\tilde{k}_i$ is the historical average, not the sample average). The constant $\hat{y}$ may then be interpreted as the "sample average yield", that is, the farm yield evaluated at the sample mean of the independent variables (at the historical mean in the case of rainfall).
...
\begin{equation}
\ln(y) = \ln(\tilde{y}) + \sum_{i = 1}^{n_k} \kappa \ln \left(\frac{k_i}{\tilde{k}_i}\right) + h \ln \left( \frac{C^*}{\tilde} \right) + \epsilon \:\:;\;\;\; \ln(\hat{y}) = \beta_0 + \sum_i \beta_i \ln(b_i)
\label{eq:regMod}
\end{equation}
