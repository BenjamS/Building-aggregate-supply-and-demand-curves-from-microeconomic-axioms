...
In the first part of the derivation, immediately below, the farm yield function is derived from axioms 1 and 2. Most of these steps are elementary and thus presented in a highly summarized form, with details provided in the Appendix. The groundwork for aggregation is then laid based on axioms 3 and 4. This includes derivation of the upper bound on farm expenditure and of the farm employment rate, as well as a discussion of conditional and unconditional means and the need to account for total factor productivity stratification based on "either/or" binary endowments (land tenure, use of oxen, etc.) that vary across the farm population.

The endogenous farm level response to tech shock and associated price changes is then formalized based on the 

(here is where we demonstrate the agreement with lipton) hic rhodis, hic saltum! All of the key differences between our approach and the conventional approach stem from this key step. 
