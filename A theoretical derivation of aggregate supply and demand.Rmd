...
Letting $C^{\#}$ denote the "average cost of production" (i.e., the cost per unit output), moreover, then
...
And the marginal cost and average cost are thus related as follows.
...
## Derivation of aggregate supply from Axioms 4-5 \label{sec:aggSupply}

### Geometric Brownian motion of prices and weather (Axiom 4)

Henceforth it is necessary to be more specific in the treatment of yield $y^*$ and output price $P$ as stochastic variables. Absent a forward contract, the output price at the time of harvest and sale $(t = T)$ is unknown to the farmer at the time of planting and input expenditures $(t = 0)$. Likewise, yield is largely dependent upon numerous stochastic meteorological processes occurring between planting and harvest which are unknown to the farmer at the time of planting. Wherever $P$ and $y^*$ appear in the equations above, then, what is really meant is $E[P(T)]\bigr|_{t = 0}$ and $E[y^*(T)]\bigr|_{t = 0}$.
...
yield is a linear function of these things then yield also gBm

\begin{equation}
E[C^{\#}(T)]^* = C^* E \left[ \frac{1}{y^*(T)} \right] \biggr|_{t = 0}
\label{eq:EcSharp}
\end{equation}



In particular, the farmer's expenditure decision at or around planting time ($C^*$) is based on their expectations regarding the yield and price they can fetch at harvest and sale.

\begin{equation}
C^* = \frac{h}{\lambda} = E[P(T)]\bigr|_{t = 0} E[y^*(T)]\bigr|_{t = 0}
\label{eq:cStar}
\end{equation}
...
\begin{equation}
E[C^{\#}(T)]^* = \frac{h}{\lambda} = E[P(T)]\bigr|_{t = 0} E[y^*(T)]\bigr|_{t = 0}
\label{eq:cSharp}
\end{equation}

<!-- And the optimal input level (Equation \ref{eq:inputDem1}) is thus more accurately represented as -->
<!-- \begin{equation} -->
<!-- x_j^* = \frac{E[P(T)]\bigr|_{t = 0} E[y^*(T)]\bigr|_{t = 0} \alpha_j}{\lambda w_j} -->
<!-- \label{eq:inputDem2} -->
<!-- \end{equation} -->
[Not necessary to specify y(t) process as C^{\#} based entirely on P(t) (by the lambda equation)]

The question remains as to what kind of stochastic process $y^*(t)$ and $P(t)$ follow. Prices generally follow a geometric Brownian motion. That is to say, changes in $P$ over a small time interval $\Delta t$ ($\Delta P = P(t + \Delta t) - P(t)$) are often accurately modeled by
...
```{r Fig1, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="center", fig.cap="\\label{fig:gBmPrice}(Left) Maize price series from (top to bottom) Eldoret market, Kenya (FPMA, 2023), Johannesburg, South Africa (JSE, 2023), and Chicago, USA (CME, 2023). (Right) The corresponding daily (monthly, in the case of Kenya) log changes exhibit the normal distribution typical of geometric Brownian motion.", echo = FALSE}
...
gg <- gg + facet_wrap(~Place, ncol = 1, scales = "free", strip.position = "left")
...
The log change of a geometric Browning motion is defined over finite time intervals $\ln(P(T) / P(\hat{t}))$ for some starting time $t<T$, and is also normally distributed, with mean and variance
...
Examples of maize prices following a geometric Brownian motion are given from different parts of the world in Figure \ref{fig:gBmPrice}. If $P(t)$ follows a geometric Brownian motion, the expected sale price $E[P(T)]\bigr|_{t = 0}$ equals the price at planting time $P(0)$ times $e^{m_p T}$.

\begin{equation}
E[P(T)]\bigr|_{t = 0} = P(0) e^{m_p T}
\end{equation}

The weather variables upon which crop yields depend generally follow geometric Brownian motion. Since yield is a function of such variables, then, by Ito's lemma, yield is also a geometric Brownian motion. Examples of weather variables following a geometric Brownian motion over a given growing season are presented in Figure \ref{fig:gBmWeather}. 


<!-- [atmospheric processes] -->
The assumption that yield and output price follow independent geometric Brownian motions (Axiom 4) is thus well supported, empirically.



<!-- Letting $b(T)$ stand for the ratio $P(T) / C^{\#}(T)$, this means that the Log change $\ln(b(T) / b(\hat{t}))$ is normally distributed with mean and variance -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- E \left[\ln \left( \frac{b(T)}{b(\hat{t})} \right) \right] &= \left(m_b - \frac{s_b^2}{2} \right) T \\ -->
<!-- Var \left[\ln \left( \frac{b(T)}{b(\hat{t})} \right) \right] &= b_{p}^2 T -->
<!-- \end{split} -->
<!-- \end{equation} -->
<!-- Where  -->
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- m_b &= m_p - m_y + \ln(C^*) \:\:;\:\:\: m_y = E\left[ \frac{\Delta y^*}{y^* \Delta t} \right]\\ -->
<!-- s_b^2 &= s_p^2 + s_y^2 \:\:;\:\:\: s_y^2 = Var\left[ \frac{\Delta y^*}{y^* \Delta t} \right] -->
<!-- \end{split} -->
<!-- \end{equation} -->
```{r Fig1, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="center", fig.cap="\\label{fig:gBmWeather}(Left) Daily average weather variable time series covering the 2018 maize long rains growing season in Eldoret, Kenya (NREL 2023). (Right) The corresponding daily log changes exhibit the normal distribution typical of geometric Brownian motion.", echo = FALSE}
...
gg <- gg + labs(title = "Daily time series of key weather variables influencing crop yields\nEldoret, Kenya, 2018 maize growing season")
gg <- gg + theme_bw()
gg <- gg + theme(axis.title.x = element_text(size = axisTitleSize),
                 axis.title.y = element_blank(),
                 axis.text = element_text(size = axisTextSize),
...
### Aggregation from farm to population level based on Axiom 5
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- E[Q(T)]\bigr|_{t = 0} &= N_f \int_{0}^{\bar{q}^*} q^* f(q^*) \: dq^* \\ -->
<!-- &= \theta E[P(T)]\bigr|_{t = 0}^{\eta} \int_{0}^{h^{-\eta}} \lambda^{-\eta} f(q^*) \: dq^* \:\:;\:\:\: \theta = N_f E[A] (g_0 E[\beta])^{\eta / h} -->
<!-- \end{split} -->
<!-- \label{eq:sup2} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \int_{0}^{h^{-\eta}} \lambda^{-\eta} f(q^*) \: dq^* = \int_{0}^{h^{-\eta}} \lambda^{-\eta} f(\lambda^{-\eta}) \: d \lambda^{-\eta} -->
<!-- \end{equation} -->

Assuming production is lognormally distributed (Axiom 5), then

\begin{equation}
\begin{split}
E[Q(T)]\bigr|_{t = 0} &= N_f \int_0^{E[\bar{q}^*(T)]|_{t = 0}} E[q^*(T)]|_{t = 0} f(E[q^*(T)]|_{t = 0})  \: d E[q^*(T)]|_{t = 0} \\
&= N_f E[E[q^*(T)]\bigr|_{t = 0}] \Phi \left(\frac{\ln\left(\frac{E[\bar{q}^*(T)]|_{t = 0}}{E[E[q^*(T)]|_{t = 0}]}\right) - \frac{s_{EqT0}^2}{2}}{s_{EqT0}} \right) \:\:;\:\:\: s_{EqT0}^2 = Var[E[q^*(T)]\bigr|_{t = 0}]
\end{split}
\end{equation}
...
&= N_f E[\tilde{q}^*(T)]\bigr|_{t = 0} \Phi \left(\frac{\ln \left( \frac{E[P(T)]\bigr|_{t = 0}^{\eta}}{E[\tilde{C}^{\#}(T)]|_{t = 0}^{\eta}} \right) - \eta^2 \frac{s_{\tilde{c}}^2}{2} T}{\eta s_{\tilde{c}} \sqrt{T}} \right) \:\:;\:\:\: s_{\tilde{C}}^2 T = Var \left[ \ln \left( \frac{\tilde{C}^{\#}(T)}{\tilde{C}^{\#}(0)} \right) \right]
\end{split}
\end{equation}

Since the definition of $\lambda$ (Equation \ref{eq:lambda4}) generally implies $P(t) = \lambda / h \: C^{\#}(t)$, it is straightforward to show by Ito's lemma that $s_{\tilde{c}} = s_p$. Recalling, moreover, that $E[P(T)]\bigr|_{t = 0} = P(0) e^{m_p T}$, aggregate supply may again be rewritten

\begin{equation}
\begin{split}
E[Q(T)]\bigr|_{t = 0} &= N_f E[\tilde{q}^*(T)]\bigr|_{t = 0} \Phi \left(\frac{\ln \left( \frac{P(0)^{\eta}}{E[\tilde{C}^{\#}(T)]|_{t = 0}^{\eta}} \right) + \left( \eta m_p - \eta^2 \frac{s_p^2}{2} \right) T}{\eta s_p \sqrt{T} \right) \:\:;\:\:\: s_p^2 T = Var \left[ \ln \left( \frac{P(T)}{P(0)} \right) \right] \\
&= N_f E[\tilde{q}^*(T)]\bigr|_{t = 0} \Phi \left( -\frac{\ln \left( \frac{E[\tilde{C}^{\#}(T)]|_{t = 0}^{\eta}}{P(0)^{\eta}} \right) - \left( \eta m_p - \eta^2 \frac{s_p^2}{2} \right) T}{\eta s_p \sqrt{T} \right)
\end{split}
\end{equation}
...
\begin{equation}
E[Q(T)]\bigr|_{t = 0} = N_f E[\tilde{q}^*(T)]\bigr|_{t = 0} (1 - \Phi(z)) \:\:;\:\:\: z = \frac{\ln \left( \frac{E[\tilde{C}^{\#}(T)]|_{t = 0}^{\eta}}{P(0)^{\eta}} \right) - \left( \eta m_p - \eta^2 \frac{s_p^2}{2} \right) T}{\eta s_p \sqrt{T}
\label{eq:sup2}
\end{equation}

Now, note that $z$ is the standard score of the Log change $\ln(P(T)^{\eta} / P(0)^{\eta} )$ evaluated at $P(T) = E[\tilde{C}^{\#}(T)]\bigr|_{t = 0}$. Therefore, $\Phi(z)$ may be interpreted as the probability that the output price at planting time $P(0)$ will fall to the mean farm's expected cost per unit output $E[\tilde{C}^{\#}]\bigr|_{t = 0}$ by harvest and sale. That is,

\begin{equation}
\Phi(z) = \text{Prob}(P(T) \leq E[\tilde{C}^{\#}(T)]\bigr|_{t = 0})
\end{equation}

And $1 - \Phi(z)$ may be roughly interpreted as the probability that the output price will remain above this level.

\begin{equation}
1 - \Phi(z) = \text{Prob}(P(T) > E[\tilde{C}^{\#}(T)]\bigr|_{t = 0})
\end{equation}
...
\begin{equation}
\frac{\partial \ln(Q)}{\partial (s_p \sqrt{T})} = -\eta \frac{\phi(-z)}{\Phi(-z)} \left( \frac{1}{\eta^2 s_p^2 T} \ln \left( \frac{E[P(T)]\bigr|_{t = 0}}{E[\tilde{C}^{\#}(T)]\bigr|_{t = 0}} \right)  + \frac{1}{2} \right)
\end{equation}
