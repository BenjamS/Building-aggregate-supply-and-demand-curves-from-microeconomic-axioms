...
The Kenya farm data includes the harvest output price as observed at harvest $P(T)$. Expected harvest price at planting can be calculated based on axiom 5 if high resolution price series data is available. The FAO Food Price Monitoring and Analysis Tool (v4) is an increasingly useful source of monthly and weekly cereal wholesale price series at key domestic marketplaces around the world, including the El Doret and Nakuru maize markets in Kenya. However, data prior to 2020 is sparse, and prior to 2007 there are only 9 consecutive monthly data points in 2005. This is too low a resolution to estimate the price drift and volatility parameters $m_p$ and $s_p$ with confidence. Instead, 


long rains mar-may
Generally how it works: If short rains (oct-dec) are bad, then price increases from planting to harvest, if short rains good, then price stays flat or goes down a little. In years where price increases over the main cropping season, volatility is also higher. Farmers planting in March of 2008 had just finished a poor short season of less than expected rain in oct-dec, and thus expected the maize price to increase from its spot price $P(0)$ at planting.
...
# Compare mod3 with unit cost model
