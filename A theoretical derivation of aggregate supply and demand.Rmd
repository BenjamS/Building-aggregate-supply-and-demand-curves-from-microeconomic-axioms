...
In the Kenya maize farm exercise farther below, for example, there are four ($n_b = 4$) binary endowment variables $b_i$ (which take a value of $1$ if the endowment is present and $0$ otherwise): use of oxen in land preparation, use of tractor in land preparation, use of hybrid maize seed, and land tenure. This means that $g$ can vary across the population. In the Kenya maize exercise, there are 14 unique values of $g$ in the population, corresponding to 14 unique combinations of the four binary endowments plus the base endowment constant $\beta_0$.
...
Because $C^{\#}$ is lognormally distributed (axiom 4), then the unconditional parameters $m_{\#}$ and $s_{\#}$ can be deduced from the conditional parameters $\bar{m}_{\#}$ and $\bar{s}_{\#}$ by solving the following system of equations.
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- E[C^*] &= E[C^{\#}] E[y^*] = E[C^{\#} | C^{\#} \leq P] E[y^* | C^{\#} \leq P] \mathcal{E}^2 \frac{\mathcal{E}^2}{\Phi \left(\frac{\ln(P) - m_{\#}}{s_{\#}} - s_{\#}\right) \Phi \left(\frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#}\right)} -->
<!-- \end{split} -->
<!-- \label{eq:condMeanC1} -->
<!-- \end{equation} -->

### Binary endowment strata

As noted earlier (Equation \ref{eq:g0Sum}), binary endowments $\beta_i b_i$, and hence TFP, vary across the population. In the Kenya maize farm example explored farther below, there are 14 distinct values of expenditure TFP, corresponding to 14 distinct combinations of the binary endowments $\beta_i b_i$. The values corresponding to each distinct combination are henceforth indexed by $\ell$. The TFP and expenditure TFP of the $\ell^{th}$ binary endowment stratum, for example, are henceforth expressed $g_{\ell}$ and $v_{\ell}$. When  appears without an index, it refers to the average across all strata. For example, $v$ refers to
<!-- The supply aggregation step in Equation \ref{eq:aggSupStep} averages aggregate supply across all binary endowment strata. -->
...
Where $N_{\ell} / N$ is the fraction of the farm sample in the $\ell^{th}$ binary endowment stratum.

The expressions $E[y^*]$ and $E[C^*]$ likewise refer to the unconditional means evaluated at $v$. To disaggregate by binary endowment stratum, note that

\begin{equation}
E[C^*]_{\ell} = e^{m_{c \ell} + frac{s^2_c}{2}}
\end{equation}

Where $m_{c \ell}$ and $s_c$ follow from the expenditure expression in Equations \ref{eq:fnCsharp}.

\begin{equation}
\begin{split}
m_{c \ell} &= \frac{1}{1 - h}(\ln(v_{\ell}) + m_k + m_{\#}) \:\:;\:\:\: m_k = E[\ln(k)] \\
s_c^2 &= \frac{1}{(1 - h)^2} (s_k^2 + s_{\#}^2) \:\:;\:\:\: s_k^2 = Var[\ln(k)]
\end{split}
\label{eq:mCsC}
\end{equation}

Whence the stratified mean yield $E[y^*]_{\ell}$ follows.

\begin{equation}
\begin{split}
E[y^*]_{\ell} &= v_{\ell} E[k] E[C^{* h}] \\
&= v_{\ell} E[k] e^{h m_{c \ell} + h^2 \frac{s_c^2}{2}}
\end{split}
\end{equation}

To evaluate the stratified conditional (observed) means, replace $m_{\#}$, $s_{\#}$ in these expressions with $\bar{m}_{\#}$, $\bar{s}_{\#}$.

## The endogenous farm level response to an exogenous technology shock
...
### The endogenous farm yield and expenditure response to exgenous shock
...
The adoption rate within single a binary endowment stratum can be calculated by evaluating this expression at the $v_{\ell}$ and $m_{c \ell}$ of interest.
<!-- \begin{equation} -->
