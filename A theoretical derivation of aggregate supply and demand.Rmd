...
4) Farms enter production only if it increases their expected net benefit

5) Farm production is lognormally distributed
...
Henceforth it is necessary to be more specific in the treatment of yield $y^*$ and output price $P$. Absent a forward contract, the output price is a stochastic variable whose value at the time of harvest $(t = T)$ is unknown to the farmer at the time of planting and input expenditures $(t = 0)$. Likewise, yield is dependent upon numerous stochastic agroclimatic processes, and is unknown to the farmer at the time of planting and input expenditure decision $C^*$. Wherever $P$ and $y^*$ appear in the equations above, then, what we really mean to say is $E[P(T)]\bigr|_{t = 0}$ and $E[y^*(T)]\bigr|_{t = 0}$. That is, farmers make their input expenditure decisions at planting time ($t = 0$) based on what they expect the values of $y^*$ and $P$ to be at harvest ($t = T$).

The farm shadow price $\lambda$ is thus more accurately written

\begin{equation}
\lambda = \frac{E[P(T)]\bigr|_{t = 0} E[y^*(T)]\bigr|_{t = 0}}{C^*} h = \left[ \frac{P(T)]}{C^{\#}} \right]\Bigr|_{t = 0} h
\label{eq:lambda4}
\end{equation}
...
The stochastic behavior of prices often follows a geometric Brownian motion. That is to say, changes in $P$ over a small time interval $\Delta t$ ($\Delta P = P(t + \Delta t) - P(t)$) are often accurately modeled by

\begin{equation}
\Delta P = \Delta P(t) m_p \Detla t - P(t) s_p \epsilon \sqrt{\Delta t}
\end{equation}
...
Geometric Brownian motion is also known to approximate the time evolution of certain [atmospheric processes] weather variables such as temperature, solar radiation, and humidity, which in turn drive yield. Assuming yield ($y^*$) follows a geometric Brownian motion, then average cost ($C^{\#}$) must also, by definition, follow a geometric Brownian motion.

Introducing the notation $b(t)$ to stand for the ratio $P(T) / C^{\#}(T)$

, then the ratio inside the brackets must also follow a geometric Brownian motion. Denoting this ratio $b(t)$, the instantaneous arithmetic returns $\Delta b / b$ are thus normally distributed with mean and variance

\begin{equation}
\begin{split}
E \left[\frac{\Delta b}{b} \right] &= m_b \Delta t  = (m_p - m_y + \ln(C^*)) \Delta t \:\:;\:\:\: m_y = E \left[\frac{\Delta y^*}{y^*} \right] \\
Var \left[\frac{\Delta b}{b} \right] &= (s_p^2 + s_y^2)  \Delta t
\end{split}
\end{equation}
...
Assuming that farms do not enter production unless they expect to make a profit (Axiom 4), then, by Equation \ref{eq:lambda}, $\lambda > h$. This, in turn, implies the following upper bound on $y^*$.

\begin{equation}
E[y^*]\bigr|_{t = 0} < (g_0 \beta)^{\eta / h}  \left(\frac{P}{h} \right)^{\eta}
\label{eq:ydUpBound}
\end{equation}
