...
\begin{equation}
E[y^*(T)]\bigr|_{t = 0} = \hat{y}^*(\mathbf{k}_{ante}, C^*)|_{\mathbf{k}_{post} = \tilde{\mathbf{k}}_{post}}) e^{\epsilon}
\end{equation}

Where $\epsilon$ is the estimation error. Now, the sample mean of this does not represent $E[\tilde{y}^*(T)]\bigr|_{t = 0}$ but rather $E[\tilde{y}^*(T) | C^# < E[P(T)]\bigr|_{t = 0}]\bigr|_{t = 0}$. In order to arrive at the former, the farm cost per unit output is first calculated as follows.
...
Where $s_{\epsilon}^2$ is the variance of the estimation error and $h$ (implied in $\eta$) is the coefficient on $\ln(C^*)$.
