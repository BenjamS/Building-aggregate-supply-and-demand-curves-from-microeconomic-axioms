...
notThese <- c("Dist", "Yield (kg/acre)", "Cost/acre", bin_vars)
...
EthetaK <- exp(sum(c(mVec * kappaVec, kappaVec^2 * sVec^2 / 2)))
thetaKvec <- matK %*% kappaVec
EthetaKsamp <- thetaKvec %>% exp() %>% mean()
EthetaK/EthetaKsamp
#EthetaKsamp2 <- matK %*% diag(kappaVec) %>% colMeans() %>% sum() %>% exp()
h <- coefVec[which(coefNames == "Cost/acre")]
# Sample mean of Cost/acre very close to lognormal expectation
EcStarH <- exp(mean(h * dfMod3b$`Cost/acre`) + sd(h * dfMod3b$`Cost/acre`)^2 / 2)  
EcStarHSamp <- exp(h * dfMod3b$`Cost/acre`)  %>% mean()
EcStarH/EcStarHSamp
beta0 <- coefVec[1]
