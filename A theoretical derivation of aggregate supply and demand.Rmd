...
To begin with, yield is partly a function of stochastic meteorological variables unknown at planting time. Wherever $y^*$ appears in the equations above, what is really meant is $E[y^*(T)]|_{t = 0}$---that is to say, the farm's expectation of what yield will be at harvest time ($t = T$) as evaluated at planting time ($t = 0$). The yield equation can now be written more carefully as follows.

\begin{equation}
E[y^*(T)]|_{t = 0} = E[E[y^*(T)]|_{t = 0}] \frac{k}{E[k]} \frac{C^{*h}}{E[C^{*h}]}
\end{equation}

Note that optimal farm expenditure per acre $C^*$ remains a deterministic variable. This is because $C^*$ primarily refers to land preparation, seed, fertilizer and chemical pest control. It may also  harvesting, and some post harvest activities (shelling, stooking, transport, etc.). The most significant of these costs are known (executed or planned) at planting time; and so $C^*$ can be treated deterministically.
...
Substituting the stochastic terms developed above for $y^*$ and $P$, the expressions for yield and aggregate supply can now be rewritten more carefully as follows.




in the deterministic aggregate supply curve (Equation \ref{eq:aggSup1}), gives the _expected_ aggregate supply curve.
...
Equation \ref{eq:condMeanYd}, relating the observed (conditional) mean and unconditional mean yield, must also be rewritten more carefully as follows.

\begin{equation}
\begin{split}
E[E[y^*(T)]|_{t = 0} | E[C^{\#}(T)]|_{t = 0} < E[P(T)]|_{t = 0}] &= \frac{E[E[y^*(T)]|_{t = 0}]} \frac{\Phi \left( \frac{\ln(P) - m_{\#}}{s_{\#}} - \eta s_{\#} \right)}{\Phi\left( \frac{\ln(P) - m_{\#}}{s_{\#}} \right)} \\
&= \frac{E[y^*]} \frac{\text{PEFS-adj. mean prob. of profit}}{\text{Farm employment rate}}
\end{split}
\end{equation}

This can be further refined into a more instructive expression by considering the stochastic character of optimal unit cost $C^{\#}$. Farm cost per acre is deterministic, but optimal unit expenditure $C^{\#}$, on the other hand, must be treated as a stochastic variable. To see this, note that Equation \ref{eq:cStar} simplifies to
...
When switching from population mean to stochastic mean, the variance likewise switches from a population variance to a stochastic variance. Most importantly for what follows, this means replacing $Var[\ln(E[C^{\#}(T)]|_{t = 0})]$ (i.e. $s^2_{\#}$) with $Var[\ln(\tilde{C}^{\#}(T))]|_{t = 0}$. 
...
And this is true for any value of $\lambda^{-1}$, including $\lambda^{-1} = E[\lambda^{-1}]$. Thus, when moving from farm population space to stochastic price space, $s^2_{\#}$ becomes $s^2_p T$.
...
Substituting the dual expressions for $m_{\#}$ and $s^2_{\#}$ in the $\Phi()$ term of Equation \ref{eq:aggSup2}, and substituting $P(0)e^{m_p T}$ for $E[P(T)]|_{t = 0}$ (via Equation \ref{eq:EPT0}), resolves the $\Phi()$ term into an economically meaningful expression.
...
&\rightarrow 1 - \Phi(z) = \text{Prob}(P(T)^{\eta} \geq E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0})
\end{split}
\label{eq:probProfit}
\end{equation}

By taking account of price stochasticity, then, the $\Phi()$ term in the aggregate supply function (Equation \ref{eq:aggSup2}) may be interpreted as the probability that the harvest output price (raised to $\eta$) will be greater than the mean farm's expected unit cost (raised to $\eta$). Because the adjustment $\eta$ is the output price elasticity of farm supply (PEFS), the probability term $1 - \Phi(z)$ is henceforth referred to as the PEFS-adjusted mean probability of profit.

Substituting this for the $\Phi()$ term in Equation \ref{eq:aggSup2}, the aggregate supply function may be rewritten

\begin{equation}
E[Q(T)]|_{t = 0} = N E[k_a] E[y^*(T)]|_{t = 0} (1 - \Phi(z)) \:\:;\:\:\: z = \frac{\ln \left(\frac{E[\tilde{C}^{\#}(T)^{\eta}]|_{t = 0}}{P(0)} \right) - \left( m_p - \eta \frac{s^2_p}{2} \right) T}{s_p \sqrt{T}}
\label{eq:aggSup3}
\end{equation}
...
The main purpose of developing the stochastic price parameter formulation of aggregate supply above is to demonstrate that the $\Phi()$ term in aggregate supply has a meaningful economic interpretation as the PEFS-adjusted mean probability of profit. For purposes of evaluating aggregate supply, the most appropriate formulation to work with is that for which the highest resolution data is available. When farm data is available, evaluation via the farm population parameter formulation in Equation \ref{eq:aggSup2} is probably the best approach. In the absence of farm data, the stochastic price parameter formulation presents a second best approach if high resolution price series data is available.
...
In the absence of both farm and price data, back of the envelope calculations can be made based on the evaluator's sense of mean observed yield, the farm employment rate, and the PEFS adjusted mean probability of profit. That is to say, Equation 


Whereby the evaluator may deduce the unconditional mean $E[]$  That is to say, the unconditional mean yield can be deduced from the estimate of observed (conditional) mean yield via .


Back of the envelope calculations in minimum data contexts since policy makers generally have or can reasonably formulate a rough idea of employment rate and probability of profit on the mean farm.
...
Price elasticity of aggregate supply and other useful derivatives
