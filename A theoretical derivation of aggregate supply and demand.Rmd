...
The stochastic movement of $y^*(t)$ is slightly less straightforward. Yield is a function of both deterministic and stochastic (mostly meteorological) variables. Insofar as the stochastic movement of meteorological variables follows a geometric Brownian motion (as in Figure \ref{fig:gBmWeather}), then it can be shown by Ito's lemma that $y^*(t)$ must also follow a geometric Brownian motion.

Because the cost per unit output $C^{\#}$ is defined as a function of $y^*(t)$, then---again by Ito's lemma---it too must follow a geometric Brownian motion. Denoting the mean and variance of the normally distributed changes $\Delta y^* / y^*$ as $m_y \Delta t$ and $s_y^2 \Delta t$, respectively, then Ito's lemma also reveals that the log changes of $C^{\#}$ have variance $s_c^2$ equal to $s_y^2$. Moreover, Equation \ref{eq:lambda3} implies $s_c^2 = s_p^2$; and thus, by transitivity, $s_y^2 = s_p^2$.
