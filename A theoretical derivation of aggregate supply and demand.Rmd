...
xx <- a0 + as.matrix(df_mod2[, bin_vars]) %*% kBinVec
tfp <- exp(xx)
kNames <- c("Cstar", "acres", "seed (kg/acre)",
                 "planting fam lab child (pers-hrs/acre)",
                 demog_vars, clim_vars)
mKvars <- colMeans(df_mod2[, kNames])
kVec <- mod2Coefs[which(coefsNames %in% kNames)]
y0 <- tfp * prod(mKvars^kVec) # yield evaluated at the sample mean
kMat <- df_mod2[, kNames] - mKvars
lkMat <- log(kMat)
lBetaK <- lkMat %*% kVec
#---
# test
log(y0) + lBetaK - mod2$fitted.values
betaK <- exp(lBetaK)
y0 * betaK - exp(mod2$fitted.values)
#---
cStar <- exp(df_mod2$Cstar)
# betaKadj <- betaK * (cStar / hCoef)^-hCoef
# y0 * betaKadj * (cStar / hCoef)^hCoef - exp(mod2$fitted.values)
kRain <- kVec[which(kNames == "Rain anomaly")]
lkRn <- as.matrix(df_mod2[, "Rain anomaly"])
betaRn <- exp(lkRn * kRain)
xRain <- exp(lkRn)
betaKadj <- betaK * (cStar / hCoef)^-hCoef * xRain^-kRain
#---
# test
y0 * betaKadj * xRain^kRain * (cStar / hCoef)^hCoef - exp(mod2$fitted.values)
#----------------------------------------------------------------------
mean(log(betaK))
...
#ERstar <- df_mod2$P * EyStar
ERstar <- EyStar * EPT02007
cStar <- exp(df_mod2$Cstar)
lam <- ERstar / cStar * hCoef
EcSharp <- cStar / EyStar
EyStarUp <- g0 * betaK * (ERstar / hCoef)^hCoef * exp(sErr^2 / 2)
#----------------------------------------------------------------------
...
# sum(EyStar < EyStarUp) / length(EyStarUp)
#----------------------------------------------------------------------
...
exp(mLam - sLam^2)
muEtaLamInv <- exp(-etaCoef * mLam + etaCoef^2 * sLam^2 / 2)
#---------------------------------------------------------------------
...
sBetaK <- sd(log(betaK))
muBetaKetaH <- exp(etaCoef / hCoef * mBetaK + (etaCoef / hCoef)^2 * sBetaK^2 / 2)
