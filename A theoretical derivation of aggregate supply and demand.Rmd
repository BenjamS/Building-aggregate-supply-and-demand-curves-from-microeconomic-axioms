...
The percentage changes in farm level expenditure TFP ($v_0$), expenditure per acre, and yield given a 1 percentage change in $g_0$ follow from Equations \ref{eq:v0forms}, \ref{eq:mCk}, and
\ref{eq:yStarT}, respectively.
...
Now, because the binary endowments $b_i$ vary across the population, the value of $v_0$ also varies. In the Kenya example, there are 14 distinct values of $v_0$, corresponding to 14 distinct combinations of the $\beta_i b_i$. The supply aggregation step in Equation \ref{eq:aggSupStep} averages aggregate supply across all binary endowment strata. Denoting the value of each distinct combination as $v_{0i}$, the average $v_0$ is effectively a population weighted sum of the $v_{0i}$.
...
Where $N_i / N$ is the fraction of farms in the $i^{th}$ binary endowment stratum. The conditional mean yield and cost disaggregated by stratum ($E[C^*| \theta]_i$, $E[y^*(T)| \theta]_i$), can be evaluated as follows.
...
\begin{equation}
\frac{\partial \ln(E[Q(T)]|_{t = 0})}{\partial \ln(g_0)} = \frac{\partial \ln E[y^*(T; k_m(T))]|_{t = 0}}{\partial \ln(g_0)}
\end{equation}

The change given adoption rate $A$ between $0-100$% is

\begin{equation}
\frac{\partial \ln(E[Q(T)]|_{t = 0})}{\partial \ln(g_0)} = A \frac{\partial \ln E[y^*(T; k_m(T))]|_{t = 0}}{\partial \ln(g_0)} = \frac{1}{1 - h} \left( 1 - \boldsymbol{\alpha} ' \frac{\partial \ln(\mathbf{w})}{\partial \ln(g_0)} \right)
\end{equation}



, given a new yield enhancing binary endowment $\beta_5$ (representing, for example, improved seed and practices), the percentage change in TFP on farms that implement the new technology may be calculated as
...
dfAdopt$dlnNu <- round(dlnNuVec, 2) #paste("dlnNu =", round(dlnNuVec, 2))
dfPlot <- dfAdopt %>% gather_("dB", "adoptRate", gatherCols)
dfPlot$dB <- as.numeric(dfPlot$dB)
gg <- ggplot(dfPlot, aes(x = dlnNu, y = dB, z = adoptRate, fill = adoptRate, group = 1))
