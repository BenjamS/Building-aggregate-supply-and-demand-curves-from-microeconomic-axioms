...
\begin{equation}
y^* = v_0 k C^{*h} \:\:;\:\:\: v_0 = \frac{E[y^*]}{E[k] E[C^{*h}]}e^{\epsilon}
\label{eq:YdEnvReg1}
\end{equation}
...
Where $\Phi()$ is the standard normal cumulative distribution function. (See Appendix A.2 for a proof.)
...
EQsT2009Aug11MoA <- 1.84 # Minister of Ag press release, Severe drought, crop failure
...
\singlespacing
```{r Fig2, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="center", fig.cap="\\label{fig:mzP}Maize monthly price series during main season (March-August) at primary markets in Kenya. Price data are from the World Food Program.", echo = FALSE}
#======================================================================
...
gg <- gg + labs(x = "Maize price (KES / kg.)")
...
There are two ways to introduce a research-induced supply curve shift. A new technology or management practice that scales yield can be introduced by shocking the unconditional mean expected yield. Recall from Equation \ref{eq:muYstarU0} that the unconditional mean expected yield can be defined (in stochastic terms),
...
\begin{equation}
\mathcal{L}^* = R^* + \lambda (B - C^*) \:\:;\:\:\: R^* = P y^*
\end{equation}

By Equation \ref{eq:cStar}, this reduces to

\begin{equation}
\mathcal{L}^* = R^* (1 - h) + \lambda B \:\:;\:\:\: R^* = P y^*
\end{equation}
...
The term $\partial B / \partial u_0$ represents any subsidies included in the technology package to encourage adoption. In the absence of subsidies, the adoption condition simplifies to

\begin{equation}
\frac{\partial \ln(y^*)}{\partial u_0} > -\frac{\partial \ln(P)}{\partial u_0}
\end{equation}
