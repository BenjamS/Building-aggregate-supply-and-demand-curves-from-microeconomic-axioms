...
dfPoints <- data.frame(`Estimate date` = reportDate,
                       ...
                       Reference = citeVec,
                       Comments = commentVec)
colnames(dfPoints) <- c("Estimate date", "Estimate\n(million MT)", "Reference", "Comments")
...
ft <- flextable(dfPoints) %>% set_caption("Table 1: Empirical estimates of main season maize harvest 2006-2012") %>% autofit()
ft
...
modelsummary(mod3, output = "latex", statistic = "p.value",
             estimate = "{estimate}{stars}", title = "Table 2: Farm yield regression results")#, coef_map = coefOrder)
#output = "markdown"
#output = "kableExtra"
#ouput="flextable"
...
The two remaining parameters required are number of farms $N_f$ and mean farm size $k_a$. For mean farm size, the Kenya farm sample mean is used, giving $k_a =$ `r muArea` acres. The number of maize farms is more difficult to assess. One sometimes sees the figure $3$ million mentioned [e.g. @njeru2022maize], but this appears to be unsupported by any hard evidence. Here, a rough estimate is deduced by dividing planted area by mean farm size. The midseason reports examined for this exercise suggest that planted maize area during the period in question ranges between 3 and 4.5 million acres [@KFSSGJul2011]. Divided by $k_a$, this gives a range between $1.56$ and $2.34$ million farms. The figure $N_f = 2$ million is used as a rough estimate.
...
The expected aggregate supply and price elasticity curves parameterized in this exercise are "short run" curves. That is to say, they are valid only insofar as the values of their fitted parameters---$h$, $N_f$, $k_a$, $k$, $m_{\#}$, $s_{\#}$, $m_p$---are accurate. Empirical estimates for years well before or well after the year of the data used to estimate the parameters will thus diverge from the curve.
...
gg <- gg + geom_point(data = dfPoints, aes(x = EPT0, y = `Estimate\n(million MT)`)) + ggrepel::geom_text_repel(data = dfPoints, aes(x = EPT0, y = `Estimate\n(million MT)`, label = `Estimate date`), size = 3, lineheight = 0.75)
gg <- gg + theme_bw()
...
The data report land preparation type (manual, oxen, tractor, or none) and land tenure type (owned by parent/relative, owned with title deed, owned without title deed, rented) at field level. Dummy variables are created for land preparation types "oxen" and "tractor", and for the land tenure type "owned with title deed".


The seed data includes details on seed type and method of acquisition. For this article, farms are considered adopters of hybrid maize seed if they use either purchased hybrid seed or a mixture of retained and purchased hybrid maize seed.
