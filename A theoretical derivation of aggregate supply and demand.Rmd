...
# date: "`r Sys.Date()`"
abstract: |
  Conventional models of the aggregate supply of agricultural commodities (particularly those used in partial equilibrium analysis) are not explicitly or formally derived from standard microeconomic axioms. Nor does the specification of such models take any explicit account of the shape of the production distribution, or, worse still, assumes that all farms in the population are identical. Such models are then, by construction, theoretically opaque, with unclear mappings between model parameters and relevant empirical phenomena, and no accounting of farm entry/exit in response to price changes. For use cases where theoretical and empirical transparency matter, here a tractable aggregate supply function is derived from 3 non-controversial microeconomic axioms, plus the assumption of lognormally distributed yields and farm size, and the assumption that output price follows a geometric Brownian motion. The resulting model expresses aggregate supply as a function of measurable technology, price, and control variables, and includes a term that accounts for farm entry/exit. The farm/entry exit term turns out to be highly instructive and intuitive, as it is proportionate to the probability that the mean farm will make a profit at harvest. The model is estimated on Kenya farm data. Calculations of supply and price elasticity of supply agree with empirical estimates. Other potential use cases are discussed, including usage in partial equilibrium models and research induced supply curve shifts.
