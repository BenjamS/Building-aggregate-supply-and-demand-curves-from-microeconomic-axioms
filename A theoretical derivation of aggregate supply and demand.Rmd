...
To calculate $m_c$ and $s_c$, note that Equations \ref{eq:paramsCondCsharp} can be rearranged into expressions for $\bar{m}_c$ and $\bar{s}_c$, whence the expression for the unconditional parameters follows immediately.
<!-- can be calculated recall from Equation \ref{eq:mCk} that $C^* = (v_0 k E[C^{\#}(T)]|_{t = 0})^{1 / (1 - h)}$, such that -->
...
Where $N_i / N$ is the fraction of farms in the $i^{th}$ binary endowment stratum. The conditional or unconditional mean yield and cost disaggregated by stratum ($E[C_i^*]$, $E[y_i^*(T)]$), can be evaluated as follows.

\begin{equation}
\begin{split}
E[C_i^*| \theta] &= e^{\bar{m}_{ci} + \frac{\bar{s}^2_c}{2}} \:\:;\:\:\: \bar{m}_{ci} = \frac{1}{1 - h}(\ln(v_{0i}) + m_k + \bar{m}_{\#})  \\
...
gg <- ggplot(dfPlot, aes(x = dB, y = dlnNu, z = adoptRate, fill = adoptRate))
