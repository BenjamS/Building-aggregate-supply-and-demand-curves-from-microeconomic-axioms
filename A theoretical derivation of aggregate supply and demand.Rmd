...
[comment here about how this differs from textbook concept of MC-AC relation?]

Moreover, combining Equations \ref{eq:lambda3} and \ref{eq:YdEnvCompact} gives yet another expression for $y^*$ which proves useful during parameterization of the aggregate supply curve farther below.
...
The expected yield at planting time for a given farm $E[y^*(T)]\bigr|_{t = 0}$ can be assessed by estimating the model in Equation \ref{eq:cobbDoug4} on the farm survey data, and then, for each farm, taking the expected value of the fitted model $\hat{y}^*$ evaluated at the observed values of the control variables, with the exception of weather variables (and any other control variables whose values are known only post harvest), which should be set to their expected values ($\tilde{k}_i$). That is, for each farm,

\begin{equation}
E[y^*(T)]\bigr|_{t = 0} = \hat{y}^*(\mathbf{k}_{ante}, C^*)|_{\mathbf{k}_{post} = \tilde{\mathbf{k}}_{post}}) e^{\frac{s_{\epsilon}^2}{2}}
\end{equation}

Where $s_{\epsilon}^2$ is the variance of the estimation error. Now, the sample mean of this does not represent $E[\tilde{y}^*(T)]\bigr|_{t = 0}$ but rather $E[\tilde{y}^*(T) | C^# < E[P(T)]\bigr|_{t = 0}]\bigr|_{t = 0}$. In order to arrive at the former, the farm cost per unit output is first calculated as follows.

\begin{equation}
C^{\#} = frac{C^*}{E[y^*(T)]\bigr|_{t = 0}}
\end{equation}

The sample mean of this represents the mean taken over the truncated distribution, $E[C^{\#} | C^{\#} < E[P(T)]\bigr|_{t = 0}]$. Given the assumption of lognormally distributed yield (Axiom 4), then, by linear independence of the logged regressors in Equation \ref{eq:cobbDoug4}, $C^{\#}$ is also lognormally distributed, such that the unobserved mean and variance of the untruncated distribution of $\ln(C^{\#})$ ($m_c$, $s_c^2$) can be implicitly calculated from the observed (i.e. sample) mean and variance ($\overline{m}_c$, $\overline{s}_c^2$) by the following formulae.
...
With $m_c$ and $s_c$ in hand, the mean cost per unit output over the untruncated population then follows as $E[C^{\#}] = e^{m_c + s_c^2/2}$. Because Equation \ref{eq:yStarCsharp} is also a model for $y^*$, the mean expected yield taken over the untruncated distribution then follows as the mean over the truncated distribution (the sample mean) multiplied by a correction term.

\begin{equation}
E[\tilde{y}^*(T)]\bigr|_{t = 0} = E[\tilde{y}^*(T) | C^# < E[P(T)]\bigr|_{t = 0}]\bigr|_{t = 0} \left( \frac{E[C^{\#}]}{E[C^{\#} | C^{\#} < E[P(T)]\bigr|_{t = 0}]} \right)^{\eta}
\end{equation}

Where $h$ (implied in $\eta$) is the coefficient on $\ln(C^*)$ when $\hat{y}^*$ is estimated.





If the available farm survey data includes control variables ($k_i$), then, with the untruncated $C^{\#}$ in hand, ... such that the model in Equation \ref{eq:cobbDoug4} can be estimated (making sure to use the untruncated mean of $C^{\#}$), then the mean yield at harvest $\tilde{y}(T)$ can be calculated directly from the estimated regression constant.



The output price $P(T)$ received by farmers after harvest might be reported in the farm survey. However, the model requires the price at planting time $P(0)$ as well as $m_p$ and $s_p$, which are generally not reported in farm surveys. In countries with a well developed forward contracts market, such information is available. Elsewhere, it is becoming increasingly available via services like the FAO Food Price Monitoring and Analysis Tool. The question of estimating over how much time...one season? the entire previous year? a running 2 or 3 year average of the one season values? 
<!-- , required to calculate the farmers' expected price (recall Equation \ref{eq:EPT0}) -->




Ideally, $C_0^{\#}$ and $h$ should be derived from detailed farm survey data. Calculation of $C_0^{\#}$ requires only farm cost and yield data. The value of $h$, on the other hand, must be estimated by regressing logged cost _and_ control variables on logged yield (recall Equation \ref{eq:cobbDoug4}). The estimated coefficient on logged cost may then be interpreted as $h$.

Accurate price parameters may be difficult to come by in regions with poorly developed farm-to-market linkages due to an entrenched intermediary market which thrives precisely on buyer-seller information asymmetry. An accurate value for $N_f$ will likewise be difficult to estimate in populations where no recent farm census data is available. Note that $N_f$ is not necessary for evaluation of the price elasticity of aggregate supply and price risk elasticity of aggregate supply (Equations \ref{eq:pElasQagg} and \ref{eq:pRiskElasQagg}).




The aggregate supply curve derived from Axioms 1-5 above is parameterized based on Kenya farm data. First, the farm level model in Equation \ref{eq:yStarC} is estimated using this data. Then, the resulting values for $h$, $\eta$, and $E[y^*(T)]\bigr|_{t = 0}$ are input into the aggregate model...   This sort of a validation but not so much a validation...
