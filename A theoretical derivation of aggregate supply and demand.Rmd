...
\begin{equation}
\frac{\partial \ln(C^*)}{\partial \ln(g_0)} = \frac{\partial \ln(y^*(T))}{\partial \ln(g_0)} = \frac{1}{1 - h}
\end{equation}
...
\begin{equation}
\begin{split}
A &= \int_0^{\bar{\bar{C^*}}} \phi \left(\frac{\ln(C^*) - m_c}{s_c} \right) d \ln(C^*) \:\:;\:\:\: \bar{\bar{C^*}} = \frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} \\
...
\begin{equation}
E[\hat{y}^*(T)]_i = v_{0i} E[k] E[C^{*h}]_i
\end{equation}
<!-- E[e^{\epsilon}]_i -->
...
Because the adoption rate is partly a function of $v_0$, and because $v_0$ varies across strata, then the adoption rate must also vary across strata. Equation \ref{eq:adoptRate} indicates that the adoption rate will be higher in strata with lower starting $v_0$, _ceteris paribus_. The adoption rate for the whole population is then

\begin{equation}
A = \sum_i \frac{N_i}{N} A_i
\end{equation}
...
\begin{equation}
\begin{split}
A_i &= \Phi\left( \frac{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} - m_{ci}}{s_c} \\
...
The total increase in aggregate supply for each stratum can be evaluated by integrating the change in farm production over the density of adopting farms $(0, \bar{\bar{y^*}})$ where $\bar{\bar{y^*}} = y^*(\bar{\bar{C^*}})$.
...
\begin{equation}
\begin{split}
\Delta Q(v_{0i}) &= N_i \int_0^{\bar{\bar{y^*}}} k_a \partial \y^* \rho(y^*) dy^* \\
...
&= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) v_{0i} E[k] E[C^{*h}] \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_c}{s_c} - h s_c  \right)
\end{split}
\end{equation}

Where the last line follows from Equation .... Recalling that $v_{0i} - E[y^*]_i / (E[k] E[C^{*h}]_i)$, this further reduces to

\begin{equation}
\begin{split}
\Delta Q(v_{0i}) &= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) E[y^*]_i \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right)
\end{split}
\end{equation}

Recall that from ... that the $\Phi$ term reduces to

\begin{equation}
\begin{split}
\Delta Q(v_{0i}) &= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) E[y^*]_i \Phi \left( \frac{\ln(\bar{\bar{C^*}}) - m_{ci}}{s_c} - h s_c  \right) \\
&= N_i E[k_a] \frac{1}{1 - h}(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(mathbf{w})) E[\tilde{y}^*(T, k_m(T))]_i (1 - \Phi ( \zeta ) \:\:;\:\:\: \zeta = \frac{\ln \left( \frac{E[\tilde{C}^{\#}(T)]|_{t = 0}}{\bar{\bar{C^{\#}(0)}}} \right) - \left( m_p - \eta \frac{s_p^2}{2} \right) T}{s_p \sqrt{T}}
\end{split}
\end{equation}

The term $1 - \Phi(\zeta)$ may be interpreted as the PEFS-adjusted mean probability that, by harvest, adoption will prove to be more profitable than not adopting. For purposes of evaluation given farm data, the Phi term better evaluated as...

The total change in aggregate welfare can be evaluated by integrating the change in farm level welfare $\partial \mathcal{L}(C^*)$ over the density of adopting farms.

\begin{equation}
\partial \mathcal{L}(C^*) =  \frac{P}{C^{\#}} (\partial \ln(\nu) + \partial \ln(g_0)) ( C^* + \bar{\bar{C^*}} )
\end{equation}

\begin{equation}
\begin{split}
\psi(v_{0i}) &= \int_0^{\bar{\bar{C^*}}} \partial \mathcal{L}(C^*) \rho(C^*) d C^* \\
&= N_i \frac{P}{C^{\#}} (\partial \ln(\nu) + \partial \ln(g_0)) \int_0^{\bar{\bar{C^*}}} ( C^* + \bar{\bar{C^*}} ) \rho(C^*) d C^* \\
&= 
\end{split}
\end{equation}
...
\begin{equation}
\frac{E[C^{\#}]}{\bar{\bar{C^{\#}}}} = \left(\frac{-h \partial B}{\partial \nu + \partial g_0}\right)^{h - 1} E[C^{*1 - h}]_i
\end{equation}



\begin{equation}
\partial \ln(E[Q(T)]|_{t = 0}) = N E[k_a] \int_0^{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)}} \partial \ln(C^*) \rho(C^*) d C^* = N E[k_a] \frac{A}{1 - h} \left(\partial \ln(g_0) - \boldsymbol{\alpha} ' \partial \ln(\mathbf{w}))
\end{equation}

The new aggregate supply curve ($E[Q(T)]|_{t = 0}'$) can then be evaluated as

\begin{equation}
E[Q(T)]|_{t = 0}' = E[Q(T)]|_{t = 0} (1 + \partial \ln(E[Q(T)]|_{t = 0}))
\end{equation}

In Appendix ..., it is shown that the total welfare impact ($\psi$) of this shift can be evaluated by integrating the normalized change in net benefit over the domain of adopting farms.
<!-- $\partial \mathcal{L}^* / \lambda$ -->

\begin{equation}
\begin{split}
\psi &= \int_0^{\frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)}} \left( \frac{C^*}{h} (\partial \ln(\nu) + \partial \ln(g_0)) + \partial B \right) \rho(C^*) d C^* \\
&= (\partial \ln(\nu) + \partial \ln(g_0)) \frac{E[C^*]}{h} \Phi \left( \frac{-h \partial B}{\partial \ln(\nu) + \partial \ln(g_0)} - m_c}{s_c} - s_c \right) + A \partial B
\end{split}
\end{equation}
...
The adoption rate for the hypothetical new technology described above ($\beta_5 = 0.3$), assuming a 40% decrease in the $\alpha_i$-adjusted output-input price ratio, with a marginal net subsidy per acre of $\partial B = 2.5$, is presented in Figure \ref{fig:v0strata} alongside the mean yield, cost, and TFP ($v_0$), disaggregated by stratum.
...
muYstarTi <- v0strat * Ek * muCstarih
...
dBvec <- seq(600, 3600, length.out = nRes)
...
adoptRateStrat <- pnorm(z)
matAdopt[i, j] <- round(sum(adoptRateStrat * stratPct), 2) * 100
}
}
...
dfx <- dfPlot
dfx$adoptRate <- round(dfx$adoptRate, 2)
#===========================================================================
# Supply curve shift
dlNu <- -0.4
dlEPT0 <-  -0.15
