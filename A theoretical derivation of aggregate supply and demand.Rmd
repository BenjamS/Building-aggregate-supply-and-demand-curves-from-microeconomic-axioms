...
mECsharpT0trunc <- mean(df_mod$Cstar - df_mod$`yield (kg/acre)`)
sECsharpT0trunc <- sd(df_mod$Cstar - df_mod$`yield (kg/acre)`)
EPT02007 <- 13
...
#yStar <- exp(df_mod1$`yield (kg/acre)`)
hist(log(EyStar / exp(df_mod1$`yield (kg/acre)`)), breaks = 15) #Nice
...
P0vec <- seq(0.1, 25, length.out = 40)
Nf <- 2.6 * 10^6
th <- 6
P02007 <- 8
#---------
...
h <- hCoef
  ...
  muEqStarT0 <- g0^(eta / hCoef) * muEtahBetaQs * muEtaLamInv * EPT0vec
