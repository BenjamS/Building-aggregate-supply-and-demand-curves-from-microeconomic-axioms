...
The conventional linear and constant elasticity of supply (CES) aggregate supply curves commonly used in ex-ante impact assessments of new agricultural technology [@alston1995science; @alston2009economics] are not explicitly derived from microeconomic axioms, but rather specified primarily for the purpose of facilitating certain calculations---especially of consumer and producer surplus---with little consideration given to the plausibility of the theoretical underpinnings and empirical implications of this choice of functional form. In particular, it is unclear whether these forms are consistent with standard microeconomic axioms regarding homogeneity, convexity/concavity, profit maximization, and the slope of farm level supply and input demand curves. (The model documentation might make passing reference to such axioms; but the models themselves are not formally derived from them.)
...
The subsequent step of translating the exogenously introduced farm level response into an aggregate supply response likewise involves no endogenous modeling of economic behavior, but is rather a set of exogenous guesswork translated into mathematical symbols. That is to say, after a long and inconclusive debate in the 1970s over whether the aggregate supply response should be represented as a pivot or parallel shift of the curve [for a survey, see @alston1995science; @norton1981evaluating; @lindner1978supply], or some combination thereof, the current predominance of parallel shift in most impact assessments emerges not on any theoretical or empirical basis, but purely as a matter of fiat [@alston2009economics].
...
For those interested in endogenous modeling of the economic supply response to exogenous shocks, here an aggregate supply curve function is derived from the following five axioms.
...
The resulting aggregate supply curve includes a term that explicitly captures farm entry/exit in response to price movements and risk, effectively accounting for the uneven distribution of endowments across inframarginal farms. The entry/exit term itself is surprisingly instructive and intuitive, as it is proportionate to the mean farm's probability of profit. All of the parameters appearing in the model have a straightforward interpretation in terms of relevant, measurable, empirical phenomena. This includes, in addition to the usual variables (technology, agroclimatic variables, price), output price volatility and length of cropping season, widening the range of policy/technology questions that can be explored.
...
When pressed on the matter, proponents of the conventional aggregate supply functions make little attempt to defend their specifications on empirical or theoretical grounds. Advocates of a particular form might offer reasons as to why their preferred specification is superior compared to the alternatives (e.g. Voon and Edwards' recommendation of the CES specification over the linear specification [-@voon1991calculation]); but no proponent of either form has offered anything like a formal derivation of their model on theoretical and/or empirical grounds. On the contrary, everyone involved in this literature acknowledges that the functional forms are specified primarily to facilitate calculations of consumer and producer surplus [@lindner1978supply; @alston1995science; @rose1980supplys]. Proponents of linear models readily admit that real world commodity supply and demand curves are almost certainly never linear [@brester2023equilibrium]. Rather, the defense of such models centers on the argument that the functional form is irrelevant for purposes of impact assessment.
...
They also find that impact assessment is insensitive to misspecification error when 1) the linear specification is assumed together with 2) a parallel shift, and assuming 3) the shift is small [@zhao1997functional]. Proponents and critics alike acknowledge that the empirical and theoretical grounds for assumptions 1 and 2 are narrow, at best [@lindner1978supply; @alston2009economic; @voon1991calculation; @rose1980supplys]. If the real shift is a pivot or mixture of pivot and shift, then Zhou et al.'s finding of insensitivity does not apply. Moreover, it is questionable whether the parallel supply curve shifts assumed in most economic surplus assessments of returns to agricultural R&D qualify as "small" [@hurley2014re; @alston2000research]. To know for sure, Zhao et al. [-@zhao1997functional] develop a method to calculate the upper bound on misspecification bias; but the present literature review finds no instance where this method has actually been implemented. Finally, Zhou et al. [-@zhao1997functional] acknowledge that their finding of insensitivity is valid only if the farm population consists entirely of identical marginal farms, a condition never actually observed in farm populations.

In synthesis, then, it is misleading to characterize the cited literature as supportive of the broad claim that impact assessments are insensitive to misspecification of the functional form of the supply curve. More accurately, the literature supports the much more limited claim that impact assessments are insensitive to misspecification when the linear specification is assumed together with a parallel shift, and when the real shift can also plausibly be assumed to be parallel; and when the shift is small, and when the farm population consists of identical marginal farms. Apart from this exceptional case identified by Zhao et al. [-@zhao1997functional], the literature indicates that impact assessments are highly sensitive to functional form, whether directly, as in the case of the CES function, or by the transitive property, i.e., through a supply curve shift which is mathematically prefigured by the functional form of the supply curve.

## Relevance to food, equality-efficiency, and price risk policy questions

[@norton1992economic; @alston1995science]
However, such a caricature is at odds with the CGIAR's long history of constructive response to criticism. This is especially true since the 2011 reforms, when mandatory accountability protocols were implemented [@leeuwis2018reforming]; but was also true prior. Plan Puebla, IR20 rice, the Bean and Cassava Programs, to name only a few examples, were all in some measure born of criticism in order to better align research objectives with the agroclimatic and socioeconomic needs of resource-poor smallholders.
[@byerlee2000targeting;@byerlee2009agriculture]
palliative measures  [@alwang2003measuring] have not been widely adopted.

Compounded...little confidence to any results regarding the distribution of net benefits amond producers and consumers.

Calculations of net benefits and their distribution based on this approach can vary considerably with the choice of functional form and type of supply curve shift. With no clear theoretical or empirical basis upon which to recommend one configuration over another, the meaning 

Because there are no theoretical 

Food policy decisions require cogent analysis of consumer-producer tradeoffs (the "food price dilemma"), as well as the distributional effects of exogenous shock across income levels. Increasingly, issues of rural-urban migration are also the impact of price volatility.  Economic surplus studies based on conventional supply curves and supply curve shifts are mathematical descriptions of an agronomic assertion,    Proponents and critics alike acknowledge that impact assessments based on the conventional aggregate supply curves can offer no such analysis (details in Literature Review).

Food policymakers require partial equilibrium modeling of the supply response to    that can provide clear insight into the food price dilemmas   ...mathematical description of an agronomic assertion. 

descriptive math of a subjective view assertion. these math descriptions are of little use, what is needed is a better understanding of supply curves and the impact on marginal costs [after Rose].

It is worth mentioning, moreover, that impact assessments vary considerably with the choice of functional form and type of supply curve shift. The   With no clear theoretical or empirical consensus upon which to argue in favor of one specification over another, 

whether same effect can be achieved through other policy instruments
food price dilemma [].
entry/exit and yield components [].
price risk

The magnitude and distribution of net benefits among consumers and producers are critically dependent upon the choice of functional form, and neither empirical nor theoretical arguments compel towards a particular model and type of shift [@alston2000research]. The conventional food price dilemma.

Net benefits are generally twice as great when the linear specification + parallel shift is used as compared to the CES specification. And the CES function implies increase in consumer surplus but a reduction in producer surplus, while the linear specification always results in an increase in both consumer and producer surplus.

## Farm heterogeneity, entry/exit

The entire debate surrounding functional form and type of shift is moot because it rests upon the assumption of 

Even the exceptional case identified by Zhou et al. [where impact assessment may be insensitive to misspecification of the supply curve] is only valid when  is void as soon as the reality of heterogeneously endowed inframarginal farms is taken into consideration. The linear model assumes a population of homogeneous farms producing identical quantities. In some sectors, one might reasonably justify this assumption by invoking competitive long run equilibrium. The case for competitive long run equilibrium is strained in the primary sector, however, where one typically sees an abundance of inframarginal farms in operation.

In some sectors, one might reasonably dismiss the need to account for heterogeneously endowed firms by invoking competitive long run equilibrium. The case for competitive long run equilibrium is strained in the primary sector, however, where one often sees an abundance of inframarginal farms in operation. (Wohlgenant [-@wohlgenant1997nature] provides an analytical exploration of the distortions that can occur when neglecting to account for heterogeneity and inframarginal production.)


(Wohlgenant [-@wohlgenant1997nature] provides an analytical exploration of the distortions that can occur when neglecting to account for heterogeneity and inframarginal production.)
...
The aggregate supply model derived in the subsequent sections is effectively one long, fierce disagreement with this passage. The sweeping [, and rather brazen, _opinion_] claim that economic theory does not tell us "very much" about the nature of aggregate supply curve shifts is particularly vexing. On the contrary, economic theory provides a set of microeconomic principles whence a firm level yield model is derived, whence "effects on individual agents", i.e., farm level response to exogenous shock, can be can modeled. Insofar as the phrase "complete theory of aggregation" refers to the specification of aggregate supply and demand curves, this too can be derived from theory by integrating individual effects over a suitable density function (i.e., Equation \ref{eq:sup1}). The firm yield model implies bounds on the domain of optimal output, such that the "exit and entry of firms" is accounted for by integrating over the domain of optimal behavior.
...
To be clear, the "precise econometric evidence or...detailed information" that the authors refer to is generally not available, but the suggestion that economic theory offers nothing in its stead is simply mistaken, ignores textbooks [frankly, brazen]. The whole point of having a theory is [to make sense of empirically observed info and to] precisely to draw inference when available information provides no clear answer to our questions. I also agree that "assumptions are unavoidable" in the sense that the standard microeconomic axioms whence $q^*$ is derived, and plausible assumptions about the shape of the farm density $\rho(q^*)$ are unavoidable,   NOT of assuming what may then be endogenously modeled on this basis. tantamount to assuming what it is the job of economists to show.   [If the inference proves wrong, then the theory is corrected.] It is not theory, then, but rather models not explicitly built up from theory, that tell us "not very much" about the nature of supply curve shifts.  ...And  insofar as the 5 axioms are unavoidable, but assumption of the supply response to exogenous technology shock, which is nearly the same as assuming the result of the impact assessment, is not unavoidable. On the contrary, it is precisely the work of the economist to model the supply response to technology shocks endogenously.
...
Where the $x_i$ are management inputs (fertilizer, etc.), the $k_i$ are control variables (demographic and agroclimatic variables, etc.), and the $\alpha_i$ and $\kappa_i$ are input and control variable elasticities of yield, respectively. The input elasticities of yield are also referred to collectively as the "technology". The product bounds $n_x$ and $n_k$ indicate the number of inputs and control variables, respectively. The variables $x_i$ and $k_i$ are assumed to be linearly independent and are divided by their population means ($\tilde{x}_i$, $\tilde{k}_i)$, such that $y_0$ may be interpreted as the mean yield. (See Appendix for details of the derivation.)

The introduction of axiom 2 then results in the profit maximizing farm yield function.

\begin{equation}
y^* = \left(y_0 \frac{\theta_{x}}{\theta_{w}}\theta_k \right)^{\frac{\eta}{h}} \left( \frac{P}{\lambda} \right)^{\eta} \:\:;\:\:\: \eta = \frac{h}{1 - h}
...
Where the $w_i$ are input prices. Note that $\eta$, the farm level output price elasticity of supply, is a function of the technology. The yield envelope---i.e., the budget constrained, profit maximizing farm yield in Equation \ref{eq:YdEnvCompact0}---is henceforth referred to simply as the farm yield. Likewise, "farm production function" henceforth refers to the budget constrained, profit maximizing production function $q^*$ obtained when $y^*$ is multiplied by the cultivated area $k_a$.

\begin{equation}
q^* = k_a y^* = k_a \left(y_0 \frac{\theta_{x}}{\theta_{w}}\theta_k \right)^{\frac{\eta}{h}} \left( \frac{C^{\#}}{h} \right)^{\eta}
\end{equation}
...
\begin{equation}
q^* = \left(y_0 \frac{\theta_{x}}{\theta_{w}} \hat{\theta}_k \right)^{\frac{\eta}{h}} \left( \frac{P}{\lambda} \right)^{\eta} \:\:;\:\:\: \hat{\theta}_k = k_a^{\kappa_a + \frac{h}{\eta}} \prod_{i \neq a} \left( \frac{k_i}{\tilde{k}_i} \right)^{\kappa_i}
\label{eq:prodFn1}
\end{equation}
...
\begin{equation}
q^* = \left(y_0 \frac{\theta_x}{\theta_{w}} \hat{\theta}_k \right)^{\frac{\eta}{h}} \left( \frac{C^{\#}}{h} \right)^{\eta}
\label{eq:prodFn2}
\end{equation}
...
\begin{equation}
q^* = y_0 \frac{\theta_{x}}{\theta_{w}} \hat{\theta}_k \left( \frac{C^*}{h} \right)^h
\label{eq:prodFn3}
\end{equation}

(Where, in the latter case, $\hat{\theta}_k = k_a^{\kappa_a + 1} \prod_{i \neq a} k_i^{\kappa_i}$.)

The latter expression comes in handy for estimation purposes because it requires information only on cost and control variables, and not on input quantities used in production. The degree of homogeneity $h$ is thus also the scale elasticity of yield ($d \ln(y^*) / d \ln(C^*) = h$), as noted in the textbooks. This form is particularly useful for estimation, as it requires only the control variables $k_i$ and input cost $C^*$, and not the management variables. That is, instead of estimating the model in Equation \ref{eq:cobbDoug3ln}, one estimates
...
\begin{equation}
Q = N \left(y_0 \frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E \left[\hat{\theta}_k^{\frac{\eta}{h}} \right] h^{-\eta} \int_{0}^{P} C^{\# \eta} f(C^{\#}) \: dC^{\#}
\end{equation}
...
The aggregate supply function thus reduces to the following tractable form.
...
This can be rewritten in a few different ways by making substitutions based on the corollary in Equation \ref{eq:useful}. In the motivating context of the present work, one is generally interested in aggregate supply as a function of output price. For the rest of the article, then, the focus is on the form below, where the fraction $C^{\#} / h$ has been replaced with $P / \lambda$.

\begin{equation}
Q = N \left( y_0 \frac{\theta_x}{\theta_w} \right)^{\frac{\eta}{h}} E \left[\hat{\theta}_k^{\frac{\eta}{h}} \right] E[\lambda^{-\eta}] P^{\eta} \Phi(\gamma)
\label{eq:sup2b}
\end{equation}
...
To evaluate this expression, a suitable stochastic model must be assumed for $P(t)$ and $C^{\#}(t)$ follow. Within a given season, prices often follow a geometric Brownian motion (gBm). That is to say, changes in $P$ over a small time interval $\Delta t$ ($\Delta P = P(t + \Delta t) - P(t)$) are often accurately modeled by
...
For what follows, it is also important to note that the log change of a gBm $\ln(P(T) / P(\hat{t}))$ is defined over any finite time interval for some starting time $\hat{t}<T$, and is also normally distributed, with mean and variance
...
Here it is tempting to introduce a sixth axiom saying that farmers will only pursue a risky bet if the probability of profit is greater than that of a coin toss (i.e., greater than $50\%$), and to thereby limit the domain of $z$ to $(-\infty, 0)$. But this is left to the discretion of the reader. [There are cases when rational economic agents pursue "out of the money" bets.]
...
# An illustrative use case: Estimation of the aggregate supply curve using Kenya farm survey data
...
priceVars <- priceVars[-which(priceVars == "Total UREA fert (KES/kg)")]
#-----------------------------------------------------------------------
...
#-----------------------------------------------------------------------
mod2Vars <- c("dist", "yield (kg/acre)", "cost/acre", "acres", "seed (kg/acre)", inputsNegCoef, bin_vars, demog_vars, clim_vars, priceVars)
...
df_mod2$dist[which(mod2$residuals < -1)]
df_mod2$dist[which(mod2$residuals > 1)]
exp(df_mod2$acres[which(mod2$residuals < -1)])
exp(df_mod2$acres[which(mod2$residuals > 1)])
...
Axioms 1 and 2 thus imply that marginal cost is proportionate to output price. This is a little different from the standard assertion that the two are equal. If they were equal, then this would be theoretical grounds upon which to presume that every farm has the same marginal cost, and then from here it is a short jump to presume that a given exogenous shock affects every farm's marginal cost identically .... but no, marginal cost varies with $\lambda$, which captures each farm's unique, subjective valuation of additional increments in production expenditure. [Even if it did not, it is invalid to assume that a shock would affect MC identically for all farms, and the authors recognize this anyway, and for this reason assume all farms marginal]
